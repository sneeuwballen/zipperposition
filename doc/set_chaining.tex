...
is equivalent to $\{t \} \cap A \subseteq \emptyset$).
...
\begin{prooftree}
    ...
    \AxiomC{$ n \geq 1 $}
    \RightLabel{singleton witness}
    \doubleLine
    \BinaryInfC{$
        \bigwedge_{i} (K \lor t_i = t_1)
        \land
        \bigwedge_{a \in A_\cap} (K \lor \{ t_1 \} \subseteq a)
        \land
        \bigwedge_{b \in B_\cap} (K \lor \{ t_1 \} \cap b \subseteq \emptyset)
    $}
\end{prooftree}
