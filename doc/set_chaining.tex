...
\begin{prooftree}
    ...
    \AxiomC{$K
        ...
        \lor \bigvee_k (x = t_k) $}
    \RightLabel{singleton elim}
    \doubleLine
    \UnaryInfC{$ \normalize{K
        \lor (  \bigcap_i ({B_i}_\cup \cup \compl{{A_i}_\cap})
            \subseteq
                \bigcup_j ({B'_j}_\cup \cup \compl{{A'_j}_\cap}) \cup \bigcup_k \{t_k\})
    }
    \doubleLine
...
\begin{prooftree}
    ...
    \UnaryInfC{$ \normalize{K
        \lor \bigvee_j
            (   \bigcap_i ({B_i}_\cup \cup \compl{{A_i}_\cap})
                \subseteq
                {B'_j}_\cup \cup \compl{{A'_j}_\cap}
            )
    }
    $}
\end{prooftree}
