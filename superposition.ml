...
(** visits non-variable positions in t. f is given the bag, the subterm,
    ...
    'a -> position -> (foterm -> 'b) -> 'c -> foterm
    -> ('a -> foterm -> position -> (foterm -> 'b) -> 'c -> 'a * foterm * 'c)
    -> 'a * foterm * 'c
    *)
...
(** Visit the term, applying the function to every subterm,
    with the given accumulator, position, context, and second accumulator.
    The function returns, in addition to accumulators, a substitution
    and a new subterm that replaces the old one. The function visits
    leaves before inner nodes.

    'a -> int list -> (foterm -> 'b) -> 'c -> foterm
    -> ('a -> foterm -> position -> (foterm -> 'b) -> 'c -> 'a * substitution * foterm * 'c)
    -> 'a * foterm * 'c
    *)
let visit bag pos ctx id t f =
  let rec aux bag pos ctx id subst t = match t.node.term with
  | Leaf _ ->
  ...
  | Node (hd::l) ->
      let bag, l, _, id =
        List.fold_left
          ...
          (bag, [hd], List.map (S.apply_subst subst) (List.tl l), id) l
      in
      let bag,subst,t,id1 = f bag (T.mk_node l) pos ctx id in
      aux bag pos ctx id1 subst t
  | _ -> assert false
  in
...
(* TODO use visit, repeteadly, to rewrite a term. Clauses that rewrite
should be kept for the proof. *)
