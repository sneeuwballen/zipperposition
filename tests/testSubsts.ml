...
let h x y z =
  T.app (__const ~ty:Type.([term; term; term] ==> term) h_) [x; y; z]
...
let test_rename =
  ( "subst.rename",
    ...
    fun () ->
      ...
      let t3' =
        h
          (S.FO.apply renaming subst (y, 1))
          t1'
          (S.FO.apply renaming subst (t3, 0))
      in
      Alcotest.(check (module T)) "must be equal" t1' t2';
      let t3'' = h a (f x (g a)) (g (g x)) in
      Alcotest.(check (testable T.pp Unif.FO.are_variant))
        "must be variants" t3'' t3';
      () )
...
let test_unify =
  ( "subst.unify",
    ...
    fun () ->
      ...
      in
      let nil ty =
        T.app (__const ~ty:Type.(forall (app list_ [bvar 0])) nil_) [T.of_ty ty]
      in
...
let suite = [test_rename; test_unify]
