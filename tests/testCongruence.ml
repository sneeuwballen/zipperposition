...
let _size_class l = List.fold_left (fun acc t -> T.size t + acc) 0 l

let _size_classes l = List.fold_left (fun acc l -> _size_class l + acc) 0 l
...
let check_classes_are_eq =
  let gen =
    QCheck.(
      list_of_size Gen.(1 -- 10) (list_of_size Gen.(1 -- 5) ArTerm.default))
  in
  let prop classes =
...
module CC_ref : sig
  ...
  let is_eq_ (cc : t) t u : bool =
    T.equal t u
    || TSet_set.exists (fun set -> T.Set.mem t set && T.Set.mem u set) cc
        ...
        T.Set.exists
          (fun u ->
            ...
            | T.App (f1, l1), T.App (f2, l2) ->
              is_eq_ cc f1 f2 && CCList.equal (is_eq_ cc) l1 l2
            | _ -> false )
          s2 )
      s1
  ...
  let rec update (cc : t) : t =
    let find_merge_ cc =
      TSet_set.to_seq cc
      |> Iter.flat_map (fun s1 ->
             assert (not (T.Set.is_empty s1));
             TSet_set.to_seq cc
             |> Iter.filter (fun s2 -> s1 != s2)
             |> Iter.map (fun s2 -> s1, s2) )
      |> Iter.find_map (fun (s1, s2) ->
             if are_congruent_ cc s1 s2 then Some (s1, s2) else None )
    in
    match find_merge_ cc with
    | None -> cc (* fixpoint *)
    | Some (s1, s2) ->
...
let props = [check_term_eq_itself; check_classes_are_eq; check_ref]
