...
let test_compare =
  ( "multiset.compare",
    ...
    fun () ->
      ...
      Alcotest.(check (module Comparison))
        "must be lt" Comparison.Lt
        (M.compare_partial f m1 m2);
...
let gen1 =
  ...
  let shrink l =
    M.to_list l |> Q.Shrink.(list ~shrink:shrink2) |> Q.Iter.map M.of_coeffs
  in
  Q.(
    small_list small_int |> map M.of_list |> set_print pp1 |> set_shrink shrink)
...
let compare_and_partial =
  ...
  in
  let prop (m1, m2) = _sign (compare' m1 m2) = _sign (M.compare m1 m2) in
  QCheck.Test.make ~name:"multiset_compare_and_compare_partial" ~long_factor:3
    ~count:1000 (Q.pair gen1 gen1) prop

(* partial order for tests *)
let partial_ord (x : int) y =
  if x = y then
    Comparison.Eq
  else if x / 5 = y / 5 && x mod 5 <> y mod 5 then
    Incomparable
  else
    CCInt.compare (x / 5) (y / 5) |> Comparison.of_total
...
let compare_partial_sym =
  let prop (m1, m2) =
    ...
    if c1 = c2 then
      ...
      Q.Test.fail_reportf "comparison: %a vs %a" Comparison.pp c1 Comparison.pp
        c2
  in
  QCheck.Test.make ~name:"multiset_compare_partial_sym" ~long_factor:3
    ~count:13_000
    Q.(pair gen1 gen1)
    prop
...
let compare_partial_trans =
  let prop (m1, m2, m3) =
    ...
    | Lt, Gt, _
    | Gt, Lt, _ ->
      Q.assume_fail () (* ignore *)
    | Eq, Eq, Eq -> true
    | Gt, Gt, Gt | Gt, Eq, Gt | Eq, Gt, Gt -> true
    | Lt, Lt, Lt | Lt, Eq, Lt | Eq, Lt, Lt -> true
    | Lt, _, Eq
    | Gt, _, Eq
    | _, Lt, Eq
    | _, Gt, Eq
    ...
    | Gt, Eq, Lt
    | Eq, Eq, (Lt | Gt) ->
      Q.Test.fail_reportf "comp %a %a %a" Comparison.pp c1 Comparison.pp c2
        Comparison.pp c3
  in
  QCheck.Test.make ~name:"multiset_compare_partial_trans" ~long_factor:3
    ~count:13_000 (Q.triple gen1 gen1 gen1) prop

let max_seq_correct =
  let prop m =
    let l1 = M.max_seq partial_ord m |> Iter.map fst |> Iter.to_list in
    let l2 =
      M.to_list m |> List.map fst
      |> List.filter (fun x -> M.is_max partial_ord x m)
    in
    if l1 = l2 then
      true
    else
      Q.Test.fail_reportf "@[max_seq %a,@ max %a@]"
...
let max_is_max =
  ...
  in
  Q.Test.make ~name:"multiset_max_l_is_max" ~long_factor:3 ~count:1000 gen prop

let suite = [ test_max; test_compare; test_cardinal_size ]
