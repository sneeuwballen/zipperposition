...
let check_ordering_inv_by_subst ~gen_t ord =
  ...
  let gen =
    QCheck.Gen.(
      pair gen_t gen_t >>= fun (t1, t2) ->
      let vars =
        Iter.of_list [ t1; t2 ] |> Iter.flat_map T.Seq.vars |> T.VarSet.of_iter
      in
...
let check_ordering_trans ~arb_t ord =
  ...
  let prop (t1, t2, t3) =
    ...
    if o12 = Comparison.Lt && o23 = Comparison.Lt then (
...
let contains_ho t =
  T.Seq.subterms t |> Iter.exists (fun t -> T.is_ho_at_root t || T.is_fun t)
  ...
  let prop t =
    ...
    |> Iter.filter_map (fun (t, i) ->
           if i > 0 then
             ...
             None)
    |> Iter.for_all (fun sub -> O.compare !ord t sub = Comparison.Gt)
  in
  QCheck.Test.make ~count ~long_factor ~name arb prop

let test_derived_ho_rpo =
  ( "ordering.derived_ho_rpo",
    `Quick,
    fun () ->
      let ord =
        O.derived_ho_rpo (Precedence.default [ a_; b_; c_; d_; e_; f_; g_; h_ ])
      in
      ...
      Alcotest.(check comp_test)
        "x a <=>? x b" Comparison.Incomparable
        (compare (Term.app x [ a ]) (Term.app x [ b ]));
      ...
      Alcotest.(check comp_test)
        "f x y <=>? x y" Comparison.Incomparable
        (compare (Term.app f [ x; y ]) (Term.app x [ y ]));
      ...
      Alcotest.(check comp_test)
        "g x > f x x" Comparison.Gt
        (compare (Term.app g [ x ]) (Term.app f [ x; x ]));
      ...
      let b = Term.const ~ty b_ in
      Alcotest.(check comp_test)
        "f (x b) <=>? x a" Comparison.Incomparable
        (compare (Term.app f [ Term.app x [ b ] ]) (Term.app x [ a ]));
      ...
      let a = Term.const ~ty a_ in
      Alcotest.(check comp_test)
        "f (x a) > x a" Comparison.Gt
        (compare (Term.app f [ Term.app x [ a ] ]) (Term.app x [ a ]));
      ...
      let b = Term.const ~ty b_ in
      Alcotest.(check comp_test)
        "f a a > f b" Comparison.Gt
        (compare (Term.app f [ a; a ]) (Term.app f [ b ])) )

let test_lambdafree_rpo =
  ( "ordering.lambdafree_rpo",
    `Quick,
    fun () ->
      let ord =
        O.lambdafree_rpo (Precedence.default [ a_; b_; c_; d_; e_; f_; g_; h_ ])
      in
      let compare = O.compare ord in
      ...
      let y = Term.var (HVar.fresh ~ty ()) in
      Alcotest.(check comp_test)
        "f x y is incomparable with x y" Comparison.Incomparable
        (compare (Term.app f [ x; y ]) (Term.app x [ y ]));
...
let test_derived_ho_kbo =
  ( "ordering.derived_ho_kbo",
    `Quick,
    fun () ->
      ...
      in
      let ord =
        O.derived_ho_kbo ~ignore_quans_under_lam:true
          (Precedence.create ~weight Precedence.Constr.alpha
      ...
      let f = Term.const ~ty:(Type.arrow [ ty ] ty) f_ in
      Alcotest.(check comp_test)
        "f b > f a" Comparison.Gt
        (compare (Term.app f [ b ]) (Term.app f [ a ]));
      ...
      let f = Term.const ~ty:(Type.arrow [ ty; ty ] ty) f_ in
      Alcotest.(check comp_test)
        "f b a > f a b" Comparison.Gt
        (compare (Term.app f [ b; a ]) (Term.app f [ a; b ]));
      ...
      Alcotest.(check comp_test)
        "z b <=>? z a" Comparison.Incomparable
        (compare (Term.app z [ b ]) (Term.app z [ a ]));
      ...
      Alcotest.(check comp_test)
        "z b a <=>? z a b" Comparison.Incomparable
        (compare (Term.app z [ b; a ]) (Term.app z [ a; b ]));
      ...
      Alcotest.(check comp_test)
        "c < f (Y X a) b" Comparison.Lt
        (compare c (Term.app f [ Term.app y [ x; a ]; b ]));
      ...
      let y = Term.var (HVar.fresh ~ty ()) in
      Alcotest.(check comp_test)
        "h (x y) > f a (x y)" Comparison.Gt
        (compare
           (Term.app h [ Term.app x [ y ] ])
           (Term.app f [ a; Term.app x [ y ] ]));
      ...
      Alcotest.(check comp_test)
        "fun y. forall x. x < forall x. x"
        (compare (Term.fun_l [ ty ] forall_x_x) forall_x_x)
        Comparison.Lt;
      ...
      Alcotest.(check comp_test)
        "fun y. z <=>? z" Comparison.Incomparable
        (compare (Term.fun_l [ ty ] z) z);
      ...
      Alcotest.(check comp_test)
        "f z > z" Comparison.Gt
        (compare (Term.app f [ z ]) z);

      (* z a <=>? z (Because of fluidity) *)
      let a = Term.const ~ty a_ in
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty ] ty) ()) in
      Alcotest.(check comp_test)
        "z a <=>? z" Comparison.Incomparable
        (compare (Term.app z [ a ]) z);

      (* lam x. z x a <=>? z (Because of fluidity) *)
      let a = Term.const ~ty a_ in
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty; ty ] ty) ()) in
      Alcotest.(check comp_test)
        "lam x. z x a <=>? z" Comparison.Incomparable
        (compare (Term.fun_l [ ty ] (Term.app z [ Term.bvar ~ty 0; a ])) z);
        ...
        (compare
           (T.app app
              [
                ty1;
                ty1;
                T.app app [ ty1; ty2; add; T.app app [ ty1; ty1; s; zero ] ];
                k;
              ])
           (T.app app
              [
                ty1;
                ty1;
                T.app app [ ty1; ty2; add; zero ];
                T.app app [ ty1; ty1; s; k ];
              ]));
        ...
        (compare
           (T.app app
              [
                ty1;
                ty1;
                T.app app [ ty1; ty2; add; T.app app [ ty1; ty1; s; x ] ];
                y;
              ])
           (T.app app
              [
                ty1;
                ty1;
                T.app app [ ty1; ty2; add; x ];
                T.app app [ ty1; ty1; s; y ];
              ])) )

let test_lambdafree_kbo =
  ( "ordering.lambdafree_kbo",
    `Quick,
    fun () ->
      (* alphabetical precedence, h has weight 2, all other symbols weight 1*)
      let weight id =
      ...
      (*app (app add (app s x)) y > app (app add x)(app s y)*)
      Alcotest.(check comp_test)
        "app (app add (app s x)) y > app (app add x)(app s y)" Comparison.Gt
        (compare
           (T.app app
              [
                ty1;
...
let test_lambda_kbo =
  ( "ordering.lambda_kbo",
    ...
    fun () ->
      ...
      let g = Term.const ~ty:(Type.arrow [ ty ] ty) g_ in
      let z =
        Term.var (HVar.fresh ~ty:(Type.arrow [ Type.arrow [ ty ] ty ] ty) ())
      in
      Alcotest.(check comp_test)
        "z g >= z f" Comparison.Geq
        (compare (Term.app z [ g ]) (Term.app z [ f ]));
      ...
      in
      Alcotest.(check comp_test)
        "z h <=>? z f" Comparison.Incomparable
        (compare (Term.app z [ h ]) (Term.app z [ f ]));
      ...
      Alcotest.(check comp_test)
        "z b c e >= z a c d" Comparison.Geq
        (compare (Term.app z [ b; c; e ]) (Term.app z [ a; c; d ]));
      ...
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty ] ty) ()) in
      Alcotest.(check comp_test)
        "y (z a) <=>? z (y a)" Comparison.Incomparable
        (compare
      ...
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty ] ty) ()) in
      Alcotest.(check comp_test)
        "f (y (z a)) < z (y a)" Comparison.Incomparable
        (compare
      ...
      Alcotest.(check bool)
        "b is max of a = b" true
        (CCList.equal T.equal (Literal.Comp.max_terms ~ord a_eq_b) [ b ]);
      Alcotest.(check bool)
        "b is max of b = a" true
        (CCList.equal T.equal (Literal.Comp.max_terms ~ord b_eq_a) [ b ]);
      ...
      Alcotest.(check bool)
        "z b is max of z a = z b" true
        (CCList.equal T.equal (Literal.Comp.max_terms ~ord za_eq_zb) [ zb ]);
      Alcotest.(check bool)
        "z b is max of z b = z a" true
        (CCList.equal T.equal (Literal.Comp.max_terms ~ord zb_eq_za) [ zb ]);
      ...
      Alcotest.(check comp_test)
        "(f b (z b) = b) > (f b (z a) = a)" Comparison.Gt
        (Literal.Comp.compare ~ord fbzb_eq_b fbza_eq_a);
      ...
      let fbzb_eq_fbza =
        Literal.mk_eq
          (Term.app f [ b; Term.app z [ b ] ])
          (Term.app f [ b; Term.app z [ a ] ])
      in
      let fbza_eq_a = Literal.mk_eq (Term.app f [ b; Term.app z [ a ] ]) a in
      Alcotest.(check comp_test)
        "(f b (z b) = f b (z a)) > (f b (z a) = a)" Comparison.Gt
        (Literal.Comp.compare ~ord fbzb_eq_fbza fbza_eq_a);
      ...
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty ] ty) ()) in
      let fbzb_eq_yb =
        Literal.mk_eq (Term.app f [ b; Term.app z [ b ] ]) (Term.app y [ b ])
      in
      let fbza_eq_ya =
        Literal.mk_eq (Term.app f [ b; Term.app z [ a ] ]) (Term.app y [ a ])
      in
      Alcotest.(check comp_test)
        "(f b (z b) = y b) >= (f b (z a) = y a)" Comparison.Geq
        (Literal.Comp.compare ~ord fbzb_eq_yb fbza_eq_ya);
      ...
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty ] ty) ()) in
      let fbzb_eq_zb =
        Literal.mk_eq (Term.app f [ b; Term.app z [ b ] ]) (Term.app z [ b ])
      in
      let fbza_eq_za =
        Literal.mk_eq (Term.app f [ b; Term.app z [ a ] ]) (Term.app z [ a ])
      in
      Alcotest.(check comp_test)
        "(f b (z b) = z b) >= (f b (z a) = z a)" Comparison.Geq
        (Literal.Comp.compare ~ord fbzb_eq_zb fbza_eq_za);
      ...
      Alcotest.(check comp_test)
        "f b (z a) = b <=>? f b (z b) = a" Comparison.Incomparable
...
let test_lambda_lpo =
  ( "ordering.lambda_lpo",
    ...
    fun () ->
      let ord =
        O.lambda_lpo (Precedence.default [ a_; b_; c_; d_; e_; f_; g_; h_ ])
      in
      let compare = O.compare ord in
      ...
      let b = Term.const ~ty b_ in
      Alcotest.(check comp_test)
        "f b b > f a" Comparison.Gt
        (compare (Term.app f [ b; b ]) (Term.app f [ a ]));
      ...
      in
      Alcotest.(check comp_test)
        "z g <=>? z f" Comparison.Incomparable
        (compare (Term.app z [ g ]) (Term.app z [ f ]));
      ...
      let z = Term.var (HVar.fresh ~ty:(Type.arrow [ ty; ty ] ty) ()) in
      Alcotest.(check comp_test)
        "lam x y. z x a >= z" Comparison.Geq
        (compare
           (Term.fun_l [ ty ]
              (Term.fun_l [ ty ] (Term.app z [ Term.bvar ~ty 1; a ])))
           z);
      ...
      let ord =
        O.lambda_lpo
          (Precedence.create Precedence.Constr.alpha
             [ add_; app_; funty_; k_; s_; zero_ ])
      in
      let x = Term.var (HVar.fresh ~ty ()) in
      let y = Term.var (HVar.fresh ~ty ()) in
