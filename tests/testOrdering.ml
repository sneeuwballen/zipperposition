...
let more_specific cmp1 cmp2 =
  Comparison.(
    ...
    | Lt, Lt | Gt, Gt | Eq, Eq | Incomparable, _ -> true
    | _, _ -> false)
...
let check_ordering_inv_by_subst ord =
  ...
  let gen =
    QCheck.Gen.(
      ...
      let vars =
        Iter.of_list [t1; t2] |> Iter.flat_map T.Seq.vars |> T.VarSet.of_seq
      in
  ...
  *)
  ...
  let prop (t1, t2, subst) =
    ...
    |> ID.Set.of_seq |> ID.Set.to_seq |> O.add_seq !ord;
...
let check_ordering_swap_args ord =
  ...
  let prop (t1, t2) =
    ...
    match o12, o21 with
    | Lt, Gt | Gt, Lt | Eq, Eq | Incomparable, Incomparable -> true
    | _ -> false
  in
  QCheck.Test.make ~count ~long_factor ~name arb prop
...
let test_kbo =
  ( "ordering.kbo",
    ...
    fun () ->
      ...
      Alcotest.(check comp_test)
        "h (x y) > f y (x a)"
        (compare (Term.app h [Term.app x [y]]) (Term.app f [y; Term.app x [a]]))
        Comparison.Gt;
      (* polymorphic example *)
