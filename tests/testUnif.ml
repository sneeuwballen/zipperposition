...
let psterm, pstmt, pstmt_l, clear_scope, unif_ty, pstyctx =
  ...
  let pt s =
    try
      ...
      t
    with e ->
      Format.eprintf "while parsing '%s'@." s;
      raise e
  and pst s =
  ...
  and pst_l s =
    ...
    let l =
      TypeInference.infer_statements_exn ~on_var:`Default ~ctx:tyctx
        ~implicit_ty_args:false (Iter.of_list l)
    in
    (* TypeInference.Ctx.exit_scope tyctx; *)
    CCVector.to_list l
  and unif_ty t u =
...
let parse_with_vars vars ~f =
  TypeInference.Ctx.with_vars pstyctx
    (CCList.map (fun (name, ty) -> Var.of_string ~ty:(psterm ty) name) vars)
    ~f
...
let pterm2 =
 fun ?(unif_types = true) ?ty s1 s2 ->
  let t1 = psterm s1 in
  let t2 = psterm s2 in
  if unif_types then unif_ty t1 t2;
  let ty = CCOpt.map psterm ty in
  ...
  T.Conv.of_simple_term_exn tyctx t1, T.Conv.of_simple_term_exn tyctx t2
...
module Task : sig
  ...
  let mk_match ?negated t1 t2 : t = mk_ ?negated @@ TMatch { t1; t2 }
  ...
  let set_with_ty ty = function
    | { view = TUnif u; _ } as r ->
      { r with view = TUnif { u with with_ty = Some ty } }
    | _ -> assert false

  let set_unif_types b = function
    | { view = TUnif u; _ } as r ->
      { r with view = TUnif { u with unif_types = b } }
    | _ -> assert false

  let is_negated r = r.negated

  let pp out = function
    | { view = TUnif { t1; t2; with_ty = None; _ }; _ } ->
      Format.fprintf out "Unif (%s, %s)" t1 t2
    | { view = TUnif { t1; t2; with_ty = Some ty; _ }; _ } ->
      Format.fprintf out "Unif (%s, %s) : %s" t1 t2 ty
    | { view = TMatch { t1; t2; _ }; _ } ->
      Format.fprintf out "Match (%s, %s) " t1 t2
...
let equal_after_sub ~subst (t1, sc1) (t2, sc2) =
  ...
  let t1 =
    Subst.FO.apply renaming subst (t1, sc1) |> Lambda.snf |> Lambda.eta_reduce
  in
  let t2 =
    Subst.FO.apply renaming subst (t2, sc2) |> Lambda.snf |> Lambda.eta_reduce
  in
  T.equal t1 t2
...
let check_variant ?(msg = "") t u =
  if Unif.FO.are_variant t u then
    ...
    Alcotest.failf "@[<2>`%a`@ and `%a`@ should be variant@,%s@]@." T.ZF.pp t
      T.ZF.pp u msg
...
let check_matches ?(msg = "") t u =
  if Unif.FO.matches ~pattern:t u then
    ...
    Alcotest.failf "@[<2>`%a`@ should match@ `%a`%s@]@." T.ZF.pp t T.ZF.pp u msg

let check_eq ?(msg = "check eq") t1 t2 = Alcotest.(check t_test) msg t1 t2
...
let check_action t u t' renaming subst a =
  match a with
  ...
  | Action.Eq { t1; t2; sc1; sc2; _ } ->
    ...
    let msg =
      Fmt.sprintf
        ...
        T.ZF.pp t T.ZF.pp u T.ZF.pp t1 sc1 T.ZF.pp t2 sc2
    in
    check_eq ~msg t1 t2
  ...
  ( "check unifiable",
    ...
    fun () ->
      try
        ...
        if negated then
          Alcotest.failf
            "@[<2>`%a`[0]@ and `%a`[1]@ should not be unifiable@]@." T.ZF.pp t
            T.ZF.pp u
        else (
          Alcotest.(check t_test) "check unified versions are equal" t' u';
          let msg =
            Fmt.sprintf
              "(@[check that unified version matches original term `%a`@ \
               `%a`@])"
              T.ZF.pp t T.ZF.pp u
          in
          ...
          if not (equal_after_sub ~subst (t, 0) (u, 1)) then
            Alcotest.failf
              "@[terms unify but are not equal under subst=%a,@ t1=`%a`,@ \
               t2=`%a`,@ t1σ=`%a`,@ t2σ=`%a`"
  ...
  ( "check matchable",
    ...
    fun () ->
      try
        ...
        if negated then
          Alcotest.failf "@[<2>`%a`[0]@ should not match pattern `%a`[1]@]@."
            T.ZF.pp u T.ZF.pp t
        else (
          let renaming = Subst.Renaming.none in
          let t' =
            Subst.FO.apply renaming subst (t, 0)
            |> Lambda.snf |> Lambda.eta_reduce
          in
          let u' =
      ...
      with Unif.Fail ->
        if not negated then
          Alcotest.failf "@[<2>`%a`[0]@ should match pattern `%a`[1]@]@."
            T.ZF.pp t T.ZF.pp u )
...
let suite_unif1 : unit Alcotest.test_case list =
  let ( =?= ) a b = Task.mk_unif a b in
  (* unif pair *)
  let ( <?> ) a b = Task.mk_unif ~negated:true a b in
  (* unif pair *)
  (* let (=?=>) a b = Task.mk_match a b in matching pair *)
  let ( >-> ) a b = Task.set_with_ty b a in
  (* specify return type *)
  let ( >?-> ) a b = Action.set_with_ty b a in
  (* specify return type *)
  let ( >>> ) a b = Task.add_action b a in
  (* specify return type *)
  ...
  let mk_tests task =
    ...
    | Task.Unif (t, u) -> check_unifiable t u actions
    | Task.Match (t, u) ->
      check_matchable ~negated:(Task.is_negated task) t u actions
    | Task.Parses t -> check_parsable t
  in
  CCList.map mk_tests
    [
      "f X b" =?= "f a Y" >>> Action.yield "f a b" >>> Action.eq "X" 0 "a" 0
      >>> Action.eq "Y" 1 "b" 0;
      ...
      "fun (x y z:term). F x" =?= "fun x y z. G x y z"
      >-> "term -> term -> term -> term"
      >>> (Action.yield "fun x y z. H x" >?-> "term -> term -> term -> term")
      >>> (Action.eq "G" 1 "fun x y z. F x" 0
          >?-> "term -> term -> term -> term");
      ...
      Task.mk_parse_and_tycheck
        "with (F0:term->term). F0 (f_ho2 (fun (Y0:term). (F0 Y0)) (fun \
         (Y0:term). X0))";
      "with (X0:term) (F0:term->term). F0 (f_ho2 (fun (Y0:term). (F0 Y0)) (fun \
       (Y0:term). X0))"
      =?= "with (F56769:term->term) (X56769:term). F56769 X56769";
      "with (F56771:term ->term). F56771 (f_ho2 (fun (Y0:term). (F56771 Y0)) \
       (fun (Y0:term). X56772))" =?= "with (F56769:term->term). F56769 X56770";
    ...
    fun () ->
      let terms =
        [
          ...
          pterm "f_ho2 (fun (x:term). Y) g", pterm "f_ho2 (fun (x:term). x) g";
        ]
      in
      terms
      |> List.iter (fun (t1, t2) ->
             try
               ...
               Alcotest.failf "@[<hv>`%a`@ and `%a@ should not match@]" T.ZF.pp
                 t1 T.ZF.pp t2
             with Unif.Fail -> ()) )

let reg_unif_makes_eq =
  ( Printf.sprintf "regression %d" __LINE__,
    `Quick,
    fun () ->
      let t1 =
        pterm
...
let suite_unif2 = [ reg_matching1; reg_stack_overflow; reg_unif_makes_eq ]
...
let check_unify_gives_unifier =
  ...
  let prop (t1, t2) =
    try
      ...
      let t1' =
        S.FO.apply renaming subst (t1, 0) |> Lambda.snf |> Lambda.eta_reduce
      in
      let t2' =
        S.FO.apply renaming subst (t2, 1) |> Lambda.snf |> Lambda.eta_reduce
      in
      if T.equal t1' t2' then
        true
      else
        QCheck.Test.fail_reportf "subst=%a,@ t1'=`%a`,@ t2'=`%a`" Subst.pp subst
          T.ZF.pp t1' T.ZF.pp t2'
  ...
  let prop (t1, t2) =
    try
      ...
      let t2' =
        S.FO.apply renaming subst (t2, 1) |> Lambda.snf |> Lambda.eta_reduce
      in
      if Unif.FO.matches ~pattern:t1 t1' && Unif.FO.matches ~pattern:t2 t2' then
        true
      else
        QCheck.Test.fail_reportf "(@[<hv2>subst=%a,@ t1'=`%a`,@ t2'=`%a`@])"
...
let check_unify_makes_eq =
  ...
  let prop (t1, t2) =
    try
      ...
      if equal_after_sub ~subst (t1, 0) (t2, 1) then
        true
      else (
        let renaming = Subst.Renaming.create () in
        QCheck.Test.fail_reportf
          "subst=%a,@ t1=`%a`,@ t2=`%a`,@ t1σ=`%a`,@ t2σ=`%a`" Subst.pp subst
          T.ZF.pp t1 T.ZF.pp t2 T.ZF.pp
          (Subst.FO.apply renaming subst (t1, 0))
          T.ZF.pp
          (Subst.FO.apply renaming subst (t2, 1))
      )
    with Unif.Fail -> QCheck.assume_fail ()
  in
  QCheck.Test.make ~long_factor:20 ~count:15_000 ~name gen prop
...
let check_equal =
  ...
  let prop t = equal_after_sub ~subst:Subst.empty (t, 0) (t, 0) in
...
let check_variant2 =
  ...
  let prop (t0, t1) =
    try
      ...
      let renaming = Subst.Renaming.create () in
      let t0' =
        Subst.FO.apply renaming subst (t0, 0) |> Lambda.snf |> Lambda.eta_reduce
      in
      let t1' =
        Subst.FO.apply renaming subst (t1, 1) |> Lambda.snf |> Lambda.eta_reduce
      in
      T.equal t0' t1'
    with Unif.Fail -> QCheck.assume_fail ()
  in
...
let check_variant_sym =
  ...
  let prop (t0, t1) = Unif.FO.are_variant t0 t1 = Unif.FO.are_variant t1 t0 in
  QCheck.Test.make ~long_factor:20 ~name (Q.pair gen gen) prop
  ...
  let prop (t1, t2) =
    try
      ...
      if T.equal t1' t2' then
        ...
        QCheck.Test.fail_reportf "@[<v>subst=%a,@ t1'=`%a`,@ t2'=`%a`@]"
          Subst.pp subst T.ZF.pp t1' T.ZF.pp t2'
    with Unif.Fail -> QCheck.assume_fail ()
  in
  ...
  let prop (t1, t2) =
    try
      ...
      let t2' =
        S.FO.apply renaming subst (t2, 1) |> Lambda.snf |> Lambda.eta_reduce
      in
      if Unif.FO.are_variant t2 t2' then
        true
      else
        QCheck.Test.fail_reportf "@[<v>subst=%a,@ t2'=`%a`@]" Subst.pp subst
          T.ZF.pp t2'
    with Unif.Fail -> QCheck.assume_fail ()
  in
  QCheck.Test.make ~long_factor:20 ~count:1000 ~name gen prop
...
let check_matching_variant_same_scope =
  ...
  let prop (t1, t2) =
    try
      ...
      let t2' =
        S.FO.apply Subst.Renaming.none subst (t2, 0)
        |> Lambda.snf |> Lambda.eta_reduce
      in
      if Unif.FO.are_variant t2 t2' then
        true
...
let check_variant_bidir_match =
  ...
  let prop (t0, t1) =
    if Unif.FO.are_variant t0 t1 then
...
let check_lits_variant_bidir_match =
  ...
  let prop (lits0, lits1) =
    if Literals.are_variant lits0 lits1 then
...
let check_ho_unify_gives_unifiers =
  ...
  let prop (t1, t2) =
    let offset =
      Iter.doubleton t1 t2 |> Iter.flat_map T.Seq.vars |> T.Seq.max_var |> succ
    in
    (* only keep proper solutions *)
    let l =
      HO_unif.unif_pairs ~fuel:20 ~offset ([ [], t1, t2 ], 0)
      |> List.filter (fun (pairs, us, _, _) ->
             pairs = [] && not (Unif_subst.has_constr us))
    in
    if l = [] then
      QCheck.assume_fail ()
    else (
      List.iter
        (fun (_, us, _, _) ->
          ...
          in
          let u2 =
            Subst.FO.apply renaming subst (t2, 0)
            |> Lambda.snf |> Lambda.eta_reduce
          in
          if not (T.equal u1 u2) then
            QCheck.Test.fail_reportf
              "(@[<hv2>bad solution@ t1'=`%a`@ t2'=`%a`@ :subst %a@])" T.ZF.pp
              u1 T.ZF.pp u2 Subst.pp subst)
        l;
      true
    )
  in
