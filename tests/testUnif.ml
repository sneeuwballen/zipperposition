...
let psterm, pstmt, pstmt_l, clear_scope, unif_ty =
  ...
  and pst_l s =
    let l =
      Parse_zf.parse_statement_list Lex_zf.token (Lexing.from_string s)
    in
    let l =
...
let pterm2 ?(unif_types = true) ?ty s1 s2 =
  let t1 = psterm s1 in
  let t2 = psterm s2 in
  if unif_types then
    unif_ty t1 t2;
...
module Task : sig
  ...
  let pp out = function
    ...
    | Unif {t1; t2; with_ty = Some ty; _} ->
      Format.fprintf out "(%s, %s) : %s" t1 t2 ty

  let parse_ = function
    | Unif {with_ty; t1; t2; unif_types; _} ->
      pterm2 ~unif_types ?ty:with_ty t1 t2
...
let check_matches ?(msg = "") t u =
  if Unif.FO.matches ~pattern:t u then
    ...
    Alcotest.failf "@[<2>`%a`@ should match@ `%a`%s@]@." T.ZF.pp t T.ZF.pp u
      msg
...
let check_unify_correct t u =
  ( "check unify is correct",
    `Quick,
    fun () ->
      let t', u', _, _ = unifier2 t u in
      let msg =
        Fmt.sprintf "(@[unify_correct `%a`@ `%a`@])" T.ZF.pp t T.ZF.pp u
      in
      check_eq ~msg t' u' )
...
let check_unifier t u ~res =
  ( "check unifier",
    ...
    fun () ->
      ...
      let msg =
        Fmt.sprintf "(@[unify `%a`@ `%a`@ :gives `%a`@])" T.ZF.pp t T.ZF.pp u
          T.ZF.pp res
      in
      check_variant ~msg t' res )

let check_unifier_matches t u =
  ( "check unifier matches unified terms",
    `Quick,
    fun () ->
      let t' = unifier t u in
      let msg =
        Fmt.sprintf "(@[unify_matches `%a`@ `%a`@])" T.ZF.pp t T.ZF.pp u
      in
      check_matches ~msg t t';
      check_matches ~msg u t' )

let check_same t u t1 sc1 t2 sc2 =
  ( "check unify makes same",
    `Quick,
    fun () ->
...
let suite_unif1 : unit Alcotest.test_case list =
  ...
  let mk_tests (pair, actions) =
    ...
    if Task.is_negated pair then
      check_unifiable ~negated:true t u :: List.map (Action.check t u) actions
    else
      check_unifiable t u :: check_unify_correct t u
      :: check_unifier_matches t u
      :: List.map (Action.check t u) actions
  in
  CCList.flat_map mk_tests
    [ ( "f X b" =?= "f a Y",
        [Action.yield "f a b"; Action.eq "X" 0 "a" 0; Action.eq "Y" 1 "b" 0] );
      ( "F a" =?= "f a (g (g a))",
      ...
      ( "X" =?= "(fun Y. X1) (fun (x y:term). c)" >-> "term",
        [Action.yield "Y" >?-> "term"] );
      ( "p_ho2 (fun a. F1 a) (fun a. F2 a)"
        =?= "p_ho2 (fun a. G a) (fun a. G a)",
  ...
  ( "regression matching",
    ...
    fun () ->
      ...
      try
        ...
        Alcotest.failf "@[<hv>`%a`@ and `%a@ should not match@]" T.ZF.pp t1
          T.ZF.pp t2
      with Unif.Fail -> () )
  ...
  let prop (t1, t2) =
    try
      ...
      if T.equal t1' t2' then
        ...
        QCheck.Test.fail_reportf "subst=%a,@ t1'=`%a`,@ t2'=`%a`" Subst.pp
          subst T.ZF.pp t1' T.ZF.pp t2'
...
let check_unifier_matches =
  ...
  let prop (t1, t2) =
    try
      ...
      let t1' = S.FO.apply renaming subst (t1, 0) |> Lambda.snf in
      let t2' = S.FO.apply renaming subst (t2, 1) |> Lambda.snf in
      if Unif.FO.matches ~pattern:t1 t1' && Unif.FO.matches ~pattern:t2 t2'
      then
        true
      else
...
let check_unify_makes_eq =
  ...
  let prop (t1, t2) =
    try
      let subst = Unif.FO.unify_syn (t1, 0) (t2, 1) in
      if Unif.FO.equal ~subst (t1, 0) (t2, 1) then
        true
      else
...
let check_equal =
  ...
  let prop t = Unif.FO.equal ~subst:Subst.empty (t, 0) (t, 0) in
