...
let pp l =
  let pp out (t, i) =
    Format.fprintf out "@[<2>`@[<2>%a@]`@ -> %d@]" T.pp t i
  in
...
module TestUnit (I : UnitIndex) = struct
  ...
  let check_all_matching_are_retrieved =
    ...
    in
    let name =
      CCFormat.sprintf "index(%s)_all_matching_are_retrieved" I.name
    in
    QCheck.Test.make ~long_factor ~name (arb 50 150) prop
...
module TestTerm (I : TermIndex) = struct
  ...
  let check_retrieved_unify =
    ...
    QCheck.Test.make ~name ~count:_count ~max_gen:_limit
      (arb arb_low_ arb_high_) prop
  ...
  let check_retrieve_all_generalizations =
    let prop =
      _check_all_satisfying_are_retrieved I.retrieve_generalizations
        _match_flip
    in
    let name =
      CCFormat.sprintf "index(%s)_retrieve_imply_generalizations" I.name
