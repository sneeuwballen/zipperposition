    ...
    let prop l =
      ...
      List.for_all
        (fun (t, _) ->
          ...
          List.for_all (fun (t', _) -> Unif.FO.matches ~pattern:t' t) retrieved)
        l
    in
    ...
    let prop l =
      ...
      List.for_all
        (fun (t, _) ->
          ...
          List.for_all
            (fun (t', _) ->
              if Unif.FO.matches ~pattern:t' t then
                List.exists (fun (t'', _) -> T.equal t' t'') retrieved
              else
                true)
            l)
        l
...
module TestTerm (I : TermIndex) = struct
  ...
  let find_all retrieve idx s_idx t s_t =
    ...
    |> Iter.fold (fun acc (t', i, _) -> (t', i) :: acc) []
        ...
        List.for_all
          (fun (t', i') ->
            try
              ...
              let res = List.exists (fun (_, i'') -> i' = i'') retrieved in
              res
            with Unif.Fail -> true)
          l)
      l

  let _match_flip ?subst t1 t2 = Unif.FO.matching ?subst ~pattern:t2 t1
  ...
  let check_retrieved_unify =
    let prop =
      _check_all_retrieved_satisfy I.retrieve_unifiables Unif.FO.unify_syn
    in
    let name = CCFormat.sprintf "index(%s)_retrieve_imply_unify" I.name in
    QCheck.Test.make ~long_factor ~name ~count ~max_gen (arb arb_low_ arb_high_)
      prop

  let check_retrieved_specializations =
    let prop =
      _check_all_retrieved_satisfy I.retrieve_specializations (fun t1 t2 ->
          Unif.FO.matching ~pattern:t1 t2)
    in
    let name =
      CCFormat.sprintf "index(%s)_retrieve_imply_specializations" I.name
    in
    QCheck.Test.make ~long_factor ~name ~count ~max_gen (arb arb_low_ arb_high_)
      prop
  ...
  let check_retrieved_generalizations =
    let prop =
      _check_all_retrieved_satisfy I.retrieve_generalizations _match_flip
    in
    let name =
      CCFormat.sprintf "index(%s)_retrieve_imply_generalizations" I.name
    in
    QCheck.Test.make ~long_factor ~name ~count ~max_gen (arb arb_low_ arb_high_)
      prop
  ...
  let check_retrieve_all_unify =
    let prop =
      _check_all_satisfying_are_retrieved I.retrieve_unifiables
        Unif.FO.unify_syn
    in
  ...
  let check_retrieve_all_specializations =
    let prop =
      _check_all_satisfying_are_retrieved I.retrieve_specializations
        (fun t1 t2 -> Unif.FO.matching ~pattern:t1 t2)
    in
    let name =
      CCFormat.sprintf "index(%s)_retrieve_imply_all_specializations" I.name
    in
    QCheck.Test.make ~long_factor ~name ~count ~max_gen (arb arb_low_ arb_high_)
      prop
  ...
  let check_retrieve_all_generalizations =
    let prop =
      _check_all_satisfying_are_retrieved I.retrieve_generalizations _match_flip
    in
    let name =
      CCFormat.sprintf "index(%s)_retrieve_imply_all_generalizations" I.name
    in
    QCheck.Test.make ~long_factor ~name ~count ~max_gen (arb arb_low_ arb_high_)
      prop
...
let props =
  let module DT = struct
    include Dtree.Make (E)

    let name = "dtree"
  end in
  let module TestDtree = TestUnit (DT) in
  let module NPDT = struct
    include NPDtree.Make (E)
