  ...
  ( "whnf1",
    ...
    fun () ->
      ...
      let redex =
        ...
        T.app
          (T.fun_ ty2
             (f (T.app (T.bvar ~ty:ty2 0) [a]) (T.app (T.bvar ~ty:ty2 0) [b])))
          [g_eta]
      in
...
let suite : unit Alcotest.test_case list =
  [test_db_shift; test_db_unshift; test_whnf1; test_whnf2; test_polymorphic_app]
...
let check_size_subterm =
  ...
  let gen =
    QCheck.Gen.(
      gen_t >>= fun t -> ArTerm.pos t >>= fun pos -> return (t, T.Pos.at t pos))
  in
...
let check_replace_id =
  let gen =
    QCheck.Gen.(gen_t >>= fun t -> ArTerm.pos t >>= fun pos -> return (t, pos))
  in
    ...
    if T.DB.is_closed t then
      if T.equal t1 t2 then
        ...
        QCheck.Test.fail_reportf
          "@[t: `@[%a@]`@ snf: `@[%a@]`,@ ref: `@[%a@]@]@." T.ZF.pp t T.ZF.pp
          t1 T.ZF.pp t2
    else
      QCheck.assume_fail ()
  in
