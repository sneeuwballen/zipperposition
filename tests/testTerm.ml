...
let test_db_shift =
  ( "db shift",
    ...
    fun () ->
      ...
      let t' =
        T.of_term_unsafe (InnerTerm.DB.shift 1 (t : T.t :> InnerTerm.t))
      in
...
let test_db_unshift =
  ( "db unshift",
    ...
    fun () ->
      ...
      let t' =
        T.of_term_unsafe (InnerTerm.DB.unshift 1 (t : T.t :> InnerTerm.t))
      in
...
let test_covers =
  ( "cover_correct",
    ...
    fun () ->
      ...
      let str = CCFormat.sprintf "%a.\n" T.pp max_cover in
      Alcotest.(check t_test)
        str max_cover
        (T.fun_ ty (h y (T.bvar ~ty 2) (T.bvar ~ty 1)));
      () )
...
let test_patterns =
  ( "patterns",
    ...
    fun () ->
      ...
      let t3 =
        T.fun_ ty (f (fun_var (T.bvar ~ty 0) (T.bvar ~ty 1)) (g (T.bvar ~ty 1)))
      in
      let t4 =
        T.fun_ ty (f (fun_var (T.bvar ~ty 0) (T.bvar ~ty 0)) (g (T.bvar ~ty 1)))
      in
      let t5 =
        T.fun_ ty
          (f (fun_var (T.bvar ~ty 0) (g (T.bvar ~ty 1))) (g (T.bvar ~ty 1)))
      in
...
let test_eta_reduce =
  ( "eta reduce",
    ...
    fun () ->
      ...
      let t1 =
        T.fun_of_fvars
          [ HVar.make ~ty 0; HVar.make ~ty 1 ]
          (T.app f_fun [ T.var_of_int ~ty 0; T.var_of_int ~ty 1 ])
      in
      let t2 = f_fun in
      ...
      let t1 =
        T.fun_of_fvars
          [ HVar.make ~ty 0 ]
          (T.app
             (T.fun_of_fvars [ HVar.make ~ty 1 ] (g (T.var_of_int ~ty 1)))
             [ T.var_of_int ~ty 0 ])
      in
      let t2 = g_fun in
      Alcotest.(check t_test) "eta reduce" (Lambda.eta_reduce t1) t2;
      ...
      let t1 =
        T.fun_of_fvars
          [ HVar.make ~ty 0; HVar.make ~ty 1 ]
          (f (T.var_of_int ~ty 1) (T.var_of_int ~ty 0))
      in
      let t2 = t1 in
      Alcotest.(check t_test) "eta reduce" (Lambda.eta_reduce t1) t2;
      ...
      let t1 =
        T.fun_of_fvars
          [ HVar.make ~ty 0; HVar.make ~ty 1 ]
          (h (g (T.var_of_int ~ty 0)) (T.var_of_int ~ty 0) (T.var_of_int ~ty 1))
      in
      let t2 =
        T.fun_of_fvars
          [ HVar.make ~ty 0 ]
          (T.app h_fun [ g (T.var_of_int ~ty 0); T.var_of_int ~ty 0 ])
      in
      Alcotest.(check t_test) "eta reduce" (Lambda.eta_reduce t1) t2;
     ...
     In HOL, we actually need to expand this subterm *)
      let lamx_px =
        T.fun_of_fvars [ HVar.make ~ty 0 ] (T.app p_fun [ T.var_of_int ~ty 0 ])
      in
     ...
     We expand on the top level, but further down we reduce *)
      ...
      let reduced =
        ...
        @@ T.fun_of_fvars
             [ HVar.make ~ty 0 ]
             (T.app q_fun [ p_fun; T.var_of_int ~ty 0 ])
      in
      Alcotest.(check t_test) "eta reduce" (Lambda.eta_reduce unreduced) reduced
  )
  ...
  ( "eta expand",
    ...
    fun () ->
      ...
      let expanded =
        T.fun_of_fvars
          [ HVar.make ~ty 0 ]
          (T.app l_fun [ subterm; T.var_of_int ~ty 0 ])
      in
      let unexpanded = l f_fun in
      Alcotest.(check t_test)
        "eta expand" expanded
        (Lambda.eta_expand unexpanded) )
...
let check_size_subterm =
  ...
  let gen =
    QCheck.Gen.(
      gen_t >>= fun t ->
      ArTerm.pos t >>= fun pos -> return (t, T.Pos.at t pos))
  in
  let pp = QCheck.Print.(pair T.to_string T.to_string) in
  let gen = QCheck.make ~print:pp gen in
  let prop (t1, t2) = T.subterm ~sub:t2 t1 && T.size t1 >= T.size t2 in
...
let check_ground_novar =
  let gen = ar_t in
  let prop t =
    (not (T.is_ground t)) || Iter.is_empty (T.Seq.vars t)
    (* ground => no vars *)
  in
...
let check_hash_mod_alpha =
  ...
  let prop (t1, t2) =
    if (not (T.equal t1 t2)) && Unif.FO.are_variant t1 t2 then
      T.hash_mod_alpha t1 = T.hash_mod_alpha t2
    else
      QCheck.assume_fail ()
  in
  QCheck.Test.make gen prop ~count:10_000 ~long_factor:20
...
let num_lam t = T.Seq.subterms t |> Iter.filter is_fun |> Iter.length
let num_var_app t = T.Seq.subterms t |> Iter.filter T.is_ho_app |> Iter.length
...
let t_show2_p pp (t, u) =
  CCFormat.sprintf "(@[`@[%a@]`,@ ref: `@[%a@]`@])" pp t T.ZF.pp u
...
let check_whnf_closed =
  ...
  in
  QCheck.Test.make gen prop ~count:10_000 ~long_factor:10
    ~name:"whnf_preserve_closed"

let check_whnf_non_redex_preserved =
  let gen = gen_ho in
  let prop t =
    match T.as_app t with
...
let check_whnf_correct =
  ...
  let prop (t, (t1, t2)) =
    if T.DB.is_closed t then
      ...
      QCheck.assume_fail ()
  in
  QCheck.Test.make gen prop ~count:10_000 ~long_factor:10 ~name:"whnf_correct"

let check_snf_closed =
  let gen = QCheck.map_keep_input ~print:t_show Lambda.snf gen_ho in
  let prop (t, t') =
    if not (T.DB.is_closed t) then
      QCheck.assume_fail ()
    else
      T.DB.is_closed t'
  in
...
module SNF = struct
  let eval_and_unshift ~by t =
    let rec aux depth t =
      ...
      | T.App (f, l) -> T.app (aux depth f) (List.map (aux depth) l)
      | T.AppBuiltin (b, l) ->
        T.app_builtin ~ty:(T.ty t) b (List.map (aux depth) l)
      | T.Fun (tyarg, body) -> T.fun_ tyarg (aux (depth + 1) body)
    in
    aux 0 t

  let rec snf_naive (t : T.t) : T.t =
    match T.view t with
    | T.Var _ | T.DB _ | T.Const _ -> t
    | T.Fun (ty_var, bod) ->
      let bod = snf_naive bod in
      T.fun_ ty_var bod
    | T.AppBuiltin (b, l) -> T.app_builtin ~ty:(T.ty t) b (List.map snf_naive l)
    | T.App (f, l) ->
    ...
    if T.DB.is_closed t then
      if T.equal t1 t2 then
        ...
        QCheck.Test.fail_reportf
          "@[t: `@[%a@]`@ snf: `@[%a@]`,@ ref: `@[%a@]@]@." T.ZF.pp t T.ZF.pp t1
          T.ZF.pp t2
    else
      QCheck.assume_fail ()
  in
  QCheck.Test.make gen_ho prop ~count:10_000 ~long_factor:10 ~name:"snf_correct"
...
let check_snf_idempotent =
  let prop t =
    ...
    if T.equal t1 t2 then
      ...
      QCheck.Test.fail_reportf
        "@[t: `@[%a@]`@ snf: `@[%a@]`,@ snf2: `@[%a@]@]@." T.ZF.pp t T.ZF.pp t1
        T.ZF.pp t2
  in
  QCheck.Test.make gen_ho prop ~count:10_000 ~long_factor:10
    ~name:"snf_idempotent"
  ...
  let prop t =
    ...
    if T.DB.is_closed t then (
      ...
      if T.equal t1 t2 then
        ...
        QCheck.Test.fail_reportf
          "@[t: `@[%a@]`@ fun_var_app: `@[%a@]`,@ snf_naive: `@[%a@]`@ snf: \
           `@[%a@]`@]@."
