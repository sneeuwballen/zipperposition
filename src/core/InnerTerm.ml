...
let update_max_db props new_db =
  if new_db > max_db then set_property props f_db_overflowed
  else
...
let _hash_norec t =
  match view t with
  | Var v ->
      Hash.combine2 1 (HVar.hash v)
  | DB v ->
      Hash.combine2 2 (Hash.int v)
  | Bind (b, varty, t') ->
      Hash.combine4 3 (Binder.hash b) (hash varty) (hash t')
  | Const s ->
      Hash.combine2 4 (ID.hash s)
  | App (f, l) ->
      Hash.combine3 10 (hash f) (Hash.list hash l)
  | AppBuiltin (b, l) ->
      Hash.combine3 20 (Builtin.hash b) (Hash.list hash l)
...
let hash_mod_alpha t : int =
  let rec aux (d : int) t =
    ...
    and h_ty = match t.ty with NoType -> 0 | HasType ty -> aux d ty in
...
let rec _eq_norec_list l1 l2 =
  ...
  | [], _ | _, [] ->
      false
  | t1 :: l1', t2 :: l2' ->
      equal t1 t2 && _eq_norec_list l1' l2'
...
let rec _eq_record_list l1 l2 =
  ...
  | [], _ | _, [] ->
      false
  | (n1, t1) :: l1', (n2, t2) :: l2' ->
      n1 = n2 && equal t1 t2 && _eq_record_list l1' l2'
  ...
  let rec aux acc = function
    ...
    | x :: xs -> (
      ...
      | _ ->
          aux (x :: acc) xs )
  in
  if Builtin.is_flattened_logical b then aux [] l else l
  ...
  | AppBuiltin (f1, l1), _ ->
      ...
      let ty =
        if Builtin.is_logical_op f1 && not (Builtin.is_quantifier f1) then
          ...
          else if List.length t_args = 1 then prop
...
module DB = struct
  ...
  let rec _to_iter ~depth t k =
    (match t.ty with NoType -> () | HasType ty -> _to_iter ~depth ty k) ;
    match view t with
    | DB v ->
        k (v, depth)
    | Var _ | Const _ ->
  ...
  let shift_real ?depth n t =
    assert (n >= 0) ;
    _fold_map ?depth ()
      ~on_bvar:(fun ~depth () i -> if i >= depth then i + n (* shift *) else i)
  ...
  let rec _replace depth ~to_replace t =
    ...
    | HasType ty -> (
        ...
        | _ when CCList.exists (equal t) to_replace -> (
          match CCList.find_idx (equal t) to_replace with
          | None ->
              assert false
          | Some (i, t') ->
              assert (equal t t') ;
  ...
  let eval env t = if DBEnv.is_empty env then t else _eval env t
  ...
  let subterms t k =
    let rec subterms t =
      ...
      | Var _ | DB _ | Const _ ->
          ()
      | Bind (_, varty, t') ->
          subterms varty ; subterms t'
      | AppBuiltin (_, l) ->
          List.iter subterms l
      | App (f, l) ->
          subterms f ; List.iter subterms l
    in
    subterms t
  ...
  let symbols t k =
    let rec symbols t =
      ...
      | Const s ->
          k s
      | App (head, l) ->
          symbols head ; List.iter symbols l
      | AppBuiltin (_, l) ->
          List.iter symbols l
      | Bind (_, varty, t') ->
          symbols varty ; symbols t'
    in
    symbols t

  let types t k =
    let rec types t =
      (match t.ty with NoType -> () | HasType ty -> k ty) ;
      match view t with
      | Var _ | DB _ | Const _ ->
          ()
      | App (head, l) ->
          types head ; List.iter types l
      | AppBuiltin (_, l) ->
          List.iter types l
      | Bind (_, _, t') ->
          types t'
...
module Pos = struct
  ...
  let rec at t pos =
    ...
    | _, P.Type pos' -> (
      match t.ty with NoType -> fail_ t pos | HasType ty -> at ty pos' )
    | _, P.Stop ->
        t
    | Var _, _ ->
        fail_ t pos
    | Bind (_, _, t'), P.Body subpos ->
        at t' subpos
    | App (t, _), P.Head subpos ->
        at t subpos
  ...
  let rec replace t pos ~by =
    match (t.ty, view t, pos) with
    | _, _, P.Stop ->
        by
    | NoType, _, P.Type _ ->
        fail_ t pos
...
let replace_m t m =
  let rec aux depth t =
    match Map.get (DB.shift depth t) m with
    | Some u ->
...
let fun_of_fvars vars body =
  if vars = [] then body
    ...
    List.fold_right (fun v body -> fun_ (HVar.ty v) body) vars body

let open_bind_fresh b t =
  let rec aux env vars t =
    match view t with
...
let rec returns ty =
  ...
  | Bind (Binder.ForallTy, _, ty') ->
      returns ty'
  | AppBuiltin (Builtin.Arrow, ret :: _) ->
      ret
  | _ ->
      ty
...
let rec head t =
  ...
  | Const s ->
      Some s
  | DB _ | Var _ | Bind (_, _, _) | AppBuiltin (_, _) ->
      None
  | App (h, _) ->
      head h
...
let rec pp_depth ?(hooks = []) depth out t =
  ...
  and _pp_root depth out t =
    ...
    | AppBuiltin (b, l) ->
        let l =
          if Builtin.is_combinator b && not !show_type_arguments then
            List.filter (fun t -> not (is_tType @@ ty_exn t)) l
          else l
        in
    ...
    | App (f, l) -> (
        ...
        | _ ->
            Format.fprintf out "@[<1>%a@ %a@]" (_pp_surrounded depth) f
              (Util.pp_list ~sep:" " (_pp_surrounded depth))
              l )
  and _pp_surrounded depth out t =
    match view t with
    | App (_, l)
      when (not !show_type_arguments)
        ...
        _pp depth out t
    | Bind _ | AppBuiltin (_, _ :: _) | App (_, _ :: _) ->
        Format.fprintf out "(@[%a@])" (_pp depth) t
    | _ ->
        _pp depth out t
  in
  _pp depth out t
...
and pp_var out v =
  ...
  | _ when needs_args ty ->
      Format.fprintf out "F%d" (HVar.id v)
  | _ ->
      HVar.pp out v
...
let rec pp_zf out t =
  ...
  and _pp_surrounded depth out t =
    match view t with
    | Bind _ | AppBuiltin (_, _ :: _) | App (_, _ :: _) ->
        Format.fprintf out "(@[%a@])" (pp_ depth) t
    | _ ->
        pp_ depth out t
  in
  pp_ 0 out t
