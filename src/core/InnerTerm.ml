...
let make_ ~ty term = {term; ty; id = ~-1; payload = No_payload}
...
module DB = struct
  ...
  let rec _to_seq ~depth t k =
    ...
    | AppBuiltin (_, l) -> List.iter (fun t -> _to_seq ~depth t k) l
    | App (f, l) ->
      _to_seq ~depth f k;
    ...
    | HasType ty ->
      ...
      | DB i ->
        if i < depth then
          ...
          bvar ~ty (i + List.length to_replace) (* shift *)
      | Const s -> const ~ty s
      | Bind (s, varty, t') ->
        ...
        | DB i ->
          ...
          | None ->
            if i >= DBEnv.size env then
              bvar ~ty (i - DBEnv.size env0) (* unshift *)
            else
              bvar ~ty i
          | Some t' ->
...
let rec pp_depth ?(hooks = []) depth out t =
  ...
  and _pp_root depth out t =
    ...
    | AppBuiltin (b, ([_; t1; t2] | [t1; t2])) when Builtin.is_infix b ->
      Format.fprintf out "(@[<1>%a@ %a@ %a@])" (_pp depth) t1 Builtin.pp b
        (_pp depth) t2
    | AppBuiltin (b, []) -> Builtin.pp out b
    | AppBuiltin (b, l) ->
...
let rec pp_zf out t =
  let rec pp_ depth out t =
    ...
    | AppBuiltin (b, ([_; t1; t2] | [t1; t2])) when Builtin.is_infix b ->
      Format.fprintf out "(@[<1>%a@ %a@ %a@])" (pp_ depth) t1 Builtin.ZF.pp b
        (pp_ depth) t2
