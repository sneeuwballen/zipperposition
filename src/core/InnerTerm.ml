...
let rec same_l_rec l1 l2 =
  ...
  | [], [] -> true
  | [], _ | _, [] -> assert false
  | x1 :: tail1, x2 :: tail2 -> equal x1 x2 && same_l_rec tail1 tail2
...
and ho_weight_uncached_ t t_ty : int =
  ...
  | Bind (Binder.Lambda, _, t') ->
    ...
    init_w (ty t') + ho_weight t'
  | Bind (_, _, t') -> ho_weight t'
  | App (f, l) ->
    if is_var f then
      1
    else
      init_w t_ty + ho_weight_l_ (ho_weight f) l
  | AppBuiltin (_, l) -> ho_weight_l_ (init_w t_ty + 1) l

and ho_weight_l_ acc = function
  | [] -> acc
  | t :: ts ->
...
let flatten_and_or b l =
  let rec aux acc = function
    ...
    | x :: xs ->
      (match view x with
      | AppBuiltin (b', ys) when Builtin.equal b b' -> aux acc (ys @ xs)
      | _ -> aux (x :: acc) xs)
  in
    ...
    let ty =
      if Builtin.is_logical_op f1 && not (Builtin.is_quantifier f1) then (
        ...
        if List.length args > 0 then
          ...
          if List.length t_args >= 2 then
            prop
          else if List.length t_args = 1 then
            arrow [ prop ] prop
          else
...
module DB = struct
  ...
  let rec _to_iter ~depth t k =
    ...
    | AppBuiltin (_, l) -> List.iter (fun t -> _to_iter ~depth t k) l
    | App (f, l) ->
      _to_iter ~depth f k;
  ...
  let rec _replace depth ~to_replace t =
    ...
    | HasType ty ->
      ...
      | _ when CCList.exists (equal t) to_replace ->
        ...
        | Some (i, t') ->
          assert (equal t t');
          bvar ~ty (depth + List.length to_replace - i - 1))
        (* replace *)
      | Var v -> var (HVar.cast ~ty v)
      | DB i ->
        if i < depth then
          ...
          bvar ~ty (i + List.length to_replace)
        (* shift *)
      | Const s -> const ~ty s
      | Bind (s, varty, t') ->
    ...
    let rec _eval env t =
      ...
      | HasType ty ->
        ...
        | DB i ->
          ...
          | None ->
            if i >= DBEnv.size env then
              bvar ~ty (i - DBEnv.size env0)
            (* unshift *)
            else
              bvar ~ty i
          | Some t' ->
        ...
        | Bind (s, varty, t') ->
          ...
          bind ~ty ~varty:varty' s t'
        | App (f, l) -> app ~ty (_eval env f) (List.map (_eval env) l)
        | AppBuiltin (s, l) -> app_builtin ~ty s (List.map (_eval env) l))
    in
    _eval env0 t

  let eval env t =
    if DBEnv.is_empty env then
      t
    else
      _eval env t
  ...
  let apply_subst subst t =
    ...
    and aux' depth ty t =
      ...
      | Bind (s, varty, t') ->
        ...
        bind ~ty ~varty:varty' s t'
      | App (f, l) -> app ~ty (aux depth f) (List.map (aux depth) l)
      | AppBuiltin (s, l) -> app_builtin ~ty s (List.map (aux depth) l)
    in
    aux 0 t
end
...
module Pos = struct
  ...
  let rec replace t pos ~by =
    ...
    | HasType ty, App (f, l), P.Head subpos -> app ~ty (replace f subpos ~by) l
...
let[@inline] as_bvar_exn t =
  match view t with
  | DB i -> i
  | _ -> invalid_arg "as_bvar_exn"

let[@inline] is_bvar_i i t =
  match view t with
  | DB j -> i = j
  | _ -> false
...
let rec pp_depth ?(hooks = []) depth out t =
  ...
  and _pp_root depth out t =
    ...
    | AppBuiltin (b, [ t1; t2 ]) when Builtin.is_infix b ->
      Format.fprintf out "(@[%a %s@ %a@])" (_pp depth) t1 (Builtin.to_string b)
        (_pp depth) t2
    ...
    | AppBuiltin (b, l) ->
      ...
      in
      if CCList.is_empty l then
        Format.fprintf out "@[%a@]" Builtin.pp b
      else
    ...
    | App (f, l) ->
      ...
      let l =
        if !show_type_arguments || is_tType (ty_exn t) then
          l
        else
          List.filter (fun t -> not (is_tType @@ ty_exn t)) l
      in
