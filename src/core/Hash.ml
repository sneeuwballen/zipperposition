...
let combine3 a b c = combine2 a b |> combine2 c

let combine4 a b c d = combine2 a b |> combine2 c |> combine2 d

let combine5 a b c d e = combine2 a b |> combine2 c |> combine2 d |> combine2 e

let pair f g (x, y) = combine2 (f x) (g y)

let opt f = function None -> 42 | Some x -> combine2 43 (f x)
