...
(** {1 Type Inference}
    ...
    - inferring the types of symbols that have not been declared (e.g. in "fof"
      or "cnf" TPTP statements) so as to enrich a {!Signature}
    - converting {i untyped} terms or formulas into {i typed} formulas, by
    ...
    with the declared constructor [nil : list(A)]. The inferred type for [f]
    ...
    Here we use a single scope when we unify and substitute type variables, the
    scope 0.

    Many functions will use an Error monad to make errors explicit. The error
    type is {!or_error}. The module {!CCError} in containers can be used to deal
    with errors (including monadic operators). *)
...
(** {2 Typing context}
    ...
    This module provides a typing context, with an applicative interface. The
    context is used to map terms to types locally during type inference. It also
    keeps and updates a signature when symbols' types are inferred.

    This module is quite low-level, and shouldn't be used in simple cases (see
...
module Ctx : sig
  ...
  (** New context with a signature and default types.
      @param default
        which types are inferred by default (if not provided then {!type_erm}
        will be used)
      @param def_as_rewrite
  ...
  (** Exit the current scope (formula, clause), meaning that all free variables'
      types are forgotten. Some free variables are bound to the [default] type
...
(** {2 Hindley-Milner Type Inference}
    ...
    This module, abstract in the exact kind of term it types, takes as input a
    signature and an {b untyped term}, and updates the typing context so that
    the {b untyped term} can be converted into a {b typed term}. *)
...
(** Infer the type of this term under the given signature. This updates the
    context's typing environment!
...
(** Safe version of {!infer_exn}. It returns [`Error s] rather than raising
    {!Error} if the typechecking fails. *)

val infer_prop_exn : Ctx.t -> untyped -> typed
(** Same as {!infer_exn} but forces the type of its result to be
    {!TypedSTerm.prop} *)
...
(** {3 Constraining types}
    ...
    This section is mostly useful for inferring a signature without converting
    untyped_terms into typed_terms. *)
...
(** [infer_statement ctx ~f st] checks and convert [st] into a typed statements,
    and a list of auxiliary type declarations for symbols that were inferred
    implicitly. *)
