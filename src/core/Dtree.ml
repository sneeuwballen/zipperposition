...
let compare_char c1 c2 =
  ...
  | Variable v1, Variable v2 ->
      compare_vars v1 v2
  | Subterm t1, Subterm t2 ->
      T.compare t1 t2
  | _ ->
      CCShims_.Stdlib.compare (char_to_int_ c1) (char_to_int_ c2)
...
let term_to_char (t : T.t) : character * T.t list =
  ...
  | ST.Var v ->
      (Variable (Type.cast_var_unsafe v), [])
  | _ when Type.is_fun (T.ty t) ->
      (Subterm t, [] (* partial app *))
  | ST.Const s ->
      (Symbol s, [])
  | ST.App (f, l) -> (
...
let pp_char out = function
  | Variable v ->
      Type.pp_typed_var out v
  | Symbol f ->
      ID.pp out f
  | Subterm t ->
      CCFormat.hbox T.pp out t
...
let rec next_rec stack len =
  match stack with
  | [] ->
      None
  | [] :: stack' ->
      next_rec stack' (len - 1)
  | (t :: next') :: stack' ->
...
module Make (E : Index.EQUATION) = struct
  ...
  let goto_leaf trie t k =
    ...
    let rec goto trie t rebuild =
      ...
      | [] -> (
        ...
        | new_leaf when trie.leaf == new_leaf ->
            root (* no change, return same tree *)
        | new_leaf ->
            rebuild {trie with leaf= new_leaf} (* replace by new leaf *) )
      | c :: t' -> (
        match CharMap.get c trie.map with
        | Some subtrie ->
            let rebuild' subtrie =
              match subtrie with
              ...
              | _ ->
                  rebuild {trie with map= CharMap.add c subtrie trie.map}
            in
  ...
  let remove dt eqn =
    ...
    let k l =
      ...
      List.filter (fun (t', eqn', _) -> t' != t || E.compare eqn eqn' <> 0) l
    in
    let tree = goto_leaf dt chars k in
    tree
  ...
  let retrieve ?(subst = S.empty) ~sign dt t k =
    ...
    let rec traverse trie iter subst =
      match iter with
      | None ->
          (* yield all equations, they all match *)
          List.iter
            (fun (_, eqn, _) ->
  ...
  let _as_graph = CCGraph.make (fun trie -> CharMap.to_iter trie.map)
