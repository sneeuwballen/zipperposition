...
type 'a num = {
  ...
  abs : 'a -> 'a;
  cmp : 'a -> 'a -> int;
  raise_to_lcm : 'a -> 'a -> 'a * 'a;
  (* factors to reach lcm *)
  hash : 'a Hash.t;
  zero : 'a;
  one : 'a;
  add : 'a -> 'a -> 'a;
...
let compare m1 m2 =
  let open CCOrd in
  let cmp_pair (s1, t1) (s2, t2) = m1.num.cmp s1 s2 <?> (T.compare, t1, t2) in
  m1.num.cmp m1.const m2.const <?> (CCOrd.list cmp_pair, m1.terms, m2.terms)

let hash m =
  Hash.combine3 42 (m.num.hash m.const)
...
let rec _merge ~num op l1 l2 =
  ...
  | (s1, t1) :: l1', (s2, t2) :: l2' ->
    ...
    | 0 ->
      let s' = op s1 s2 in
      if num.cmp num.zero s' = 0 then
        _merge ~num op l1' l2' (* t disappears *)
      else
        (s', t1) :: _merge ~num op l1' l2'
...
let _fmap f e =
  let terms =
    CCList.filter_map
      (fun (s, t) ->
        let s' = f s in
        if e.num.cmp e.num.zero s' = 0 then
          None (* [t] doesn't occur anymore *)
        else
          Some (s', t) )
      e.terms
  in
...
let mk_const ~num s = {num; const = s; terms = []}

let singleton ~num coeff t =
  assert (Type.equal (T.ty t) num.ty);
  if num.cmp num.zero coeff = 0 then
    mk_const ~num coeff (* 0 *)
  else (
...
let of_list ~num s l =
  List.fold_left (fun e (s, t) -> add e s t) (mk_const ~num s) l

let map f e =
  let const = {e with terms = []} in
  List.fold_left (fun e (n, t) -> add e n (f t)) const e.terms

let add_const e s = {e with const = e.num.add e.const s}

let remove e t =
  {e with terms = List.filter (fun (_, t') -> not (T.equal t t')) e.terms}

let remove_const e = {e with const = e.num.zero}

let add_list m l = List.fold_left (fun m (c, t) -> add m c t) m l

module Seq = struct
  let terms m k = List.iter (fun (_, t) -> k t) m.terms

  let vars m = Iter.flat_map T.Seq.vars (terms m)

  let coeffs m k = List.iter k m.terms

  let coeffs_swap m k = List.iter (fun (x, y) -> k (y, x)) m.terms
end
...
let var_occurs ~var e = List.exists (fun (_, t) -> T.var_occurs ~var t) e.terms
...
let difference e1 e2 = sum e1 (uminus e2)
...
let dominates ~strict m1 m2 =
  match comparison m1 m2 with
...
let split m =
  let const1, const2 =
    if m.num.sign m.const >= 0 then
      m.const, m.num.zero
    else
      m.num.zero, m.num.abs m.const
  in
  let rec partition = function
    | [] -> [], []
    | (c, t) :: l' ->
      let l1, l2 = partition l' in
      if m.num.sign c > 0 then
        (c, t) :: l1, l2
      else
        l1, (m.num.uminus c, t) :: l2
  in
...
let apply_subst renaming subst (m, sc) =
  map (fun t -> Subst.FO.apply renaming subst (t, sc)) m

let apply_subst_no_simp renaming subst (m, sc) =
  { m with
    terms =
      List.map (fun (c, t) -> c, Subst.FO.apply renaming subst (t, sc)) m.terms
  }

let is_ground m = List.for_all (fun (_, t) -> T.is_ground t) m.terms

let fold f acc m = CCList.foldi (fun acc i (n, t) -> f acc i n t) acc m.terms
...
let fold_max ~ord f acc m =
  (* set of max terms *)
  let max =
    Seq.terms m |> MT.Seq.of_seq MT.empty
    |> MT.max_seq (Ordering.compare ord)
    |> Iter.map fst |> T.Seq.add_set T.Set.empty
  in
  CCList.foldi
...
let pp_ ~mult ~add ~pp_t out e =
  let pp_pair out (s, t) =
    if e.num.cmp s e.num.one = 0 then
      pp_t out t
    else
      Format.fprintf out "%s %s %a" (e.num.to_string s) mult pp_t t
  in
  match e.terms with
  ...
  | _ :: _ ->
    Format.fprintf out "%a%s%s"
      (Util.pp_list ~sep:add pp_pair)
      e.terms add (e.num.to_string e.const)
...
let pp_tstp out e =
  let rec pp_pair out (s, t) =
    if e.num.cmp s e.num.one = 0 then
      ...
      Format.fprintf out "$product(%s, %a)" (e.num.to_string s) T.TPTP.pp t
  and pp_list buf l =
    match l with
    | [] -> ()
    | [(s, t)] -> pp_pair buf (s, t)
...
let nth m n = try List.nth m.terms n with _ -> _fail_idx m n
...
module Focus = struct
  ...
  let focus_term_exn m t =
    match focus_term m t with
  ...
  let to_monome t = add t.rest t.coeff t.term
  ...
  let pp out t =
    ...
    let pp_focused out t =
      if num.cmp num.one t.coeff = 0 then
        T.pp out t.term
      else
        Format.fprintf out "%s·%a" (num.to_string t.coeff) T.pp t.term
    in
    if is_zero t.rest then
      Format.fprintf out "[%a]" pp_focused t
    else
      Format.fprintf out "[%a] + %a" pp_focused t pp t.rest

  let is_max ~ord mf =
    List.for_all
      (fun (_, t) ->
        match Ordering.compare ord mf.term t with
  ...
  let _apply_subst how subst (mf, sc) =
    ...
    (* if [term] occurs in the new [rest], remove it and add its
       coefficient. *)
    let coeff, rest =
      if mem rest term then
        rest.num.add mf.coeff (find_exn rest term), remove rest term
      else
        mf.coeff, rest
    in
    if rest.num.sign coeff = 0 then
      failwith "Monome.Focus.apply_subst: coeff 0";
    {coeff; rest; term}
  ...
  let rec _iter_self ~num ~subst c t l rest const scope k =
    ...
    | (c', t') :: l' ->
      if
        Unif.FO.equal ~subst:(US.subst subst) (Scoped.make t scope)
          (Scoped.make t' scope)
      then (* we do not have a choice, [t = t'] is true *)
        _iter_self ~num ~subst (num.add c c') t l' rest const scope k
      else (
        ( try
            (* maybe we can merge [t] and [t'] *)
            let subst' =
              Unif.FO.unify_full ~subst (Scoped.make t scope)
                (Scoped.make t' scope)
            in
            (* move back [rest] into the main list, some terms might be equal
                 to [t] now *)
            _iter_self ~num ~subst:subst' (num.add c c') t (l' @ rest) [] const
              scope k
          with Unif.Fail -> () );
  ...
  let unify_self_monome ?(subst = US.empty) (m, sc) k : unit =
    let num = m.num in
    let rec choose_first subst l rest =
      match l with
      | [] -> ()
      | (c, t) :: l' ->
        choose_second subst c t l' rest;
        choose_first subst l' ((c, t) :: rest)
    and choose_second subst c t l rest =
      match l with
      | [] -> ()
      | (c', t') :: l' ->
  ...
  let unify_ff ?(subst = US.empty) (mf1, sc1) (mf2, sc2) k =
    ...
    try
      ...
      _iter_self ~num ~subst mf1.coeff mf1.term mf1.rest.terms []
        mf1.rest.const sc1 (fun (mf1, subst) ->
          _iter_self ~num ~subst mf2.coeff mf2.term mf2.rest.terms []
            mf2.rest.const sc2 (fun (mf2, subst) -> k (mf1, mf2, subst)) )
    with Unif.Fail -> ()
  ...
  let unify_mm ?(subst = US.empty) (m1, sc1) (m2, sc2) k =
    ...
    let rec choose_first subst l1 rest1 cst1 l2 rest2 cst2 k =
      match l1, l2 with
      | [], _ | _, [] -> ()
      | (c1, t1) :: l1', (c2, t2) :: l2' ->
...
let variant ?(subst = Subst.empty) (m1, sc1) (m2, sc2) k =
  assert (m1.num == m2.num);
  let rec traverse_lists subst (c1, t1) l1' rest2 l2 =
    match l2 with
    | [] -> () (* fail *)
    | (c2, t2) :: l2' ->
      if m1.num.cmp c1 c2 = 0 then (
        try
  ...
  let rec traverse_lists subst (c1, t1) l1' rest2 l2 =
    ...
    | (c2, t2) :: l2' ->
      if m1.num.cmp c1 c2 <= 0 then (
        try
          let subst =
            Unif.FO.matching_adapt_scope ~subst ~pattern:(t1, sc1) (t2, sc2)
          in
          if m1.num.cmp c1 c2 = 0 then
            start subst l1' (rest2 @ l2')
...
let of_term_exn (type a) (num : a num) t =
  ...
  let rec of_term t =
    ...
    | T.AppBuiltin (b, _) when Builtin.is_arith b -> raise NotLinear
    | T.AppBuiltin _ | T.Var _ | T.Const _ | T.Fun _ | T.App _ | T.DB _ ->
      singleton ~num num.one t
  in
  of_term t
...
module Int = struct
  ...
  let of_term t = try Some (of_term_exn t) with NotLinear -> None

  let mk_const n = T.builtin ~ty:num.ty (Builtin.mk_int n)

  (* a.t *)
  let mk_product a t =
    if num.equal a num.one then
      t
    else
  ...
  let normalize_wrt_zero m =
    if is_const m then
      ...
      if Z.equal Z.one gcd then
        m
      else
        _fmap (fun c -> Z.div c gcd) m )
  ...
  let reduce_same_factor m1 m2 t =
    try
      ...
      *)
      ...
      Util.debugf 5
        "@[reduce same factor:@ %a, %a have gcd %a,@ mult by %a, %a@]"
        (fun k -> k pp m1 pp m2 pp_z gcd pp_z d2 pp_z d1);
      product m1 d2, product m2 d1
    with Not_found -> raise (Invalid_argument "Monome.reduce_same_factor")
    ...
    if Z.sign c <= 0 then
      ...
      try
        Some
          (_fmap
             (fun s ->
               if Z.(equal (erem s c) zero) then Z.divexact s c else raise Exit
               )
             e)
      with Exit -> None )
  ...
  let factorize e =
    let gcd =
      if Z.equal e.const Z.zero then (
        match e.terms with
        | [] -> Z.one
        | (c, _) :: terms' ->
          List.fold_left (fun gcd (c, _) -> Z.gcd c gcd) c terms' )
      else
        List.fold_left (fun gcd (c, _) -> Z.gcd c gcd) e.const e.terms
    in
    let gcd = Z.abs gcd in
    if Z.equal Z.one gcd || Z.sign gcd = 0 then
      None
    else (
      match quotient e gcd with
      | None -> assert false
      | Some e' -> Some (e', gcd) )
  ...
  module Solve = struct
    ...
    let diophant2 a b const =
      ...
      let u, v, gcd =
        ...
        if Z.sign r <> 0 then
          failwith
            (CCFormat.sprintf
               "unsolvable diophantine equation %a x + %a y = %a" pp_z a pp_z b
               pp_z const)
        else (
    ...
    let rec diophant_l l const =
      ...
      | a1 :: a2 :: l' ->
        ...
        (* first, solve [a1 * u1 + a2 * u2 = gcd_1_2]. We then
            ...
            and we're done. *)
        ( match diophant_l (gcd_1_2 :: l') const with
        | [], _ -> assert false
        | u_1_2 :: u', gcd ->
        ...
        List.mapi
          (fun i _li ->
            if i = 0 then (
              ...
              sum (singleton c_prev k.(i - 1)) (singleton c_i k.(i)) ) )
          l
    ...
    let lower_zero ?fresh_var ~strict m =
      if is_const m then
        ...
        | _ :: _ :: _ ->
          ...
          let gcd =
            List.fold_left (fun gcd (c, _) -> Z.gcd gcd c) m.const m.terms
          in
          ...
          let c' =
            if Z.sign r = 0 then
              if strict then
                (* already a multiple of gcd. take the previous one, gcd * (q-1) *)
                Z.mul (Z.pred q) gcd
    ...
    let lt_zero ?fresh_var m = lower_zero ?fresh_var ~strict:true m

    let leq_zero ?fresh_var m = lower_zero ?fresh_var ~strict:false m

    let neq_zero ?fresh_var m = lt_zero ?fresh_var m
  end
end
...
module Rat = struct
  ...
  let to_multiset m = Seq.terms m |> Multisets.MT.Seq.of_seq Multisets.MT.empty
end
