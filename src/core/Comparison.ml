...
let of_total ord = match ord with 0 -> Eq | x when x > 0 -> Gt | _ -> Lt

let merge_with_Geq = function Lt | Leq -> Incomparable | Eq -> Geq | cmp -> cmp

let merge_with_Leq = function Gt | Geq -> Incomparable | Eq -> Leq | cmp -> cmp

let smooth = function Gt -> Geq | Lt -> Leq | cmp -> cmp
...
let is_Lt_or_Leq = function Lt | Leq -> true | _ -> false

let is_Lt_or_Leq_or_Eq = function Lt | Leq | Eq -> true | _ -> false
