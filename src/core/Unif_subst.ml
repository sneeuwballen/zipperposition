...
module FO = struct
  ...
  let rename_to_new_scope ~counter (t0, scope0) (t1, scope1) =
    ...
    in
    let subst = empty in
    let subst = Term.vars t0 |> Term.VarSet.to_seq |> Iter.fold (add_renaming scope0) subst in
    let subst = Term.vars t1 |> Term.VarSet.to_seq |> Iter.fold (add_renaming scope1) subst in
