...
let equal_com lit1 lit2 =
  ...
  | Equal ->
    lit1.op = lit2.op
    && ( (M.equal lit1.left lit2.left && M.equal lit1.right lit2.right)
       || (M.equal lit1.left lit2.right && M.equal lit1.right lit2.left) )
...
let hash lit =
  CCHash.combine3 (CCHash.poly lit.op) (M.hash lit.left) (M.hash lit.right)
...
let pp out m =
  Format.fprintf out "%a %s %a" M.pp m.left
    ( match m.op with
    | Equal -> "="
    | Less -> "<" )
    M.pp m.right
...
let pp_tstp out m =
  match m.op with
  | Equal -> Format.fprintf out "%a = %a" M.pp_tstp m.left M.pp_tstp m.right
  | Less ->
...
let fold f acc m =
  let acc = Iter.fold f acc (Monome.Seq.terms m.left) in
  Iter.fold f acc (Monome.Seq.terms m.right)
...
module Subsumption = struct
  ...
  let _matching_postcond (m1, sc1) (m2, sc2) (subst, c1, c2) =
    ...
    ( Util.debugf 5
        ...
        (fun k ->
          k M.pp m1 sc1 (Q.to_string c1) M.pp m2 sc2 (Q.to_string c2) S.pp
            subst );
      false )
  ...
  let matching ~protect ~subst m1 sc1 m2 sc2 k =
    ...
    let rec init_with_coeff ~subst c1 l1 rest1 t2 c2 rest2 =
      ...
      | [] ->
        ...
        check_other_terms ~subst ~scale1:(Q.div c2 g) ~scale2:(Q.div c1 g)
          rest1 rest2
      | (c1', t1) :: l1' ->
  ...
  let matching2 ~subst l1 r1 sc1 l2 r2 sc2 k =
    ...
    if M.is_const l1 && M.is_const l2 then (* only one problem *)
      ...
      matching ~protect ~subst l1 sc1 l2 sc2 (fun (subst, c1, c2) ->
  ...
  let check ~subst lit1 sc1 lit2 sc2 k =
    ...
    | Equal, Equal ->
      ...
      matching2 ~subst l1 r1 sc1 l2 r2 sc2 (fun (subst, c1, c2) ->
          if
            Q.(
              equal
          ...
          then
            k subst );
      matching2 ~subst l1 r1 sc1 r2 l2 sc2 (fun (subst, c1, c2) ->
          if
            Q.(
              equal
                (c1 * (M.const r1 - M.const l1))
...
let _to_coeffs lit =
  Iter.append (M.Seq.coeffs_swap lit.left) (M.Seq.coeffs_swap lit.right)

(* FIXME: ignore multiplicities, for now, but would need to
   bring everything to same numerator? *)
let to_multiset lit =
  _to_coeffs lit |> Iter.map fst |> Multisets.MT.Seq.of_seq Multisets.MT.empty
...
module Seq = struct
  let terms lit k =
    M.Seq.terms lit.left k;
    M.Seq.terms lit.right k
...
module Focus = struct
  ...
  let focus_term lit t =
    match M.Focus.focus_term lit.left t with
    | Some mf1 ->
