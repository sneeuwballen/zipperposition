...
let compare_rule r1 r2 =
  ...
  | T_rule _, _ | L_rule _, _ -> CCInt.compare (to_int r1) (to_int r2)
...
type defined_cst = {
  ...
  mutable defined_positions : defined_positions lazy_t
        ...
        reduce_l l (fun l' ->
            ...
            | T.Const id ->
              ...
              | Some (r, subst, sc_r, l_rest) ->
                ...
                let rhs =
                  Subst.FO.apply Subst.Renaming.none subst (r.term_rhs, sc_r)
                in
    ...
    | T.App (f, _) ->
      ...
      | T.Const id ->
        ...
        |> Iter.filter_map (fun r ->
               ...
               with Unif.Fail -> None )
      | _ -> Iter.empty )
    | T.Fun _ | T.Var _ | T.DB _ | T.AppBuiltin _ -> Iter.empty
end
...
module Lit = struct
  ...
  let add_eq_rule (r : Rule.t) : unit =
    ...
    | Literal.Equation (t, u, sign) ->
      ...
      if sign && (not !allow_pos_eqn_rewrite_) && T.is_var t && T.is_var u then
        ...
        Util.invalid_argf
          "Rewrite.Lit.add_eq_rule:@ invalid equation type `@[%a@]`@ for rule \
           `%a`"
      ...
      CCArray.findi
        (fun i lit ->
          ...
          | Some (rule, subst, tags) ->
            ...
            Util.debugf ~section 5
              "@[<2>rewrite `@[%a@]`@ :into `@[<v>%a@]`@ :with @[%a@]@ :rule \
               `%a`@]" (fun k ->
                k Literal.pp lit
                  (Util.pp_list
  ...
  let normalize_clause lits = Util.with_prof prof_lit_rw normalize_clause_ lits
  ...
  | L_rule r ->
    ...
    | Literal.Prop (t, _) ->
      ...
      | T.Classic.App (id, args) ->
        ...
        id, args, rhs
      | _ -> fail () )
    | Literal.True | Literal.False | Literal.Equation _ | Literal.Int _
    | Literal.Rat _ ->
      fail () )
...
module Defined_cst = struct
  ...
  let mk_rule_proj_ (p : Ind_ty.projector) proof : rule =
    ...
    let ty_vars =
      CCList.init n_ty_vars (fun i -> HVar.make ~ty:Type.tType i)
    in
    let _, ty_args, _ =
    ...
    T_rule
      (Term.Rule.make ~proof id ty_proj (List.map T.var ty_vars @ [t]) rhs)
  ...
  let mk_rule_cstor_ (c : Ind_ty.constructor) proof : rule =
    ...
    let rhs = T.var x in
    T_rule
      (Term.Rule.make ~proof c_id c_ty (List.map T.var ty_vars @ args) rhs)
