...
(* disabled this rule since extsup takes care of reasoning like this *)
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
    ...
    let t1,t2 = CCPair.map_same
        (fun t -> ((Lambda.eta_reduce t) :> InnerTerm.t))
        (Term.of_term_unsafe t1,Term.of_term_unsafe t2) in
    (* re-deref after eta reduction *)
    let t1,sc1 = US.deref subst (t1,sc1)
    and t2,sc2 = US.deref subst (t2,sc2) in
