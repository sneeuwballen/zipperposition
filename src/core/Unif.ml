...
let rec unif_list subst ~op (l1, sc1) (l2, sc2) k =
  match l1, l2 with
  | [], [] -> k subst
  | [], _ | _, [] -> ()
  | x1 :: tail1, x2 :: tail2 ->
...
let pp_protected out = function
  | P_scope c -> Format.fprintf out "protect[%d]" c
  | P_vars s -> Format.fprintf out "protect{@[%a@]}@])" (T.VarSet.pp HVar.pp) s
...
module Inner = struct
  ...
  let bind ?(check = true) subst v t =
    if check && occurs_check ~depth:0 subst v t then
      fail ()
    else if S.mem subst v then
      fail ()
    else
      S.bind subst v t
  ...
  let update ?(check = true) subst v t =
    if check && occurs_check ~depth:0 subst v t then
      fail ()
    else if not (S.mem subst v) then
      fail ()
    else
      S.update subst v t

  (* is the type of [t] prop, or some other non-syntactically unifiable type? *)
  let has_non_unifiable_type_or_is_prop (t : T.t) : bool =
    match T.ty t with
    | T.NoType -> false
  ...
  let restrict_to_scope subst (t, sc_t) ~into:scope : Unif_subst.t * term =
    let rec aux sc_t subst t : US.t * term =
      match T.ty t with
      | T.NoType -> subst, t
      | T.HasType ty ->
        ...
        ( match T.view t with
        | T.Var v ->
          ( match Subst.find (US.subst subst) (v, sc_t) with
          | Some (u, sc_u) ->
            if sc_t = scope then (
              ...
              subst, T.var v )
            else if T.is_var u && sc_u = scope then
  ...
  let rec whnf_deref_rec (subst : US.t) (t, sc_t) : US.t * T.t =
    ...
    | T.Var _ ->
      ...
      if T.equal t u then
        ...
        whnf_deref_rec subst (u, sc_t)
    (* fixpoint, maybe [u] is reducible *)
    | T.App (f0, l) ->
      let subst, f = whnf_deref_rec subst (f0, sc_t) in
      let t = if T.equal f0 f then t else T.app ~ty:(T.ty_exn t) f l in
      (* now reduce to WHNF *)
  ...
  let lift_terms (l : T.t list) (rhs : T.t) : T.t =
    ...
    let vars = List.map (fun t -> HVar.fresh ~ty:(T.ty_exn t) ()) l in
    (* now replace [l] by [vars] *)
    let body =
      let m = List.map2 (fun t v -> t, T.var v) l vars |> T.Map.of_list in
      T.replace_m rhs m
    in
  ...
  let is_match_op = function
    | O_match_protect _ -> true
    | _ -> false
    ...
    let inter =
      List.filter
        (fun t ->
          match T.view t with
          | T.DB i ->
            DBEnv.mem bvars.B_vars.left i && List.exists (T.is_bvar_i i) l2
          | _ -> assert false )
        l1
    in
  ...
  *)
  let rec unif_rec ~op ~root ~bvars subst t1s t2s : unif_subst =
    let t1, sc1 = US.deref subst t1s and t2, sc2 = US.deref subst t2s in
    match T.ty t1, T.ty t2 with
    | T.NoType, T.NoType ->
      assert (t1 == t2 && t1 == T.tType);
      subst
    | T.NoType, _ | _, T.NoType -> fail ()
  ...
  and unif_term ~op ~root ~bvars subst t1 sc1 t2 sc2 : unif_subst =
    ...
    | T.Var _, _ when is_whnf t2 sc2 -> unif_vars ~op subst t1 sc1 t2 sc2
    | _, T.Var _ when is_whnf t1 sc1 -> unif_vars ~op subst t1 sc1 t2 sc2
    | T.DB i, T.DB j -> if i = j then subst else raise Fail
    | T.Const f, T.Const g ->
      if ID.equal f g then
        subst
      else if
        op = O_unify && (not root) && has_non_unifiable_type_or_is_prop t1
      then (
    ...
    | ( T.App ({T.term = T.Const id1; _}, l1),
      ...
      then (
        ...
        fail ()
    | ( T.App ({T.term = T.Var _ | T.DB _ | T.Bind (Binder.Lambda, _, _); _}, _),
        _ )
    | ( _,
        T.App ({T.term = T.Var _ | T.DB _ | T.Bind (Binder.Lambda, _, _); _}, _)
      )
    ...
    | _, T.Bind (Binder.Lambda, _, _) ->
      ...
      | O_match_protect (P_scope sc2') | O_variant (P_scope sc2') ->
        assert (sc2 = sc2');
        if sc1 = sc2' then (* no renaming at all, same scope already *)
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
    match T.view t1, T.view t2, op with
    | T.Var v1, T.Var v2, O_equal ->
      if HVar.equal T.equal v1 v2 && sc1 = sc2 then
        subst
      else
        fail ()
    | T.Var v1, T.Var v2, _ when HVar.equal T.equal v1 v2 && sc1 = sc2 -> subst
    ...
    | _, T.Var v2, O_unify ->
      if occurs_check ~depth:0 (US.subst subst) (v2, sc2) (t1, sc1) then
        ...
        US.bind subst (v2, sc2) (t1, sc1)
    | _ -> fail ()

  (* fail *)

  (* unify lists pairwise *)
  and unif_list ~op ~bvars subst l1 sc1 l2 sc2 : unif_subst =
    match l1, l2 with
    | [], [] -> subst
    | _, [] | [], _ -> fail ()
    | t1 :: l1', t2 :: l2' ->
  ...
  and unif_ho ~op ~root ~bvars subst t1_0 t2_0 ~scope : unif_subst =
    ...
    let same_rigid_head () =
      ...
      then (
        ...
        fail ()
    in
    match T.view f1, T.view f2 with
    | _ when T.equal f1 f2 -> same_rigid_head ()
    | T.Bind (Binder.Lambda, _, _), T.Bind (Binder.Lambda, _, _) ->
      assert (l1 = [] && l2 = []);
      let new_vars1, f1, new_vars2, f2 = T.open_bind2 Binder.Lambda f1 f2 in
    ...
    | T.Const id1, T.Const id2 ->
      ...
      then (
        ...
        fail ()
    | T.DB i1, T.DB i2 -> if i1 = i2 then same_rigid_head () else fail ()
    | T.Var _, _ when l1 = [] ->
      unif_rec ~op ~bvars ~root subst (t1, scope) (t2, scope) (* to bind *)
    | _, T.Var _ when l2 = [] ->
      unif_rec ~op ~bvars ~root subst (t1, scope) (t2, scope) (* to bind *)
    | T.Const f, T.Var _ when partial_skolem_fail f l1 l2 -> fail ()
    | T.Var _, T.Const g when partial_skolem_fail g l2 l1 -> fail ()
    | T.Var v1, T.Const _ ->
      ( match op with
    ...
    | T.Const _, T.Var v2 ->
      ...
      then (* flex/rigid pattern unif *)
        ...
        fail ()
    | T.Var v1, T.Var v2
      when op = O_unify && !_allow_pattern_unif
           && distinct_bvar_l ~bvars:bvars.B_vars.left l1
           && distinct_bvar_l ~bvars:bvars.B_vars.right l2 ->
      (* flex/flex equation for pattern unif *)
      flex_flex_unif subst ~bvars ~ty_ret:(T.ty_exn t1) ~scope v1 l1 v2 l2
    | T.Var v1, T.Var v2
      when is_match_op op && !_allow_pattern_unif
           && distinct_bvar_l ~bvars:bvars.B_vars.left l1
           && distinct_bvar_l ~bvars:bvars.B_vars.right l2
           && CCList.subset ~eq:T.equal l2 l1 ->
      (* TODO: use equality mod subst for [subset] check *)
      flex_flex_matching ~op subst ~bvars ~ty_ret:(T.ty_exn t2) ~scope v1 l1 v2
        l2
    | T.Var _, T.Var _ ->
      (* λfree-HO: unify with currying, "from the right" *)
      let l1, l2 = pair_lists_right f1 l1 f2 l2 in
  ...
  and flex_rigid ~bvars subst f1 l1 t2 ~scope : unif_subst =
    ...
    let subst =
      unif_rec ~op:O_unify ~root:true ~bvars:B_vars.empty subst (f1, scope)
        (rhs, scope)
    in
  ...
  and proj_fun ~bvars subst (t, sc_t) : unif_subst =
    ...
    | T.DB i ->
      if DBEnv.mem bvars i then
        proj_fun_l ~bvars subst (l, sc_t)
      else
        fail () (* this variable is not in scope anymore *)
    | T.Var v ->
      if l = [] then
        subst
  ...
  and proj_fun_l ~bvars subst (l, sc) : unif_subst =
    List.fold_left (fun subst t -> proj_fun ~bvars subst (t, sc)) subst l
  ...
  *)
  and flex_flex_unif ~bvars subst ~ty_ret v1 l1 v2 l2 ~scope : unif_subst =
    Util.debugf ~section 5
      ...
      (fun k ->
        k HVar.pp v1 (Util.pp_list T.pp) l1 scope HVar.pp v2
          (Util.pp_list T.pp) l2 scope US.pp subst B_vars.pp bvars );
  ...
  *)
  ...
  let unify_syn ?(subst = Subst.empty) a b : Subst.t =
    ...
    if US.has_constr subst then
      raise Fail
    else
      US.subst subst

  let matching ?(subst = Subst.empty) ~pattern b =
    if Scoped.same_scope pattern b then
      invalid_arg "Unif.matching: same scopes";
    let scope = Scoped.scope b in
    Util.with_prof prof_matching
      (fun () ->
        let subst = US.of_subst subst in
        let subst =
          unif_rec subst pattern b ~root:true
            ~op:(O_match_protect (P_scope scope)) ~bvars:B_vars.empty
        in
      ...
      (fun () ->
        ...
        let subst =
          unif_rec subst
            ...
            (Scoped.make b scope) ~op:(O_match_protect (P_vars blocked))
  ...
  let variant ?(subst = Subst.empty) ((_, sc1) as a) ((t2, sc2) as b) =
    ...
    let op =
      let protect =
        if sc1 = sc2 then
          P_vars (T.Seq.vars t2 |> T.VarSet.of_seq)
        else
          P_scope sc2
      in
      O_variant protect
...
module Ty = struct
  ...
  let matching =
    ( matching
      :> ?subst:subst -> pattern:term Scoped.t -> term Scoped.t -> subst )
  ...
  let are_unifiable_full = (are_unifiable_full :> term -> term -> bool)

  let are_unifiable_syn = (are_unifiable_syn :> term -> term -> bool)

  let matches = (matches :> pattern:term -> term -> bool)
...
let () =
  Options.add_opts
    [ ( "--partial-skolem",
        ...
        " allow partial application of skolem constants (sound only assuming \
         the axiom of choice)" );
      ( "--no-unif-pattern",
        Arg.Clear _allow_pattern_unif,
        " disable pattern unification" );
      ( "--unif-pattern",
        Arg.Set _allow_pattern_unif,
        " enable pattern unification" ) ]
