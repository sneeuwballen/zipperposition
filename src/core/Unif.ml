...
let occurs_check ~depth subst (v, sc_v) t =
  let rec check ~depth (t, sc_t) =
    ...
    | T.HasType ty -> (
        ...
        | T.App (hd, l) ->
            check ~depth (hd, sc_t) || check_l ~depth l sc_t )
  and check_l ~depth l sc =
    ...
    | [t] ->
        check ~depth (t, sc)
    | t :: tail ->
        check ~depth (t, sc) || check_l ~depth tail sc
  in
  check ~depth t
...
let pp_op out = function
  | O_unify ->
      CCFormat.string out "unify"
  | O_equal ->
      CCFormat.string out "equal"
  | O_variant p ->
      Format.fprintf out "(@[variant %a@])" pp_protected p
  | O_match_protect p ->
      Format.fprintf out "(@[match %a@])" pp_protected p
...
module Inner = struct
          ...
          | T.Var v -> (
            ...
            | Some (u, sc_u) ->
                if sc_t = scope then
                  ...
                  (subst, T.var v')
            | None ->
                if sc_t = scope then (subst, T.var (HVar.cast ~ty v))
                else
  ...
  let rec whnf_deref_rec (subst : US.t) (t, sc_t) : US.t * T.t =
    ...
    | T.Var _ ->
        ...
        if T.equal t u then (subst, u) else whnf_deref_rec subst (u, sc_t)
    ...
    | T.App (f0, l) ->
        ...
        if T.equal t u then (subst, t) else whnf_deref_rec subst (u, sc_t)
        (* reduce further? *)
    | _ ->
        (subst, t)
  ...
  module B_vars : sig
    type t = private {left: T.t DBEnv.t; right: T.t DBEnv.t}
    ...
    type t = {left: T.t DBEnv.t; right: T.t DBEnv.t}
  ...
  let env_l_dense (e : 'a DBEnv.t) : 'a list =
    DBEnv.to_list e |> List.map (function Some x -> x | None -> assert false)
  ...
  let restrict_fun1 subst ~op ~ty ~to_:subset ~scope (v, args) =
    ...
    match op with
    | O_match_protect _ ->
        if n = n_old then subst else fail ()
    | _ ->
        (* fresh variable *)
  ...
  *)
  ...
  and unif_term ~op ~root ~bvars subst t1 sc1 t2 sc2 : unif_subst =
    ...
    | _, T.Var _ when is_whnf t1 sc1 ->
        unif_vars ~op subst t1 sc1 t2 sc2
    | T.DB i, T.DB j ->
        if i = j then subst else raise Fail
    | T.Const f, T.Const g ->
        if ID.equal f g then subst
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
    ...
    | T.Var v1, T.Var v2, O_equal ->
        if HVar.equal T.equal v1 v2 && sc1 = sc2 then subst else fail ()
    | T.Var v1, T.Var v2, _ when HVar.equal T.equal v1 v2 && sc1 = sc2 ->
        subst
  ...
  and proj_fun ~op ~bvars subst (t, sc_t) : unif_subst =
    ...
    | T.DB i ->
        if DBEnv.mem bvars i then proj_fun_l ~op ~bvars subst (l, sc_t)
  ...
  *)
  ...
  let unify_syn ?(subst = Subst.empty) a b : Subst.t =
    ...
    if US.has_constr subst then raise Fail else US.subst subst
