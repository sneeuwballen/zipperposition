...
let norm_logical_disagreements ?(mode = `Conservative) b args args' :
  ...
  let sort =
    CCList.sort (fun t1 t2 ->
        ...
        <?> (CCInt.compare, Term.hash t1, Term.hash t2))
  in
...
module Inner = struct
  ...
  let restrict_fun1 subst ~op ~ty ~to_:subset ~scope (v, args) =
  ...
  *)
  ...
  and unif_term ~op ~root ~bvars subst t1 sc1 t2 sc2 : unif_subst =
    ...
    | ( T.App ({ T.term = T.Const id1; _ }, l1),
      ...
      if ID.equal id1 id2 && List.length l1 = List.length l2 then
        ...
        fail ()
    | ( T.App
          ({ T.term = T.Var _ | T.DB _ | T.Bind (Binder.Lambda, _, _); _ }, _),
        _ )
    | ( _,
        T.App
          ({ T.term = T.Var _ | T.DB _ | T.Bind (Binder.Lambda, _, _); _ }, _) )
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
    ...
    let t1, t2 =
      CCPair.map_same
        (fun t -> (Lambda.eta_reduce ~expand_quant:false t :> InnerTerm.t))
        (Term.of_term_unsafe t1, Term.of_term_unsafe t2)
    in
    ...
    | _, _, _
      when (not !_unif_bool)
           &&
           match T.open_poly_fun t1 with
           | _, _, ret -> T.type_is_prop ret ->
      fail ()
    | _, _, _
      when (not !_unif_bool)
           &&
           match T.open_poly_fun t2 with
           | _, _, ret -> T.type_is_prop ret ->
      fail ()
      ...
      if occurs_check ~depth:0 (US.subst subst) (v1, sc1) (t2, sc2) then
        ...
        let derefed =
          Scoped.map
            ...
            derefed
        in
        if snd @@ derefed == sc2 && T.equal (fst @@ derefed) t2 then
          subst
        else
          fail ()
      ) else
      ...
      if occurs_check ~depth:0 (US.subst subst) (v2, sc2) (t1, sc1) then
        ...
        let derefed =
          Scoped.map
            ...
            derefed
        in
        if snd @@ derefed == sc1 && T.equal (fst @@ derefed) t1 then
          subst
        else
          fail ()
      ) else
  ...
  and proj_fun ~op ~bvars subst (t, sc_t) : unif_subst =
    ...
    | T.DB i ->
      if DBEnv.mem bvars i then
        proj_fun_l ~op ~bvars subst (l, sc_t)
      else
  ...
  and proj_fun_l ~op ~bvars subst (l, sc) : unif_subst =
    List.fold_left (fun subst t -> proj_fun ~op ~bvars subst (t, sc)) subst l
  ...
  *)
  ...
  let variant ?(subst = Subst.empty) ((_, sc1) as a) ((t2, sc2) as b) =
    ...
    let op =
      let protect =
        if sc1 = sc2 then
          P_vars (T.Seq.vars t2 |> T.VarSet.of_iter)
        else
          P_scope sc2
      in
      O_variant protect
