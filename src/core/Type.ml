...
let view t =
  ...
  | T.AppBuiltin (Builtin.Arrow, ret :: l) ->
      Fun (l, ret)
  | T.AppBuiltin (Builtin.Prop, []) ->
      Builtin Prop
  | T.AppBuiltin (Builtin.TType, []) ->
      Builtin TType
  | T.AppBuiltin (Builtin.Term, []) ->
      Builtin Term
  | T.AppBuiltin (Builtin.TyInt, []) ->
      Builtin Int
  | T.AppBuiltin (Builtin.TyRat, []) ->
      Builtin Rat
  | T.AppBuiltin (Builtin.TyReal, []) ->
      Builtin Real
  | _ ->
      assert false
...
let as_var_exn ty = match view ty with Var v -> v | _ -> invalid_arg "as_var_exn"
...
let builtin = function TType -> tType | Prop -> prop | Term -> term | Int -> int | Rat -> rat | Real -> real
...
let bvar i = T.bvar ~ty:T.tType i

let forall ty = T.bind ~ty:T.tType ~varty:T.tType Binder.forall_ty ty

let rec forall_n n ty = if n = 0 then ty else forall (forall_n (n - 1) ty)
...
let def id = ID.payload_find id ~f:(function Payload_def d -> Some d | _ -> None)

let def_exn id = match def id with Some d -> d | None -> raise Not_found
...
module Seq = struct
  ...
  let has_bools_only t = T.Seq.subterms t |> Iter.for_all (fun ty -> is_fun ty || is_prop ty)
end
...
let rec expected_args ty =
  match view ty with
  | Fun (args, ret) ->
      args @ expected_args ret
  | Forall ty' ->
      expected_args ty'
...
let () =
  Printexc.register_printer (function
    | ApplyError msg ->
        Some (Util.err_spf "@[<2>Type.ApplyError:@ %s@]" msg)
    | _ ->
        None )
...
let rec ty_eq ty ty' =
  ...
  | DB i, DB i' ->
      i = i'
  | App (f, l), App (f', l') ->
      if List.length l = List.length l' then ID.equal f f' && List.for_all2 ty_eq l l' else false
  | _ ->
      false
...
let apply ty0 args0 =
  let rec aux ty args env =
    ...
    | T.DB _, _ ->
        ...
        aux ty args env
    | _ ->
        err_applyf_ "@[<2>Type.apply:@ expected quantified or function type,@ but got @[%a@] @[(args: %a)@]"
          T.pp_zf ty (CCList.pp T.pp) args
  and aux_l ty_ret exp_args args env =
    ...
    | [], _ -> (
      ...
      | _ ->
          err_applyf_ "@[<2>Type.apply:@ unexpected arguments [@[%a@]]@]" (Util.pp_list T.pp_zf) args )
    | exp :: exp_args', a :: args' ->
        (* expected type: [exp];  [a]: actual value, whose type must match [exp] *)
        let exp' = T.DB.eval env exp in
        if T.equal exp' (T.ty_exn a) then aux_l ty_ret exp_args' args' env
        else
          err_applyf_
            "@[<2>Type.apply:@ wrong argument type,@ expected `@[_ : %a@]`@ but got `@[%a : %a@]`@ when \
             applying `%a` to@ [@[%a@]]@ in env [%a]@]"
...
module TPTP = struct
  ...
  let rec pp_tstp_rec depth out t =
    match view t with
    | Builtin Prop ->
        CCFormat.string out "$o"
    | Builtin TType ->
        CCFormat.string out "$tType"
    | Builtin Term ->
        CCFormat.string out "$i"
    | Builtin Int ->
        CCFormat.string out "$int"
    | Builtin Rat ->
        CCFormat.string out "$rat"
    | Builtin Real ->
        CCFormat.string out "$real"
    | Var v ->
        Format.fprintf out "X%d" (HVar.id v)
    | DB i ->
        Format.fprintf out "Tb%d" (depth - i - 1)
    | App (p, []) ->
        ID.pp_tstp out p
    | App (p, args) ->
        Format.fprintf out "@[<2>%a(%a)@]" ID.pp_tstp p (Util.pp_list (pp_tstp_rec depth)) args
    | Fun (args, ret) ->
        Format.fprintf out "%a > %a" (pp_l depth) args (pp_tstp_rec depth) ret
    | Forall ty' ->
        Format.fprintf out "!>[Tb%d:$tType]: %a" depth (pp_inner (depth + 1)) ty'

  and pp_inner depth out t =
    match view t with
    | Fun _ ->
        Format.fprintf out "(@[%a@])" (pp_tstp_rec depth) t
    | _ ->
        pp_tstp_rec depth out t

  and pp_l depth out l =
    match l with
    | [] ->
        assert false
    | [ty] ->
        pp_inner depth out ty
    | _ ->
  ...
  let rec pp_ho_depth depth out t =
    match view t with
    | Builtin Prop ->
        CCFormat.string out "$o"
    | Builtin TType ->
    ...
    | App (p, []) ->
        ID.pp_tstp out p
    | App (p, args) ->
        Format.fprintf out "@[<2>%a @@ %a @]" ID.pp_tstp p (Util.pp_list ~sep:" @ " (pp_inner depth)) args
    | Fun (args, ret) ->
  ...
  and pp_inner depth out t =
    match view t with
    | Fun _ | App (_, _ :: _) ->
        Format.fprintf out "(@[%a@])" (pp_ho_depth depth) t
    | _ ->
        pp_ho_depth depth out t
  ...
  and pp_l depth out l =
    ...
    | [ty] ->
        pp_inner depth out ty
    | _ ->
        Format.fprintf out "@[%a@]" (Util.pp_list ~sep:" > " (pp_inner depth)) l

  let pp_ho ?(depth = 0) out t = pp_ho_depth depth out t

  let pp_typed_var out v =
    match view (HVar.ty v) with
    (* | Builtin Term -> HVar.pp out v implicit *)
    | _ ->
        Format.fprintf out "@[%a : %a@]" HVar.pp_tstp v (pp_ho ~depth:0) (HVar.ty v)

  let to_string = CCFormat.to_string pp
end

let pp_typed_var_gen ~pp_ty out v =
  match view (HVar.ty v) with
  | Builtin TType ->
  ...
  | Forall _ | Fun _ ->
      Format.fprintf out "(@[F%d:%a@])" (HVar.id v) pp_ty (HVar.ty v)
  | _ ->
      Format.fprintf out "(@[%a:%a@])" HVar.pp v pp_ty (HVar.ty v)
...
let rec pp_rec depth out t =
  match view t with
  | Builtin b ->
      pp_builtin out b
  | Var v ->
      let ty = HVar.ty v in
      if is_tType ty then Format.fprintf out "A%d" (HVar.id v) else HVar.pp out v
  | DB i ->
      Format.fprintf out "T%i" (depth - i - 1)
  | App (p, []) ->
      ID.pp out p
  | App (p, args) ->
      Format.fprintf out "@[<2>%a %a@]" ID.pp p (Util.pp_list ~sep:" " (pp_inner_app depth)) args
  | Fun (args, ret) ->
      Format.fprintf out "@[%a →@ %a@]" (Util.pp_list ~sep:" → " (pp_inner_fun depth)) args (pp_rec depth) ret
  | Forall ty' ->
      Format.fprintf out "@[Π T%i.@ %a@]" depth (pp_inner_fun (depth + 1)) ty'

and pp_inner_fun depth out t =
  match view t with Fun _ -> Format.fprintf out "(@[%a@])" (pp_rec depth) t | _ -> pp_rec depth out t

and pp_inner_app depth out t =
  match view t with
  | Fun _ | App (_, _ :: _) ->
      Format.fprintf out "(@[%a@])" (pp_rec depth) t
  | _ ->
      pp_rec depth out t
...
let mangle (ty : t) : string =
  ...
  let rec aux buf t =
    ...
    | Fun (args, ret) ->
        ...
        aux buf ret
    | Forall f ->
        Printf.bprintf buf "pi_%a" aux f
  in
...
module Conv = struct
  ...
  let create () =
    {vars= Var.Subst.empty; n= 0; hvars= VarMap.empty; bvars_to_db= VarMap.empty; max_vars= ref None}
  ...
  let exit_bvar ~handle ctx v =
    let new_map = VarMap.map (fun x -> x - 1) ctx.bvars_to_db in
    if CCOpt.is_some handle then ctx.bvars_to_db <- VarMap.add v (CCOpt.get_exn handle) new_map
    else ctx.bvars_to_db <- new_map

  let find_bvar ctx v = VarMap.find_opt v ctx.bvars_to_db
  ...
  let get_maxvar ctx = CCOpt.get_or ~default:0 !(ctx.max_vars)

  let incr_maxvar ctx = set_maxvar ctx (get_maxvar ctx + 1)
  ...
  let of_simple_term_exn ctx t =
    let rec aux depth v2db t =
      match PT.view t with
      | PT.Var v -> (
        match Var.Subst.find v2db v with
        | Some i ->
      ...
      | PT.Bind (Binder.ForallTy, v, t') ->
          ...
          forall t'
      | PT.Record _ ->
          failwith "cannot convert record-type into type"
      | PT.Meta (_, {contents= Some ty'}, _) ->
          aux depth v2db ty'
      | PT.Bind _ | PT.AppBuiltin _ | PT.Meta _ | PT.Ite _ | PT.Match _ | PT.Let _ | PT.Multiset _ ->
          raise (Error t)
    and aux_var v =
      match Var.Subst.find ctx.vars v with
      | Some v ->
          v
      | None ->
          (* free variable *)
  ...
  let of_simple_term ctx t = try Some (of_simple_term_exn ctx t) with Error _ -> None

  let rec to_simple_term ?(env = DBEnv.empty) ctx t =
    let rec aux env t =
      match view t with
      | Builtin Prop ->
          PT.builtin ~ty:PT.tType Builtin.Prop
      | Builtin TType ->
          PT.builtin ~ty:PT.tType Builtin.TType
      | Builtin Term ->
          PT.builtin ~ty:PT.tType Builtin.Term
      | Builtin Int ->
          PT.builtin ~ty:PT.tType Builtin.TyInt
      | Builtin Rat ->
          PT.builtin ~ty:PT.tType Builtin.TyRat
      | Builtin Real ->
          PT.builtin ~ty:PT.tType Builtin.TyReal
      | Var v ->
          let v = aux_var v in
          PT.var v
      | DB i ->
          PT.var (DBEnv.find_exn env i)
      | App (s, l) ->
          (* s : type -> type -> ... -> type *)
      ...
      | Fun (args, ret) ->
          ...
          PT.Ty.fun_ args ret
      | Forall t' ->
          let v = Var.of_string ~ty:PT.tType (CCFormat.sprintf "V%d" (DBEnv.size env)) in
          let t' = aux (DBEnv.push env v) t' in
          PT.bind ~ty:PT.tType Binder.forall_ty v t'
    and aux_var v = var_to_simple_var ~prefix:"A" ctx v in
    aux env t

  and var_to_simple_var ?(prefix = "A") ctx v =
    try VarMap.find v ctx.hvars
    with Not_found ->
      let v' = Var.of_string ~ty:(to_simple_term ctx (HVar.ty v)) (CCFormat.sprintf "%s%d" prefix (HVar.id v)) in
      ...
      | DB i ->
          assert (
            if i >= 0 && i < List.length env then true
              ...
              Format.printf "%d not in %a@." i (CCFormat.Dump.list pp) env ;
              false ) ) ;
          assert (
            if equal ty (List.nth env i) then true
            else (
              Format.printf "%a:%a or %a@." pp t pp ty pp (List.nth env i) ;
              false ) ) ;
          bvar i
      | App (f, l) ->
          app f (List.map (aux env) l)
      | Fun (args, ret) ->
          arrow (List.map (aux env) args) (aux env ret)
      | Builtin b ->
          builtin b
      | Forall body ->
