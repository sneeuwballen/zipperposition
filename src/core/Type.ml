...
let[@inline] is_bvar ty =
  match T.view ty with
  | T.DB _ -> true
  | _ -> false

let[@inline] is_app ty =
  match T.view ty with
  | T.App _ -> true
  | _ -> false

let[@inline] is_const ty =
  match T.view ty with
  | T.Const _ -> true
  | _ -> false
...
let[@inline] is_forall ty =
  match T.view ty with
  | T.Bind (Binder.ForallTy, _, _) -> true
  | _ -> false
...
let arity ty =
  ...
  let rec traverse n_forall ty =
    ...
    | Forall ty' -> traverse (n_forall + 1) ty'
    | Var _ -> NoArity
    | Builtin _ -> Arity (n_forall, 0)
    | DB _ | App _ -> Arity (n_forall, 0)
  in
  traverse 0 ty
...
let apply ty0 args0 =
  ...
  and aux_l ty_ret exp_args args env =
    ...
    | exp :: exp_args', a :: args' ->
      ...
      if T.equal exp' (T.ty_exn a) then
        aux_l ty_ret exp_args' args' env
      else
        err_applyf_
          "@[<2>Type.apply:@ wrong argument type,@ expected `@[_ : %a@]`@ but \
           got `@[%a : %a@]`@ when applying `%a` to@ [@[%a@]]@ in env [%a]@]"
...
module Conv = struct
  ...
  let of_simple_term_exn ctx t =
    let rec aux depth v2db t =
      ...
      | PT.Bind _ | PT.AppBuiltin _ | PT.Meta _ | PT.Ite _ | PT.Match _
      | PT.Let _ | PT.Multiset _ ->
        raise (Error t)
    and aux_var v =
  ...
  let rec aux env t =
    ...
    | T.HasType ty ->
      ...
      | DB i ->
        ...
        assert (
          if equal ty (List.nth env i) then
            true
          else (
