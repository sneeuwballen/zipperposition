...
(** This module computes precedences that satisfy a list of constraints. See
    {!Precedence.Constr} for more details on constraints. *)
...
(** Add a precedence constraint with its priority. The lower the priority, the
    stronger influence the constraint will have. *)
...
(** Parameters db_w and lmb_w correspond to the weight de-Bruijn and lambda
    abstraction given for computation of KBO. *)
val mk_precedence :
     db_w:int
  -> lmb_w:int
