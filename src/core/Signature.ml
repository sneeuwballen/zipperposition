...
let find_exn { sym_map; ty_map } s : Type.t =
  let t, _ = ID.Map.find s sym_map in
  t
...
let declare ({ sym_map; ty_map } as signature) id ty =
  ...
  with Not_found ->
    ...
    let ty_map =
      ...
      Type.Map.add ty (ID.Set.add id id_set) ty_map
    in
    { sym_map; ty_map }
...
let arity signature s =
  ...
  | Type.NoArity ->
    failwith
      (CCFormat.sprintf "symbol %a has ill-formed type %a" ID.pp s Type.TPTP.pp
         ty)
  | Type.Arity (a, b) -> a, b
  ...
  let ty_map =
    ID.Map.fold
      (fun id (ty, _) acc ->
        ...
        Type.Map.add ty (ID.Set.add id id_set) acc)
      smap Type.Map.empty
  in
  ty_map
...
module Seq = struct
  let symbols s = ID.Map.to_iter s.sym_map |> Iter.map fst
  let types s = ID.Map.to_iter s.sym_map |> Iter.map snd |> Iter.map fst
