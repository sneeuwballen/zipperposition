...
let empty = mk_ Name.Map.empty Type.Map.empty

let is_empty { sym_map; ty_map } =
  let sym_map_empty = Name.Map.is_empty sym_map in
  assert ((not sym_map_empty) || Type.Map.is_empty ty_map);
  sym_map_empty

let mem { sym_map; ty_map } s = Name.Map.mem s sym_map

let find_exn { sym_map; ty_map } s : Type.t =
  let t, _ = Name.Map.find s sym_map in
  t

let find signature s = try Some (find_exn signature s) with Not_found -> None

let find_by_type { sym_map; ty_map } ty =
  let res = Type.Map.get_or ty ty_map ~default:Name.Set.empty in
  assert (
    Name.Set.for_all
      (fun id ->
        let ty', _ = Name.Map.find id sym_map in
        Type.equal ty' ty)
      res);
  res
    ...
    | AlreadyDeclared (id, old, new_) ->
      Some
        (CCFormat.sprintf
           ...
           Name.pp id Type.pp old Type.pp new_)
    | _ -> None)
...
let declare ({ sym_map; ty_map } as signature) id ty =
  ...
  with Not_found ->
    ...
    let sym_map = Name.Map.add id (ty, false) sym_map in
    let ty_map =
      let id_set = Type.Map.get_or ty ty_map ~default:Name.Set.empty in
      Type.Map.add ty (Name.Set.add id id_set) ty_map
    in
    { sym_map; ty_map }

let cardinal sgn = Name.Map.cardinal sgn.sym_map
...
let arity signature s =
  ...
  | Type.NoArity ->
    failwith
      (CCFormat.sprintf "symbol %a has ill-formed type %a" Name.pp s
         Type.TPTP.pp ty)
  | Type.Arity (a, b) -> a, b

let is_ground { sym_map; _ } =
  Name.Map.for_all (fun _ (ty, _) -> Type.is_ground ty) sym_map

let ty_map_of_s_map smap =
  let ty_map =
    Name.Map.fold
      (fun id (ty, _) acc ->
        let id_set = Type.Map.get_or ty acc ~default:Name.Set.empty in
        Type.Map.add ty (Name.Set.add id id_set) acc)
      smap Type.Map.empty
  in
  ty_map
      ...
      (fun s t1 t2 ->
        ...
        | Some (ty1, c1), Some (ty2, c2) ->
          if Type.equal ty1 ty2 then
            ...
            raise (AlreadyDeclared (s, ty1, ty2))
...
let sym_in_conj s sgn =
  snd (Name.Map.get_or s sgn.sym_map ~default:(Type.int, false))

let set_sym_in_conj s signature =
  let t = find_exn signature s in
  let sym_map = Name.Map.add s (t, true) signature.sym_map in
  { signature with sym_map }

module Seq = struct
  let symbols s = Name.Map.to_iter s.sym_map |> Iter.map fst
  let types s = Name.Map.to_iter s.sym_map |> Iter.map snd |> Iter.map fst
  let to_iter s = Name.Map.to_iter s.sym_map
  let of_iter l = Iter.fold (fun s (id, ty) -> declare s id ty) empty l
end

let to_set s = Seq.symbols s |> Name.Set.of_iter
let to_list s = Name.Map.to_list s.sym_map
let iter s f = Name.Map.iter f s.sym_map

let fold s acc f =
  Name.Map.fold (fun s (ty, c) acc -> f acc s (ty, c)) s.sym_map acc
...
let is_not_bool signature s = not (is_bool signature s)

(** {2 IO} *)

let pp out s =
  let pp_pair out (s, (ty, c)) =
    Format.fprintf out "@[<hov2>%a:@ %a %B@]" Name.pp s Type.pp ty c
  in
