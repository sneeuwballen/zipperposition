...
let find signature s =
  try Some (ID.Map.find s signature) with Not_found -> None

let find_exn signature s = ID.Map.find s signature
...
let arity signature s =
  ...
  | Type.NoArity ->
    failwith
      (CCFormat.sprintf "symbol %a has ill-formed type %a" ID.pp s Type.pp ty)
  | Type.Arity (a, b) -> a, b
...
module Seq = struct
  let symbols s = ID.Map.to_seq s |> Iter.map fst

  let types s = ID.Map.to_seq s |> Iter.map snd
...
let iter s f = ID.Map.iter f s

let fold s acc f = ID.Map.fold (fun s ty acc -> f acc s ty) s acc
