...
*)
type feature = A | B | DB of int | N | S of ID.t | BI of Builtin.t | Ignore
...
and gfpf_root ~depth t =
  ...
  | T.DB i ->
      if i < depth then DB i else Ignore
  | T.Var _ ->
      A
  | T.Const c ->
      S c
...
let pp_feature out = function
  ...
  | B ->
      CCFormat.fprintf out "B"
  | DB i ->
      CCFormat.fprintf out "DB %d" i
  | N ->
      CCFormat.fprintf out "N"
  | S id ->
      CCFormat.fprintf out "S %a" ID.pp id
  | BI b ->
      CCFormat.fprintf out "BI %a" Builtin.pp b
  | Ignore ->
      CCFormat.fprintf out "I"
...
let cmp_feature f1 f2 =
  ...
  | DB i, DB j ->
      compare i j
  | _ ->
      feat_to_int_ f1 - feat_to_int_ f2
...
let compatible_features_match f1 f2 =
  match f1 with
  | S s1 -> (
    match f2 with S s2 -> ID.equal s1 s2 | Ignore -> true | _ -> false )
  | BI b1 -> (
    match f2 with BI b2 -> Builtin.equal b1 b2 | Ignore -> true | _ -> false )
  | Ignore | B ->
      true
  | A -> (
    match f2 with A | DB _ | S _ | Ignore | BI _ -> true | _ -> false )
  | DB i -> (
    match f2 with DB j -> i = j | Ignore -> true | _ -> false )
  | N -> (
    match f2 with N | Ignore -> true | _ -> false )
  ...
  let is_empty idx =
    let rec is_empty trie =
      ...
      | Leaf l ->
          Leaf.is_empty l
      | Node map ->
          FeatureMap.for_all (fun _ trie' -> is_empty trie') map
    in
    is_empty idx.trie
  ...
  let remove_if idx t leaf_cleaner =
    ...
    let rec recurse trie features =
      ...
      | Node map, f :: features' ->
          ...
          if FeatureMap.is_empty map then Empty else Node map
      | Leaf leaf, [] ->
          let leaf = leaf_cleaner leaf t in
          if Leaf.is_empty leaf then Empty else Leaf leaf
      | Node _, [] | Leaf _, _ :: _ ->
          failwith "different feature length in fingerprint trie"
    in
    let features = idx.fp t in
  ...
  let iter idx f =
    let rec iter trie f =
      ...
      | Empty ->
          ()
      | Node map ->
          FeatureMap.iter (fun _ subtrie -> iter subtrie f) map
      | Leaf leaf ->
          Leaf.iter leaf f
    in
    iter idx.trie f
