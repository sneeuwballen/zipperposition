...
let rec gfpf pos t =
  match pos, T.Classic.view t with
  | [], T.Classic.Var _ -> A
  | [], T.Classic.DB _ -> B
  | [], _
    when (not (Unif.Ty.type_is_unifiable @@ T.ty t)) || Type.is_fun (T.ty t) ->
    NonFO
  ...
  | _, T.Classic.AppBuiltin _ | _, T.Classic.NonFO -> NonFO
...
module Make (X : Set.OrderedType) = struct
  ...
  let retrieve_unifiables ?(subst = Unif_subst.empty) (idx, sc_idx) t k =
    ...
    traverse ~compatible idx features (fun leaf ->
        Leaf.fold_unify ~subst (leaf, sc_idx) t k )
