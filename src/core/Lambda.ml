...
module Inner = struct
  ...
  let rec snf_rec t =
    ...
    | T.HasType ty ->
      ...
      | T.App (f, l) ->
        let f' = snf_rec f in
        if not (T.equal f f') then
          snf_rec (T.app ~ty f' l)
        else (
  ...
  let eta_reduce_rec t =
    let rec aux t =
      ...
      | T.HasType ty ->
        ...
        | T.Bind
            (Binder.Lambda, _, {T.term = T.App (f, [{T.term = T.DB 0; _}]); _})
          when not (T.DB.contains f 0) ->
          (* [fun x. f x --> f] *)
          T.DB.unshift 1 f
        | T.Bind (b, varty, body) -> T.bind ~ty ~varty b (aux body)
        | T.App (_, []) -> assert false
        | T.App (f, l) ->
          let f' = aux f in
          let l' = List.map aux l in
          if T.equal f f' && T.same_l l l' then
            t
          else
            T.app ~ty (aux f) (List.map aux l)
        | T.AppBuiltin (b, l) -> T.app_builtin ~ty b (List.map aux l) )
    in
    aux t
...
let snf t = Inner.snf_rec (t : T.t :> IT.t) |> T.of_term_unsafe

let eta_expand t = Inner.eta_expand (t : T.t :> IT.t) |> T.of_term_unsafe

(*|> CCFun.tap (fun t' ->
  if t != t' then Format.printf "@[eta_expand `%a`@ into `%a`@]@." T.pp t T.pp t')*)

let eta_reduce t = Inner.eta_reduce (t : T.t :> IT.t) |> T.of_term_unsafe
