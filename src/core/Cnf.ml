...
module Flatten = struct
  ...
  let rec map_m f l =
    match l with
    | [] -> return []
    | x :: tail -> f x >>= fun x -> map_m f tail >|= fun tail -> x :: tail
  ...
  let complete_eq t u =
    ...
    let vars =
      List.mapi
        (fun i ty ->
          let name =
            try List.nth names i with _ -> Printf.sprintf "x_%d" i
          in
  ...
  *)
  let flatten_rec ?of_ (ctx : Skolem.ctx) stmt (pos : position)
    ...
    let rec aux pos vars t =
      ...
      | T.Bind (Binder.Lambda, _, _) ->
        ...
        aux Pos_inner vars body >|= fun body -> T.fun_l fun_vars body
...
let rec nnf f =
  ...
  match F.view f with
  | F.Atom _ | F.Neq _ | F.Eq _ | F.True | F.False -> f
  | F.Not f' ->
    ( match F.view f' with
    | F.Not f'' -> nnf f''
    | F.Neq (a, b) -> F.eq a b
  ...
  | F.Imply (f1, f2) -> nnf (F.or_ [F.not_ f1; f2])
  | F.Equiv (f1, f2) ->
...
let rec will_yield_lit f =
  ...
  | F.And _ | F.Or _ | F.Imply _ | F.Equiv _ | F.Xor _ -> false
  ...
  let rec maybe_rename ~polarity a b f =
    ...
    *)
    let c1, c2 =
      ...
      | `Both ->
        E.(
          (a */ p true f) +/ (b */ p false f), a +/ b +/ p true f +/ p false f)
    in
...
let flatten ~ctx seq : _ Iter.t =
  ...
  let flatten_def stmt d : _ Stmt.def_rule list =
    ...
    | Stmt.Def_term {vars; id; ty; args; rhs; as_form} ->
      flatten_rec_l ~of_:id ctx stmt Pos_inner vars args
      >>= (fun args ->
            flatten_rec_l ~of_:id ctx stmt Pos_toplevel vars args
            >>= fun args ->
    ...
    | Stmt.Def_form {vars; lhs; rhs; polarity; as_form} ->
      (let of_ =
         ...
         | _ -> None
       in
       map_sliteral (flatten_rec ?of_ ctx stmt Pos_inner vars) lhs
       >>= fun lhs ->
...
let simplify_and_rename ~ctx ~disable_renaming ~preprocess seq =
  ...
  let process_form ~is_goal stmt f =
    Util.debugf ~section 4 "@[<2>simplify and rename@ `@[%a@]`@]" (fun k ->
        k T.pp f );
    (* preprocessing *)
...
let cnf_of_seq ?(opts = []) ?(ctx = Skolem.create ()) seq =
  ...
  let v =
    flatten ~ctx seq |> simplify_and_rename ~ctx ~disable_renaming ~preprocess
  in
  ...
  let conv_form_sk f : (ID.t * type_) list * clause list =
    ...
    let clauses =
      ...
      with NotCNF _ | SLiteral.NotALit _ ->
        ...
        Util.debugf ~section 4 "@[<2>... CNF:@ `@[%a@]`@]" (fun k ->
...
let cnf_of ?opts ?ctx st = cnf_of_seq ?opts ?ctx (Iter.return st)
...
let type_declarations seq =
  ...
  seq |> Iter.flat_map Seq.ty_decls |> ID.Map.of_seq
...
let convert seq =
  ...
  let conv_statement st =
    Util.debugf ~section 5 "@[<2>@{<yellow>convert@}@ `@[%a@]`@]" (fun k ->
        k pp_c_statement st );
    ...
    let res =
      ...
      | Stmt.Rewrite d -> Stmt.rewrite ~attrs ~proof (conv_def d)
      | Stmt.TyDecl (id, ty) ->
    ...
    res
  in
  Iter.map conv_statement seq |> CCVector.of_seq |> CCVector.freeze
