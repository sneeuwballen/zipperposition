...
let enum_prop ?(mode = `Full) ((v : Term.var), sc_v) ~offset :
...
module U = struct
  ...
  let classify_pair (_, t, u) =
    match T.is_ho_app t, T.is_ho_app u with
    | false, false -> P_rigid_rigid
    | false, true | true, false -> P_flex_rigid
  ...
  let mk_pb ~subst ~penalty ~offset pairs : pb = {subst; penalty; offset; pairs}

  (* normalize problem w.r.t rigid/rigid equations *)
  let normalize_pb sc (pb : pb) : pb option =
    match flatten_rigid_rigid sc pb.subst pb.pairs with
    | None -> None
    | Some pairs ->
  ...
  *)
    ...
    while st.fuel > 0 && not (Queue.is_empty st.queue) do
      ...
      | Some ({penalty; offset; subst; pairs = (env, t1, t2) :: pairs_tl} as pb)
        ...
        ( try
            ...
            (* unify types *)
            let subst =
              Unif.Ty.unify_full ~subst (T.ty t1, sc) (T.ty t2, sc)
            in
            (* unify terms *)
            ...
            match T.view hd1, T.view hd2 with
            ...
            | T.DB a, T.DB b ->
              if a = b then
                push_new "db" ~penalty ~offset ~subst []
              else
                fail ()
            | T.Var _, _ when l1 = [] ->
  ...
  let norm_subst offset sc us =
    if !enable_norm_subst then
      Util.with_prof prof_norm_subst (norm_subst_ offset sc us) ()
    else
      us
