...
let enum_prop ?(mode = `Full) ?(add_var = false) ((v : Term.var), sc_v)
  ...
  (* FIXME: what to do? *)
    ...
    in
    ignore l_symbols;
    (* stiffle compiler warning -- symbols might be reintroduced *)
    let combs = l_combinators in
    if mode == `Combinators then
      combs
    else
      lambdas
  )
  ...
  let unif_rigid ~sc ~subst ~offset env v args t :
    ...
    and imitate =
      ...
      | T.AppBuiltin (b, l) when vars_right = [] && T.args t = [] ->
        ...
        let offset, f_vars =
          ty_args_t |> List.map (Type.arrow all_ty_args) |> mk_fresh_vars offset
        in
  ...
  *)
  ...
  let unif_loop (st : state) : unit =
    ...
    while st.fuel > 0 && not (Queue.is_empty st.queue) do
      ...
      | Some
          ({ penalty; offset; subst; pairs = (env, t1, t2) :: pairs_tl } as pb)
        ->
        ...
        (try
           ...
           | T.Var _, T.Var _ ->
             ...
             Util.debugf ~section 5 "(@[ho_unif.all_flex_flex@ %a@])" (fun k ->
                 k pp_pb pb);
