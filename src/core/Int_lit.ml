...
let equal lit1 lit2 =
  ...
  | Divides d1, Divides d2 ->
    d1.sign = d2.sign && d1.power = d2.power && Z.equal d1.num d2.num
    && M.equal d1.monome d2.monome
  | _, _ -> false

let equal_com lit1 lit2 =
  match lit1, lit2 with
  | Binary (op1, x1, y1), Binary (op2, x2, y2)
...
let hash lit =
  ...
  | Divides d ->
    Hash.combine5 20 (Hash.bool d.sign) (Z.hash d.num) (M.hash d.monome)
      (Hash.int d.power)
...
let make op m1 m2 =
  ...
  | Lesseq ->
    ...
    ( match Monome.Int.factorize m' with
    | Some (m'', g) when Z.gt g Z.one ->
      ...
      else if Z.equal c Z.zero then (* no constant, just divide by gcd *)
        _make_split op m''
      else (
...
module U = struct
  ...
  let _add_prime n = ZTbl.replace (Lazy.force _table) n None
  ...
  let rec _merge l1 l2 =
    ...
    | p1 :: l1', p2 :: l2' ->
      ...
      | 0 -> {prime = p1.prime; power = p1.power + p2.power} :: _merge l1' l2'
      | n when n < 0 -> p1 :: _merge l1' l2
      | _ -> p2 :: _merge l1 l2' )
...
let pp out = function
  | Binary (op, l, r) ->
    Format.fprintf out "@[%a %s@ %a@]" M.pp l
      ( match op with
      | Equal -> "="
      | Different -> "≠"
      | Less -> "<"
...
let pp_tstp out = function
  ...
  | Divides d when d.sign ->
    let nk = Z.pow d.num d.power in
    Format.fprintf out "$remainder_e(%a, %s) = 0" M.pp_tstp d.monome
      (Z.to_string nk)
  | Divides d ->
    let nk = Z.pow d.num d.power in
    Format.fprintf out "$remainder_e(%a, %s) != 0" M.pp_tstp d.monome
...
let pp_zf out = function
  | Binary (Equal, l, r) -> Format.fprintf out "%a = %a" M.pp_zf l M.pp_zf r
  | Binary (Different, l, r) ->
    Format.fprintf out "%a != %a" M.pp_zf l M.pp_zf r
  | Binary (Less, l, r) -> Format.fprintf out "(%a < %a)" M.pp_zf l M.pp_zf r
  | Binary (Lesseq, l, r) ->
    Format.fprintf out "(%a <= %a)" M.pp_zf l M.pp_zf r
  | Divides d when d.sign ->
    let nk = Z.pow d.num d.power in
...
let map f = function
  ...
  | Divides d ->
    mk_divides ~sign:d.sign d.num ~power:d.power (M.map f d.monome)

let fold f acc = function
  | Binary (_, m1, m2) ->
    let acc = Iter.fold f acc (Monome.Seq.terms m1) in
    Iter.fold f acc (Monome.Seq.terms m2)
  | Divides d -> Iter.fold f acc (Monome.Seq.terms d.monome)
...
let generic_unif m_unif ~subst (lit1, sc1) (lit2, sc2) k =
  ...
  | ( Binary (((Equal | Different) as op1), x1, y1),
      Binary (((Equal | Different) as op2), x2, y2) )
    when op1 = op2 ->
    (* try both ways *)
    unif4 m_unif ~subst x1 y1 sc1 x2 y2 sc2 k
  | Binary (op1, x1, y1), Binary (op2, x2, y2) ->
    if op1 = op2 then
      m_unif ~subst (x1, sc1) (x2, sc2) (fun subst ->
          m_unif ~subst (y1, sc1) (y2, sc2) k )
  | Divides d1, Divides d2 ->
    if Z.equal d1.num d2.num && d1.power = d2.power && d1.sign = d2.sign then
      m_unif ~subst (d1.monome, sc1) (d2.monome, sc2) k
  | Binary _, Divides _ | Divides _, Binary _ -> ()
...
module Subsumption = struct
  ...
  let _matching_postcond (m1, sc1) (m2, sc2) (subst, c1, c2) =
    ...
    let m1 = M.product m1 c1 and m2 = M.product m2 c2 in
    let d = M.difference m1 m2 in
    M.is_const d
    ||
    ( Util.debugf 5
        "@[<2>postcond:@ %a[%d] scaled %s,@ and %a[%d] scaled %s@ with %a@]"
        (fun k ->
          k M.pp m1 sc1 (Z.to_string c1) M.pp m2 sc2 (Z.to_string c2) S.pp
            subst );
      false )

  (* match lists together exactly. No scaling authorized *)
  let rec match_lists ~protect ~subst l1 rest1 sc1 l2 sc2 k =
    match l1, l2 with
    | [], [] ->
      if rest1 = [] then
        k subst
    ...
    | (c1, t1) :: l1', (c2, t2) :: l2' when Z.leq c1 c2 ->
      ( try
          let subst =
            Unif.FO.matching_adapt_scope ~protect ~subst
              ~pattern:(Scoped.make t1 sc1) (Scoped.make t2 sc2)
          in
          if Z.equal c1 c2 then
            match_lists ~protect ~subst (rest1 @ l1') [] sc1 l2' sc2 k
          else
            match_lists ~protect ~subst l1' rest1 sc1
              ((Z.(c2 - c1), t2) :: l2')
              sc2 k
        with Unif.Fail -> () );
      (* ignore [t1] for now *)
  ...
  let matching ~protect ~subst m1 sc1 ~scale2 m2 sc2 k =
    ...
    let rec init_with_coeff ~subst c1 l1 rest1 t2 c2 rest2 =
      ...
      | [] ->
        ...
        if scale2 then (
          ...
          check_other_terms ~subst ~scale1:(Z.divexact c2 g)
            ~scale2:(Z.divexact c1 g) rest1 rest2 )
        else if
          (* can only scale [c1], so it only works if [c1] divides [c2] *)
          Z.(equal (c2 mod c1) zero)
        then
          check_other_terms ~subst ~scale1:(Z.divexact c2 c1) ~scale2:Z.one
            rest1 rest2
        else
          ()
      | (c1', t1) :: l1' ->
    ...
    and (* match other terms with the given scaling coeff *)
        check_other_terms ~subst ~scale1 ~scale2 l1 l2 =
      let l1 = List.map (fun (c, t) -> Z.(c * scale1), t) l1
      and l2 = List.map (fun (c, t) -> Z.(c * scale2), t) l2 in
      match_lists ~protect ~subst l1 [] sc1 l2 sc2 (fun subst ->
          assert (
            _matching_postcond (Scoped.make m1 sc1) (Scoped.make m2 sc2)
              (subst, scale1, scale2) );
          k (subst, scale1, scale2) )
    in
    match M.coeffs m2 with
    | [] ->
      ( match M.coeffs m1 with
      | [] -> k (subst, Z.one, Z.one)
      | _ :: _ -> () (* fail *) )
    | (c2, t2) :: l2 ->
  ...
  let matching2 ~subst l1 r1 sc1 ~scale2 l2 r2 sc2 k =
    ...
    if M.is_const l1 && M.is_const l2 then (* only one problem *)
      matching ~protect ~subst r1 sc1 ~scale2 r2 sc2 k
    else
      matching ~protect ~subst l1 sc1 ~scale2 l2 sc2 (fun (subst, c1, c2) ->
          let r1 = List.map (fun (c, t) -> Z.(c * c1), t) (M.coeffs r1)
          and r2 = List.map (fun (c, t) -> Z.(c * c2), t) (M.coeffs r2) in
          match_lists ~protect ~subst r1 [] sc1 r2 sc2 (fun subst ->
              k (subst, c1, c2) ) )
  ...
  let check ~subst lit1 sc1 lit2 sc2 k =
    ...
    | Binary (Equal, l1, r1), Binary (Lesseq, l2, r2) ->
      ...
      matching2 ~subst l1 r1 sc1 ~scale2:true l2 r2 sc2 (fun (subst, c1, c2) ->
          if
            Z.(
              leq
                (c1 * (M.const r1 - M.const l1))
    ...
    | Binary (Lesseq, l1, r1), Binary (Different, l2, r2) ->
      ...
      matching2 ~subst l1 r1 sc1 ~scale2:true l2 r2 sc2 (fun (subst, c1, c2) ->
          ...
          then
            k subst );
      matching2 ~subst l1 r1 sc1 ~scale2:true r2 l2 sc2 (fun (subst, c1, c2) ->
          if
            Z.(
              lt
                (c1 * (M.const r1 - M.const l1))
      ...
      matching2 ~subst r1 l1 sc1 ~scale2:true r2 l2 sc2 (fun (subst, c1, c2) ->
          if
            Z.(
              gt
          ...
          then
            k subst );
      matching2 ~subst r1 l1 sc1 ~scale2:true l2 r2 sc2 (fun (subst, c1, c2) ->
          if
            Z.(
              gt
                (c1 * (M.const l1 - M.const r1))
    ...
    (* XXX: bad, because  2a = b will subsume 2|b but cannot participate
        ...
        *)
    | Divides d1, Divides d2
      when d1.sign = d2.sign && Z.equal d1.num d2.num
      ...
      matching ~protect ~subst d1.monome sc1 ~scale2:false d2.monome sc2
        (fun (subst, c1, c2) ->
          if
            ...
            && (d1.sign || Z.equal c1 Z.one)
          then
            k subst )
    | Divides d1, Divides d2
      when d1.sign && (not d2.sign) && Z.equal d1.num d2.num
           && d1.power >= d2.power ->
      ...
      matching ~protect ~subst d1.monome sc1 ~scale2:false d2.monome sc2
        (fun (subst, c1, c2) ->
          if
            Z.(
              gt
                ( ((c1 * M.const d1.monome) - (c2 * M.const d2.monome))
                mod (d2.num ** d2.power) )
                zero)
          then
            k subst )
...
let are_variant lit1 lit2 = not (Iter.is_empty (variant (lit1, 0) (lit2, 1)))
...
let apply_subst_no_simp renaming subst (lit, sc) =
  ...
  | Divides d ->
    Divides
      {d with monome = M.apply_subst_no_simp renaming subst (d.monome, sc)}
...
let is_trivial = function
  ...
  | Binary (Different, m1, m2) ->
    ...
    let gcd =
      M.coeffs m |> List.fold_left (fun c1 (c2, _) -> Z.gcd c1 c2) Z.one
    in
...
let is_absurd = function
  | Binary (Equal, m1, m2) ->
    ...
    (* absurd if: either it's a=0, with a a constant, or if
       the GCD of all coefficients does not divide the constant
       (unsolvable diophantine equation) *)
    (M.is_const m && M.sign m <> 0) || Z.sign (Z.rem (M.const m) gcd) <> 0
...
let fold_terms ?(pos = P.stop) ?(vars = false) ?ty_args ~which ~ord ~subterms
    lit k =
  (* function to call at terms *)
  let at_term ~pos t k =
    if subterms then
      T.all_positions ?ty_args ~vars ~pos t k
    else if
      (* don't do anything if [t] is a var and [vars=false] *)
      vars || not (T.is_var t)
    then
      k (t, pos)
  and fold_monome =
...
let _to_coeffs lit =
  ...
  | Divides d -> M.Seq.coeffs_swap d.monome

let to_multiset lit =
  _to_coeffs lit |> Multisets.MT.Seq.of_coeffs Multisets.MT.empty

let max_terms ~ord lit =
  let m = to_multiset lit in
  Multisets.MT.max (Ordering.compare ord) m
  |> Multisets.MT.to_list |> List.map fst
...
let to_form = function
  | Binary (op, m1, m2) ->
    ...
    | Less ->
      let t =
        T.app_builtin Builtin.Less ~ty:Type.prop [T.ty t1 |> T.of_ty; t1; t2]
      in
      SLiteral.atom t true
    | Lesseq ->
      let t =
        T.app_builtin Builtin.Lesseq ~ty:Type.prop [T.ty t1 |> T.of_ty; t1; t2]
      in
      SLiteral.atom t true )
  | Divides d ->
    let nk = Z.pow d.num d.power in
    ...
    let t1 =
      T.app_builtin ~ty:Type.prop Builtin.Remainder_e [T.of_ty (T.ty t); t; nk]
    in
...
module Focus = struct
  ...
  let mk_div ?(sign = true) num ~power m = Div {power; num; sign; monome = m}
  ...
  let get_exn lit pos =
    match get lit pos with
    | None ->
      invalid_arg
        (CCFormat.sprintf "wrong position %a for focused arith lit %a" P.pp pos
           pp lit)
    | Some x -> x
  ...
  let focus_term lit t =
    match lit with
    | Binary (op, m1, m2) ->
      ( match M.Focus.focus_term m1 t with
      | Some mf1 ->
    ...
    | Divides d ->
      ( match M.Focus.focus_term d.monome t with
      | None -> None
      | Some mf -> Some (Div {d with monome = mf}) )

  let focus_term_exn lit t =
    match focus_term lit t with
  ...
  let replace a by =
    ...
    | Div d ->
      mk_divides ~sign:d.sign d.num ~power:d.power
        (M.sum (MF.rest d.monome) by)

  let focused_monome = function
    | Left (_, mf, _) | Right (_, _, mf) -> mf
    | Div d -> d.monome

  let opposite_monome = function
    | Left (_, _, m) | Right (_, m, _) -> Some m
    | Div _ -> None
  ...
  let fold_terms ?(pos = P.stop) lit k =
    ...
    | Binary (op, m1, m2) ->
      MF.fold_m
        ~pos:P.(append pos (left stop))
        m1 ()
        (fun () mf pos -> k (Left (op, mf, m2), pos));
      MF.fold_m
        ~pos:P.(append pos (right stop))
        m2 ()
        (fun () mf pos -> k (Right (op, m1, mf), pos))
    | Divides d ->
      MF.fold_m ~pos d.monome () (fun () mf pos ->
          k (Div {d with monome = mf}, pos) )

  (* is the focused term maximal in the arithmetic literal? *)
  let is_max ~ord = function
    | Left (_, mf, m) | Right (_, m, mf) ->
      let t = MF.term mf in
      let terms = Iter.append (M.Seq.terms m) (MF.rest mf |> M.Seq.terms) in
      Iter.for_all (fun t' -> Ordering.compare ord t t' <> Comparison.Lt) terms
    | Div d ->
  ...
  let map_lit ~f_m ~f_mf lit =
    match lit with
    | Left (op, mf, m) -> Left (op, f_mf mf, f_m m)
    | Right (op, m, mf) -> Right (op, f_m m, f_mf mf)
    | Div d -> Div {d with monome = f_mf d.monome}

  let product lit z =
    map_lit ~f_mf:(fun mf -> MF.product mf z) ~f_m:(fun m -> M.product m z) lit
  ...
  let unify ?(subst = Unif_subst.empty) (lit1, sc1) (lit2, sc2) k =
    let _set_mf lit mf =
      ...
      | Div d -> Div {d with monome = mf}
    in
    MF.unify_ff ~subst
      (focused_monome lit1, sc1)
      (focused_monome lit2, sc2)
      (fun (mf1, mf2, subst) -> k (_set_mf lit1 mf1, _set_mf lit2 mf2, subst))
  ...
  let scale_power lit power =
    ...
    | Left _ | Right _ -> invalid_arg "scale_power: not a divisibility lit"

  let op = function
    | Left (op, _, _) | Right (op, _, _) -> `Binary op
    | Div _ -> `Divides
