...
type t =
  ...
  ; ty: t option
  ; mutable hash: int
  ; (* computed lazily *) loc: location option }
...
let rec deref t =
  match t.term with Meta (_, {contents= Some t'}, _) -> deref t' | _ -> t

let[@inline] must_deref t : bool =
  match t.term with Meta (_, {contents= Some _}, _) -> true | _ -> false

let view t =
  match t.term with
  | Meta (_, {contents= Some t'}, _) ->
      (deref t').term
  | v ->
      v
...
and hash_rec_ t =
  match view t with
  | Var s ->
      Hash.combine2 1 (Var.hash s)
  | Const s ->
      Hash.combine2 2 (ID.hash s)
  | App (s, l) ->
      Hash.combine3 3 (hash s) (Hash.list hash l)
  | Multiset l ->
      Hash.combine2 4 (Hash.list hash l)
  | AppBuiltin (b, l) ->
      Hash.combine3 5 (Builtin.hash b) (Hash.list hash l)
  | Bind (s, v, t') ->
      Hash.combine4 6 (Binder.hash s) (Var.hash v) (hash t')
  | Record (l, rest) ->
      Hash.combine3 7 (Hash.opt hash rest)
        (Hash.list (Hash.pair Hash.string hash) l)
  | Ite (a, b, c) ->
      Hash.combine4 8 (hash a) (hash b) (hash c)
  | Let (_, u) ->
      Hash.combine2 9 (hash u)
  | Match (u, _) ->
      Hash.combine2 10 (hash u)
  | Meta (id, _, _) ->
      Var.hash id
...
let rec compare t1 t2 =
  ...
  if h1 <> h2 then CCInt.compare h1 h2 (* compare by hash, first *)
    ...
    | AppBuiltin (b1, l1), AppBuiltin (b2, l2) ->
        Builtin.compare b1 b2 <?> (CCOrd.list compare, l1, l2)
    ...
    | Meta (id1, _, _), Meta (id2, _, _) ->
        Var.compare id1 id2
    | Ite (a1, b1, c1), Ite (a2, b2, c2) ->
        CCList.compare compare [a1; b1; c1] [a2; b2; c2]
    | Let (l1, t1), Let (l2, t2) ->
        CCOrd.(compare t1 t2 <?> (list (pair Var.compare compare), l1, l2))
...
let rec pp out t =
  ...
  | App (f, l) -> (
      ...
      | _ ->
          Format.fprintf out "@[<2>%a@ %a@]" pp_inner f
  ...
  | Let (l, u) ->
      let pp_binding out (v, t) =
        Format.fprintf out "@[%a := %a@]" Var.pp_fullc v pp t
      in
  ...
  | Match (u, l) ->
      let pp_branch out (c, vars, rhs) =
        Format.fprintf out "@[<2>case@ @[%a%a%a@] ->@ %a@]" ID.pp c.cstor_id
          ...
          vars pp rhs
      in
      Format.fprintf out "@[<hv>@[<hv2>match %a with@ %a@]@ end@]" pp u
...
and pp_infix_ b out l =
  ...
  | [] ->
      assert false
  | [t] when b = Builtin.Arrow ->
      pp out t
  | [t] ->
      pp_inner out t
  | t :: l' ->
      Format.fprintf out "@[%a@]@ %a %a" pp_inner t Builtin.pp b (pp_infix_ b)
        l'
...
and pp_var_ty out v =
  ...
  | AppBuiltin (Builtin.Term, []) ->
      ()
  | _ ->
      Format.fprintf out ":%a" pp_inner ty
...
let app_builtin ?loc ~ty b (l : t list) =
  ...
  | Builtin.Not, [f'] -> (
    ...
    | AppBuiltin (Builtin.Eq, l) ->
        mk_ Builtin.Neq l
    | AppBuiltin (Builtin.Neq, l) ->
        mk_ Builtin.Eq l
    | AppBuiltin (Builtin.Not, [t]) ->
        t
    | AppBuiltin (Builtin.True, []) ->
        mk_ Builtin.False []
    | AppBuiltin (Builtin.False, []) ->
        mk_ Builtin.True []
    | _ ->
        mk_ b l )
...
let app ?loc ~ty s l =
  ...
  | _, [] ->
      s
  | App (f, l'), _ ->
      make_ ?loc ~ty (App (f, l' @ l))
  | AppBuiltin (b, l'), _ ->
      make_ ?loc ~ty (AppBuiltin (b, l' @ l))
  | _ ->
      make_ ?loc ~ty (App (s, l))
...
let map_ty t ~f =
  {t with hash= ~-1; ty= (match t.ty with None -> None | Some x -> Some (f x))}
...
module Seq = struct
  ...
  let subterms_with_bound t k =
    let rec iter set t =
      ...
      | Ite (a, b, c) ->
          iter set a ; iter set b ; iter set c
      | Let (l, u) ->
          let set' =
            List.fold_left
...
let rec is_ground t =
  ...
  | Const _ ->
      true
  | App (f, l) ->
      is_ground f && List.for_all is_ground l
  | AppBuiltin (_, l) ->
      List.for_all is_ground l
  | Ite (a, b, c) ->
      is_ground a && is_ground b && is_ground c
  | Let (l, u) ->
      is_ground u && List.for_all (fun (_, t) -> is_ground t) l
  | Match (u, l) ->
      is_ground u && List.for_all (fun (_, _, t) -> is_ground t) l
  | Bind (_, v, t') ->
      is_ground (Var.ty v) && is_ground t'
  | Record (l, rest) ->
...
let as_id_app t =
  ...
  | Const id ->
      Some (id, ty_exn t, [])
  | App ({term= Const id; ty= Some ty; _}, l) ->
      Some (id, ty, l)
  | _ ->
      None
...
let map ~f ~bind:f_bind b_acc t =
  match view t with
  | Var v ->
      var ?loc:t.loc (Var.update_ty ~f:(f b_acc) v)
  | Const id ->
      const ?loc:t.loc id ~ty:(f b_acc (ty_exn t))
  | App (hd, l) ->
...
module Ty = struct
  ...
  let rec view (t : t) : view =
    ...
    | Var v ->
        Ty_var v
    | App (f, l) -> (
      match t_view_ f with Const id -> Ty_app (id, l) | _ -> assert false )
    | Const id ->
        Ty_app (id, [])
    | Bind (Binder.ForallTy, v, t) ->
        Ty_forall (v, t)
    | Record (l, None) ->
        Ty_record (l, None)
    | Record (l, Some r) -> (
      match t_view_ r with Var r -> Ty_record (l, Some r) | _ -> assert false )
    | Meta (_, {contents= Some ty'}, _) ->
        view ty'
    | Meta (v, o, k) ->
        Ty_meta (v, o, k)
    | AppBuiltin (Builtin.Prop, []) ->
        Ty_builtin Prop
    | AppBuiltin (Builtin.TType, []) ->
        Ty_builtin TType
    | AppBuiltin (Builtin.TyInt, []) ->
        Ty_builtin Int
    | AppBuiltin (Builtin.TyRat, []) ->
        Ty_builtin Rat
  ...
  let fun_ ?loc args ret =
    ...
    | [], _ ->
        ret
    | _, Ty_fun (args', ret') ->
        mk_fun_ ?loc (args @ args') ret'
    | _ ->
        mk_fun_ ?loc args ret
  ...
  let order ty : int =
    let rec aux ty =
      ...
      | Ty_app (_, l) ->
          List.fold_left (fun o arg -> max o (aux arg)) 0 l
      | _ ->
          0
    in
    max 1 (aux ty)
  ...
  let mangle ty =
    let add_id buf id =
      let s =
        ID.name id |> CCString.filter (function '#' -> false | _ -> true)
      in
      Buffer.add_string buf s ; Buffer.add_string buf "_u"
    in
    let rec aux buf t =
      match view t with
      | Ty_builtin TType ->
    ...
    aux buf ty ; Buffer.contents buf
  ...
  let mangle_except_builtin ty =
    ...
    | Ty_builtin TType ->
        STerm.builtin Builtin.TType
    | Ty_builtin Int ->
        STerm.builtin Builtin.TyInt
    | Ty_builtin Rat ->
        STerm.builtin Builtin.TyRat
    | Ty_builtin Real ->
        STerm.builtin Builtin.TyReal
    | Ty_builtin Prop ->
        STerm.builtin Builtin.Prop
    | Ty_builtin Term ->
        STerm.builtin Builtin.Term
    | _ ->
        STerm.const (mangle ty)
  ...
  let rec is_quantifier_free t : bool =
    ...
    | Ty_forall _ ->
        false
    | Ty_builtin _ | Ty_var _ | Ty_meta _ ->
        true
    | Ty_app (_, l) ->
        List.for_all is_quantifier_free l
    | Ty_fun (l, ret) ->
        List.for_all is_quantifier_free l && is_quantifier_free ret
    | Ty_multiset u ->
        is_quantifier_free u
    | Ty_record (l, _) ->
        List.for_all (fun (_, t) -> is_quantifier_free t) l

  let rec is_prenex ty =
    match view ty with
    | Ty_forall (_, bod) ->
        is_prenex bod
    | _ ->
        is_quantifier_free ty
  ...
  let rec is_mono t =
    ...
    | Ty_builtin _ ->
        true
    | Ty_app (_, l) ->
        List.for_all is_mono l
    | Ty_fun (l, ret) ->
        List.for_all is_mono l && is_mono ret
    | Ty_multiset t ->
        is_mono t
    | Ty_record (l, rest) ->
        List.for_all CCFun.(compose snd is_mono) l && rest = None
    | Ty_meta _ | Ty_var _ | Ty_forall (_, _) ->
        false
end
...
module Form = struct
  ...
  let is_prop_ (t : term) : bool =
    match t.ty with Some ty -> Ty.is_prop ty | None -> false
  ...
  let view (t : term) =
    ...
    | AppBuiltin (Builtin.False, []) ->
        False
    | AppBuiltin (Builtin.And, l) when Ty.is_prop (ty_exn t) ->
        And l
    | AppBuiltin (Builtin.Or, l) when Ty.is_prop (ty_exn t) ->
        Or l
    | AppBuiltin (Builtin.Not, [f]) ->
        Not f
    ...
    | AppBuiltin ((Builtin.(Eq | Neq) as hd), l) when Ty.is_prop (ty_exn t) -> (
      ...
      | [x; l; r] ->
          if hd = Builtin.Eq then Eq (l, r) else Neq (l, r)
      | _ ->
          invalid_arg "equality encoded wrongly" )
    | Bind (Binder.Forall, v, t) ->
        Forall (v, t)
    | Bind (Binder.Exists, v, t) ->
        Exists (v, t)
    | Bind ((Binder.ForallTy | Binder.Lambda), _, _) ->
        assert false
    | Ite _
    | Match _
    | Let _
  ...
  let close_forall ?loc f =
    ...
    let tyvars, vars =
      List.partition (fun v -> Ty.returns_tType (Var.ty v)) (free_vars f)
    in
...
let is_monomorphic t =
  Seq.subterms t |> Iter.for_all (fun t -> Ty.is_mono (ty_exn t))
...
module Subst = struct
  ...
  let add subst v t =
    if mem subst v then
      Util.invalid_argf "@[<2>var `@[%a@]`@ already bound in {@[%a@]}@]"
        Var.pp_full v pp subst ;
  ...
  let rec eval_ ~rename_binders ~recursive subst t =
    ...
    | _ ->
        map subst t
          ~bind:
            ( if rename_binders then rename_var ~rename_binders
              else eval_binders ~recursive ~rename_binders ~t )
  ...
  and rename_var ~rename_binders subst v =
    let v' =
      Var.copy v
      |> Var.update_ty ~f:(eval_ ~rename_binders ~recursive:true subst)
    in
    (* (re-)bind [v] to [v'] *)
  ...
  and eval_var ~rename_binders ~recursive ~t subst v =
    ...
    | None ->
        var ?loc:t.loc
          (Var.update_ty v ~f:(eval_ ~recursive ~rename_binders subst))
    | Some t' ->
        if recursive then (
          assert (t != t') ;
...
let rec rename subst t =
  ...
  | _ ->
      map subst t ~bind:bind_rename_var ~f:rename
...
let pp_stack out l =
  let pp_ty out = function
    | None ->
        ()
    | Some ty ->
        Format.fprintf out ":%a" pp ty
  in
  let pp_frame out (t1, t2) =
    Format.fprintf out "@[<2>unifying `@[%a@,%a@]`@ and `@[%a@,%a@]`@]" pp t1
      pp_ty (ty t1) pp t2 pp_ty (ty t2)
  in
...
let () =
  Printexc.register_printer (function
    | UnifyFailure (msg, st, loc) ->
        Some
          (CCFormat.sprintf "@[<2>@{<Red>unification error@}:@ %s@ %a@,%a@]" msg
             pp_stack st Loc.pp_opt loc )
    | _ ->
        None )
...
let occur_check_ ~allow_open ~subst v t =
  ...
  let rec check bound t =
    ...
    | Var v' -> (
      ...
      | None ->
          (not allow_open) && not (Var.Set.mem bound v')
      | Some t' ->
          check bound t' )
    | Ite (a, b, c) ->
        check bound a || check bound b || check bound c
    | Let (l, u) ->
        List.exists (fun (_, t) -> check bound t) l
        ||
        let bound =
          List.fold_left (fun set (v, _) -> Var.Set.add set v) bound l
        in
        check bound u
    | Match (u, l) ->
        check bound u
        || List.exists
             (fun (_, vars, rhs) ->
               let bound =
                 if allow_open then bound
                 else List.fold_left Var.Set.add bound vars
               in
               check bound rhs )
             l
    | Const _ ->
...
let unify ?(allow_open = false) ?loc ?(st = UStack.create ())
    ?(subst = Subst.empty) t1 t2 =
  let stack = ref [] in
  ...
  and unify_terms subst t1 t2 =
    ...
    | App (f1, l1), App (f2, l2) when List.length l1 = List.length l2 ->
        unif_rec subst f1 f2 ; unif_l subst l1 l2
    | App (f1, l1), App (f2, l2) ->
        ...
        if n1 > n2 then (
          ...
          let f1' =
            app f1 hd1 ~ty:Ty.(fun_ (List.map ty_exn tl1) (ty_exn t1))
          in
          unif_rec subst f1' f2 ; unif_l subst tl1 l2 )
        else
          let hd2, tl2 = CCList.take_drop (n2 - n1) l2 in
          let f2' =
            app f2 hd2 ~ty:Ty.(fun_ (List.map ty_exn tl2) (ty_exn t2))
          in
          unif_rec subst f1 f2' ; unif_l subst l1 tl2
    | Ite (a1, b1, c1), Ite (a2, b2, c2) ->
        unif_rec subst a1 a2 ; unif_rec subst b1 b2 ; unif_rec subst c1 c2
    | Let (l, u), _ ->
    ...
    | AppBuiltin (b1, l1), AppBuiltin (b2, l2)
      when List.length l1 = List.length l2 ->
        if Builtin.equal b1 b2 then unif_l subst l1 l2
          ...
          fail_ "%a and %a are not compatible (subst {@[%a@]})" Builtin.pp b1
            Builtin.pp b2 Subst.pp subst
  ...
  and unif_record_rest ~ty subst r rest =
    ...
    | None, [] ->
        ()
    | None, _ :: _ ->
        fail_ "row is absent, cannot match %a" pp_fields rest
    | Some t, _ -> (
      ...
      | _ ->
          fail_ "record row @[%a@] is not a unification variable" pp t )
  in
  unif_rec subst t1 t2

let apply_unify ?gen_fresh_meta ?allow_open ?loc ?st ?(subst = Subst.empty) ty l
    =
  Util.debugf ~section 5 "@[<>apply `%a`@ to [@[%a@]]@]" (fun k ->
      k pp ty (Util.pp_list pp) l ) ;
  let rec aux subst ty l =
    ...
    | Ty.Ty_forall (v, ty'), a :: l' ->
        ...
        Util.debugf ~section 5 "@[bind `%a` to `@[%a@]`@]" (fun k ->
            k Var.pp_fullc v pp a ) ;
    ...
    | Ty.Ty_meta _, _ -> (
      ...
      | None ->
          fail_uniff_ ?loc [] "cannot apply type `@[%a@]`@ to `@[%a@]`" pp ty
            (Util.pp_list pp) l
      | Some g ->
  ...
  and aux_l subst exp ret l =
    ...
    | _, [] ->
        Subst.eval subst (Ty.fun_ exp ret)
    | [], _ ->
        aux subst ret l
    | exp_a :: exp', a :: l' ->
...
let try_alpha_renaming f1 f2 =
  let rec aux subst = function
    ...
    | (f1, f2) :: rest -> (
      ...
      | Var v, Var v' -> (
        ...
        | Some t -> (
          ...
          | Var v'' when Var.equal v' v'' && equal (Var.ty v') (Var.ty v'') ->
              aux subst rest
          | _ ->
              fail_unif_ [(f1, f2)] "double binding" )
        | None ->
      ...
      | AppBuiltin (hd_x, _xs), AppBuiltin (hd_y, _ys)
        when Builtin.equal hd_x hd_y ->
          fail_unif_ [(f1, f2)] "arity mismatch"
      | Bind (b, v, body), Bind (b', v', body') when Binder.equal b b' ->
          assert (not @@ Subst.mem subst v) ;
          let subst =
            if not (Var.equal v v') then Subst.add subst v (var v') else subst
          in
          aux subst ((Var.ty v, Var.ty v') :: (body, body') :: rest)
      | _ ->
          fail_unif_ [(f1, f2)] "mismatch or unknown constructors" )
  in
  try
    ...
    if Var.Set.intersection_empty vars1 vars2 then (
      ...
      Util.debugf ~section 5
        "Alpha renaming succeeded:@ of %a@ and %a@ with subst %a" (fun k ->
          k pp f1 pp f2 Subst.pp subst ) ;
      Some subst )
    else None
  with UnifyFailure (msg, st, _) ->
    Util.debugf ~section 1 "Alpha renaming failed: %s@ %a" (fun k ->
        k msg pp_stack st ) ;
    None
...
let simplify_formula t =
  ...
  let simplify_and_or t b l =
    ...
    let netural_el, absorbing_el =
      if b = Builtin.And then (F.true_, F.false_) else (F.false_, F.true_)
    in
  ...
  let simpl_and_or_imp premise conclusion =
    ...
    let premise_terms =
      match view premise with AppBuiltin (And, l) -> l | _ -> [premise]
    in
    let conc_terms =
      match view conclusion with AppBuiltin (Or, l) -> l | _ -> [conclusion]
    in
    let is_true =
      List.exists (fun c -> List.exists (equal c) premise_terms) conc_terms
    in
...
let rec erase t =
  match view t with
  | Var v ->
      STerm.var (Var.to_string v)
  | Const s ->
      STerm.const (ID.to_string s)
  | App (f, l) ->
      STerm.app (erase f) (List.map erase l)
  | Bind (b, v, t) ->
      STerm.bind b
        [(STerm.V (Var.to_string v), Some (erase (Var.ty v)))]
        (erase t)
  | AppBuiltin (b, l) ->
      STerm.app_builtin b (List.map erase l)
  | Ite (a, b, c) ->
      STerm.ite (erase a) (erase b) (erase c)
  | Match (u, l) ->
      let u = erase u in
      let l =
        List.map
      ...
      let rest =
        CCOpt.map
          (fun t ->
            match view t with
            | Var v ->
                STerm.V (Var.to_string v)
            | _ ->
                failwith "cannot erase non-variable record raw" )
          rest
      in
      STerm.record (List.map (fun (n, t) -> (n, erase t)) l) ~rest
  | Meta _ ->
      failwith "cannot erase meta"

let rec depth t =
  match view t with
  | Var _ | Meta _ | Const _ ->
      0
  | App (f, l) ->
      depth_l (f :: l)
  | Bind (b, v, t) ->
