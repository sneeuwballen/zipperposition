...
let rec compare t1 t2 =
  ...
  if h1 <> h2 then
    ...
    | App (s1, l1), App (s2, l2) ->
      CCOrd.(compare s1 s2 <?> (CCOrd.list compare, l1, l2))
    | Bind (s1, v1, t1), Bind (s2, v2, t2) ->
      CCOrd.(Binder.compare s1 s2 <?> (compare, v1, v2) <?> (compare, t1, t2))
    | AppBuiltin (b1, l1), AppBuiltin (b2, l2) ->
      CCOrd.(Builtin.compare b1 b2 <?> (CCOrd.list compare, l1, l2))
...
let rec pp out t =
  ...
  | Let (l, u) ->
    let pp_binding out (v, t) =
      Format.fprintf out "@[%a := %a@]" Var.pp v pp t
    in
...
module Form = struct
  ...
  let view (t : term) =
    ...
    | Ite _ | Match _ | Let _ | Multiset _ | Record _ | Meta _ | Var _
    | Const _ | App _ | AppBuiltin _ ->
      Atom t
...
module Subst = struct
  ...
  let rec eval_ ~recursive subst t =
    match view t with
    | Var v ->
      ( match Var.Subst.find subst v with
      | None -> var ?loc:t.loc (Var.update_ty v ~f:(eval_ ~recursive subst))
      | Some t' ->
        assert (t != t');
        if recursive then
          ...
          t' )
    | _ -> map subst t ~bind:rename_var ~f:(eval_ ~recursive)
  ...
  let eval subst t =
    if is_empty subst then t else eval_ ~recursive:true subst t

  let eval_nonrec subst t =
    if is_empty subst then t else eval_ ~recursive:false subst t
end
...
let pp_stack out l =
  ...
  let pp_frame out (t1, t2) =
    Format.fprintf out "@[unifying `@[%a@,%a@]` and `@[%a@,%a@]`@]" pp t1 pp_ty
      (ty t1) pp t2 pp_ty (ty t2)
  in
...
let apply_unify ?gen_fresh_meta ?allow_open ?loc ?st ?(subst = Subst.empty) ty
    l =
