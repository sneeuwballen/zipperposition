...
let rec compare t1 t2 =
  ...
  in
  if h1 <> h2 then
    CCInt.compare h1 h2 (* compare by hash, first *)
  else (
...
let rec pp out t =
  ...
  | App (f, l) ->
    let l =
      if !InnerTerm.show_type_arguments || is_tType (ty_exn t) then
        l
      else
        List.filter (fun t -> not (ty_exn t |> is_tType)) l
    in
    let as_infix =
      match view f with
...
let is_fun = function
  | { term = Bind (Binder.Lambda, _, _); _ } -> true
  | _ -> false
...
module Form = struct
  ...
  let view (t : term) =
    ...
    | Ite _ | Match _ | Let _ | Multiset _ | Record _ | Meta _ | Var _ | Const _
    | App _ | AppBuiltin _ ->
      Atom t
...
module Subst = struct
  ...
  let rec eval_ ~rename_binders ~recursive subst t =
    ...
    | _ ->
      map subst t
        ~bind:
          (if rename_binders then
             rename_var ~rename_binders
           else
             eval_binders ~recursive ~rename_binders ~t)
  ...
  let eval ?(rename_binders = true) subst t =
    if is_empty subst then
      ...
      eval_ ~rename_binders ~recursive:true subst t

  let eval_nonrec subst t =
    if is_empty subst then
      t
    else
      eval_ ~rename_binders:true ~recursive:false subst t
end
...
let rec rectify_aux ?(pref = "v_") ~cnt ~subst t =
  ...
  | Bind (b, v_old, body) ->
    ...
    ( bind ~ty:t_ty b v body,
      if CCOpt.is_none old then
        Subst.remove subst v_old
      else
        Subst.add subst v_old (CCOpt.get_exn old) )
  | AppBuiltin (b, fs) ->
...
and handle_var ~pref ?(rename = true) ~cnt ~subst v t_ty =
  ignore pref;
  if rename && Subst.mem subst v then
    Subst.find_exn subst v, subst, v
  else (
           ...
           (fun (_, vars, rhs) ->
             let bound =
               if allow_open then
                 ...
                 List.fold_left Var.Set.add bound vars
             in
             check bound rhs)
           l
    | Const _ -> false
...
let unify ?(allow_open = false) ?loc ?(st = UStack.create ())
  ...
  and unify_terms subst t1 t2 =
    ...
    | Meta (_, r, _), _ ->
      assert (!r = None);
      if occur_check_ ~allow_open ~subst t1 t2 then
        fail_ "occur check"
      else
        UStack.bind ~st r (Subst.eval subst t2)
    | _, Meta (_, r, _) ->
      assert (!r = None);
      if occur_check_ ~allow_open ~subst t2 t1 then
        fail_ "occur check"
      else
        UStack.bind ~st r (Subst.eval subst t1)
    | Var v1, Var v2 ->
  ...
  and unif_record_fields kind subst l1 l2 =
    ...
    | (n1, t1) :: l1', (n2, t2) :: l2', `MustMatch ->
      if n1 = n2 then (
        ...
        fail_ "fields %a and %a do not match" pp_fields l1 pp_fields l2
...
let apply_unify ?gen_fresh_meta ?allow_open ?loc ?st ?(subst = Subst.empty) ty l
  ...
  let rec aux subst ty l =
    ...
    | ( ( Ty.Ty_var _ | Ty.Ty_app _ | Ty.Ty_builtin _ | Ty.Ty_multiset _
        | Ty.Ty_record _ ),
        _ ) ->
      fail_uniff_ ?loc [] "cannot apply type `@[%a@]`@ to `@[%a@]`" pp ty
        (Util.pp_list pp) l
...
let simplify_formula t =
  ...
  let simplify_and_or t b l =
    ...
    if exists_double l || List.exists (equal absorbing_el) l then
      absorbing_el
    else (
  ...
  let rec aux t =
    ...
    | AppBuiltin (((Eq | Equiv) as b), [ x; y ]) when Ty.is_prop (ty_exn x) ->
      ...
      if equal x y then
        F.true_
      else if equal x F.true_ then
        y
      else if equal x F.false_ then
        F.not_ y
      else if equal y F.true_ then
        x
      else if equal y F.false_ then
        F.not_ x
      else if equal x (F.not_ y) || equal y (F.not_ x) then
        F.false_
      else
