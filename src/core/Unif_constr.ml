...
type t = {t1: term; sc1: Scoped.scope; t2: term; sc2: Scoped.scope; tags: Proof.tag list}
...
let apply_subst renaming subst (c : t) : term * term =
  (Subst.apply renaming subst (c.t1, c.sc1), Subst.apply renaming subst (c.t2, c.sc2))

let apply_subst_l renaming subst (l : t list) : _ list = List.map (apply_subst renaming subst) l
...
let equal c1 c2 = T.equal c1.t1 c2.t1 && T.equal c1.t2 c2.t2 && c1.sc1 = c2.sc1 && c1.sc2 = c2.sc2

let compare c1 c2 : int =
  let open CCOrd.Infix in
  T.compare c1.t1 c2.t1 <?> (T.compare, c1.t2, c2.t2) <?> (CCOrd.int, c1.sc1, c2.sc1)
