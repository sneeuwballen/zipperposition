...
let make size =
  { size; stack = CCList.range 0 size |> List.map (fun _ -> None) }
...
let rec push_none_multiple env n =
  if n <= 0 then
    ...
    push_none (push_none_multiple env (n - 1))
...
let of_list l =
  ...
  List.fold_left (fun env (db, v) -> set env db v) env l

let to_list e = e.stack
let to_list_i e = List.mapi (fun i x -> CCOpt.map (CCPair.make i) x) e.stack
