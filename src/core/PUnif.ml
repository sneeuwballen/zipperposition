...
let project_hs_one ~counter pref_types i type_ui =
  ...
  let pref_args =
    List.mapi (fun i ty -> T.bvar ~ty (n_args_free - i - 1)) pref_types
  in
  let new_vars =
    List.map
      (fun ty ->
...
let imitate_one ~scope ~counter s t =
  try OSeq.nth 0 (JP_unif.imitate_onesided ~scope ~counter s t)
...
let proj_lr ~counter ~scope ~subst s t flag max_app_projs =
  ...
  if get_op flag ProjApp < max_app_projs then
    l
  else
  ...
  |> CCList.filter_map (fun ((i, _) as p) ->
         if i < List.length args_s then (
           ...
           if
             ...
             || T.is_bvar argss_arr.(i)
                && T.is_bvar hd_t
                && not (T.equal argss_arr.(i) hd_t)
           then
             None
           else
             Some p
  ...
  |> CCList.filter_map (fun (i, ty) ->
         ...
         match
           PatternUnif.unif_simple ~subst ~scope (T.of_ty arg_ret_ty)
             (T.of_ty hd_ret_ty)
         with
         | Some subst' ->
...
let proj_hs ~counter ~scope ~flex s =
  CCList.map fst
  @@ proj_lr ~counter ~scope ~subst:Subst.empty flex s Int32.zero max_int
...
let k_subset ~k l =
  let rec aux i acc l =
    if i = 0 then
      ...
      match l with
      | x :: xs -> OSeq.interleave (aux i acc xs) (aux (i - 1) (x :: acc) xs)
      | [] -> assert false
    )
  in
...
let elim_subsets_rule ?(max_elims = None) ~elim_vars ~counter ~scope t u depth =
  ...
  let same_args, diff_args =
    List.mapi
      (fun i ty ->
        if
          i < Array.length args_t
          && i < Array.length args_u
          && T.equal args_t.(i) args_u.(i)
        then
          `Left (T.bvar ~ty (pref_len - i - 1))
        else
          `Right (T.bvar ~ty (pref_len - i - 1)))
      pref_tys
    ...
    try
      let simp_proj, func_proj =
        ...
        if is_ident_last then
          ...
          proj_lr ~counter ~scope ~subst s t flag
            (get_option PUP.k_max_app_projections)
      ...
      if depth < get_option PUP.k_max_depth then (
        ...
        | `Flex x, `Flex y when HVar.equal Type.equal x y ->
          ...
          if remaining_elims > 0 then
            subset_elimination ~max_elims:(Some remaining_elims) ~counter ~scope
              s t
            |> OSeq.map (fun (sub, inc) ->
                   let flag' =
                     CCList.fold_left
                       (fun acc _ -> inc_op acc Elim)
                       flag
                       (CCList.replicate inc None)
                   in
                   Some (sub, flag'))
          else
            OSeq.return (Some (elim_trivial ~counter ~scope x, flag))
        | `Flex x, `Flex y ->
          (* all rules  *)
          let ident =
            if get_op flag Ident < get_option PUP.k_max_identifications then
              ...
              |> OSeq.map (fun x ->
                     ...
                     let subs_t =
                       T.of_term_unsafe
                       @@ fst (snd (List.hd (Subst.to_list subst)))
                     in
          ...
          in
          let projs_ident =
            if OSeq.is_empty projs && OSeq.is_empty ident then
              OSeq.empty
            else
              OSeq.append projs (delay depth ident)
          in
          if not (OSeq.is_empty projs_ident) then
            projs_ident
          else
            OSeq.return
              (Some (flex_flex_diff_trivial ~scope ~counter x y, flag))
