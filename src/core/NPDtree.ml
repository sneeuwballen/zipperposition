    ...
    let rec traverse trie iter =
      ...
      | Some i ->
        ...
        | As_app (s, _) ->
          ...
          ( match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (skip i) (* skip subterm *) ) )
    in
    try
...
module MakeTerm (X : Set.OrderedType) = struct
  ...
  let is_empty n =
    n.star = None && SIMap.is_empty n.map && Leaf.is_empty n.leaf
    ...
    let rec traverse trie iter =
      ...
      | Some i ->
        ...
        | As_app (s, l) ->
          ...
          ( match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (skip i) (* skip subterm of [t] *)
          ) )
    in
    try
      traverse (fst dt) (iterate (fst t));
