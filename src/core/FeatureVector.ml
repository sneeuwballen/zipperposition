...
module Make (C : Index.CLAUSE) = struct
  ...
  let empty_trie n =
    match n with
    | TrieNode m when IntMap.is_empty m ->
        true
    | TrieLeaf set when CSet.is_empty set ->
        true
    | _ ->
        false
        ...
        try
          ...
          let rebuild' subtrie =
            ...
            | _ when empty_trie subtrie ->
                rebuild (TrieNode (IntMap.remove c m))
            | _ ->
                rebuild (TrieNode (IntMap.add c subtrie m))
          in
  ...
  type t = {trie: trie; features: Feature.t list}

  let empty_with features = {trie= TrieNode IntMap.empty; features}
  ...
  let iter idx f =
    let rec iter = function
      | TrieLeaf set ->
          CSet.iter f set
      | TrieNode map ->
          IntMap.iter (fun _ t' -> iter t') map
    in
    iter idx.trie
  ...
  let fold f acc idx =
    let rec fold acc = function
      | TrieLeaf set ->
          CSet.fold (fun x acc -> f acc x) set acc
      | TrieNode map ->
          IntMap.fold (fun _ t' acc -> fold acc t') map acc
    in
    fold acc idx.trie
end
