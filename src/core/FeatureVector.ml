...
module Make (C : Index.CLAUSE) = struct
  ...
  module Feature = struct
    ...
    let rec _depth_term depth t =
      ...
      match T.view t with
      | T.Var _ | T.Const _ | T.DB _ -> depth
      | T.Fun _ -> assert false (* we just opened a function*)
      | T.AppBuiltin (_, l) | T.App (_, l) ->
        if CCList.is_empty l || T.is_var (T.head_term t) then
          depth
    ...
    let rec fold_lower fv node =
      ...
      | i :: fv', TrieNode map ->
        IntMap.iter
          (fun j subnode ->
            if j <= i then
              fold_lower fv' subnode (* go in the branch *)
            else
              ())
          map
    ...
    let rec fold_higher fv node =
      ...
      | i :: fv', TrieNode map ->
        IntMap.iter
          (fun j subnode ->
            if j >= i then
              fold_higher fv' subnode (* go in the branch *)
            else
              ())
          map
      | _ -> failwith "number of features in feature vector changed"
