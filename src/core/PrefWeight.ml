...
type node_tag = Var | Sym of ID.t | BT of Builtin.t

module NodeTagMap = Map.Make (struct
  let tag_to_id = function Var -> 0 | Sym _ -> 1 | BT _ -> 2
...
type trie = Empty | Node of trie NodeTagMap.t  (** {2 Unix index} *)
