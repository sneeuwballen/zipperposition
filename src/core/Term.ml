...
module Seq = struct
  ...
  let subterms t k =
    let rec aux t =
      k t;
      match view t with
      | AppBuiltin _ | Const _ | Var _ | DB _ -> ()
      | Fun (_, u) -> aux u
      | App (f, l) ->
        aux f;
        List.iter aux l
  ...
  let ty_vars t = subterms t |> Iter.flat_map (fun t -> Type.Seq.vars (ty t))
...
let symbols ?(init = ID.Set.empty) t = ID.Set.add_seq init (Seq.symbols t)
...
module AC (A : AC_SPEC) = struct
  ...
  let symbols seq =
    seq |> Iter.flat_map seq_symbols |> ID.Set.add_seq ID.Set.empty
end
  ...
  let pp_depth ?hooks:_ depth out t =
    ...
    let rec pp_rec out t =
      ...
      | AppBuiltin (b, ([t; u] | [_; t; u])) when Builtin.TPTP.is_infix b ->
        Format.fprintf out "(@[%a %a@ %a@])" pp_rec t Builtin.TPTP.pp b pp_rec
          u
      | AppBuiltin (b, l) when Builtin.TPTP.fixity b = Builtin.Infix_nary ->
        Format.fprintf out "(@[%a@])"
          (Util.pp_list ~sep:(Builtin.TPTP.to_string b) pp_rec)
          l
      | AppBuiltin (b, l) ->
        Format.fprintf out "(@[<hov2>%a@ %a@])" Builtin.TPTP.pp b
          (Util.pp_list pp_rec) l
      | Const s -> ID.pp_tstp out s
      | App (f, l) ->
        Format.fprintf out "@[<hov2>%a(@,%a)@]" pp_rec f
          (Util.pp_list ~sep:", " pp_rec)
          l
      | Fun _ ->
...
module Conv = struct
  ...
  let of_simple_term_exn ctx t =
    ...
    let rec aux t =
      ...
      | PT.Bind (Binder.Lambda, v, body) ->
        ...
        fun_ ty_arg body
      | PT.Bind _ | PT.Meta _ | PT.Record _ | PT.Ite _ | PT.Let _
      | PT.Match _ | PT.Multiset _ ->
  ...
  let to_simple_term ?(allow_free_db = false) ?(env = DBEnv.empty) ctx t =
    ...
    and aux_ty ty = Type.Conv.to_simple_term ~env ctx ty in
    aux_t env t
end
