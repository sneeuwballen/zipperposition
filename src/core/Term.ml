...
let view t =
  ...
  | T.Const s ->
      Const s
  | T.Bind (Binder.Lambda, ty, t') ->
      Fun (Type.of_term_unsafe ty, t')
  | _ ->
      assert false
...
module Classic = struct
  ...
  let view t : view =
    ...
    | T.DB i ->
        DB i
    | _ when not (Type.is_unifiable @@ ty t) ->
        NonFO
    | T.Const s ->
        App (s, [])
    | T.AppBuiltin (b, l) ->
        AppBuiltin (b, l)
    | T.App (f, l) -> (
      match T.view f with T.Const id -> App (id, l) | _ -> NonFO )
...
module Seq = struct
  ...
  let subterms ?(include_builtin = false) ?(include_app_vars = true)
    ...
    let rec aux t =
      ...
      match view t with
      | AppBuiltin (_, l) ->
          if include_builtin then List.iter aux l
      | Const _ | Var _ | DB _ ->
          ()
  ...
  let typed_symbols ?(include_types = false) t =
    ...
    let ty_syms =
      if include_types then
        Iter.flat_map (fun t -> Type.Seq.symbols (ty t)) (subterms t)
        |> Iter.map (fun ty_id -> (ty_id, Type.tType))
      else Iter.empty
    in
...
let weight ?(var = 1) ?(sym = fun _ -> 1) t =
  let rec weight t =
    if Type.is_tType (ty t) then 0
      ...
      | App (hd, l) ->
          List.fold_left (fun s t' -> s + weight t') (weight hd) l
      | Fun (_, u) ->
          1 + weight u
      | Const s ->
          sym s
...
let is_linear t =
  ...
  let rec aux t =
    ...
    | Fun (_, body) ->
        aux body
    | Var v ->
        if VarTbl.mem var_set v then false else (VarTbl.add var_set v () ; true)
    | _ ->
        true
  in
  let res = aux t in
  VarTbl.clear var_set ; res
...
let rec is_fo_term t =
  match view t with
  | Var _ ->
      not (Type.is_fun (ty t) || Type.is_prop (ty t))
  | AppBuiltin _ ->
      equal t true_ || equal t false_
  | App (hd, l) ->
...
let in_fool_fragment t =
  ...
  let rec aux ~top t =
    ...
    | App (hd, l) ->
        T.is_const hd && List.for_all (aux ~top:false) l
    | Var _ ->
...
let lambda_depth t =
  let rec aux acc t =
    match view t with
    | AppBuiltin (_, l) ->
        max_d_l (List.map (aux acc) l)
    | App (hd, l) ->
        max_d_l (List.map (aux acc) (hd :: l))
    | Fun (_, u) ->
        aux (inc_depth acc) u
    | Var _ | DB _ | Const _ ->
        acc
...
let comb_depth t =
  ...
  let rec aux ~comb_streak acc t =
    ...
    | AppBuiltin (_, l) ->
        max_d_l (List.map (aux ~comb_streak:false acc) l)
    | App (hd, l) ->
        max_d_l (List.map (aux ~comb_streak:false acc) (hd :: l))
    | Fun (_, u) ->
        invalid_arg "lambdas should have been removed."
    | Var _ | DB _ | Const _ ->
        acc
  in
...
module DB = struct
  ...
  let unskolemize sk_to_vars t =
    let rec aux depth subt =
      ...
      | Some i ->
          bvar ~ty:(ty subt) (depth + i)
      | None -> (
        match view subt with
        | Const _ | Var _ | DB _ ->
            subt
        | Fun (v_ty, body) ->
            fun_ v_ty (aux (depth + 1) body)
        | App (f, l) ->
...
let rebuild_rec t =
  let rec aux env t =
    ...
    | Const id ->
        const ~ty id
    | App (f, l) ->
        app (aux env f) (List.map (aux env) l)
    | AppBuiltin (b, l) ->
        app_builtin ~ty b (List.map (aux env) l)
    | Fun (ty_arg, bod) ->
  ...
  | None -> (
      ...
      if List.length args != 0 then
        ...
        | Builtin _ ->
            (ty_list, ty)
        | _ ->
            Printf.printf "that's not supposed to happen\n" ;
            assert false )
...
let mangle_term ty_list str_term_list t =
  let rec aux env ty_list str_term_list t =
    ...
    match view t with
    | Var _ | DB _ | Const _ ->
        (ty_list, str_term_list, T.cast ~ty:(new_ty :> t) t)
    | App (f, l) -> (
        ...
        | Const f_id ->
            ...
            let ty_list, str_term_list, new_term_args =
...
let rec is_properly_encoded t =
  ...
  | AppBuiltin (_, l) ->
      List.for_all is_properly_encoded l
  | App (hd, l) ->
      List.for_all is_properly_encoded (hd :: l)
  | Fun (_, u) ->
