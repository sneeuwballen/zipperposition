...
struct
  ...
  let hs_proj_flex_rigid ~counter ~scope ~flex u depth =
    ...
    if IntSet.mem flex_hd_id !ident_vars then
      OSeq.empty
    else (
      let projections = PUnif.proj_hs ~counter ~scope ~flex u in
      ...
      OSeq.append (OSeq.of_list simp_projs)
        (if CCList.is_empty func_projs then
           OSeq.empty
         else
           OSeq.of_list func_projs)
    )
  ...
  let proj_rule ~counter ~scope s t depth =
    let maybe_project u =
      ...
      if IntSet.mem flex_hd_id !ident_vars then
        ...
        JP_unif.project_onesided ~scope ~counter u
    in
    ...
    then
      ...
      | `Flex x, `Flex y when HVar.equal Type.equal x y ->
        ...
        OSeq.append
          ...
          @@ PUnif.elim_subsets_rule ~max_elims:None ~elim_vars ~counter ~scope
               s t depth)
      ...
      | `Flex _, `Rigid | `Rigid, `Flex _ ->
        let flex, rigid =
          if Term.is_var (T.head_term s) then
            s, t
          else
            t, s
        in
