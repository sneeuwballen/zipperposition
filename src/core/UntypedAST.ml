...
let pp_statement out st =
  ...
  match st.stmt with
  | Include s -> fpf out "@[<2>include \"%s\"@]@." (String.escaped s)
  | Decl (id, ty) ->
  ...
  | Rewrite t -> fpf out "@[<2>rewrite%a @[%a@]@]." pp_attrs attrs T.pp t
  | Data l ->
    ...
    fpf out "@[<v>data%a@ %a@]." pp_attrs attrs
      ...
      l
  | Assert f -> fpf out "@[<2>assert%a@ @[%a@]@]." pp_attrs attrs T.pp f
  | Lemma f -> fpf out "@[<2>lemma%a@ @[%a@]@]." pp_attrs attrs T.pp f
  | Goal f -> fpf out "@[<2>goal%a@ @[%a@]@]." pp_attrs attrs T.pp f
