...
module Make (E : Map.OrderedType) = struct
  ...
  let compare_partial f m1 m2 =
    ...
    let rec check_left ~neq ~max1 m1 ~max2 m2 =
      match m1 with
      | [] ->
    ...
    and filter_with ~neq ~max1 x1 n1 m1' ~max2 m2 rest2 =
      match m2 with
      | _ when Z.sign n1 <= 0 ->
      ...
      | (x2, n2) :: m2' ->
        ...
        | Comparison.Eq ->
          ...
          if c < 0 then (* remove x1 *)
            check_left ~neq ~max1 m1' ~max2
              ((x2, Z.(n2 - n1)) :: List.rev_append m2' rest2)
  ...
  let max f m =
    max_seq f m |> Iter.fold (fun m (c, t) -> add_coeff m c t) empty
