...
module Weight = struct
  ...
  let compare a b : int =
    if a.omega = b.omega then
      CCInt.compare a.one b.one
    else
      CCInt.compare a.omega b.omega

  let sign a : int = compare a zero

  let pp out (a : t) : unit =
    if a.omega = 0 then
      CCFormat.int out a.one
    else if a.one = 0 then
      Format.fprintf out "%d@<1>·@<1>ω" a.omega
    else
      Format.fprintf out "%d@<1>·@<1>ω+%d" a.omega a.one
  ...
  *)
  ...
  let unary_first ~signature _ s1 s2 =
    ...
    let weight _sig s =
      if is_post_cnf_skolem ~sig_ref:_sig s then
        min_int
      else if is_unary _sig s then
        max_int
      else if not (Signature.mem !_sig s) then
        max_int - 1
      else
...
let weight_modarity ~signature =
  ...
  fun a ->
    ...
    if is_post_cnf_skolem ~sig_ref:_sig a then
      default_weight
    else
      Weight.int (arity + 4)
...
let weight_arity0 ~signature =
  ...
  | a ->
    ...
    in
    if is_post_cnf_skolem ~sig_ref:(ref empty_sig) a then
      default_weight
    else
      Weight.int res
...
let lambda_def_weight lm_w db_w base_weight clauses =
  ...
  let find_def lhs rhs =
    let try_extracting lhs rhs =
      ...
      | T.Const id -> CCOpt.return_if (T.is_ground rhs) (id, rhs)
  ...
  let eval_weight ~weights t =
    let rec aux t =
      ...
      | Term.AppBuiltin (hd, args) ->
        Weight.( + )
          (if Builtin.is_quantifier hd then
             Weight.omega
           else
             Weight.one)
          (aux_l args)
...
let add_list ~signature p l =
  ...
  let cutoff = 30 in
  let snapshot =
    if CCList.length l < cutoff then
      mk_snapshot l
    else
      List.fast_sort p.constr (p.snapshot @ l)
  in
