...
let () =
  Printexc.register_printer (function
    | NotALit f ->
        Some (CCFormat.sprintf "@[<2>formula@ @[%a@]@ is not a lit@]" T.pp f)
    | _ ->
        None )
...
let equal eq a b =
  ...
  | True, True | False, False ->
      true
  | Atom (t1, b1), Atom (t2, b2) ->
      b1 = b2 && eq t1 t2
  | Eq (a1, a2), Eq (b1, b2) | Neq (a1, a2), Neq (b1, b2) ->
      (eq a1 b1 && eq a2 b2) || (eq a1 b2 && eq a2 b1)
  | True, _ | False, _ | Atom _, _ | Eq _, _ | Neq _, _ ->
      false
...
let of_form f =
  match F.view f with
  | F.Not f' -> (
    match F.view f' with F.Atom t -> Atom (t, false) | _ -> not_lit f )
  | F.Eq (t1, t2) ->
      Eq (t1, t2)
  | F.Neq (t1, t2) ->
      Neq (t1, t2)
