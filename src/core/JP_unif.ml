...
let iterate ?(flex_same = false) ~scope ~counter u v l =
  ...
  let positions =
    ...
    |> OSeq.flat_map (fun v ->
        ...
        |> (fun l ->
        if not flex_same then
          l
        else
          List.filter (fun (_, _, _, _, ty) -> Type.is_fun ty) l)
        |> OSeq.of_list)
  in
  ...
  let types_w_seq =
    OSeq.iterate
      ...
      []
  in
  if OSeq.is_empty positions then
    OSeq.empty
  else
    types_w_seq
  ...
  and find_disagreement_aux s t =
    if T.is_type s then
      ...
      | T.Fun (ty_s, s'), T.Fun (ty_t, t') ->
        ...
        if not (Type.equal ty_s ty_t) then
          OSeq.return ((s, t), [])
        else
          find_disagreement_aux s' t'
      | _ -> OSeq.return ((s, t), [])
