...
let project ~scope ~counter u v (_ : (T.var * int) list) =
  OSeq.append (project_onesided ~scope ~counter u) (project_onesided ~scope ~counter v)
...
let project_hs_one ~counter pref_types i type_ui =
  ...
  let new_vars =
    pref_types_ui |> List.map (fun ty -> T.var (H.fresh_cnt ~counter ~ty:(Type.arrow pref_types ty) ()))
  in
...
let imitate_onesided ~scope ~counter u v =
  ...
  if
    T.is_var head_u (* u has a varaible head *) && (not (T.is_bvar head_v)) && not (T.is_fun head_v)
    (* the head of v is not a bound variable or a lambda-expression *)
  then
...
let identify ~scope ~counter u v (_ : (T.var * int) list) =
  ...
  if T.is_var head_u && T.is_var head_v (* TODO: necessary when args_u or args_v is empty? *) then
    ...
    in
    let matrix_head =
      T.var (H.fresh_cnt ~counter ~ty:(Type.arrow (prefix_types_u @ prefix_types_v) return_type) ())
    in
...
let iterate_one ~counter types_w prefix_types return_type i type_ul =
  ...
  let inner_lambda_expr =
    ...
    let bvars_u_under_w =
      prefix_types |> List.rev |> List.mapi (fun i ty -> T.bvar ~ty (i + List.length types_w)) |> List.rev
    in
    ...
    let vars_y =
      prefix_types_ul
      |> List.map (fun ty -> T.var (H.fresh_cnt ~counter ~ty:(Type.arrow (prefix_types @ types_w) ty) ()))
    in
  ...
  let var_x =
    T.var
      (H.fresh_cnt ~counter
         ~ty:(Type.arrow (prefix_types @ [Type.arrow types_w return_type_ul]) return_type)
         () )
  in
...
let iterate ?(flex_same = false) ~scope ~counter u v l =
  ...
  let positions =
    ...
    |> OSeq.flat_map (fun v ->
           let prefix_types, return_type = Type.open_fun (HVar.ty v) in
           prefix_types
           |> List.mapi (fun i type_ul -> (v, prefix_types, return_type, i, type_ul))
           |> List.fast_sort (fun (_, _, _, _, x) (_, _, _, _, y) ->
                  List.length (Type.expected_args y) - List.length (Type.expected_args x) )
           |> (fun l -> if not flex_same then l else List.filter (fun (_, _, _, _, ty) -> Type.is_fun ty) l)
           |> OSeq.of_list )
  in
  (* The tuple `w` can be of any length. Hence we use the sequence [[alpha], [alpha, beta], [alpha, beta, gamma], ...] *)
  let types_w_seq =
    OSeq.iterate (fun types_w -> Type.var (H.fresh_cnt ~counter ~ty:Type.tType ()) :: types_w) []
  in
  if OSeq.is_empty positions then OSeq.empty
  else
    types_w_seq
                  ...
                  if Type.is_fun type_ul then
                    ...
                    @@ Some
                         (US.FO.singleton (v, scope)
                            (iterate_one ~counter types_w prefix_types return_type i type_ul, scope) )
                  else
                    OSeq.append
                      ...
                      | Type.Var alpha ->
                          ...
                          let prefix_types' =
                            prefix_types |> CCList.map (fun ty -> S.apply_ty ty_subst (ty, scope))
                          in
                          let return_type' = S.apply_ty ty_subst (return_type, scope) in
                          OSeq.return
                          @@ Some
                               (US.FO.bind ty_subst (v', scope)
                                  (iterate_one ~counter types_w prefix_types' return_type' i alpha', scope) )
                      | _ ->
                          OSeq.return None ) )
...
let find_disagreement s t =
  ...
  let rec find_disagreement_l ?(applied_var = None) ?(argindex = 0) ss tt =
    ...
    | s' :: ss', t' :: tt' ->
        ...
        |> OSeq.map (fun ((u, v), l) ->
               match applied_var with Some x -> ((u, v), (T.as_var_exn x, argindex) :: l) | None -> ((u, v), l) )
        |> fun seq ->
        if Term.is_type s' && not (OSeq.is_empty seq) then
          ...
          seq
        else OSeq.append seq (find_disagreement_l ~applied_var ~argindex:(argindex + 1) ss' tt')
    | _, _ ->
        raise (Invalid_argument "types of unified terms should be equal")
  and find_disagreement_aux s t =
    if T.is_type s then if Term.equal s t then OSeq.empty else OSeq.return ((s, t), [])
    else
      ...
      | T.AppBuiltin (f, ss), T.AppBuiltin (g, tt) when Builtin.equal f g && List.length ss = List.length tt ->
          ...
          find_disagreement_l ss tt
      | T.Var _, T.Var _ when T.equal s t ->
          OSeq.empty
      | T.DB i, T.DB j when i = j ->
          OSeq.empty
      | T.Const a, T.Const b when ID.equal a b ->
          OSeq.empty
      | T.Fun (ty_s, s'), T.Fun (ty_t, t') ->
          ...
          if not (Type.equal ty_s ty_t) then OSeq.return ((s, t), []) else find_disagreement_aux s' t'
      | _ ->
          OSeq.return ((s, t), [])
  in
  ...
  | Some d ->
      Some d
  | None -> (
    match s () with OSeq.Nil -> None | OSeq.Cons (d, _) -> Some d )
...
let unify ~scope ~counter t0 s0 =
  let rec unify_terms ?(rules = []) t s =
    ...
    | Some ((u, v), l) ->
        ...
        let subst_seq =
          if T.is_type t then OSeq.return (unif_simple ~scope s t, "unif_ty_args")
          else if Type.equal (T.ty u) (T.ty v) then
            ...
            ; (iterate ~flex_same:false ~scope ~counter, "proj_hs") ]
            (* iterate must be last in this list because it is the only one with infinitely many child nodes *)
            |> OSeq.of_list
            |> OSeq.flat_map (fun (rule, rulename) -> rule u v l |> OSeq.map (fun subst -> (subst, rulename)))
          else
        ...
        |> OSeq.map (fun (osubst, rulename) ->
               ...
               | Some subst ->
                   ...
                   in
                   unifiers |> OSeq.map (CCOpt.map (fun unifier -> US.merge subst unifier))
  ...
  | Some type_unifier ->
      ...
      (* ... then terms. *)
      let term_unifiers =
        if Term.is_fo_term t' && Term.is_fo_term s' then (
          Util.debugf 1 "Doing first-order unif %a = %a" (fun k -> k T.pp t0 T.pp s0) ;
          OSeq.return (unif_simple ~scope t' s') )
        else (
          Util.debugf 1 "Doing JP unif %a = %a" (fun k -> k T.pp t0 T.pp s0) ;
  ...
  ZProf.with_prof prof_jp_unify
    (fun () ->
      ...
      let add_renaming scope subst v =
        if US.FO.mem subst (v, scope) then subst
        else
