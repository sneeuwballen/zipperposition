...
let unif_simple ~scope t s =
  try
    let type_unifier =
      Unif.FO.unify_syn ~subst:Subst.empty (t, scope) (s, scope)
    in
...
let project ~scope ~counter u v (_ : (T.var * int) list) =
  OSeq.append
    (project_onesided ~scope ~counter u)
    (project_onesided ~scope ~counter v)
...
let project_hs_one ~counter pref_types i type_ui =
  ...
  let pref_args =
    pref_types |> List.mapi (fun i ty -> T.bvar ~ty (n_args_free - i - 1))
  in
  let new_vars =
    pref_types_ui
    |> List.map (fun ty ->
...
let imitate_onesided ~scope ~counter u v =
  ...
  then
    ...
    let subst =
      US.FO.singleton (T.as_var_exn head_u, scope) (subst_value, scope)
    in
    OSeq.return subst
  else OSeq.empty
...
let imitate ~scope ~counter u v (_ : (T.var * int) list) =
  OSeq.append
    (imitate_onesided ~scope ~counter u v)
    (imitate_onesided ~scope ~counter v u)
...
let identify ~scope ~counter u v (_ : (T.var * int) list) =
  ...
  if
    T.is_var head_u
    && T.is_var head_v (* TODO: necessary when args_u or args_v is empty? *)
  then
    ...
    let matrix_head =
      T.var
        (H.fresh_cnt ~counter
           ~ty:(Type.arrow (prefix_types_u @ prefix_types_v) return_type)
           () )
    in
    ...
    let subst =
      US.FO.singleton (T.as_var_exn head_u, scope) (subst_value_u, scope)
    in
    let subst =
      US.FO.bind subst (T.as_var_exn head_v, scope) (subst_value_v, scope)
    in
    OSeq.return subst
  else OSeq.empty
...
let iterate_one ~counter types_w prefix_types return_type i type_ul =
  ...
  let inner_lambda_expr =
    ...
    let bvars_u_under_w =
      prefix_types |> List.rev
      |> List.mapi (fun i ty -> T.bvar ~ty (i + List.length types_w))
      |> List.rev
    in
    let bvars_w =
      types_w |> List.rev |> List.mapi (fun i ty -> T.bvar ~ty i) |> List.rev
    in
    let bvar_ul_under_w =
      T.bvar ~ty:type_ul (List.length prefix_types - 1 - i + List.length types_w)
    in
    let vars_y =
      prefix_types_ul
      |> List.map (fun ty ->
    ...
    let matrix =
      T.app bvar_ul_under_w
        (vars_y |> List.map (fun y -> T.app y (bvars_u_under_w @ bvars_w)))
    in
    T.fun_l types_w matrix
  in
  let bvars_u =
    prefix_types |> List.rev |> List.mapi (fun i ty -> T.bvar ~ty i) |> List.rev
  in
  let var_x =
    T.var
      (H.fresh_cnt ~counter
         ~ty:
           (Type.arrow
              (prefix_types @ [Type.arrow types_w return_type_ul])
              return_type )
         () )
  in
...
let iterate ?(flex_same = false) ~scope ~counter u v l =
  ...
  let types_w_seq =
    OSeq.iterate
      (fun types_w ->
        Type.var (H.fresh_cnt ~counter ~ty:Type.tType ()) :: types_w )
      []
  in
...
let find_disagreement s t =
  ...
  and find_disagreement_aux s t =
    if T.is_type s then
      if Term.equal s t then OSeq.empty else OSeq.return ((s, t), [])
    else
      ...
      | T.AppBuiltin (f, ss), T.AppBuiltin (g, tt)
        when Builtin.equal f g && List.length ss = List.length tt ->
    ...
    | Some ((u, v), l) ->
        ...
        let subst_seq =
          if T.is_type t then
            ...
            let add_some f u v l =
              f ~scope ~counter u v l |> OSeq.map (fun s -> Some s)
            in
  ...
  | Some type_unifier ->
      ...
      let term_unifiers =
        if Term.is_fo_term t' && Term.is_fo_term s' then (
          Util.debugf 1 "Doing first-order unif %a = %a" (fun k ->
              k T.pp t0 T.pp s0 ) ;
