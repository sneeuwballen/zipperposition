...
module KBO : ORD = struct
  ...
  let mk_balance t1 t2 : var_balance =
    if T.is_ground t1 && T.is_ground t2 then
      ...
      let vb =
        { offset = minvar;
          (* offset of variables to 0 *)
          pos_counter = 0;
          neg_counter = 0;
          balance = AllocCache.Arr.make alloc_cache width 0 }
      in
      Obj.set_tag (Obj.repr vb.balance) Obj.no_scan_tag;
      (* no GC scan *)
      vb )
  ...
  let rec kbo ~prec t1 t2 =
    ...
    and balance_weight_rec wb terms y ~pos res =
      match terms with
      | [] -> wb, res
      | t :: terms' ->
    ...
    and tckbo (wb : W.t) t1 t2 =
      ...
      | _ when T.equal t1 t2 ->
        wb, Eq (* do not update weight or var balance *)
      | T.Var x, T.Var y ->
      ...
      | T.DB i, T.DB j -> wb, if i = j then Eq else Incomparable
      | _ ->
    ...
    and tckbo_rec wb f g ss ts =
      if prec_compare prec f g = Eq then (
        match prec_status prec f with
        | Prec.Multiset -> tckbocommute wb ss ts
        | Prec.Lexicographic -> tckbolex wb ss ts
        | Prec.LengthLexicographic -> tckbolenlex wb ss ts )
      else (
  ...
  let might_flip prec t s =
    ...
    | Some (Head.I g), Some (Head.I f) ->
      List.exists
        ...
        CCList.(0 --^ term_arity)
    | Some (Head.V _), Some (Head.I _) | Some (Head.I _), Some (Head.V _) ->
      true
    | _ -> assert false
end
    ...
    if T.equal s t then
      ...
      | _ ->
        ...
        | Some head1, Some head2 ->
          rpo6_composite ~prec s t head1 head2 (Head.term_to_args s)
            (Head.term_to_args t)
        | _ -> Incomparable ) )
...
let kbo prec =
  ...
  let compare prec a b =
    CCCache.with_cache cache (fun (a, b) -> KBO.compare_terms ~prec a b) (a, b)
  in
  { cache;
    compare;
    name = KBO.name;
    prec;
    might_flip = KBO.might_flip;
    monotonic = true }

let lfhokbo_arg_coeff prec =
  let cache = mk_cache 256 in
  let compare prec a b =
    CCCache.with_cache cache
      (fun (a, b) -> LFHOKBO_arg_coeff.compare_terms ~prec a b)
      (a, b)
  in
  { cache;
    compare;
    name = LFHOKBO_arg_coeff.name;
    prec;
    might_flip = LFHOKBO_arg_coeff.might_flip;
    monotonic = false }

let rpo6 prec =
  let cache = mk_cache 256 in
  let compare prec a b =
    CCCache.with_cache cache (fun (a, b) -> RPO6.compare_terms ~prec a b) (a, b)
  in
  let cache_might_flip = mk_cache 256 in
  let might_flip prec a b =
    CCCache.with_cache cache_might_flip
      (fun (a, b) -> RPO6.might_flip prec a b)
      (a, b)
  in
...
let default_of_list l = rpo6 (Prec.default l)
