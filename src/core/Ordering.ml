...
let is_eta_reduced_term_stable_wrt_flip t =
  ...
  | T.AppBuiltin _ | T.DB _ | T.Const _ -> true
  | _ -> false
...
let prec_compare prec a b =
  match a, b with
  | Head.I a, Head.I b ->
    (match Prec.compare prec a b with
    | 0 -> C.Eq
    | n when n > 0 -> Gt
    | _ -> Lt)
  | Head.B a, Head.B b ->
    (match Builtin.compare a b with
    | 0 -> Eq
    | n when n > 0 -> Gt
    | _ -> Lt)
  | Head.DB a, Head.DB b ->
    (match CCInt.compare a b with
    | 0 -> Eq
    | n when n > 0 -> Gt
    | _ -> Lt)
  ...
  | Head.V x, Head.V y ->
    if HVar.equal Type.equal x y then
      Eq
    else
      Incomparable
...
module TermBoolAsKey = struct
  ...
  let compare (t1, b1) (t2, b2) =
    if CCBool.equal b1 b2 then
      T.compare t1 t2
    else
      CCBool.compare b1 b2
end
...
module MakeKBO (P : PARAMETERS) : ORD = struct
  ...
  let add_pos_var balance var ~below_lam =
    ...
    if n = 0 then
      balance.pos_counter <- balance.pos_counter + 1
    else if n = -1 then
      balance.neg_counter <- balance.neg_counter - 1;
  ...
  let weight prec ~below_lam = function
    | Head.Q _ ->
      if P.ignore_deep_quants && below_lam then
        W.one
      else
        W.omega
    | Head.B _ -> W.one
  ...
  let rec kbo ~prec t1 t2 =
    ...
    and tckbo_rec wb f g ss ts ~below_lam =
      ...
      if prec_compare prec f g = C.Eq then (
        ...
        | Prec.Lexicographic -> tckbolex wb ss ts ~below_lam:ss_below_lam
...
module MakeRPO (P : PARAMETERS) : ORD = struct
  ...
  let rec rpo6 ~prec s t =
    ...
    then (
      ...
      | _, T.Var var ->
        if T.var_occurs ~var s then
          Gt
        else
          Incomparable
      | T.Var var, _ ->
        if T.var_occurs ~var t then
          Lt
        else
          Incomparable
      | _ ->
...
module EPO : ORD = struct
  ...
  and epo_behind_cache ~prec (t, tt) (s, ss) =
    ...
    then
      ...
      | (T.Var _, []), (T.Var _, []) -> Incomparable
      | _, (T.Var var, []) ->
        if T.var_occurs ~var t || CCList.exists (T.var_occurs ~var) tt then
          Gt
        else
          Incomparable
      | (T.Var var, []), _ ->
        if T.var_occurs ~var s || CCList.exists (T.var_occurs ~var) ss then
          Lt
        else
          Incomparable
      | _ ->
  ...
  and epo_composite ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    ...
    | Eq ->
      ...
      | Head.V _ ->
        if c = C.Gt then
          epo_check_e4 ~prec (t, tt) (s, ss) (g, gg) (f, ff)
        else if c = Lt then
  ...
  and epo_check_e2_e3 ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if ff = [] || epo ~prec (t, tt) (chop (f, ff)) = Gt then
      Gt
    else
      epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e2_e3_inv ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if gg = [] || epo ~prec (chop (g, gg)) (s, ss) = Lt then
      Lt
    else
      epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e4 ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if ff = [] || epo ~prec (chop (g, gg)) (chop (f, ff)) = Gt then
      Gt
    else
      epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e4_inv ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if gg = [] || epo ~prec (chop (g, gg)) (chop (f, ff)) = Lt then
      Lt
    else
      epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)
  ...
  let rec lfhokbo_arg_coeff ~prec t s =
    ...
    let lfhokbo_lenlex ts ss =
      if List.length ts = List.length ss then
        lfhokbo_lex ts ss
      else if List.length ts > List.length ss then
        Gt
      else
        Lt
    in
    ...
    let lfhokbo_composite g f ts ss =
      ...
      | _ ->
        ...
        if List.length ts = 1 && (hd_ts_s = Gt || hd_ts_s = Eq) then
          Gt
        else if List.length ss = 1 && (hd_ss_t = Gt || hd_ss_t = Eq) then
          Lt
        else
          Incomparable
    in
    ...
    if T.equal t s then
      ...
      | Some wt, Some ws ->
        if WP.compare wt ws > 0 then
          Gt
        (* by rule C1 *)
        else if WP.compare wt ws < 0 then
          Lt
        (* by rule C1 *)
...
module Polynomial = struct
  ...
  let incr_counter poly sign k =
    if sign > 0 then
      poly.pos_counter <- poly.pos_counter + k
    else if sign < 0 then
      poly.neg_counter <- poly.neg_counter + k

  let create_zero () =
    { hashtbl = ULH.create 16; pos_counter = 0; neg_counter = 0 }
...
let consider_poly t s cmp =
  match cmp with
  | C.Gt | Geq ->
    if is_more_polymorphic t s then
      cmp
    else
      Incomparable
  | Lt | Leq ->
    if is_more_polymorphic s t then
      cmp
    else
      Incomparable
  | _ -> cmp
...
module LambdaKBO : ORD = struct
  ...
  let categorize_var_arg var arg arg_ty (some_args, extra_args) =
    if is_problematic_type arg_ty then
  ...
  let analyze_weight_diff w =
    ...
    | true, false ->
      if W.sign (Polynomial.constant_monomial w) > 0 then
        Gt
      else
        Geq
    | false, true ->
    ...
    | AppBuiltin (b, _), Var _ ->
      consider_weights_poly ~prec t s
        (if Builtin.as_int b = 0 then
           ...
           C.Incomparable)
    | Var _, _ | _, Var _ -> consider_weights_poly ~prec t s C.Incomparable
...
module LambdaLPO : ORD = struct
  ...
  and compare_subs_both_ways ~prec t ts s ss =
    if check_subs ~prec ts s then
      C.Gt
    else if check_subs ~prec ss t then
      C.Lt
    else
      C.Incomparable
  ...
  and do_compare_terms ~prec t s =
    ...
    | Var y, Var x ->
      if
        HVar.id y = HVar.id x && List.for_all2 (cannot_flip ~prec) t_args s_args
      then
        cw_ext (do_compare_terms ~prec) t_args s_args
      else
        C.Incomparable
    | Var _, Fun (_, s_body) ->
      if check_subs ~prec [ s_body ] t then
        C.Lt
      else
        C.Incomparable
    | Var _, (DB _ | Const _) ->
      if check_subs ~prec s_args t then
        C.Lt
      else
        C.Incomparable
    | Var _, AppBuiltin (s_b, s_bargs) ->
      if check_subs ~prec (List.rev_append s_bargs s_args) t then
        C.Lt
      else
        C.Incomparable
    | (Const _ | DB _), Var _ ->
      if check_subs ~prec t_args s then
        C.Gt
      else
        C.Incomparable
    | Const gid, Const fid ->
    ...
    | AppBuiltin (t_b, t_bargs), Var _ ->
      if check_subs ~prec (List.rev_append t_bargs t_args) s then
        C.Gt
      else
        C.Incomparable
    ...
    | DB j, DB i ->
      if j > i then
        consider_poly t s (compare_rest ~prec t s_args)
      else if j = i then
        compare_regular_args ~prec t t_args s s_args
      else
        consider_poly t s (C.opp (compare_rest ~prec s t_args))
    | Fun (_, t_body), Var _ ->
      if check_subs ~prec [ t_body ] s then
        C.Gt
      else
        C.Incomparable
...
let lambda_kbo prec =
  ...
  in
  let normalize_cmp t =
    if Term.is_fo_term t then
      t
    else
      Lambda.eta_expand (Lambda.snf t)
  in
...
let subterm =
  let compare _ t1 t2 =
    if T.equal t1 t2 then
      C.Eq
    else if T.subterm ~sub:t1 t2 then
      Lt
    else if T.subterm ~sub:t2 t1 then
      Gt
    else
      Incomparable
  in
