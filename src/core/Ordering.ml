...
let is_term_stable_wrt_flip t =
  ...
  | T.AppBuiltin _ | T.DB _ | T.Const _ ->
      true
  | T.Fun _ ->
      is_eta_reduced_term_stable_wrt_flip (Lambda.eta_reduce t)
  | _ ->
      false
...
module Head = struct
  ...
  let pp out = function
    ...
    | B b ->
        Builtin.pp out b
    | Q Forall ->
        CCString.pp out "FORALL"
    | Q Exists ->
        CCString.pp out "EXISTS"
    | V x ->
        HVar.pp out x
    | DB i ->
        CCInt.pp out i
  ...
  let rec term_to_head s =
    ...
    | T.AppBuiltin (Builtin.ForallConst, [_; _]) ->
        Q Forall
    | T.AppBuiltin (Builtin.ExistsConst, [_; _]) ->
        Q Exists
    | T.AppBuiltin (fid, _) ->
        B fid
    | T.Const fid ->
...
let prec_compare prec a b =
  match (a, b) with
  | Head.I a, Head.I b -> (
    match Prec.compare prec a b with 0 -> C.Eq | n when n > 0 -> Gt | _ -> Lt )
  | Head.B a, Head.B b -> (
    match Builtin.compare a b with 0 -> Eq | n when n > 0 -> Gt | _ -> Lt )
  | Head.DB a, Head.DB b -> (
    match CCInt.compare a b with 0 -> Eq | n when n > 0 -> Gt | _ -> Lt )
  | Head.LAM, Head.LAM ->
      Eq
  | Head.Q Forall, Head.Q Forall ->
      Eq
  | Head.Q Exists, Head.Q Exists ->
      Eq
  | Head.Q Forall, Head.Q Exists ->
      Gt
  | Head.Q Exists, Head.Q Forall ->
      Lt
  ...
  | Head.B _, Head.Q _ ->
      Lt
  | Head.V x, Head.V y ->
      if HVar.equal Type.equal x y then Eq else Incomparable
  | Head.V _, _ ->
      Incomparable
  | _, Head.V _ ->
      Incomparable
...
module MakeKBO (P : PARAMETERS) : ORD = struct
  ...
  let add_pos_var balance var ~below_lam =
    ...
    if n = 0 then balance.pos_counter <- balance.pos_counter + 1
  ...
  let add_neg_var balance var ~below_lam =
    let n = TermBoolTbl.get_or balance.balance (var, below_lam) ~default:0 in
    if n = 0 then balance.neg_counter <- balance.neg_counter + 1
  ...
  let weight prec ~below_lam = function
    | Head.Q _ ->
        if P.ignore_deep_quants && below_lam then W.one else W.omega
    | Head.B _ ->
        W.one
    | Head.I s ->
        Prec.weight prec s
  ...
  let rec kbo ~prec t1 t2 =
    ...
    and tckbolex wb terms1 terms2 ~below_lam =
      ...
      | [], [] ->
          (wb, C.Eq)
      | t1 :: terms1', t2 :: terms2' -> (
        match tckbo wb t1 t2 ~below_lam with
        | wb', C.Eq ->
    ...
    and tckbo (wb : W.t) t1 t2 ~below_lam =
      if T.equal t1 t2 then (wb, C.Eq) (* do not update weight or var balance *)
    ...
    and tckbo_rec wb f g ss ts ~below_lam =
      ...
      if prec_compare prec f g = C.Eq then
        match prec_status prec f with
        | Prec.Multiset ->
            assert false
        | Prec.Lexicographic ->
...
module MakeRPO (P : PARAMETERS) : ORD = struct
  ...
  let rec rpo6 ~prec s t =
    ...
    then
      ...
      | Head.V _, Head.V _ ->
          Incomparable
      | _, Head.V _ ->
          if has_subterm s t then Gt else Incomparable
      | Head.V _, _ ->
      ...
      | T.Var _, T.Var _ ->
          Incomparable
      | _, T.Var var ->
          if T.var_occurs ~var s then Gt else Incomparable
      | T.Var var, _ ->
  ...
  and rpo6_composite ~prec s t f g ss ts =
    ...
    | Eq -> (
      match prec_status prec f with
      | Prec.Multiset ->
          cMultiset ~prec s t ss ts
      | Prec.Lexicographic ->
          cLMA ~prec s t ss ts
      | Prec.LengthLexicographic ->
          cLLMA ~prec s t ss ts )
    | Gt ->
        cMA ~prec s ts
    | Lt ->
        C.opp (cMA ~prec t ss)
  ...
  and cLLMA ~prec s t ss ts =
    if List.length ss = List.length ts then cLMA ~prec s t ss ts
    else if List.length ss > List.length ts then cMA ~prec s ts
    else C.opp (cMA ~prec t ss)
  ...
  and alpha ~prec ss t =
    ...
    | [] ->
        Incomparable
    | s :: ss' -> (
      match rpo6 ~prec s t with Eq | Gt -> Gt | _ -> alpha ~prec ss' t )

  let compare_terms ~prec x y =
    let _span = ZProf.enter_prof prof_rpo in
    let compare = rpo6 ~prec x y in
    ZProf.exit_prof _span ; compare
  ...
  let might_flip prec s t = not (cannot_flip ~prec s t)
end

module EPO : ORD = struct
  let name = "epo"

  let rec epo ~prec (t, tt) (s, ss) =
    CCCache.with_cache _cache
      (fun ((t, tt), (s, ss)) -> epo_behind_cache ~prec (t, tt) (s, ss))
      ((t, tt), (s, ss))
  ...
  and epo_composite ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    ...
    | Gt ->
        epo_check_e2_e3 ~prec (t, tt) (s, ss) (g, gg) (f, ff)
    | Lt ->
        epo_check_e2_e3_inv ~prec (t, tt) (s, ss) (g, gg) (f, ff)
    | Eq -> (
        let c =
          match prec_status prec g with
          | Prec.Multiset ->
              assert false
          | Prec.Lexicographic ->
              epo_lex ~prec gg ff
  ...
  and epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if
      gg != []
      &&
      let c = epo ~prec (s, ss) (chop (g, gg)) in
      c = Lt || c = Eq
    then Gt
    else if
      ff != []
      &&
      let c = epo ~prec (t, tt) (chop (f, ff)) in
      c = Lt || c = Eq
    then Lt
    else Incomparable

  and epo_check_e2_e3 ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if ff = [] || epo ~prec (t, tt) (chop (f, ff)) = Gt then Gt
    else epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e2_e3_inv ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if gg = [] || epo ~prec (chop (g, gg)) (s, ss) = Lt then Lt
    else epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e4 ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if ff = [] || epo ~prec (chop (g, gg)) (chop (f, ff)) = Gt then Gt
    else epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)

  and epo_check_e4_inv ~prec (t, tt) (s, ss) (g, gg) (f, ff) =
    if gg = [] || epo ~prec (chop (g, gg)) (chop (f, ff)) = Lt then Lt
    else epo_check_e1 ~prec (t, tt) (s, ss) (g, gg) (f, ff)
  ...
  and epo_llex ~prec gg ff =
    ...
    if m < n then Lt else if m > n then Gt else epo_lex ~prec gg ff
  ...
  and epo_lex ~prec gg ff =
    ...
    | gg_hd :: gg_tl, ff_hd :: ff_tl ->
        ...
        if c = Eq then epo_lex ~prec gg_tl ff_tl else c
    | _ :: _, [] ->
        Gt
    | [], _ :: _ ->
        Lt
...
module LambdaFreeKBOCoeff : ORD = struct
  ...
  module Weight_indet = struct
    ...
    type t = Weight of var | Arg_coeff of var * int

    let compare x y =
      match (x, y) with
      | Weight x', Weight y' ->
          HVar.compare Type.compare x' y'
      | Arg_coeff (x', i), Arg_coeff (y', j) ->
          let c = HVar.compare Type.compare x' y' in
          if c <> 0 then c else abs (i - j)
      | Weight _, Arg_coeff (_, _) ->
          1
      | Arg_coeff (_, _), Weight _ ->
          -1

    let pp out (a : t) : unit =
      match a with
      | Weight x ->
          Format.fprintf out "w_%a" HVar.pp x
      | Arg_coeff (x, i) ->
          Format.fprintf out "k_%a_%d" HVar.pp x i
  ...
  let rec weight prec t =
    ...
    let arg_coeff_multiplier t i =
      match T.view t with
      | T.Const fid ->
          Some (WP.mult_const (Prec.arg_coeff prec fid i))
      | T.Var x ->
          Some (WP.mult_indet (WI.Arg_coeff (x, i)))
      | _ ->
          None
    in
    ...
    match T.view t with
    | T.App (f, args) ->
        app_weight (weight prec f) (arg_coeff_multiplier f) args
    | T.AppBuiltin (_, args) ->
        app_weight (Some (WP.const W.one)) (fun _ -> Some (fun x -> x)) args
    | T.Const fid ->
        Some (WP.const (Prec.weight prec fid))
    | T.Var x ->
        Some (WP.indet (WI.Weight x))
    | _ ->
        None

  let rec lfhokbo_arg_coeff ~prec t s =
    (* lexicographic comparison *)
    let rec lfhokbo_lex ts ss =
      match (ts, ss) with
      | [], [] ->
          C.Eq
      | _ :: _, [] ->
      ...
      | [], _ :: _ ->
          Lt
      | t0 :: t_rest, s0 :: s_rest -> (
        match lfhokbo_arg_coeff ~prec t0 s0 with
        | C.Gt ->
            Gt
        | Lt ->
            Lt
        | Eq ->
            lfhokbo_lex t_rest s_rest
        | _ ->
            Incomparable )
    in
    let lfhokbo_lenlex ts ss =
      if List.length ts = List.length ss then lfhokbo_lex ts ss
      else if List.length ts > List.length ss then Gt
      else Lt
    in
    ...
    let lfhokbo_composite g f ts ss =
      ...
      | _ ->
          ...
          if List.length ts = 1 && (hd_ts_s = Gt || hd_ts_s = Eq) then Gt
          else if List.length ss = 1 && (hd_ss_t = Gt || hd_ss_t = Eq) then Lt
          else Incomparable
    in
    ...
    let lfhokbo_same_weight t s =
      ...
      | _ ->
          let g, f = (Head.term_to_head t, Head.term_to_head s) in
          lfhokbo_composite g f (Head.term_to_args t) (Head.term_to_args s)
    in
    if T.equal t s then Eq
    else
...
module Polynomial = struct
  ...
  let compare unk2 unk1 =
    match (unk2, unk1) with
    | EtaUnknown y, EtaUnknown x ->
        HVar.compare Type.compare y x
    | EtaUnknown _, _ ->
        1
    | _, EtaUnknown _ ->
        -1
    | WeightUnknown ts, WeightUnknown ss ->
        CCList.compare T.compare ts ss
    | WeightUnknown _, _ ->
        1
    | _, WeightUnknown _ ->
        -1
  ...
  let hash_unknown = function
    | EtaUnknown var ->
        HVar.hash var
    | WeightUnknown ts ->
        Hash.list Term.hash ts
    | CoeffUnknown (ts, i) ->
        Hash.list Term.hash ts + i + 1
  ...
  let create_zero () = {hashtbl= ULH.create 16; pos_counter= 0; neg_counter= 0}

  let is_zero poly = poly.pos_counter + poly.neg_counter = 0

  let all_coeffs_nonnegative poly = poly.neg_counter = 0

  let all_coeffs_nonpositive poly = poly.pos_counter = 0
...
let same_length_lex_ext f ys xs =
  let rec lex ys xs =
    ...
    | y :: ys, x :: xs -> (
      match f y x with
      | C.Geq ->
          C.merge_with_Geq (lex ys xs)
      | Eq ->
          lex ys xs
      | Leq ->
          C.merge_with_Leq (lex ys xs)
      | cmp ->
          cmp )
    | _, _ ->
        assert false
...
let consider_poly t s cmp =
  match cmp with
  | C.Gt | Geq ->
      if is_more_polymorphic t s then cmp else Incomparable
  | Lt | Leq ->
      if is_more_polymorphic s t then cmp else Incomparable
  | _ ->
      cmp
...
module LambdaKBO : ORD = struct
  ...
  let consider_weight w cmp =
    ...
    , match analyze_weight_diff w with
      | Geq ->
          C.merge_with_Geq cmp
      | Eq ->
          cmp
      | Leq ->
          C.merge_with_Leq cmp
      | cmp' ->
          cmp' )
  ...
  let rec process_args ~prec ts ss =
    ...
    if CCList.is_empty ts && CCList.is_empty ss then (w, C.Eq)
    else
      let ws, cmp = same_length_lex_ext_data (process_terms ~prec) ts ss in
  ...
  and process_terms ~prec t s =
    ...
    | Fun (t_ty, t_body), Fun (s_ty, s_body) -> (
      ...
      | Eq ->
          process_terms ~prec t_body s_body
      | cmp ->
          consider_weights_poly ~prec t_body s_body cmp )
    | Fun _, (DB _ | Const _ | AppBuiltin _) ->
        consider_weights_poly ~prec t s Gt
    | DB _, Fun _ ->
        consider_weights_poly ~prec t s Lt
    | DB j, DB i ->
    ...
    | Const gid, Const fid -> (
      ...
      | 0 -> (
        ...
        | 0 -> (
          ...
          | cmp ->
              consider_weights_poly ~prec t s cmp )
        | n ->
            consider_weights_poly ~prec t s (C.of_total n) )
      | n when n > 0 ->
          consider_weights_poly ~prec t s Gt
      | _ ->
          consider_weights_poly ~prec t s Lt )
    | Const _, AppBuiltin _ ->
        consider_weights_poly ~prec t s Gt
...
module LambdaLPO : ORD = struct
  ...
  let rec cannot_flip ~prec t s =
    ...
    || T.equal t s
    || is_term_stable_wrt_flip t && is_term_stable_wrt_flip s
       && not
  ...
  and compare_regular_args ~prec t ts s ss =
    ...
    | ti :: ts', si :: ss' -> (
      ...
      | C.Gt ->
          compare_rest ~prec t ss'
      | C.Geq ->
          C.merge_with_Geq (compare_regular_args ~prec t ts' s ss')
      | C.Eq ->
          compare_regular_args ~prec t ts' s ss'
      | C.Leq ->
          C.merge_with_Leq (compare_regular_args ~prec t ts' s ss')
      | C.Lt ->
          C.opp (compare_rest ~prec s ts')
      | C.Incomparable ->
          compare_subs_both_ways ~prec t ts' s ss' )
    | _, _ ->
        assert false
  ...
  and do_compare_terms ~prec t s =
    ...
    | Const gid, Const fid -> (
      ...
      | 0 -> (
        ...
        | 0 -> (
          ...
          | Eq ->
              compare_regular_args ~prec t t_args s s_args
          | Lt ->
              C.opp (compare_rest ~prec s t_args)
          | _ ->
              compare_subs_both_ways ~prec t t_args s s_args )
        | n when n > 0 ->
            compare_rest ~prec t s_args
        | _ ->
            C.opp (compare_rest ~prec s t_args) )
    ...
    | AppBuiltin (t_b, t_bargs), AppBuiltin (s_b, s_bargs) -> (
        ...
        | 0 -> (
          ...
          | n when n > 0 ->
              compare_rest ~prec t all_s_args
          | _ ->
              C.opp (compare_rest ~prec s all_t_args) )
        | n when n > 0 ->
            compare_rest ~prec t all_s_args
        | _ ->
            C.opp (compare_rest ~prec s all_t_args) )
    ...
    | Fun (t_ty, t_body), Fun (s_ty, s_body) -> (
      ...
      | Gt ->
          compare_rest ~prec t [s_body]
      | Eq ->
          do_compare_terms ~prec t_body s_body
      | Lt ->
          C.opp (compare_rest ~prec s [t_body])
      | _ ->
          compare_subs_both_ways ~prec t [t_body] s [s_body] )
    | App _, _ | _, App _ ->
        assert false
  ...
  { ord with
    compare=
      (fun prec a b ->
        ...
        match f_res with
        | C.Geq ->
            C.merge_with_Geq (ord.compare prec a' b')
        | Eq ->
            ord.compare prec a' b'
        | Leq ->
            C.merge_with_Leq (ord.compare prec a' b')
        | _ ->
            f_res ) }
...
let none =
  ...
  let monotonic = true in
  { cache_compare= dummy_cache_
  ; compare
  ; prec= Prec.default []
  ; name= "none"
  ; might_flip
  ; cache_might_flip= dummy_cache_
  ; monotonic }
