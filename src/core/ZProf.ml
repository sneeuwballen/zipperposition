...
type span = No_span | Span of {start: timestamp; prof: t}
...
module TEF = struct
  ...
  let emit_sep_ oc = if !first_ then first_ := false else output_string oc ",\n"
...
let make name =
  let prof = {prof_name= name; prof_total= 0.; prof_calls= 0; prof_max= 0.} in
  (* register profiler *)
...
let[@inline] message f = if __prof && Lazy.force active_ then message_real_ f
...
let pp_prof () =
  if Lazy.force prof_active_ then Format.eprintf "%a@." show_profilers () ;
  TEF.teardown ()

let setup () = TEF.setup () ; at_exit pp_prof
