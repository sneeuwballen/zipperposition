...
let stat_refine_calls = Util.mk_stat "ho_refine.calls"
let stat_refine_steps = Util.mk_stat "ho_refine.steps"
...
let refine_step ((c:t),sc_combs) (t,sc_pair): _ list =
  ...
  in
  Util.incr_stat stat_refine_calls;
  begin
    seq
    ...
    |> CCFun.tap
      (fun substs ->
         if not @@ CCList.is_empty substs then (
           ...
           Util.add_stat stat_refine_steps (List.length substs);
           Util.debugf ~section 4
             "(@[ho_refine.step@ `@[%a@]`@ :substs (@[<hv>%a@])@])"
             (fun k->k T.pp t (Util.pp_list pp_subst) substs);
         );
      )
  end
