...
(** A signal can be used to decouple the (unique) emitter of some event, and
    (multiple) receivers for this event. The signal is typically created by the
    emitter (in the same module), and exposed; then, observers can register to
...
(** Register a handler to the signal; the handler returns [true] if it wants to
    continue being notified, [false] otherwise *)
