...
let _mono_ty_args_per_clause =
  ref {relative_bound= Some 1000000.0; absolute_cap= Some 500; relative_floor= 1000000}
...
let match_type ~pattern ~mono_type = Unif.Ty.matching_same_scope ~pattern mono_type ~scope:0
...
let typed_sym term =
  ...
  let get_typed_sym app_term =
    ...
    match T.head f with Some id when ty_well_formed ty -> Some (id, T.ty_args f) | _ -> None
  in
...
let iter_union ~eq iter_1 iter_2 = remove_duplicates (Iter.append iter_1 iter_2) ~eq
...
let max_nb len bound =
  ...
  in
  raise_opt rel_cap_opt bound.absolute_cap (fun rel_cap abs_cap -> min abs_cap rel_cap)
...
let derive_type_arg_subst mono_map poly_map =
  ...
  let combine fun_sym (old_poly_args, new_poly_args) subst_acc =
    ...
    let old_poly_subst = Iter.persistent_lazy (type_arg_iter_subst new_mono_args old_poly_args) in
...
let apply_ty_arg_subst subst poly_ty_args max_new_mono_opt max_new_poly_opt =
  ...
  let subst_ty_domain =
    Iter.map
      (fun (ty_var : InnerTerm.t HVar.t) -> HVar.make ~ty:(Ty.of_term_unsafe ty_var.ty) ty_var.id)
      subst_domain
  in
  ...
  let ty_vars ty_args =
    remove_duplicates ~eq:ty_var_eq
      (List.fold_left (fun acc ty -> Iter.append (Ty.Seq.vars ty) acc) Iter.empty ty_args)
  in
  ...
  in
  let max_new_mono = match max_new_mono_opt with None -> Iter.length mono_candidates | Some max -> max in
  let max_new_poly = match max_new_poly_opt with None -> Iter.length poly_candidates | Some max -> max in
...
let max_new_ty_args fun_sym mono_map poly_map =
  ...
  let max_mono_bound =
    max_nb (Iter.length (fst all_mono_ty_args) + Iter.length (snd all_mono_ty_args)) !_mono_ty_args_per_sym
  in
  let max_poly_bound =
    max_nb (Iter.length (fst all_poly_ty_args) + Iter.length (snd all_poly_ty_args)) !_poly_ty_args_per_sym
  in
...
let apply_subst_map mono_map poly_map new_subst =
  ...
  let update_maps fun_sym (old_poly_ty_args, new_poly_ty_args)
    ...
    let remaining_clause_mono =
      raise_opt remaining_clause_mono (Some (Iter.length all_new_mono_ty_args)) ( - )
    in
    let remaining_clause_poly =
      raise_opt remaining_clause_poly (Some (Iter.length all_new_poly_ty_args)) ( - )
    in
...
let mono_step_clause mono_map poly_map susbt_clause_map curr_iteration =
  ...
  let new_mono_map_all, new_poly_map_all, used_substs_iter = apply_subst_map mono_map poly_map new_subst_all in
...
let map_initialisation_step (mono_map, clause_poly_map, pb_subst_map) (clause_id, literals) =
  ...
  let remove_duplicate_init map =
    ArgMap.map (fun (_, iter) -> (Iter.empty, Iter.persistent (remove_duplicates ~eq:ty_arg_eq iter))) map
  in
...
let generate_monomorphising_subst subst_map ty_var_iter max_new_subst =
  ...
  in
  let ty_var_eq = HVar.equal Ty.ty_eq in
  let remaining_ty_vars ty_var_iter subst = Iter.diff ~eq:ty_var_eq ty_var_iter (subst_ty_domain subst) in
...
let clause_ty_vars lit_arr =
  ...
  let all_vars =
    Iter.persistent_lazy
      (remove_duplicates ~eq:var_eq (Iter.flat_map (fun lit -> Literal.Seq.vars lit) (Iter.of_array lit_arr)))
  in
  ...
  let rec iter_mono_subst iteration new_subst_limit =
    ...
    if iteration < 0 then Iter.empty
      ...
      if Iter.length prev_subst >= new_subst_limit then Iter.take new_subst_limit prev_subst
        ...
        let curr_subst =
          generate_monomorphising_subst iter_subst_map vars_to_instantiate !_monomorphising_subst_per_clause
        in
...
let monomorphise_problem_base clause_list =
  ...
  let final_mono, final_non_mono, subst_map_res =
    let init_mono_map =
      ArgMap.map (fun (_, init_iter) -> (Iter.persistent Iter.empty, Iter.persistent init_iter)) init_mono_map
    in
  ...
  let total_clause_limit =
    let relative_max = int_of_float (!_new_clauses_relative_bound *. float_of_int (List.length clause_list)) in
    max !_e_max_derived relative_max
  in
  let new_clauses =
    List.fold_left new_clauses_fold (Iter.empty, total_clause_limit) poly_clause_list
    |> fst |> remove_duplicates ~eq:clause_eq
  in
...
let run_with_time_limit limit f =
  ...
  let () = Sys.set_signal Sys.sigalrm (Sys.Signal_handle (fun _ -> raise Out_of_time)) in
...
let rec convert_type ty =
  ...
  else match view ty with Builtin _ -> ty | _ -> Ty.const (ID.make (Ty.mangle ty))
...
let () =
  Options.add_opts
    [ ( "--sym-mono-ty-args"
      ...
      , " parameters for controlling the number of new monomorphic type argument for each symbol per clause per \
         iteration" )
    ; ( "--sym-poly-ty-args"
      , Arg.String
          (fun s ->
      ...
      , " parameters for controlling the number of new polymorphic type argument for each symbol per clause per \
         iteration" )
    ; ( "--clause-mono-ty-args"
      , Arg.String
          (fun s ->
    ...
    ; ( "--monomorphising-subst-per-clause"
      ...
      , " maximum number of instantiating substitutions that can be generated by monomorphisation algorithm per \
