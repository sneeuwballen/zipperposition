...
let to_int_ = function
  ...
  | True ->
      if !_t_bigger_false then 1 else 0
  | False ->
      if !_t_bigger_false then 0 else 1
  | Not ->
      2
  | And ->
      3
...
let hash s =
  match s with
  | Int i ->
      Hash.combine2 1 (Z.hash i)
  | Rat r ->
      Hash.combine2 2 (Hash.string (Q.to_string r))
  | c ->
      Hash.combine2 3 (Hashtbl.hash c)
...
let is_logical_op = function
  | And | Or | Not | Imply | Equiv | Xor | ForallConst | ExistsConst ->
      true
  | _ ->
      false
...
let is_quantifier = function ForallConst | ExistsConst -> true | _ -> false
...
module Tag = struct
  ...
  let pp out = function
    ...
    | T_ho ->
        Fmt.string out "ho"
    | T_live_cnf ->
        Fmt.string out "live_cnf"
    | T_ho_norm ->
        Fmt.string out "ho_norm"
    | T_dont_increase_depth ->
        Fmt.string out "dont_increase_depth"
    | T_ext ->
        Fmt.string out "extensionality"
    | T_ind ->
        Fmt.string out "ind"
    | T_data ->
        Fmt.string out "data"
    | T_distinct ->
        Fmt.string out "distinct_constants"
    | T_ac id ->
        Fmt.fprintf out "(ac %a)" ID.pp_full id
    | T_cannot_orphan ->
        Fmt.fprintf out "cannot orphan"
end
...
module TPTP = struct
  let to_string = function
    ...
    | Prop ->
        "$o"
    | Multiset ->
        failwith "cannot print this symbol in TPTP"
    | ForallConst ->
        "!!"
    | ExistsConst ->
        "??"
  ...
  let is_connective = function Int _ | Rat _ -> false | _ -> true
end
...
module ArithOp = struct
  ...
  let sign = function
    ...
    | Rat n ->
        Q.sign n
    | s ->
        _ty_mismatch "cannot compute sign of symbol %a" pp s
  ...
  let zero_of_ty = function `Rat -> zero_rat | `Int -> zero_i

  let one_of_ty = function `Rat -> one_rat | `Int -> one_i
  ...
  let is_zero = function
    ...
    | Rat n ->
        Q.sign n = 0
    | s ->
        _ty_mismatch "not a number: %a" pp s
  ...
  let is_minus_one = function
    | Int n ->
        Z.equal n Z.minus_one
    | Rat n ->
        Q.equal n Q.minus_one
    | s ->
        _ty_mismatch "not a number: %a" pp s
  ...
  let floor s =
    ...
    | Int _ ->
        s
    | Rat n ->
        mk_int (Q.to_bigint n)
    | s ->
        _ty_mismatch "not a numeric constant: %a" pp s
  ...
  let ceiling s =
    ...
    | Int _ ->
        s
    | Rat _ ->
        failwith "Q.ceiling: not implemented" (* TODO *)
    | s ->
        _ty_mismatch "not a numeric constant: %a" pp s
  ...
  let truncate s =
    ...
    | Int _ ->
        s
    | Rat n when Q.sign n >= 0 ->
        mk_int (Q.to_bigint n)
    | Rat _ ->
        failwith "Q.truncate: not implemented" (* TODO *)
    | s ->
        _ty_mismatch "not a numeric constant: %a" pp s
  ...
  let round s =
    ...
    | Int _ ->
        s
    | Rat _ ->
        failwith "Q.round: not implemented" (* TODO *)
    | s ->
        _ty_mismatch "not a numeric constant: %a" pp s
  ...
  let err2_ s1 s2 =
    match (s1, s2) with
    | Int _, Rat _ | Rat _, Int _ ->
        _ty_mismatch "incompatible numeric types: %a and %a" pp s1 pp s2
    | _ ->
        _ty_mismatch "not numeric constants: %a, %a" pp s1 pp s2
  ...
  let remainder_e s1 s2 =
    ...
    | Int n1, Int n2 ->
        mk_int (Z.rem n1 n2)
    | _ ->
        difference s1 (product (quotient_e s1 s2) s2)

  let remainder_t s1 s2 =
    match (s1, s2) with
    | Int n1, Int n2 ->
        mk_int (Z.rem n1 n2)
    | _ ->
        difference s1 (product (quotient_t s1 s2) s2)

  let remainder_f s1 s2 =
    match (s1, s2) with
    | Int n1, Int n2 ->
        mk_int (Z.rem n1 n2)
    | _ ->
        difference s1 (product (quotient_f s1 s2) s2)

  let to_int s = match s with Int _ -> s | _ -> floor s

  let to_rat s =
    match s with
    | Int n ->
        mk_rat (Q.of_bigint n)
    | Rat _ ->
        s
    | _ ->
        _ty_mismatch "not a numeric constant: %a" pp s
  ...
  let divides a b =
    ...
    | Rat i, Rat _ ->
        Q.sign i <> 0
    | Int a, Int b ->
        Z.sign a <> 0 && Z.sign (Z.rem b a) = 0
    | _ ->
        _ty_mismatch "divides: expected two numerical types"
...
module ZF = struct
  let to_string = function
    ...
    | Prop ->
        "prop"
    | Term ->
        "term" (* XXX needs to be declared! *)
    | Xor | Multiset ->
        failwith "cannot print this symbol in ZF"
    | ForallConst ->
        "!!"
    | ExistsConst ->
        "??"
