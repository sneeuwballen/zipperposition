...
let is_arith = function
  ...
  | To_int | To_rat | Less | Lesseq | Greater | Greatereq ->
    true
  | _ -> false
...
let fixity = function
  | And | Or -> Infix_nary
  | Imply | Equiv | Xor | Eq | Neq | HasType | Sum | Difference | Product
  | Quotient | Quotient_e | Quotient_f | Quotient_t | Remainder_e
  | Remainder_t | Remainder_f | Less | Lesseq | Greater | Greatereq ->
    Infix_binary
  | _ -> Prefix
...
module TPTP = struct
  ...
  let fixity = function
    | And | Or -> Infix_nary
    | Imply | Equiv | Xor | Eq | Neq | HasType -> Infix_binary
    | _ -> Prefix
  ...
  let connectives = Set.of_seq (Iter.of_list [and_; or_; equiv; imply])
...
module ArithOp = struct
  ...
  let parse_num s =
    if String.contains s '/' then
      mk_rat (Q.of_string s)
    else
      mk_int (Z.of_string s)
  ...
  let quotient s1 s2 =
    ...
    | Int n1, Int n2 ->
      ...
      if Z.sign r = 0 then
        ...
        _ty_mismatch "non-exact integral division: %a / %a" pp s1 pp s2
    | Rat n1, Rat n2 ->
      if Q.sign n2 = 0 then
  ...
  let divides a b =
    ...
    | Int a, Int b -> Z.sign a <> 0 && Z.sign (Z.rem b a) = 0
...
module ZF = struct
  let to_string = function
    ...
    | Xor | Multiset -> failwith "cannot print this symbol in ZF"
    | ForallConst -> "!!"
    | ExistsConst -> "??"
    | Grounding -> "$$grounding"
    | TyInt -> "int"
