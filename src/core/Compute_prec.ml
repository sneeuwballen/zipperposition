...
let empty =
  {constrs= []; constr_rules= []; last_constr= Precedence.Constr.alpha; weight_rule= _default_weight; status= []}
    ...
    List.fold_left
      (fun (weights, arg_coeff) input ->
        try
          ...
          in
          ( (fun constant ->
              if ID.name constant = name then Precedence.Weight.int (List.hd values) else weights constant )
          , fun constant -> if ID.name constant = name then List.tl values else arg_coeff constant )
        with Failure _ | Not_found -> failwith "Syntax error in custom weights" )
...
let weight_fun_of_prec ?(rank = None) ~symbols ~prec_fun =
  ...
  let w_tbl =
    ...
    |> CCList.foldi (fun tbl idx f -> ID.Map.add f (symbol_to_rank idx) tbl) ID.Map.empty
  in
  function
  | id ->
...
let force_const_weight ~weight ~signature = function
  | Some c_w -> (
      function
      | s -> (
        try if snd (Signature.arity signature s) == 0 then Precedence.Weight.int c_w else weight s
...
let mk_precedence ~db_w ~lmb_w ~signature t seq =
  ...
  (* set of symbols *)
  let symbols = seq |> Iter.flat_map Statement.Seq.symbols |> ID.Set.of_iter |> ID.Set.to_list in
  (* constraints *)
  let constrs = t.constrs @ List.map (fun (p, rule) -> (p, rule seq)) t.constr_rules in
  Util.debugf ~section 2 "@[<2>%d precedence constraint(s)@]" (fun k -> k (List.length constrs)) ;
  ...
  let weight =
    if !_from_prec then weight_fun_of_prec ~rank:!_rank ~symbols:(ID.Set.of_list symbols) ~prec_fun:constr
    else t.weight_rule seq
  in
...
let () =
  Options.add_opts
    [ ("--alpha-precedence", Arg.Set _alpha_precedence, " use pure alphabetical precedence")
