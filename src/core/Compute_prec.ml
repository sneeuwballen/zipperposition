    ...
    List.fold_left
      (fun (weights, arg_coeff) input ->
        ...
        with Failure _ | Not_found ->
          failwith "Syntax error in custom weights")
...
let force_const_weight ~weight ~signature = function
  | Some c_w ->
    ...
    | s ->
      (try
         if snd (Signature.arity signature s) == 0 then
           Precedence.Weight.int c_w
         else
           weight s
...
let mk_precedence ~db_w ~lmb_w ~signature t seq =
  ...
  (* constraints *)
  let constrs =
    t.constrs @ List.map (fun (p, rule) -> p, rule seq) t.constr_rules
  in
  Util.debugf ~section 2 "@[<2>%d precedence constraint(s)@]" (fun k ->
      k (List.length constrs));
  let constr = Precedence.Constr.compose_sort constrs in
  let constr = Precedence.Constr.compose constr t.last_constr in
  let constr =
    if !_alpha_precedence then
      Precedence.Constr.alpha
    else
      constr
  in
  let weight =
  ...
  Options.add_opts
    [
      ...
      ( "--kbo-weight-fun-from-precedence",
        Arg.Bool (( := ) _from_prec),
        " assign to each symbol the weight equal to the number of symbols \
         greater than it in the precedence" );
      ...
      ( "--weights",
        ...
        " set weights, e.g. f=2,g=3,h=1, or weights and argument coefficients, \
         e.g. f=2:3:4,g=3:2" );
    ]
