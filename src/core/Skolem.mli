...
(** A Skolem symbol is a witness for an existential property, used in {!CNF}.
    ...
    Typically, we transform [∃x. p(x)] into [p(sk_x)] where [sk_x] is a fresh
...
(** [skolem_form ~ctx subst var f] returns a term [t] made of a new symbol
    applied to the free variables of [f] that do not occur in [subst]. This term
    should replace the variable [var], occurring free in [f].

    For instance, [skolem_form ~ctx ø Y p(a, b, Y, X)] will yield something like
    [sk42(X)]. *)
...
(** List of new definitions, that were introduced since the last call to
    {!new_definitions}. The list can be obtained only once, after which those
    definitions are not "new" anymore.

    Will call {!remove_def} so there is no risk of re-using a definition with a
    new polarity. *)
