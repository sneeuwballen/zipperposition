...
module Result = struct
  ...
  let make_tc (type a) ~of_exn ~to_exn ~compare ~to_form
      ...
      ?(is_stmt = false) ?(flavor = fun _ -> `Vanilla) () : a result_tc =
  ...
  let form_tc : form result_tc =
    make_tc
      ...
      ~flavor:(fun f ->
        if T.equal f F.false_ then `Proof_of_false else `Vanilla )
      ()
...
module S = struct
  ...
  let pp_tstp out proof =
    ...
    traverse ~order:`DFS proof (fun p ->
        ...
        if Result.is_stmt (result p) then
          ...
          Format.fprintf out "tff(@[%s, %s,@ @[%a@],@ @[%a@]%a@]).@," p_name
            role
            (Result.pp_in Output_format.tptp)
  ...
  let pp_zf out proof =
    ...
    traverse ~order:`DFS proof (fun p ->
        ...
        let infos =
          (info_name :: info_from) @ info_rule @ info_status
          @ Step.infos p.step
        in
        if Result.is_stmt (result p) then
