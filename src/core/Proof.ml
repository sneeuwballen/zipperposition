...
module Parent = struct
  ...
  let from_subst_proj p (subst : Subst.Projection.t) : t =
    if Subst.Projection.is_empty subst then
      P_of p
    else
      P_subst (p, subst)
    ...
    let pp_step status out (rule, parents) =
      ...
      | [] ->
        Format.fprintf out "inference(@[%a,@ [status(%s)]@])" Rule.pp rule
          status
      | _ :: _ ->
        Format.fprintf out "inference(@[%a,@ [status(%s)],@ [@[%a@]]@])" Rule.pp
          rule status pp_parents parents
    in
    match k with
  ...
  let compare a b =
    ...
    | Res (r1, x1), Res (r2, x2) ->
      if r1.res_id <> r2.res_id then
        CCInt.compare r1.res_id r2.res_id
      else (
...
module Step = struct
  ...
  let is_assert p =
    match p.kind with
    | Intro (_, R_assert) -> true
    | _ -> false

  let is_assert_like p =
    match p.kind with
    | Intro (_, (R_assert | R_def | R_decl)) -> true
    | _ -> false

  let is_goal p =
    match p.kind with
    | Intro (_, (R_goal | R_lemma)) -> true
    | _ -> false
    ...
    let rec aux p =
      let init =
        CCOpt.get_or ~default:0
          (CCOpt.map
             (fun r ->
               if CCString.equal (Rule.name r) name then
                 1
               else
                 0)
             (rule p))
  ...
  let tags p =
    match p.kind with
    | Simplification (_, tags) | Inference (_, tags) -> dedup_tags tags
    | _ -> []
  ...
  let pp_dot_seq ~name out seq =
    CCGraph.Dot.pp_all
      ...
      ~attrs_v:(fun p ->
        ...
        else if is_pure_bool p then
          `Color "cyan3" :: shape :: attrs
        else if has_absurd_lits p then
          `Color "orange" :: shape :: attrs
        else if is_def p then
          `Color "navajowhite" :: shape :: attrs
        else if Step.is_goal @@ step p then
          `Color "green" :: shape :: attrs
        else if Step.is_trivial @@ step p then
          `Color "cyan" :: shape :: attrs
