...
module Src = struct
  ...
  let pp_role out = function
    | R_decl ->
        CCFormat.string out "decl"
    | R_assert ->
        CCFormat.string out "assert"
    | R_goal ->
        CCFormat.string out "goal"
    | R_def ->
        CCFormat.string out "def"
    | R_lemma ->
        CCFormat.string out "lemma"
  ...
  let pp out src =
    ...
    | From_file (src, attrs) -> (
        ...
        match src.name with
        | None ->
            Format.fprintf out "'%s'%a" file UA.pp_attrs attrs
        | Some name ->
            Format.fprintf out "'%s' in '%s'%a" name file UA.pp_attrs attrs )
  ...
  let to_attrs src : UntypedAST.attrs =
    ...
    | From_file (f, attrs) -> (
      match f.name with
      | None ->
          app "file" [quoted f.file] :: attrs
      | Some n ->
          app "file" [quoted f.file; quoted n] :: attrs )
end
...
module Kind = struct
  ...
  let pp out k =
    match k with
    | Intro (src, R_goal) ->
        Format.fprintf out "goal %a" Src.pp src
    | Intro (src, R_lemma) ->
        Format.fprintf out "lemma %a" Src.pp src
    | Intro (src, R_assert) ->
        Src.pp out src
    | Intro (src, (R_def | R_decl)) ->
        Src.pp out src
    | Inference (rule, tags) ->
    ...
    | Trivial ->
        CCFormat.string out "trivial"
    | By_def id ->
        Format.fprintf out "by_def(%a)" ID.pp id
    | Define (id, src) ->
  ...
  let pp_tstp out (k, parents) =
    ...
    | Intro (src, (R_assert | R_goal | R_def | R_decl)) ->
        Src.pp_tstp out src
    | Inference (rule, _) | Simplification (rule, _) ->
        pp_step "thm" out (rule, parents)
    | Esa rule ->
        pp_step "esa" out (rule, parents)
    | Intro (_, R_lemma) ->
        Format.fprintf out "lemma"
    | Trivial ->
        assert (parents = []) ;
        Format.fprintf out "trivial([status(thm)])"
    | By_def _ ->
        Format.fprintf out "by_def([status(thm)])"
    | Define _ ->
        Format.fprintf out "define([status(thm)])"
end
...
module Result = struct
  ...
  let name (Res (r, x)) =
    ...
    | None ->
        assert false
    | Some x -> (
      match r.res_name with None -> None | Some f -> Some (f x) )
...
module Step = struct
  ...
  let rule p =
    match p.kind with
    | Intro _ | Trivial | By_def _ | Define _ ->
        None
    | Esa rule | Simplification (rule, _) | Inference (rule, _) ->
        Some rule
  ...
  let combine_dist o p =
    ...
    | None, None ->
        None
    | (Some _ as res), None | None, (Some _ as res) ->
        res
    | Some x, Some y ->
        Some (min x y)
      ...
      | _ -> (
          let d =
            ...
            | [] ->
                None
            | [p] ->
                (Parent.proof p).step.dist_to_goal
            | [p1; p2] ->
                combine_dist (Parent.proof p1).step.dist_to_goal p2
            | p :: l ->
                List.fold_left combine_dist (Parent.proof p).step.dist_to_goal l
          in
  ...
  let pp out step =
    ...
    | Trivial ->
        Format.fprintf out "@[<2>trivial%a@]" pp_infos step.infos
    | By_def id ->
...
module S = struct
    ...
    traverse ~order:`DFS proof (fun p ->
        ...
        and info_status =
          ...
          | Inference _ | Simplification _ ->
              [mk_status "inference"]
          | Esa _ ->
              [mk_status "equisatisfiable"]
          | Intro (src, R_lemma) ->
              mk_status "lemma" :: Src.to_attrs src
          | Intro (src, R_goal) ->
              mk_status "goal" :: Src.to_attrs src
          | Intro (src, R_assert) ->
              mk_status "assert" :: Src.to_attrs src
          | Intro (src, R_def) ->
              mk_status "def" :: Src.to_attrs src
          | Intro (src, R_decl) ->
              mk_status "decl" :: Src.to_attrs src
          | Trivial ->
              [mk_status "trivial"]
          | By_def _ | Define _ ->
              []
  ...
  let pp_in o out proof =
    ...
    | Output_format.O_none ->
        Util.debug ~section 1 "proof printing disabled"
