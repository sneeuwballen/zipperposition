  ...
  | Sum l ->
    let pp_pair out (n, b) =
      if b = Zero then
        ...
        Fmt.fprintf out "@<1>%d · ω^@[%a@]" n pp_inner b
    in
...
let rec compare (a : t) (b : t) : int =
  ...
  | Zero, Sum _ -> -1
  | Sum _, Zero -> 1
  | Sum l1, Sum l2 -> CCList.compare compare_pair l1 l2
...
and add_l l1 l2 : (int * t) list =
  ...
  | (n1, b1) :: tail1, (n2, b2) :: tail2 ->
    let c = compare b1 b2 in
    if c = 0 then
      (n1 + n2, b1) :: add_l tail1 tail2
    else if c > 0 then
      (n1, b1) :: add_l tail1 l2
    else
      (n2, b2) :: add_l l1 tail2
...
let normalize_ (l : (int * t) list) : (int * t) list =
  let rec merge_succ l =
    ...
    | (n1, b1) :: ((n2, b2) :: tail2 as tail) ->
      ...
      assert (c >= 0);
      if c = 0 then
        merge_succ ((n1 + n2, b1) :: tail2)
      else
        (n1, b1) :: merge_succ tail
  in
  (* sort by decreasing order *)
  List.sort (fun pair1 pair2 -> compare_pair pair2 pair1) l |> merge_succ

let rec mult (a : t) (b : t) : t =
  match a, b with
  | Zero, _ | _, Zero -> zero
