...
let and_ ?loc = function
  | [] -> true_
  | [x] -> x
  | l -> app_builtin ?loc Builtin.and_ l

let or_ ?loc = function
  | [] -> false_
  | [x] -> x
  | l -> app_builtin ?loc Builtin.or_ l
...
let close_all s t =
  let vars =
    Seq.free_vars t |> StringSet.of_seq |> StringSet.elements
...
module TPTP = struct
  ...
  let rec pp out t =
    ...
    | AppBuiltin (Builtin.Or, l) -> Util.pp_list ~sep:" | " pp_surrounded out l
