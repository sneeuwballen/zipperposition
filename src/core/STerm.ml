...
let rec compare t1 t2 =
  ...
  | Bind (s1, v1, t1), Bind (s2, v2, t2) ->
    ...
    if c = 0 then (
      ...
      if c' = 0 then
        CCOrd.list compare_typed_var v1 v2
      else
        c'
    ) else
      c
  ...
  | Let (l1, t1), Let (l2, t2) ->
    CCOrd.(compare t1 t2 <?> (list (pair compare_var compare), l1, l2))
...
let rec hash t =
  ...
  | Ite (a, b, c) -> Hash.combine4 50 (hash a) (hash b) (hash c)
...
let int_ i = builtin (Builtin.make_payload (Builtin.Int i))
let of_int i = int_ (Z.of_int i)
let rat n = builtin (Builtin.make_payload (Builtin.Rat n))
let real r = builtin (Builtin.make_payload (Builtin.Real r))
...
let ty_int = builtin Builtin.ty_int
let ty_rat = builtin Builtin.ty_rat
let ty_real = builtin Builtin.ty_real
let forall_ty ?loc vars t = bind ?loc Binder.forall_ty vars t

let ty_unfold =
  let rec aux acc ty =
    match ty.term with
    | AppBuiltin (b, ret :: l) -> aux (l @ acc) ret
    | _ -> acc, ty
  in
  aux []
...
let unfold_bind b =
  let rec aux acc t =
    match t.term with
    | Bind (b', vars, t) when b = b' -> aux (List.rev_append vars acc) t
    | _ -> List.rev acc, t
  in
  aux []
...
module Seq = struct
  let subterms t k =
    let rec iter t =
      ...
      match t.term with
      | Var _ | Const _ -> ()
      | List l | AppBuiltin (_, l) -> List.iter iter l
      | App (f, l) ->
        iter f;
        List.iter iter l
      | Ite (a, b, c) ->
      ...
      | Match (u, l) ->
        ...
        List.iter
          (function
            | Match_case (_, _, rhs) | Match_default rhs -> iter rhs)
          l
      | Bind (_, _, t') -> iter t'
      | Record (l, _) -> List.iter (fun (_, t') -> iter t') l
    in
    iter t
  ...
  let subterms_with_bound t k =
    ...
    let rec iter bound t =
      ...
      match t.term with
      | Var _ | Const _ -> ()
      | AppBuiltin (_, l) | List l -> List.iter (iter bound) l
      | App (f, l) ->
        iter bound f;
        List.iter (iter bound) l
      | Bind (_, v, t') ->
      ...
      | Match (u, l) ->
        ...
        List.iter
          ...
          l
      | Record (l, _) -> List.iter (fun (_, t') -> iter bound t') l
    in
    iter StringSet.empty t
...
let rec pp out t =
  ...
  | AppBuiltin (_b, []) when Builtin.equal _b Builtin.tType ->
    CCFormat.string out "type"
  | AppBuiltin (_b, []) when Builtin.equal _b Builtin.tType ->
    CCFormat.string out "int"
  | AppBuiltin (_b, []) when Builtin.equal _b Builtin.ty_rat ->
...
module TPTP = struct
  let pp_id out s =
    if Util.tstp_needs_escaping s then
      CCFormat.fprintf out "'%s'" s
    else
  ...
  let rec pp out t =
    ...
    | AppBuiltin (_b_eq, ([ _; a; b_eq ] | [ a; b_eq ]))
      ...
      Format.fprintf out "@[%a =@ %a@]" pp_surrounded a pp_surrounded b_eq
    | AppBuiltin (_b_neq, ([ _; a; b_neq ] | [ a; b_neq ]))
      when Builtin.equal _b_neq Builtin.neq ->
      Format.fprintf out "@[%a !=@ %a@]" pp_surrounded a pp_surrounded b_neq
    | AppBuiltin (_b, [ ret; a ]) ->
  ...
  and pp_typed_var out (v, o) =
    match o with
    | None -> pp_var out v
    | Some { term = AppBuiltin (_b_term, []); _ }
      when Builtin.equal _b_term Builtin.term ->
      pp_var out v (* implicit type *)
    | Some ty -> Format.fprintf out "%a:%a" pp_var v pp_surrounded ty

  and pp_surrounded out t =
    match t.term with
    | AppBuiltin (_, _ :: _) | Bind _ -> Format.fprintf out "(@[%a@])" pp t
    | _ -> pp out t
...
module TPTP_THF = struct
  ...
  let rec pp out t =
    ...
    | Var v -> pp_var out v
    | Const s -> pp_id out s
    | AppBuiltin (_b_and, l) when List.length l >= 2 ->
      Util.pp_list ~sep:" & " pp_force_surrounded out l
    | AppBuiltin (_b_or, l) when List.length l >= 2 ->
      Util.pp_list ~sep:" | " pp_force_surrounded out l
    | AppBuiltin (_b_not3, [ a ]) when Builtin.equal _b_not3 Builtin.not_ ->
      Format.fprintf out "@[~@[%a@]@]" pp_force_surrounded a
    | AppBuiltin (_b_imply, [ a; b_imply ])
      when Builtin.equal _b_imply Builtin.imply ->
      Format.fprintf out "@[%a =>@ %a@]" pp_force_surrounded a
        pp_force_surrounded b_imply
    ...
    | AppBuiltin (_b_eq, ([ _; a; b_eq ] | [ a; b_eq ]))
      when Builtin.equal _b_eq Builtin.eq ->
      Format.fprintf out "@[%a =@ %a@]" pp_force_surrounded a
        pp_force_surrounded b_eq
    | AppBuiltin (_b_neq, ([ _; a; b_neq ] | [ a; b_neq ]))
      when Builtin.equal _b_neq Builtin.neq ->
      Format.fprintf out "@[%a !=@ %a@]" pp_force_surrounded a
        pp_force_surrounded b_neq
    | AppBuiltin (b, []) when b == Builtin.not_ || Builtin.is_logical_binop b ->
  ...
  and pp_surrounded out t =
    match t.term with
    | AppBuiltin (_, _ :: _) | App (_, _) | Bind _ ->
      Format.fprintf out "( @[%a@] )" pp t
    | _ -> pp out t

  and pp_force_surrounded out t = Format.fprintf out "( @[%a@] )" pp t
...
module ZF = struct
  ...
  let rec pp out t =
    ...
    | Const s -> pp_id out s
    | List l -> Format.fprintf out "[@[<hv>%a@]]" (Util.pp_list ~sep:"," pp) l
    | AppBuiltin (_b, _) when Builtin.equal _b Builtin.tType ->
      CCFormat.string out "type"
    ...
    | (AppBuiltin (s, [ a; b ]) | AppBuiltin (s, [ _; a; b ]))
      ...
      Format.fprintf out "@[@[%a@]@ %a @[%a@]@]" pp_surrounded a Builtin.ZF.pp s
        pp_surrounded b
    | AppBuiltin (s, l) ->
  ...
  and pp_surrounded out t =
    match t.term with
    | AppBuiltin (_, _ :: _) | App _ | Ite _ | Bind _ ->
      Format.fprintf out "(@[%a@])" pp t
    | _ -> pp out t
