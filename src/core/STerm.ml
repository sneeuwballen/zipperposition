...
type t = {term: view; loc: location option; attrs: attr list}
...
let rec compare t1 t2 =
  ...
  | Var s1, Var s2 ->
      CCShims_.Stdlib.compare s1 s2
  | Const s1, Const s2 ->
      String.compare s1 s2
  | App (s1, l1), App (s2, l2) ->
      let c = compare s1 s2 in
      if c = 0 then CCOrd.list compare l1 l2 else c
  ...
  | Bind (s1, v1, t1), Bind (s2, v2, t2) ->
      ...
      if c = 0 then
        ...
        if c' = 0 then CCOrd.list compare_typed_var v1 v2 else c'
      else c
...
let rec hash t =
  ...
  | Const s ->
      Hash.string s
  | AppBuiltin (s, l) ->
      Hash.combine3 10 (Builtin.hash s) (Hash.list hash l)
  | App (s, l) ->
      Hash.combine3 20 (hash s) (Hash.list hash l)
  | List l ->
      Hash.combine2 30 (Hash.list hash l)
  | Bind (s, v, t') ->
      Hash.combine4 40 (Binder.hash s) (Hash.list hash_ty_var v) (hash t')
  | Ite (a, b, c) ->
      Hash.combine4 50 (hash a) (hash b) (hash c)
  | Match (u, _) ->
      Hash.combine2 60 (hash u)
  | Let (_, u) ->
...
let app ?loc s l =
  ...
  | _, [] ->
      s
  | AppBuiltin (s1, l1), _ ->
      app_builtin ?loc s1 (l1 @ l)
  | App (s1, l1), _ :: _ ->
      make_ ?loc (App (s1, l1 @ l))
  | _, _ :: _ ->
      make_ ?loc (App (s, l))
...
let bind ?loc s v l =
  ...
  | _ :: _, _ ->
      make_ ?loc (Bind (s, v, l))

let ite ?loc a b c = make_ ?loc (Ite (a, b, c))

let match_ ?loc t l = make_ ?loc (Match (t, l))

let let_ ?loc l u = match l with [] -> u | _ -> make_ ?loc (Let (l, u))

let with_ ?loc l u = match l with [] -> u | _ -> make_ ?loc (With (l, u))
...
and pp_typed_var out = function
  | v, None ->
      pp_var out v
  | v, Some ty ->
      Format.fprintf out "%a:%a" pp_var v pp ty
...
module TPTP = struct
  ...
  let rec pp out t =
    ...
    | Match _ ->
        failwith "cannot print `match` in TPTP"
    | Record _ ->
        failwith "cannot print records in TPTP"
...
module TPTP_THF = struct
  ...
  let rec pp out t =
    ...
    | With (_, u) ->
        pp out u (* skip bindings *)
    | Ite _ ->
        failwith "cannot print `ite` in TPTP"
    | Let _ ->
        failwith "cannot print `let` in TPTP"
    | Match _ ->
        failwith "cannot print `match` in TPTP"
...
let rec apply_subst (subst : subst) (t : t) : t =
  ...
  match t.term with
  | Var (V s) ->
      StrMap.get_or ~default:t s subst
  | Var Wildcard ->
      t
  | Const c ->
      const ?loc c
  ...
  | Match (u, l) ->
      ...
      let l =
        List.map
          (function
            | Match_default rhs ->
                Match_default (apply_subst subst rhs)
            | Match_case (c, vars, rhs) ->
  ...
  | List l ->
      list_ ?loc (List.map (apply_subst subst) l)
  | Record (l, row) ->
