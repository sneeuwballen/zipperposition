...
module Ctx = struct
  ...
  let bind_meta ctx (v, r, k) =
    ...
    | None, `Generalize ->
      let v' = Var.copy v in
      Util.debugf ~section 5 "@[<2>generalize type meta_var %a@]" (fun k ->
          k Var.pp v );
      r := Some (T.var v')
  ...
  let declare ?loc ctx s ty =
    ...
    in
    if doit then (
      Util.debugf ~section 3 "@{<yellow>declare@} %a:@ @[%a@]" (fun k ->
          k ID.pp s T.pp ty );
  ...
  let decls =
    List.map
      (fun d ->
        ...
        if T.Ty.returns_tType ty then
          error_ ?loc
            "in definition of %a,@ equality between types is forbidden" ID.pp
            id;
...
let infer_statements_exn ?def_as_rewrite ?on_var ?on_undef ?on_shadow ?ctx
