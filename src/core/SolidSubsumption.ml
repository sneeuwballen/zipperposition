...
struct
  ...
  let bvar_or_const t = Term.is_const t || Term.is_bvar t

  let app_builtin b args repls =
    if TS.for_all bvar_or_const repls then
      AppBuiltin (b, args, repls)
    else
      invalid_arg "replacements must be bound vars or constants"
  ...
  let app hd args repls =
    if TS.for_all bvar_or_const hd then
      ...
      invalid_arg "head of multiterm is bound var or constant"
  ...
  let rec of_term term =
    ...
    | AppBuiltin (b, args) -> app_builtin b (List.map of_term args) TS.empty
    | App (hd, args) -> app (TS.singleton hd) (List.map of_term args) TS.empty
    | Fun (ty, body) -> fun_ ty (of_term body)
  ...
  let rec pp out =
    ...
    | App (hds, args, repls) ->
      CCFormat.fprintf out "|{@[%a@]}(@[%a@])|@[%a@]|" (TS.pp ~pp_sep:sepc T.pp)
        hds
  ...
  let cover t solids : multiterm =
    ...
    let rec aux ~depth s_args t : multiterm =
      ...
      | Fun _ ->
        ...
        fun_l ty_args res
      | DB i when i >= depth ->
        if TS.is_empty db_hits then
          raise SolidMatchFail
        else
          repl db_hits
      | _ -> repl (TS.add t db_hits)
    in
  ...
  let solid_match ~subst ~pattern ~target =
    ...
    let rec aux subst l r =
      if T.is_type l then
        if T.is_ground l && T.is_ground r && T.equal l r then
          subst
        else
          raise SolidMatchFail (* polymorphism not supported now *)
      else (
        match T.view l with
        | AppBuiltin (b, args) ->
          (match T.view r with
          | AppBuiltin (b', args') when Builtin.equal b b' ->
            (try
             ...
             with
            | Unif.Fail -> raise SolidMatchFail
            | Invalid_argument _ ->
  ...
  let sign l =
    if L.is_positivoid l then
      1
    else
      -1

  let cmp_by_sign l1 l2 = CCOrd.int (sign l1) (sign l2)
  let cmp_by_weight l1 l2 = CCOrd.int (L.ho_weight l1) (L.ho_weight l2)
