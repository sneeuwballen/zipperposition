...
struct
  ...
  let app_builtin b args repls =
    if TS.for_all bvar_or_const repls then AppBuiltin (b, args, repls)
  ...
  let cover t solids : multiterm =
    ...
    let rec aux ~depth s_args t : multiterm =
      ...
      | Fun _ ->
          ...
          fun_l ty_args res
      | DB i when i >= depth ->
          if TS.is_empty db_hits then raise SolidMatchFail else repl db_hits
      | _ ->
          repl (TS.add t db_hits)
    in
  ...
  let refine_subst_w_term subst var t =
    if not @@ MS.mem var subst then MS.add var t subst
    else
    ...
    let rec aux subst l r =
      if T.is_type l then
        ...
        | Fun _ ->
            ...
            aux subst body body'
        | Var x ->
            refine_subst_w_term subst x (cover r [])
        | _ ->
            if T.equal l r then subst else raise SolidMatchFail
    in
    if
  ...
  let sign l = if L.is_positivoid l then 1 else -1
  ...
  let lit_matchers ~subst ~pattern ~target k =
    ...
    | L.Equation (lhs, rhs, sign) -> (
      ...
      | _ ->
          () )
    | L.True -> (
      match target with L.True -> k subst | _ -> () )
    | L.False -> (
