...
let equal l1 l2 =
  ...
  | Equation _, _ | Prop _, _ | True, _ | False, _ | Int _, _ | Rat _, _ ->
    false
...
let equal_com l1 l2 =
  ...
  | Equation (l1, r1, sign1), Equation (l2, r2, sign2) ->
    sign1 = sign2
    && ((T.equal l1 l2 && T.equal r1 r2) || (T.equal l1 r2 && T.equal r1 l2))
  | Prop (p1, sign1), Prop (p2, sign2) -> sign1 = sign2 && T.equal p1 p2
  | True, True | False, False -> true
...
let sign = function
  | Prop (_, sign) | Equation (_, _, sign) -> sign
  | False -> false
...
let is_eqn = function
  | Equation _ | Prop _ -> true
  | Int _ | Rat _ | True | False -> false

let is_eq lit = is_eqn lit && is_pos lit
let is_neq lit = is_eqn lit && is_neg lit

let is_prop = function
  | Prop _ | True | False -> true
  | Int _ | Rat _ | Equation _ -> false
...
let ty_error_ a b =
  let msg =
    CCFormat.sprintf
      "@[<2>Literal: incompatible types in equational lit@ for `@[%a : %a@]`@ \
       and `@[%a : %a@]`@]"
      T.pp a Type.pp (T.ty a) T.pp b Type.pp (T.ty b)
  in
  raise (Type.ApplyError msg)

let has_num_ty t = Type.equal Type.int (T.ty t) || Type.equal Type.rat (T.ty t)
...
and mk_num_eq t u sign =
  ...
  if Type.equal Type.int (T.ty t) then
    ...
    if sign then
      ...
      Monome.Rat.of_term u >>= fun m2 -> return (Rat (AL.mk_eq m1 m2))
    else
      None (* no "neq" literal for rationals *)
  else
    assert false
...
and mk_prop p sign =
  ...
  | T.AppBuiltin
      ( ( (Builtin.Less | Builtin.Lesseq | Builtin.Greater | Builtin.Greatereq)
        as b ),
        [_; t; u] )
    when has_num_ty t ->
    (* arith conversion *)
    ( match mk_num_prop b t u sign with
    | None -> Prop (p, sign)
    | Some lit -> lit )
  | _ ->
...
and mk_num_prop builtin t u sign : t option =
  ...
  if Type.equal Type.int (T.ty t) then
    ...
    Monome.Int.of_term u >|= fun m2 ->
    let mk_pred t u =
      match builtin, sign with
      | Builtin.Less, true | Builtin.Greatereq, false -> AL.mk_less t u
      | Builtin.Lesseq, true | Builtin.Greater, false -> AL.mk_lesseq t u
      | Builtin.Greater, true | Builtin.Lesseq, false -> AL.mk_less u t
      | Builtin.Greatereq, true | Builtin.Less, false -> AL.mk_lesseq u t
      | _ -> assert false
    in
    Int (mk_pred m1 m2)
  else if Type.equal Type.rat (T.ty t) then
    ...
    match builtin, sign with
    | Builtin.Less, true | Builtin.Greatereq, false ->
      Some (Rat (AL.mk_less t u))
    | Builtin.Greater, true | Builtin.Lesseq, false ->
      Some (Rat (AL.mk_less u t))
    | Builtin.Lesseq, true
    | Builtin.Greater, false
    | Builtin.Greatereq, true
    | Builtin.Less, false ->
      None (* cannot encode this, would yield 2 lits *)
    | _ -> assert false
  else
    assert false
...
let mk_arith_op op m1 m2 =
  ...
  if Int_lit.is_trivial alit then
    mk_tauto
  else if Int_lit.is_absurd alit then
    mk_absurd
  else
    Int alit
...
module Seq = struct
  ...
  let symbols lit = Iter.flat_map T.Seq.symbols (terms lit)
...
module UnifOp = struct
  type 'subst op = {
    term : subst:'subst -> term Scoped.t -> term Scoped.t -> 'subst Iter.t;
    monomes :
      'a. subst:'subst -> 'a Monome.t Scoped.t -> 'a Monome.t Scoped.t ->
      'subst Iter.t
  }
end
...
let unif_lits op ~subst (lit1, sc1) (lit2, sc2) k =
  ...
  | Int o1, Int o2 ->
    Int_lit.generic_unif op.monomes ~subst (o1, sc1) (o2, sc2) (fun s ->
        k (s, [Builtin.Tag.T_lia]) )
  | Rat o1, Rat o2 ->
    Rat_lit.generic_unif op.monomes ~subst (o1, sc1) (o2, sc2) (fun s ->
        k (s, [Builtin.Tag.T_lra]) )
  | _, _ -> ()
...
let variant ?(subst = S.empty) lit1 lit2 k =
  let op =
    UnifOp.
      { term =
          (fun ~subst t1 t2 k ->
            try k (Unif.FO.variant ~subst t1 t2) with Unif.Fail -> () );
        monomes = (fun ~subst m1 m2 k -> Monome.variant ~subst m1 m2 k) }
  in
  unif_lits op ~subst lit1 lit2 (fun (subst, tags) ->
      if Subst.is_renaming subst then k (subst, tags) )
...
let _eq_subsumes ~subst l1 r1 sc1 l2 r2 sc2 k =
  ...
  and equate_root ~subst l2 r2 k =
    ( try
        let subst =
          Unif.FO.matching_adapt_scope ~subst ~pattern:(Scoped.make l1 sc1)
            (Scoped.make l2 sc2)
        in
        let subst =
          Unif.FO.matching_adapt_scope ~subst ~pattern:(Scoped.make r1 sc1)
            (Scoped.make r2 sc2)
        in
        k subst
      with Unif.Fail -> () );
...
let subsumes ?(subst = Subst.empty) (lit1, sc1) (lit2, sc2) k =
  ...
  | Int o1, Int o2 ->
    ...
    Int_lit.subsumes ~subst (o1, sc1) (o2, sc2) (fun s ->
        k (s, [Builtin.Tag.T_lia]) )
...
let unify ?(subst = US.empty) lit1 lit2 k =
  let op =
    UnifOp.
      { term =
          (fun ~subst t1 t2 k ->
            try k (Unif.FO.unify_full ~subst t1 t2) with Unif.Fail -> () );
...
let apply_subst renaming subst (lit, sc) =
  apply_subst_ subst (lit, sc) ~f_term:(S.FO.apply renaming)
  ...
  | Equation (l, r, sign) ->
    Equation
      ( S.FO.apply renaming subst (l, sc),
        ...
        sign )
  | Prop (p, sign) -> Prop (S.FO.apply renaming subst (p, sc), sign)
  | True | False -> lit

let apply_subst_list renaming subst (lits, sc) =
  List.map (fun lit -> apply_subst renaming subst (lit, sc)) lits
...
let vars lit = Seq.vars lit |> T.VarSet.of_seq |> T.VarSet.to_list

let var_occurs v lit =
  match lit with
  | Prop (p, _) -> T.var_occurs ~var:v p
  | Equation (l, r, _) -> T.var_occurs ~var:v l || T.var_occurs ~var:v r
  | Int _ | Rat _ -> Iter.exists (T.var_occurs ~var:v) (Seq.terms lit)
  | True | False -> false
...
let is_ground lit =
  ...
  | Int _ | Rat _ -> Iter.for_all T.is_ground (Seq.terms lit)
  | True | False -> true
...
let to_multiset lit =
  ...
  | Int alit ->
    AL.Seq.to_multiset alit |> Multisets.MT.Seq.of_coeffs Multisets.MT.empty
  | Rat o ->
...
let fold_terms ?(position = Position.stop) ?(vars = false) ?ty_args ~which
    ?(ord = Ordering.none) ~subterms lit k =
  (* function to call at terms *)
  let at_term ~pos t =
    if subterms then
      T.all_positions ?ty_args ~vars ~pos t k
    else if T.is_var t && not vars then
      () (* ignore *)
    else
      k (t, pos)
  in
  ...
  | Equation (l, r, _) ->
    ...
    | `Max ->
      ...
      | Comparison.Gt -> at_term ~pos:P.(append position (left stop)) l
      | Comparison.Lt -> at_term ~pos:P.(append position (right stop)) r
...
let to_ho_term (lit : t) : T.t option =
  match lit with
  | Prop (t, true) -> Some t
...
module Comp = struct
  ...
  *)
  ...
  let _some_term_dominates f l1 l2 =
    List.exists (fun x -> List.for_all (fun y -> f x y = Comparison.Gt) l2) l1
  ...
  let _cmp_by_polarity l1 l2 =
    ...
    match p1, p2 with
    | true, true | false, false -> Comparison.Eq
  ...
  let _cmp_by_kind l1 l2 =
    ...
    let _to_int = function
      ...
      | Equation _ | Prop _ -> 30
      (* eqn and prop are really the same thing *)
    in
  ...
  let _cmp_specific ~ord l1 l2 =
    ...
    | Int (AL.Binary (op1, x1, y1)), Int (AL.Binary (op2, x2, y2)) ->
      ...
      let left =
        Multisets.MMT.doubleton (MI.to_multiset x1) (MI.to_multiset y1)
      in
      let right =
        Multisets.MMT.doubleton (MI.to_multiset x2) (MI.to_multiset y2)
      in
    ...
    | Int (AL.Divides d1), Int (AL.Divides d2) ->
      ...
      (* TODO: Bezout-normalize, then actually compare Monomes. *)
        ...
        C.Incomparable
    | ( Rat {Rat_lit.op = o1; left = l1; right = r1},
        Rat {Rat_lit.op = o2; left = l2; right = r2} ) ->
      assert (o1 = o2);
  ...
  let compare ~ord l1 l2 =
    let f =
      Comparison.(
        _cmp_by_maxterms ~ord @>> _cmp_by_polarity @>> _cmp_by_kind
        @>> _cmp_specific ~ord)
    in
    let res = f l1 l2 in
    res
...
module Pos = struct
  ...
  let split lit pos =
    ...
    | Int (AL.Divides d), P.Arg (i, pos') ->
      let term =
        try snd (Monome.nth d.AL.monome i) with _ -> _fail_lit lit pos
      in
  ...
  let replace lit ~at ~by =
    ...
    | Prop (p, sign), P.Left pos' -> Prop (T.Pos.replace p pos' ~by, sign)
    | True, _ | False, _ ->
      lit (* flexible, lit can be the result of a simplification *)
  ...
  let is_max_term ~ord lit pos =
    let module AL = Int_lit in
    match lit, pos with
    | Equation (l, r, _), P.Left _ -> Ordering.compare ord l r <> Comparison.Lt
...
module View = struct
  ...
  let get_eqn lit position =
    ...
    | True, _ | False, _ | Int _, _ | Rat _, _ -> None
