...
let ty_error_ a b =
  let msg =
    CCFormat.sprintf "@[<2>Literal: incompatible types in equational lit@ for `@[%a : %a@]`@ and `@[%a : %a@]`@]"
...
module Seq = struct
  let terms lit k = match lit with Equation (l, r, _) -> k l ; k r | True | False -> ()
...
let variant ?(subst = S.empty) lit1 lit2 k =
  let op = UnifOp.{term= (fun ~subst t1 t2 k -> try k (Unif.FO.variant ~subst t1 t2) with Unif.Fail -> ())} in
...
let as_ho_predicate (lit : t) : _ option =
  ...
  | Equation (lhs, rhs, _) when is_predicate_lit lit -> (
      ...
      match (T.view hd_t, args_t) with T.Var v, _ :: _ -> Some (v, hd_t, args_t, is_positivoid lit) | _ -> None )
  | _ ->
      None
...
let is_ho_unif lit =
  match lit with Equation (t, u, _) when is_negativoid lit -> Term.is_ho_app t || Term.is_ho_app u | _ -> false
...
module Comp = struct
  ...
  let _maxterms2 ~ord l r =
    match O.compare ord l r with C.Gt | Geq -> [l] | Lt | Leq -> [r] | Eq -> [l] | Incomparable -> [l; r]
  ...
  *)
  ...
  let rec _find_dominators ~met_geq_or_leq f ts1 ts2 =
    ...
    | t :: ts1 ->
        ...
        if List.for_all (fun cmp -> cmp = C.Gt) cmps then StrictDominator
          ...
          let met_geq_or_leq' = met_geq_or_leq || List.exists (fun cmp -> cmp = C.Geq || cmp = C.Leq) cmps in
  ...
  let _cmp_by_maxterms ~ord l1 l2 =
    ...
    match
      (_find_dominators ~met_geq_or_leq:false f maxs1 maxs2, _find_dominators ~met_geq_or_leq:false f maxs2 maxs1)
    with
