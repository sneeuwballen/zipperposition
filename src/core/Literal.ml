...
let compare l1 l2 =
  ...
  | True, True | False, False ->
      0
  | _, _ ->
      __to_int l1 - __to_int l2

let fold f acc lit =
  match lit with Equation (l, r, _) -> f (f acc l) r | True | False -> acc
...
let heuristic_weight weight = function
  | Equation (l, _, _) as lit when is_predicate_lit lit ->
      weight l
  | Equation (l, r, _) ->
      weight l + weight r
  | True | False ->
...
module Seq = struct
  let terms lit k =
    match lit with Equation (l, r, _) -> k l ; k r | True | False -> ()
  ...
  let typed_symbols ?(include_types = false) lit =
    Iter.flat_map (T.Seq.typed_symbols ~include_types) (terms lit)
end
...
let subsumes ?(subst = Subst.empty) (lit1, sc1) (lit2, sc2) k =
  ...
  | _ ->
      matching ~subst ~pattern:(lit1, sc1) (lit2, sc2) k
...
let is_ho_constraint = function
  | Equation (l, r, _) as lit when is_negativoid lit ->
      T.is_ho_at_root l || T.is_ho_at_root r
  | _ ->
      false

let is_constraint = function
  | Equation (t, u, _) as lit when is_negativoid lit ->
      T.is_var t || T.is_var u
  | _ ->
      false
...
let var_occurs v lit =
  match lit with
  | Equation (l, r, _) ->
      T.var_occurs ~var:v l || T.var_occurs ~var:v r
  | True | False ->
      false
...
let to_multiset lit =
  ...
  | Equation (l, r, _) ->
      Multisets.MT.doubleton l r
  | True | False ->
      Multisets.MT.singleton T.true_
...
let rec cannot_be_eq (t1 : term) (t2 : term) : Builtin.Tag.t list option =
  ...
  | TC.AppBuiltin (Builtin.Int z1, []), TC.AppBuiltin (Builtin.Int z2, []) ->
      if Z.equal z1 z2 then None
      else Some [Builtin.Tag.T_lia; Builtin.Tag.T_cannot_orphan]
  | TC.AppBuiltin (Builtin.Rat n1, []), TC.AppBuiltin (Builtin.Rat n2, []) ->
      if Q.equal n1 n2 then None
      else Some [Builtin.Tag.T_lra; Builtin.Tag.T_cannot_orphan]
...
let is_absurd lit =
  assert (no_prop_invariant lit) ;
  match lit with
  | Equation (l, r, false) when T.equal l r ->
      true
  | Equation (l, r, true) ->
      CCOpt.is_some (cannot_be_eq l r)
  | False ->
      true
  | Equation _ | True ->
      false
...
let is_absurd_tags lit =
  ...
  match lit with
  | Equation (l, r, true) ->
      cannot_be_eq l r |> CCOpt.get_or ~default:[]
  | Equation _ | False ->
      []
  | True ->
      assert false

let fold_terms ?(position = Position.stop) ?(vars = false) ?(var_args = true)
    ?(fun_bodies = true) ?ty_args ~which ?(ord = Ordering.none) ~subterms lit k
    =
  assert (no_prop_invariant lit) ;
  ...
  let at_term ~pos t =
    if subterms then
      T.all_positions ~filter_formula_subterms ?ty_args ~vars ~var_args
        ~fun_bodies ~pos t k
...
let as_ho_predicate (lit : t) : _ option =
  ...
  | Equation (lhs, rhs, _) when is_predicate_lit lit -> (
      ...
      | T.Var v, _ :: _ ->
          Some (v, hd_t, args_t, is_positivoid lit)
      | _ ->
          None )
  | _ ->
      None
...
let is_ho_unif lit =
  match lit with
  | Equation (t, u, _) when is_negativoid lit ->
      Term.is_ho_app t || Term.is_ho_app u
  | _ ->
      false
...
let normalize_eq lit =
  let as_neg t =
    match T.view t with T.AppBuiltin (Not, [f]) -> Some f | _ -> None
  in
  ...
  if List.for_all (fun h -> not (h out lit)) hooks then
    ...
    | Equation (p, t, _) when is_predicate_lit lit ->
        Format.fprintf out "@[%s%a@]"
          (if is_positivoid lit then "" else "¬")
          T.pp p
    | True ->
  ...
  *)
  ...
  let rec _find_dominators ~met_geq_or_leq f ts1 ts2 =
    ...
    | t :: ts1 ->
        ...
        if List.for_all (fun cmp -> cmp = C.Gt) cmps then StrictDominator
          ...
          let met_geq_or_leq' =
            met_geq_or_leq
            || List.exists (fun cmp -> cmp = C.Geq || cmp = C.Leq) cmps
          in
  ...
  let _cmp_by_maxterms ~ord l1 l2 =
    ...
    | NoDominator met_geq_or_leq1, NoDominator met_geq_or_leq2 ->
        if met_geq_or_leq1 || met_geq_or_leq2 then C.Eq
          (* check next criterion *)
        else
          ...
          if T.Set.equal set1 set2 then Eq (* check next criterion *)
            ...
            Incomparable
    | StrictDominator, NoDominator _ ->
        Gt
    | NoDominator _, StrictDominator ->
        Lt
    | NonstrictDominator, NoDominator _ ->
        Geq
    | NoDominator _, NonstrictDominator ->
        Leq
    | _, _ ->
        assert false
  ...
  let _cmp_by_term_multiset ~ord l1 l2 =
    Multisets.MT.compare_partial (Ordering.compare ord) (to_multiset l1)
      (to_multiset l2)
...
module Conv = struct
  ...
  let rec try_hooks x hooks =
    ...
    | [] ->
        None
    | h :: hooks' -> (
      match h x with None -> try_hooks x hooks' | Some _ as res -> res )
  ...
  let lit_to_tst ?(ctx = T.Conv.create ()) lit =
    ...
    | SLiteral.True ->
        TypedSTerm.Form.true_
    | SLiteral.False ->
        TypedSTerm.Form.false_
...
module View = struct
  ...
  let get_eqn lit position =
    ...
    | Equation (l, r, sign), P.Left _ ->
        Some (l, r, sign)
    | Equation (l, r, sign), P.Right _ ->
        Some (r, l, sign)
    | True, _ | False, _ ->
        None
    | _ ->
        invalid_arg "get_eqn: wrong literal or position"
...
let as_inj_def lit =
  ...
  | Some (l, r, false) -> (
    try
      ...
      let vars_l, vars_r =
        (List.map _as_var (T.args l), List.map _as_var (T.args r))
      in
