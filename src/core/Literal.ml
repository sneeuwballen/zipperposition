...
let no_prop_invariant = function
  | Equation (lhs, rhs, sign) ->
    if T.is_true_or_false rhs && not sign then (
      CCFormat.printf "literal wrongly encoded: @[%a@] @[%a@] @[%b@]@." T.pp lhs
        T.pp rhs sign;
      false
    ) else
      true
    ...
    | None ->
      ...
      | Equation (l, r, _) ->
        ...
        if Type.is_prop (Term.ty l) then
          if T.is_true_or_false r then
            SLiteral.atom l sign
          else (
            let hd =
              if sign then
                Builtin.Equiv
