...
let lift_lambdas st = 
  let get_ll_sym_and_type ~proof f = 
    let _, body = TST.unfold_binder Binder.Forall f in
    ...
    | TST.AppBuiltin(b, [lhs;rhs]) 
      ...
      | TST.App(hd, _) when TST.is_const hd -> 
        ty_decl ~proof (TST.head_exn hd) (TST.ty_exn hd)
      | _ -> invalid_arg "wrongly encoded lambda lifting" end
    | _ ->  invalid_arg "wrongly encoded lambda lifting" in
  ...
  let assert_defs def = 
    ...
    [get_ll_sym_and_type ~proof def; assert_ ~proof def] in
