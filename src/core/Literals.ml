...
let equal lits1 lits2 =
  let rec check i = if i = Array.length lits1 then true else Lit.equal lits1.(i) lits2.(i) && check (i + 1) in
  if Array.length lits1 <> Array.length lits2 then false else check 0

let equal_com lits1 lits2 =
  let rec check i =
    if i = Array.length lits1 then true else Lit.equal_com lits1.(i) lits2.(i) && check (i + 1)
  in
  if Array.length lits1 <> Array.length lits2 then false else check 0
...
let compare_multiset ~ord (l1 : t) (l2 : t) =
  ...
  M.compare_partial_l (Literal.Comp.compare ~ord) (Array.to_list l1) (Array.to_list l2)

let hash lits = Hash.array Lit.hash lits

let variant ?(subst = S.empty) (a1, sc1) (a2, sc2) =
  Unif.unif_array_com ~size:`Same (subst, []) (a1, sc1) (a2, sc2) ~op:(fun (subst, t1) x y k ->
      Lit.variant ~subst x y (fun (s, t2) -> k (s, t1 @ t2)) )
  |> Iter.filter (fun (s, _) -> Subst.is_renaming s)

let are_variant a1 a2 = not (Iter.is_empty (variant (Scoped.make a1 0) (Scoped.make a2 1)))

let matching ?(subst = S.empty) ~pattern:(a1, sc1) (a2, sc2) =
  Unif.unif_array_com ~size:`Same (subst, []) (a1, sc1) (a2, sc2) ~op:(fun (subst, t1) x y k ->
      Lit.matching ~subst ~pattern:x y (fun (s, t2) -> k (s, t1 @ t2)) )

let matches a1 a2 = not (Iter.is_empty (matching ~pattern:(Scoped.make a1 0) (Scoped.make a2 1)))
...
let vars lits = Iter.of_array lits |> Iter.flat_map Lit.Seq.vars |> T.VarSet.of_iter |> T.VarSet.to_list

let is_ground lits = CCArray.for_all Lit.is_ground lits
...
let apply_subst renaming subst (lits, sc) = Array.map (fun lit -> Lit.apply_subst renaming subst (lit, sc)) lits
...
module MLI = Multiset.Make (struct
  ...
  let compare (l1, i1) (l2, i2) = if i1 = i2 then Lit.compare l1 l2 else CCShims_.Stdlib.compare i1 i2
end)
...
let _to_multiset_with_idx lits = CCArray.foldi (fun acc i x -> MLI.add acc (x, i)) MLI.empty lits
...
let maxlits_l ~ord lits =
  ...
  let max = MLI.max_seq (_compare_lit_with_idx ~ord) m |> Iter.map fst |> Iter.to_list in
...
let maxlits ~ord lits =
  ...
  let m = _to_multiset_with_idx lits in
  let max =
    MLI.max_seq (_compare_lit_with_idx ~ord) m |> Iter.map (fun (x, _) -> snd x) |> Iter.to_list |> BV.of_list
  in
  ZProf.exit_prof _span ; max
...
let is_absurd lits = CCArray.for_all Lit.is_absurd lits

let apply_subst renaming subst (lits, sc) = CCArray.map (fun l -> Lit.apply_subst renaming subst (l, sc)) lits
...
module Pos = struct
  let _fail_lits lits pos =
    let msg =
      CCFormat.sprintf "@[invalid position @[%a@]@ in lits [@[%a@]]@]" Position.pp pos (CCFormat.array Lit.pp)
        lits
    in
    invalid_arg msg

  let _fail_pos pos =
    let msg = CCFormat.sprintf "@[<2>invalid literal-array position@ @[%a@]@]" Position.pp pos in
    invalid_arg msg
...
module Conv = struct
  ...
  let to_forms ?hooks lits = Array.to_list (Array.map (Lit.Conv.to_form ?hooks) lits)
  ...
  let to_tst lits =
    ...
    let vars = clause_vars |> T.VarSet.to_list |> CCList.map (fun v -> T.Conv.to_simple_term ctx (T.var v)) in
    let disjuncts =
      match or_args with
      | [] ->
  ...
  let to_s_form ?allow_free_db ?ctx ?hooks lits =
    let ctx =
      if CCOpt.is_some ctx then CCOpt.get_exn ctx
        ...
        res
    in
    Array.to_list lits |> List.map (Literal.Conv.to_s_form ?hooks ?allow_free_db ~ctx) |> TypedSTerm.Form.or_
end
...
module View = struct
  ...
  let _unwrap2 ~msg f x y = match f x y with Some z -> z | None -> invalid_arg msg
...
let fold_terms ?(vars = false) ?(var_args = true) ?(fun_bodies = true) ?ty_args ~(which : [< `All | `Max]) ~ord
    ~subterms ~eligible lits k =
  let rec aux i =
    if i = Array.length lits then ()
...
let pp_tstp_closed out lits =
  let pp_vars out = function
    | [] ->
        ()
    | l ->
        Format.fprintf out "![@[%a@]]:@ " (Util.pp_list ~sep:", " Type.TPTP.pp_typed_var) l
  in
...
let pp_zf_closed out lits = pp_vars_gen ~pp_var:Type.ZF.pp_typed_var ~pp_lits:pp_zf out lits
...
let is_unique_max_horn_clause ~ord lits = BV.cardinal (pos lits) = 1 && BV.cardinal (maxlits ~ord lits) = 1
...
let unshielded_vars ?(filter = fun _ -> true) lits : _ list =
  vars lits |> List.filter (fun var -> filter var && not (is_shielded var lits))

let vars_distinct lits =
  let dif_vars = vars lits in
  let dif_ids = List.sort_uniq CCOrd.int @@ List.map HVar.id dif_vars in
...
let is_polymorphism_safe idx lits =
  ...
  let at_idx = Iter.filter (fun var -> Type.is_tType (HVar.ty var)) (Literal.Seq.vars lits.(idx)) in
