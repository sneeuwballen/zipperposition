...
let vars lits =
  Iter.of_array lits |> Iter.flat_map Lit.Seq.vars |> T.VarSet.of_seq
  |> T.VarSet.to_list
...
module Pos = struct
  ...
  let _fail_pos pos =
    let msg =
      CCFormat.sprintf "@[<2>invalid literal-array position@ @[%a@]@]"
        Position.pp pos
    in
    invalid_arg msg
...
module View = struct
  ...
  let get_arith lits pos =
    match pos with
    | Position.Arg (idx, pos') when idx < Array.length lits ->
      Lit.View.focus_arith lits.(idx) pos'
    | _ -> None

  let get_rat lits pos =
    match pos with
    | Position.Arg (idx, pos') when idx < Array.length lits ->
  ...
  let get_arith_exn = _unwrap2 ~msg:"get_arith: improper position" get_arith

  let get_rat_exn = _unwrap2 ~msg:"get_rat: improper position" get_rat
end
  ...
  let rec aux i =
    if i = Array.length lits then
      ...
      | Lit.Equation (l, r, sign) when sign_ok sign ->
        ...
        | Comparison.Gt -> k (l, r, sign, Position.(arg i @@ left @@ stop))
        | Comparison.Lt -> k (r, l, sign, Position.(arg i @@ right @@ stop))
      ...
      | Lit.Prop _ | Lit.Equation _ | Lit.Int _ | Lit.Rat _ | Lit.True
      | Lit.False ->
        () );
      aux (i + 1) )
  in
...
let fold_arith ~eligible lits k =
  let rec aux i =
    if i = Array.length lits then
      ...
      ( match Lit.View.get_arith lits.(i) with
      | None -> ()
      | Some x ->
...
let fold_arith_terms ~eligible ~which ~ord lits k =
  let module M = Monome in
  let module MF = Monome.Focus in
  fold_arith ~eligible lits (fun (a_lit, pos) ->
      ...
      in
      Int_lit.Focus.fold_terms ~pos a_lit (fun (foc_lit, pos) ->
...
let fold_rat ~eligible lits k =
  let rec aux i =
    if i = Array.length lits then
      ...
      ( match Lit.View.get_rat lits.(i) with
      | None -> ()
      | Some x ->
        let pos = Position.(arg i stop) in
        k (x, pos) );
...
let fold_rat_terms ~eligible ~which ~ord lits k =
  let module M = Monome in
  let module MF = Monome.Focus in
  fold_rat ~eligible lits (fun (a_lit, pos) ->
      (* do we use the given term? *)
      let do_term =
        match which with
        | `All -> fun _ -> true
        | `Max ->
          ...
          fun t -> CCList.mem ~eq:T.equal t max_terms
      in
      Rat_lit.Focus.fold_terms ~pos a_lit (fun (foc_lit, pos) ->
          let t = Rat_lit.Focus.term foc_lit in
          if do_term t then k (t, foc_lit, pos) ) )

let fold_terms ?(vars = false) ?ty_args ~(which : [< `All | `Max]) ~ord
  ...
  let rec aux i =
    if i = Array.length lits then
      ...
      Lit.fold_terms
        ...
        ?ty_args ~vars ~which ~ord ~subterms lits.(i) k;
      aux (i + 1) )
  in
  aux 0

let symbols ?(init = ID.Set.empty) lits =
  Iter.of_array lits |> Iter.flat_map Lit.Seq.symbols |> ID.Set.add_seq init
...
let pp out lits =
  ...
  pp_gen ~l:"[" ~r:"]" ~false_:"⊥" ~sep:(CCFormat.return "@ ∨ ") ~pp_lit
    out lits
...
let pp_vars out lits =
  pp_vars_gen ~pp_var:Type.pp_typed_var ~pp_lits:pp out lits

let pp_tstp out lits =
  let pp_lit = CCFormat.hovbox Lit.pp_tstp in
  pp_gen ~l:"(" ~r:")" ~false_:"$false" ~sep:(CCFormat.return "@ | ") ~pp_lit
    out lits
...
let unshielded_vars ?(filter = fun _ -> true) lits : _ list =
  vars lits
  |> List.filter (fun var -> filter var && not (is_shielded var lits))
