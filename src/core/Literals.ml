...
let vars lits =
  Iter.of_array lits |> Iter.flat_map Lit.Seq.vars |> T.VarSet.of_iter
  |> T.VarSet.to_list
...
module Conv = struct
  ...
  let to_tst lits =
    ...
    let vars =
      clause_vars |> T.VarSet.to_list
      |> CCList.map (fun v -> T.Conv.to_simple_term ctx (T.var v))
    in
    let disjuncts =
      match or_args with
...
let symbols ?(init = Name.Set.empty) ?(include_types = false) lits =
  Iter.of_array lits
  |> Iter.flat_map (Lit.Seq.symbols ~include_types)
  |> Name.Set.add_iter init
...
let pp out lits =
  ...
  pp_gen ~l:"[" ~r:"]" ~false_:"⊥" ~sep:(CCFormat.return "@ ∨ ") ~pp_lit out
    lits
...
let is_polymorphism_safe idx lits =
  let all =
    Iter.filter (fun var -> Type.is_tType (HVar.ty var)) (Seq.vars lits)
  in
  let at_idx =
    Iter.filter
      (fun var -> Type.is_tType (HVar.ty var))
      (Literal.Seq.vars lits.(idx))
  in
  Iter.for_all
    (fun ty_var -> Iter.exists (HVar.equal Type.equal ty_var) at_idx)
    all
