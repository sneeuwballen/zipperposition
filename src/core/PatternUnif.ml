...
let get_bvars args =
  ...
  if List.for_all T.is_bvar args then (
    ...
    if List.length no_dup = List.length res then
      Some (CCArray.of_list no_dup)
    else
      None
  ) else
    None
...
let norm subst scope t =
  let t =
    if Type.is_ground (T.ty t) then
      t
    else
      S.apply subst (t, scope)
  in
...
let rec build_term ?(depth = 0) ~subst ~scope ~counter var bvar_map t =
  let rec args_same ls1 ls2 =
    match ls1, ls2 with
    | Some x :: xs, y :: ys -> T.equal x y && args_same xs ys
    | [], [] -> true
    | _ -> false
  in
  ...
  | T.Var _ ->
    ...
    if T.equal t' t then
      ...
      build_term ~subst ~scope ~counter ~depth var bvar_map t'
  | T.Const _ -> t, subst
  | T.App (hd, args) ->
    if T.is_var hd then (
      ...
      if not (US.FO.mem subst (Term.as_var_exn hd, scope)) then (
        ...
        if List.length matrix != List.length args then (
          ...
          in
          res_term, subst
        ) else if args_same new_args args then
          t, subst
        else
          T.app hd (CCList.filter_map CCFun.id new_args), subst
      ) else (
        ...
        let t' =
          if T.equal hd hd' then
            ...
            let args =
              if Type.equal (T.ty hd) (T.ty hd') then
                args
              else
                List.map (fun t -> S.apply subst (t, scope)) args
            in
            T.app hd' args
          )
        in
  ...
  | T.AppBuiltin (b, args) ->
    let new_args, subst =
      List.fold_right
        ...
        args ([], subst)
    in
    if List.for_all2 T.equal args new_args then
      t, subst
    else
      T.app_builtin ~ty:(Term.ty t) b new_args, subst
      ...
      | T.Var _, T.Var _ ->
        let subst =
          if T.equal hd_s hd_t then
            ...
            flex_diff ~counter ~scope ~subst hd_s hd_t args_s args_t
        in
