...
let pp out ty =
  let ppvars out = function
    | [] ->
        ()
    | l ->
        Format.fprintf out " %a" (Util.pp_list HVar.pp) l
  in
...
let as_inductive_ty id =
  ID.payload_find id ~f:(function Payload_ind_type ty -> Some ty | _ -> None)
...
let as_inductive_ty_exn id =
  ...
  | Some ty ->
      ty
  | None ->
      invalid_declf_ "%a is not an inductive type" ID.pp id
...
let is_inductive_simple_type ty =
  try is_inductive_ty (TypedSTerm.head_exn ty) with Not_found -> false
...
let is_recursive (t : t) =
  ...
  if new_ then
    Util.debugf ~section 3 "(@[is_recursive@ :ty %a@ :res %B@])" (fun k ->
        k pp t res ) ;
  res
...
let as_constructor id =
  ID.payload_find id ~f:(function
    | Payload_ind_cstor (cstor, ity) ->
        Some (cstor, ity)
    | _ ->
        None )

let as_constructor_exn id =
  match as_constructor id with
  | None ->
      raise (NotAnInductiveConstructor id)
  | Some x ->
      x
...
let contains_inductive_types t =
  T.Seq.subterms t |> Iter.exists (fun t -> is_inductive_type (T.ty t))
