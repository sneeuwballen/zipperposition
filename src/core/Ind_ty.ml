...
type constructor = {cstor_name: ID.t; cstor_ty: Type.t; cstor_args: (Type.t * projector) list}
...
type id_or_tybuiltin = I of ID.t | B of Type.builtin
...
let pp out ty =
  let ppvars out = function [] -> () | l -> Format.fprintf out " %a" (Util.pp_list HVar.pp) l in
  Format.fprintf out "@[%a%a@]" ID.pp ty.ty_id ppvars ty.ty_vars

let type_hd ty =
  let _, _, ret = Type.open_poly_fun ty in
  match Type.view ret with Type.Builtin b -> Some (B b) | Type.App (s, _) -> Some (I s) | _ -> None
...
let as_inductive_ty id = ID.payload_find id ~f:(function Payload_ind_type ty -> Some ty | _ -> None)

let as_inductive_ty_exn id =
  match as_inductive_ty id with Some ty -> ty | None -> invalid_declf_ "%a is not an inductive type" ID.pp id

let is_inductive_ty id = match as_inductive_ty id with Some _ -> true | None -> false

let is_inductive_type ty = match type_hd ty with Some (I id) -> is_inductive_ty id | Some (B _) | None -> false

let is_inductive_simple_type ty = try is_inductive_ty (TypedSTerm.head_exn ty) with Not_found -> false

let as_inductive_type ty =
  match Type.view ty with
  | Type.App (id, l) -> (
    match as_inductive_ty id with None -> None | Some ity -> Some (ity, l) )
...
let is_recursive (t : t) =
  ...
  if new_ then Util.debugf ~section 3 "(@[is_recursive@ :ty %a@ :res %B@])" (fun k -> k pp t res) ;
  res
...
let is_rec_ (top : t) : bool =
  let rec find_in_ity (seen : t list) (ity : t) : bool =
    if CCList.mem ~eq:equal ity seen then false (* loop *)
      ...
      List.exists (fun cstor -> find_in_ty_args seen cstor.cstor_ty) ity.ty_constructors
  and find_in_ty_args seen ty =
    match Type.view ty with
    | Type.Forall ty' ->
        find_in_ty_args seen ty'
    | Type.Fun (args, _) ->
        List.exists (find_in_ty seen) args
    | Type.App _ | Type.Builtin _ | Type.Var _ | Type.DB _ ->
        false
  and find_in_ty (seen : t list) (ty : Type.t) =
    match Type.view ty with
    | Type.Forall ty' ->
        find_in_ty seen ty'
    | Type.App (id, l) ->
    ...
    | Type.Builtin _ | Type.Var _ | Type.DB _ ->
        false
  in
  find_in_ity [] top
...
let declare_ty id ~ty_vars constructors ~proof =
  ...
  if constructors = [] then invalid_declf_ "Ind_types.declare_ty %a: no constructors provided" ID.pp id ;
  (* check that [ty] is not declared already *)
  ( match as_inductive_ty id with
  | Some _ ->
      invalid_declf_ "inductive type %a already declared" ID.pp id
  | None ->
      () ) ;
  let rec ity =
    { ty_id= id
...
let as_constructor id =
  ID.payload_find id ~f:(function Payload_ind_cstor (cstor, ity) -> Some (cstor, ity) | _ -> None)

let as_constructor_exn id =
  match as_constructor id with None -> raise (NotAnInductiveConstructor id) | Some x -> x

let is_constructor s = match as_constructor s with Some _ -> true | None -> false

let contains_inductive_types t = T.Seq.subterms t |> Iter.exists (fun t -> is_inductive_type (T.ty t))
...
let as_projector id = ID.payload_find id ~f:(function Payload_ind_projector p -> Some p | _ -> None)
