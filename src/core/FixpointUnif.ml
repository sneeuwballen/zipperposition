  ...
  let rec aux ~depth ~under_var t =
    ...
    | T.App (hd, args) when T.is_var hd ->
      ...
      if T.equal hd var then
        if under_var || not no_prefix then
          None
        else
          raise NotUnif
      else
    ...
    | T.App (hd, args) ->
      ...
      | Some hd' ->
        CCOpt.map
          (fun args' ->
            if T.same_l args args' && T.equal hd hd' then
              t
            else
              T.app hd' args')
    ...
    | T.Var _ ->
      ...
      if T.equal var t then
        if under_var || Type.is_fun (T.ty t) then
          None
        else
          raise NotUnif
      else
  ...
  and aux_l ~depth ~under_var args =
    ...
    | x :: xs ->
      ...
      | None ->
        ignore xs';
        None
      | Some t -> CCOpt.map (fun ts -> t :: ts) xs')
  in
  ...
  let driver s t scope subst =
    ...
    if T.is_var s && T.is_var t then
      if T.equal s t then
        ...
        US.FO.bind subst (T.as_var_exn s, scope) (t, scope)
