...
module type S = sig
  ...
  val update :
    ?check:bool -> subst -> ty HVar.t Scoped.t -> term Scoped.t -> subst
  (** [bind subst v t] replaces the binding of [v] to [t], but fails if [v]
      occurs in [t] (performs an occur-check first)
      @param check if true, perform occur check
  ...
  (** [matching ~pattern scope_p b scope_b] returns [sigma] such that
      [sigma pattern = b], or fails. Only variables from the scope of [pattern]
  ...
  val matching_same_scope :
    ?protect:ty HVar.t Iter.t ->
    ?subst:subst ->
    scope:int ->
    pattern:term ->
    term ->
    subst
  (** matches [pattern] (more general) with the other term. The two terms live
      in the same scope, which is passed as the [scope] argument. It needs to
      gather the variables of the other term to make sure they are not bound.
      @param scope the common scope of both terms
      @param protect
        a sequence of variables to protect (they cannot be bound during
        matching!). Variables of the second term are automatically protected. *)
  ...
  val matching_adapt_scope :
    ...
    term Scoped.t ->
    subst
  (** Call either {!matching} or {!matching_same_scope} depending on whether the
      given scopes are the same or not.
  ...
  (** Succeeds iff the first term is a variant of the second, ie if they are
      alpha-equivalent *)

  val equal : subst:subst -> term Scoped.t -> term Scoped.t -> bool
  [@@deprecated "this function doesn't work anymore on HO terms"]
  (** [equal subst t1 s1 t2 s2] returns [true] iff the two terms are equal under
      the given substitution, i.e. if applying the substitution will return the
      same term. *)
