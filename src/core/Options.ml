...
(** {1 Global CLI options}
    ...
    Those options can be used by any program that parses command line arguments
    using the standard module {!Arg}. It may modify some global parameters, and
    return a parameter type for other options. *)

let _stats = ref false

type input_format = I_tptp | I_zf | I_tip | I_dk | I_guess
...
let input_format_of_string s =
  ...
  | "dk" ->
      I_dk
  | s ->
      failwith ("unknown input format " ^ s)
...
let print_format_of_string s =
  ...
  | "dk" | "zf" ->
      O_zf
  | _ ->
      failwith ("unknown print format " ^ s)
...
let mk_debug_opts () =
  ...
  |> CCList.cons
       ( "-d"
       , Arg.Int (Util.Section.set_debug Util.Section.root)
       , " alias for --debug" )
...
let make () =
  List.rev_append
    ...
    ; ( "--backtrace"
      , switch_opt true Printexc.record_backtrace
      , " enable backtraces" )
    ; ( "-bt"
      , switch_opt true Printexc.record_backtrace
      , " enable backtraces (alias for --backtrace)" )
    ; ( "--no-backtrace"
      , switch_opt false Printexc.record_backtrace
      , " disable backtraces" )
    ; ("--color", switch_opt true CCFormat.set_color_default, " enable colors")
    ; ( "--no-color"
      , switch_opt false CCFormat.set_color_default
      , " disable colors" )
    ; ( "-nc"
      , switch_opt false CCFormat.set_color_default
      , " alias for --no-color" )
    ...
    ; ( "--output"
      , Arg.String set_out
      , " choose printing format (zf, tptp, default, none)" )
    ; ("-o", Arg.String set_out, " alias for --output")
    ; ( "--break"
      , Arg.Set Util.break_on_debug
      , " wait for user input after each debug message" )
    ; ( "--show-ty-args"
      , Arg.Set InnerTerm.show_type_arguments
      , " show type arguments in terms" )
    ; ( "--hide-ty-args"
      , Arg.Clear InnerTerm.show_type_arguments
      , " hide type arguments in terms" )
    ; ( "--t-bigger-f"
      , Arg.Bool (( := ) Builtin._t_bigger_false)
      , " is T bigger than F" ) ]
