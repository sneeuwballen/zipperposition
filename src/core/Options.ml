...
let mk_debug_opts () =
  ...
  |> Iter.filter_map (fun (name, sec) ->
         if name = "" then None
         else Some ("--debug." ^ name, Arg.Int (Util.Section.set_debug sec), " debug level for section " ^ name) )
  |> Iter.to_list
  |> CCList.cons ("-d", Arg.Int (Util.Section.set_debug Util.Section.root), " alias for --debug")
