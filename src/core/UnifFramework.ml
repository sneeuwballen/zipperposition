...
module type PARAMETERS = sig
  ...
  val identify_scope :
    T.t Scoped.t -> T.t Scoped.t -> T.t * T.t * Scoped.scope * S.t

  val identify_scope_l :
       T.t list Scoped.t
    -> T.t list Scoped.t
    -> T.t list * T.t list * Scoped.scope * S.t
...
module type S = sig
  ...
  val unify_scoped_l :
    T.t list Scoped.t -> T.t list Scoped.t -> S.FO.t option OSeq.t
end
...
module type US = sig
  ...
  val unify_scoped_l :
    T.t list Scoped.t -> T.t list Scoped.t -> US.t option OSeq.t
end
...
module Make (P : PARAMETERS) = struct
  ...
  let do_unif ~bind_cnt ~hits_cnt problem subst unifscope =
    ...
    let delay steps res () =
      let skipper =
        int_of_float (Flex_state.get_exn PUP.k_skip_multiplier P.flex_state)
      in
      if steps != 0 && steps mod skipper == 0 then
        OSeq.append
          (OSeq.of_list (CCList.replicate (steps / skipper) None))
          res ()
      else res ()
    in
    let rec aux ?(root = false) subst problem =
      let decompose args_l args_r rest flag =
        ...
        let to_classify, rest =
          if List.length rest <= 15 then (new_args @ rest, [])
          else (new_args, rest)
        in
        let sort_class =
          List.sort (fun (l, r, _) (l', r', _) ->
              let l, l' =
                CCPair.map_same
                  (fun t -> T.head_term @@ snd @@ T.open_fun t)
                  (l, l')
              in
              let r, r' =
                CCPair.map_same
              ...
              in
              if
                ((not (Term.is_app l)) || not (Term.is_app r))
                && ((not (Term.is_app l')) || not (Term.is_app r'))
              then 0
              else if (not (Term.is_app l)) || not (Term.is_app r) then -1
              else if (not (Term.is_app l')) || not (Term.is_app r') then 1
              else
                Term.ho_weight l + Term.ho_weight r - Term.ho_weight l'
                - Term.ho_weight r' )
        in
        let classify_one s =
        ...
        if Flex_state.get_exn PragUnifParams.k_sort_constraints P.flex_state
        then
          sort_class rr @ sort_class fr @ sort_class unsure @ rest
          @ sort_class ff
      ...
      | (lhs, rhs, flag) :: rest -> (
        match
          PatternUnif.unif_simple ~subst ~scope:unifscope
            (T.of_ty (T.ty lhs))
            (T.of_ty (T.ty rhs))
        with
        | None ->
        ...
        | Some subst -> (
            ...
            let body_lhs, body_rhs, _ =
              eta_expand_otf ~subst ~scope:unifscope pref_lhs pref_rhs body_lhs
                body_rhs
            in
            let (hd_lhs, args_lhs), (hd_rhs, args_rhs) =
              (T.as_app body_lhs, T.as_app body_rhs)
            in
            if Term.is_type lhs then (
              assert (Term.is_type rhs) ;
              try
                let subst =
                  Unif.FO.unify_syn ~subst (lhs, unifscope) (rhs, unifscope)
                in
                aux subst rest
              ...
              | T.AppBuiltin (b1, args1), T.AppBuiltin (b2, args2) ->
                  let args_lhs = args_lhs @ args1
                  and args_rhs = args_rhs @ args2 in
                  if Builtin.equal b1 b2 then
                    try
                      let mode =
                        Flex_state.get_exn PragUnifParams.k_logop_mode
                          P.flex_state
                      in
                      let args_lhs, args_rhs =
                        Unif.norm_logical_disagreements ~mode b1 args_lhs
                          args_rhs
                      in
                  ...
                  | None ->
                      let args_unif =
                        if
                          ...
                          && T.equal hd_lhs hd_rhs
                        then (
                          incr bind_cnt ;
                          delay !bind_cnt (fun () ->
                              decompose_and_cont args_lhs args_rhs rest flag
                                subst () ) )
                        else OSeq.empty
                      in
                      let all_oracles =
                        P.pb_oracle (body_lhs, unifscope) (body_rhs, unifscope)
                          flag subst unifscope
                      in
                      let res =
                        OSeq.map
                          (fun sub_flag_opt ->
  ...
  let try_lfho_unif ((s, _) as t0) ((t, _) as t1) =
    ...
    let eligible_for_lfho t =
      ...
      let no_lams t =
        if Iter.exists T.is_fun (T.Seq.subterms ~include_builtin:true t) then
          raise LambdaFound
        else true
      in
      let rec aux t =
    ...
    in
    if
      Flex_state.get_exn PUP.k_try_lfho P.flex_state
      && eligible_for_lfho s && eligible_for_lfho t
    then
      try OSeq.return (Some (Unif_subst.subst (Unif.FO.unify_full t0 t1)))
      with Unif.Fail -> OSeq.empty
    else OSeq.empty
  ...
  let unify_scoped t0s t1s =
    ...
    try
      OSeq.append (try_lfho_unif t0s t1s)
        (do_unif ~bind_cnt ~hits_cnt [(lhs, rhs, P.init_flag)] subst unifscope)
      |> OSeq.map (fun opt ->
             CCOpt.map
               (fun subst ->
                 let norm t =
                   T.normalize_bools @@ Lambda.eta_expand @@ Lambda.snf t
                 in
                 ...
                 if not (T.equal l r && Type.equal (Term.ty l) (Term.ty r)) then (
                   ...
                   CCFormat.printf "orig:@[%a@]@.=?=@.@[%a@]@." (Scoped.pp T.pp)
                     t0s (Scoped.pp T.pp) t1s ;
                   CCFormat.printf "new:@[%a:%a@]@.=?=@.@[%a:%a@]@." T.pp l
                     Type.pp (T.ty l) T.pp r Type.pp (T.ty r) ;
                   assert false ) ;
                 subst )
               opt )
    with Unif.Fail -> OSeq.empty
  ...
  let unify_scoped_l t0s t1s =
    ...
    let problem =
      List.map (fun (a, b) -> (a, b, P.init_flag)) (List.combine lhs rhs)
    in
    let bind_cnt = ref 0 in
    (* number of created binders *)
    let hits_cnt = ref 0 in
    (* number of unifiers found *)
    try do_unif ~bind_cnt ~hits_cnt problem subst unifscope
    with Unif.Fail -> OSeq.empty
end
