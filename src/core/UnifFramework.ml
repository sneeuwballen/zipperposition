...
module Make (P : PARAMETERS) = struct
  ...
  let do_unif ~bind_cnt ~hits_cnt problem subst unifscope =
    let max_infs =
      if Flex_state.get_exn PUP.k_max_inferences P.flex_state < 0 then
        max_int
      else
        Flex_state.get_exn PUP.k_max_inferences P.flex_state
    in
    let delay steps res () =
      let skipper =
    ...
    let rec aux ?(root = false) subst problem =
      let decompose args_l args_r rest flag =
        let rec zipped_with_flag = function
          ...
          | _, _ -> invalid_arg "lists must be of the same size."
        in
        let new_args = zipped_with_flag (args_l, args_r) in
        let to_classify, rest =
          if List.length rest <= 15 then
            new_args @ rest, []
          else
            new_args, rest
        in
        let sort_class =
          List.sort (fun (l, r, _) (l', r', _) ->
              ...
              then
                0
              else if (not (Term.is_app l)) || not (Term.is_app r) then
                -1
              else if (not (Term.is_app l')) || not (Term.is_app r') then
                1
              else
        ...
        let classify_one s =
          let rec follow_bindings t =
            ...
            if T.equal hd derefed then
              hd
            else
              follow_bindings derefed
          in
          ...
          if Term.is_type lhs then (
            ...
            | T.AppBuiltin (b1, args1), T.AppBuiltin (b2, args2) ->
              ...
              if Builtin.equal b1 b2 then (
                try
                  let mode =
                    Flex_state.get_exn PragUnifParams.k_logop_mode P.flex_state
                  in
                  let args_lhs, args_rhs =
                    Unif.norm_logical_disagreements ~mode b1 args_lhs args_rhs
                  in
  ...
  let try_lfho_unif ((s, _) as t0) ((t, _) as t1) =
    ...
    let eligible_for_lfho t =
      ...
      let rec aux t =
        match T.view t with
        | T.App (hd, args) when T.is_const hd -> List.for_all no_lams args
