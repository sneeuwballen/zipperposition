...
module Feature : sig
  ...
  let compare f1 f2 : int =
    ...
    | N i1, N i2 ->
        CCInt.compare i1 i2
    | S s1, S s2 ->
        ID.Set.compare s1 s2
    | M m1, M m2 ->
        ID.Map.compare CCInt.compare m1 m2
    | L m1, L m2 ->
        Util.Int_set.compare m1 m2
    | N _, _ | S _, _ | M _, _ | L _, _ ->
...
module Make (C : Index_intf.CLAUSE) = struct
  ...
  module Feature_fun = struct
    ...
    type t = {name: string; f: Index_intf.lits -> Index_intf.labels -> feature}
      ...
      let subterms_depth t k =
        let rec recurse depth t =
          if not (T.is_app_var t) then (
            ...
            | Fun (_, u) ->
                recurse depth u
                (* not increasing the depth when we go under a fun
    ...
    let depth_sym_minus = make "depth_sym-" (depth_sym_ SLiteral.is_neg)
  end
  ...
  let empty_trie n =
    match n with
    | TrieNode m when Feat_map.is_empty m ->
        true
    | TrieLeaf set when C_set.is_empty set ->
        true
    | _ ->
        false
    ...
    let rec goto trie i rebuild =
      if i = IArray.length fv then
        ...
        | TrieLeaf set as leaf -> (
          (* found leaf *)
          match k set with
          | new_leaf when leaf == new_leaf ->
              root (* no change, return same tree *)
          | new_leaf ->
              rebuild new_leaf (* replace by new leaf *) )
        | TrieNode _ ->
            assert false
      else
        match (trie, IArray.get fv i) with
        | TrieNode m, c -> (
          try
            (* insert in subtrie *)
            ...
            let rebuild' subtrie =
              ...
              | _ when empty_trie subtrie ->
                  rebuild (TrieNode (Feat_map.remove c m))
              | _ ->
                  rebuild (TrieNode (Feat_map.add c subtrie m))
            in
  ...
  type t = {trie: trie; funs: feature_funs}

  let empty_with funs = {trie= TrieNode Feat_map.empty; funs}
    ...
    let rec fold_higher i node =
      if i = IArray.length fv then
        ...
        | TrieNode map, feat ->
            Feat_map.iter
              (fun feat' subnode ->
                if check ~feat_query:feat ~feat_tree:feat' then
                  fold_higher (i + 1) subnode (* go in the branch *)
                else () )
              map
        | TrieLeaf _, _ ->
            assert false
  ...
  let iter idx f =
    let rec iter = function
      | TrieLeaf set ->
          C_set.iter f set
      | TrieNode map ->
          Feat_map.iter (fun _ t' -> iter t') map
    in
    iter idx.trie
  ...
  let fold f acc idx =
    let rec fold acc = function
      | TrieLeaf set ->
          C_set.fold (fun x acc -> f acc x) set acc
      | TrieNode map ->
          Feat_map.fold (fun _ t' acc -> fold acc t') map acc
    in
    fold acc idx.trie
end
