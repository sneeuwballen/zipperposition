  ...
  let pp out (f : t) : unit =
    ...
    | M m ->
      Fmt.fprintf out "(@[mset@ %a@])"
        Fmt.(seq (pair ~sep:silent ID.pp int))
        (ID.Map.to_seq m)
    | L l ->
...
module Make (C : Index_intf.CLAUSE) = struct
  ...
  module Feature_fun = struct
    ...
    let set_sym_ filter lits _ = symbols_ filter lits |> ID.Set.of_seq |> mk_s
        ...
        | TrieNode m, c ->
          ( try
            ...
            with Not_found ->
              (* no subtrie found *)
              let subtrie =
                if i + 1 = IArray.length fv then
                  TrieLeaf C_set.empty
                else
                  TrieNode Feat_map.empty
              and rebuild' subtrie =
                match subtrie with
                | _ when empty_trie subtrie ->
                  rebuild (TrieNode (Feat_map.remove c m))
