...
struct
            ...
            List.map
              (fun arg ->
                if Type.is_fun (T.ty arg) then
                  ...
                  if (T.is_bvar arg && T.is_ground arg) || not exception_on_error then arg
      ...
      let rec limit_combinations max_c l =
        if max_c <= 1 then CCList.map (fun l -> [List.hd l]) l
          ...
          | x :: xs ->
              ...
              let x, max_c = if n <= max_c then (x, max_c / n) else (CCList.take (max max_c 1) x, 0) in
              x :: limit_combinations max_c xs
      in
      match combs_limit with None -> aux l | Some max -> aux (limit_combinations max l)
  ...
  let cover_rigid_skeleton ?(covers_limit = None) t solids =
    ...
    let combs_limit = match covers_limit with None -> None | Some x -> if x < 0 then None else Some x in
    ...
    let rec aux ?(recurse = true) ~depth s_args t : T.t list =
      ...
      let db_hits = CCList.filter_map (fun (s, s_db) -> if T.equal s t then Some s_db else None) sols_as_db in
      let rest =
        try
          match T.view t with
          | AppBuiltin (hd, args) ->
      ...
      in
      if CCList.is_empty db_hits && CCList.is_empty rest then raise CoveringImpossible
  ...
  let collect_flex_flex ~counter ~flex_args t =
    let replace_var ~bvar_tys ~target =
      if Type.returns_tType (T.ty target) || (CCList.is_empty flex_args && CCList.is_empty (T.args target)) then
        (target, [])
      else
        let bvars = List.mapi (fun i ty -> (i, ty)) bvar_tys |> List.rev_map (fun (i, ty) -> T.bvar ~ty i) in
        ...
        let flex_args_db =
          List.mapi (fun i a -> T.bvar ~ty:(T.ty a) (n_bvars + (num_f_args - 1 - i))) flex_args
        in
  ...
  let solve_flex_flex_diff ~subst ~counter ~scope lhs rhs =
    ...
    let hd_l, args_l, n_l = (T.as_var_exn @@ T.head_term lhs, T.args lhs, List.length @@ T.args lhs) in
    let hd_r, args_r, n_r = (T.as_var_exn @@ T.head_term rhs, T.args rhs, List.length @@ T.args rhs) in
          ...
          CCList.flatten
            (List.mapi
               (fun i arg ->
                 ...
                 List.combine (CCList.replicate n (T.bvar (n_l - i - 1) ~ty:(T.ty arg))) arg_covers )
               args_l )
        in
        let covered_r =
          CCList.flatten
            (List.mapi
               (fun i arg ->
                 ...
                 let n = List.length arg_covers in
                 List.combine arg_covers (CCList.replicate n (T.bvar (n_r - i - 1) ~ty:(T.ty arg))) )
               args_r )
        in
        let all_covers = covered_l @ covered_r in
        assert (List.for_all (fun (l_arg, r_arg) -> Type.equal (Term.ty l_arg) (Term.ty r_arg)) all_covers) ;
  ...
  let cover_flex_rigid ~subst ~counter ~scope flex rigid =
    ...
    let subst =
      List.fold_left
        (fun subst (lhs, rhs) -> US.subst (solve_flex_flex_diff ~subst ~counter ~scope lhs rhs))
  ...
  let var_conditions s t =
    (T.is_linear s || T.is_linear t) && T.VarSet.is_empty (T.VarSet.inter (T.vars s) (T.vars t))
  ...
  let build_constraints args1 args2 rest =
    let rf, other =
      ...
      |> CCList.partition (fun (s, t) -> T.is_const (T.head_term s) && T.is_const (T.head_term t))
    in
    rf @ rest @ other

  let project_flex_rigid ~subst ~scope flex rigid =
    assert (T.is_var (T.head_term flex)) ;
    let flex_var, flex_args = (T.as_var_exn @@ T.head_term flex, List.mapi (fun i a -> (i, a)) (T.args flex)) in
  ...
  let rec unify ~scope ~counter ~subst constraints =
    ...
    | (s, t) :: rest ->
        ...
        if not (Term.equal s' t') then
          ...
          | T.Var _, T.Var _ ->
              let solver = if not (T.equal hd_s hd_t) then solve_flex_flex_diff else solve_flex_flex_same in
          ...
          | T.Var _, _ ->
              ...
              @ CCList.flat_map
                  (fun (subst, s, t) -> unify ~scope ~counter ~subst:(US.of_subst subst) ((s, t) :: rest))
                  projected
          | _, T.Var _ ->
          ...
          | T.AppBuiltin (hd_s, args_s'), T.AppBuiltin (hd_t, args_t')
            when Builtin.equal hd_s hd_t
                 && List.length args_s' + List.length args_s = List.length args_t' + List.length args_t -> (
            try
              let mode = Flex_state.get_exn PragUnifParams.k_logop_mode St.st in
              let args_lhs, args_rhs =
                Unif.norm_logical_disagreements ~mode hd_s (args_s' @ args_s) (args_t' @ args_t)
              in
  ...
  let unify_scoped ?(subst = US.empty) ?(counter = ref 0) t0_s t1_s =
    try
      let res =
        ...
        else if Scoped.scope t0_s != Scoped.scope t1_s then raise (Invalid_argument "scopes should be the same")
        else
