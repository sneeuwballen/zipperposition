...
struct
  ...
  let eta_expand_otf ~subst ~scope pref1 pref2 t1 t2 =
    ...
    if List.length pref1 = List.length pref2 then
      t1, t2, pref1
    else (
          ...
          List.map
            (fun arg ->
              if Type.is_fun (T.ty arg) then (
                ...
                if (T.is_bvar arg && T.is_ground arg) || not exception_on_error
                then
                  arg
                else
                  raise NotInFragment
                ...
                if T.is_ground arg || not exception_on_error then
                  arg
                else
                  raise NotInFragment
              ))
  ...
  let all_combs ~combs_limit l =
    ...
    if CCList.for_all (fun l -> List.length l = 1) l then
      [ CCList.flatten l ]
    else (
      let rec limit_combinations max_c l =
        if max_c <= 1 then
          CCList.map (fun l -> [ List.hd l ]) l
        else (
          match l with
          | [] -> []
          | x :: xs ->
  ...
  let cover_rigid_skeleton ?(covers_limit = None) t solids =
    ...
    let rec aux ?(recurse = true) ~depth s_args t : T.t list =
      ...
      let sols_as_db =
        List.mapi (fun i t -> t, T.bvar ~ty:(T.ty t) (n - i - 1 + depth)) s_args
      in
      let db_hits =
        CCList.filter_map
          (fun (s, s_db) ->
  ...
  let collect_flex_flex ~counter ~flex_args t =
    ...
    let rec aux ~bvar_tys t =
      ...
      | Fun (ty, body) ->
        ...
        T.fun_ ty body', cons
      | Var _ -> replace_var ~bvar_tys ~target:t
      | _ -> t, []
    in
  ...
  let norm t =
    if Term.is_fun (T.head_term t) then
      Lambda.whnf t
    else
      t
  ...
  let rec norm_deref subst (t, sc) =
    ...
    let t' =
      ...
      | T.Var _ ->
        ...
        if T.equal tt u then
          u
        else
          norm_deref subst (u, sc)
      | T.App (f0, l) ->
        let f = norm_deref subst (f0, sc) in
        let t =
          if T.equal f0 f then
            tt
          else
            T.app f l
        in
      ...
      CCList.filter_map
        (fun (i, fa) ->
          ...
          | Some id' when ID.equal id id' && Type.equal (T.ty fa) ret_ty ->
            ...
            in
            let flex_arg =
              if CCList.is_empty trailing_tys then
                fa
              else
                T.fun_l trailing_tys fa
            in
  ...
  let rec unify ~scope ~counter ~subst constraints =
    ...
    | (s, t) :: rest ->
      ...
      if not (Term.equal s' t') then (
        ...
        | T.Var _, T.Var _ ->
          let solver =
            if not (T.equal hd_s hd_t) then
              solve_flex_flex_diff
            else
              solve_flex_flex_same
          in
          let subst =
        ...
        | T.Var _, _ ->
          let projected =
            project_flex_rigid ~subst:(US.subst subst) ~scope body_s' body_t'
          in
          let covered =
          ...
          @ CCList.flat_map
              (fun (subst, s, t) ->
                unify ~scope ~counter ~subst:(US.of_subst subst) ((s, t) :: rest))
              projected
        | _, T.Var _ ->
          let projected =
            project_flex_rigid ~subst:(US.subst subst) ~scope body_t' body_s'
          in
          let covered =
