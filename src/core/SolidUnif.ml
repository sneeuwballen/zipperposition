...
struct
  ...
  let solidify ?(limit = true) ?(exception_on_error = true) t =
    let rec aux t =
      ...
      if not (Type.is_ground (T.ty t)) then raise NotInFragment ;
      match T.view t with
      | AppBuiltin (hd, args) ->
      ...
      | App (hd, args) ->
          ...
          if limit && List.length args > solid_limit then raise NotInFragment ;
          let args' =
            List.map
              (fun arg ->
                if Type.is_fun (T.ty arg) then
                  ...
                  if
                    (T.is_bvar arg && T.is_ground arg) || not exception_on_error
                  then arg
                  else raise NotInFragment
                else
                  let arg = Lambda.snf arg in
                  if T.is_ground arg || not exception_on_error then arg
                  else raise NotInFragment )
              args
          in
        ...
        if max_c <= 1 then CCList.map (fun l -> [List.hd l]) l
          ...
          | x :: xs ->
              ...
              let x, max_c =
                ...
                else (CCList.take (max max_c 1) x, 0)
              in
              x :: limit_combinations max_c xs
      in
      match combs_limit with
      | None ->
          aux l
      | Some max ->
          aux (limit_combinations max l)
  ...
  let cover_rigid_skeleton ?(covers_limit = None) t solids =
    ...
    assert (
      List.for_all
        (fun t -> (not @@ Type.is_fun @@ T.ty t) || T.is_bvar t)
        solids ) ;
    let n = List.length solids in
    let combs_limit =
      match covers_limit with
      | None ->
          None
      | Some x ->
    ...
    let rec aux ?(recurse = true) ~depth s_args t : T.t list =
      ...
      let db_hits =
        CCList.filter_map
          (fun (s, s_db) -> if T.equal s t then Some s_db else None)
          sols_as_db
      in
      let rest =
        try
          ...
          | AppBuiltin (hd, args) ->
              if CCList.is_empty args then [t]
                ...
                let args_combined =
                  all_combs ~combs_limit (List.map (aux ~depth s_args) args)
                in
          ...
          | App (hd, args) when recurse ->
              if Term.is_var hd then [t]
                ...
                let hd, args =
                  (T.head_term_mono t, CCList.drop_while T.is_type args)
                in
                let covered_hd =
                  aux ~recurse:(recurse && not (T.is_app hd)) ~depth s_args hd
                in
                let covered_args = List.map (aux ~depth s_args) args in
                let cobmined =
                  all_combs ~combs_limit (covered_hd :: covered_args)
                in
      ...
      in
      if CCList.is_empty db_hits && CCList.is_empty rest then
        raise CoveringImpossible
      else CCList.interleave db_hits rest
    in
    let _span = ZProf.enter_prof prof_cover_rigid in
    try
      let res = aux ~depth:0 solids t in
      ZProf.exit_prof _span ; res
    with CoveringImpossible -> ZProf.exit_prof _span ; []

  let collect_flex_flex ~counter ~flex_args t =
    let replace_var ~bvar_tys ~target =
      if
        Type.returns_tType (T.ty target)
        || (CCList.is_empty flex_args && CCList.is_empty (T.args target))
      then (target, [])
      else
        let bvars =
          List.mapi (fun i ty -> (i, ty)) bvar_tys
          |> List.rev_map (fun (i, ty) -> T.bvar ~ty i)
        in
        ...
        let flex_args_db =
          List.mapi
            (fun i a -> T.bvar ~ty:(T.ty a) (n_bvars + (num_f_args - 1 - i)))
            flex_args
        in
    ...
    let rec aux ~bvar_tys t =
      ...
      | App (hd, args) ->
          if Term.is_var hd then replace_var ~bvar_tys ~target:t
          else
  ...
  let solve_flex_flex_diff ~subst ~counter ~scope lhs rhs =
    ...
    let rhs =
      solidify @@ Lambda.whnf
      @@ Subst.FO.apply Subst.Renaming.none subst (rhs, scope)
    in
    ...
    let hd_l, args_l, n_l =
      (T.as_var_exn @@ T.head_term lhs, T.args lhs, List.length @@ T.args lhs)
    in
    let hd_r, args_r, n_r =
      (T.as_var_exn @@ T.head_term rhs, T.args rhs, List.length @@ T.args rhs)
    in
    assert (not @@ HVar.equal Type.equal hd_l hd_r) ;
    let cover_rigid_skeleton =
      let limit = get_op PUP.k_max_unifs_solid_ff in
      cover_rigid_skeleton ~covers_limit:(Some limit)
    in
    let res =
      if CCList.is_empty args_l && CCList.is_empty args_r then
        ...
        assert (
          List.for_all
            (fun (l_arg, r_arg) -> Type.equal (Term.ty l_arg) (Term.ty r_arg))
            all_covers ) ;
        let fresh_var_ty =
          Type.arrow (List.map (fun (a_l, _) -> T.ty a_l) all_covers) (T.ty lhs)
        in
        let fresh_var = T.var (HVar.fresh_cnt ~counter ~ty:fresh_var_ty ()) in
        let subs_l =
          T.fun_l (List.map T.ty args_l)
            (T.app fresh_var (List.map fst all_covers))
        in
        let subs_r =
          T.fun_l (List.map T.ty args_r)
            (T.app fresh_var (List.map snd all_covers))
        in
  ...
  let solve_flex_flex_same ~subst ~counter ~scope lhs rhs =
    ...
    in
    let res =
      if CCList.is_empty args_l && CCList.is_empty args_r then US.of_subst subst
      else (
        ...
        let same_args =
          ...
          |> List.mapi (fun i (a, b) ->
                 if T.equal a b then Some (T.bvar ~ty:(T.ty a) (n_l - i - 1))
                 else None )
          |> CCList.filter_map CCFun.id
        in
        let fresh_var_ty =
          Type.arrow (List.map (fun t -> T.ty t) same_args) (T.ty lhs)
        in
        let fresh_var = T.var (HVar.fresh_cnt ~counter ~ty:fresh_var_ty ()) in
        let subs_val =
          T.fun_l (List.map T.ty args_l) (T.app fresh_var same_args)
        in
  ...
  let cover_flex_rigid ~subst ~counter ~scope flex rigid =
    ...
    let rigid =
      Lambda.snf @@ Subst.FO.apply Subst.Renaming.none subst (rigid, scope)
    in
    let rigid_orig = rigid in
    let flex, rigid = (solidify flex, solidify rigid) in
    let flex_args = T.args flex in
    let to_bind, flex_constraints =
      collect_flex_flex ~counter ~flex_args rigid
    in
    let subst =
      List.fold_left
        (fun subst (lhs, rhs) ->
          US.subst (solve_flex_flex_diff ~subst ~counter ~scope lhs rhs) )
        subst flex_constraints
    in
    ...
    let res =
      if CCList.is_empty rigid_covers then raise NotUnifiable
      else
        ...
        if CCList.is_empty flex_args then (
          ...
          if not (Term.DB.is_closed rigid) then (
            CCFormat.printf "@[%a@]=?=@[%a@]@ has bound vars in RHS@." T.pp flex
              T.pp rigid_orig ;
            assert false ) ;
          ...
          List.map
            (fun r ->
              ...
              let subs_flex =
                Subst.FO.bind' subst (head_var, scope) (closed_rigid, scope)
              in
  ...
  let norm t = if Term.is_fun (T.head_term t) then Lambda.whnf t else t

  let rec norm_deref subst (t, sc) =
    let pref, tt = T.open_fun t in
    let t' =
      match T.view tt with
      | T.Var _ ->
          let u, _ = US.FO.deref subst (tt, sc) in
          if T.equal tt u then u else norm_deref subst (u, sc)
      | T.App (f0, l) ->
          let f = norm_deref subst (f0, sc) in
          let t = if T.equal f0 f then tt else T.app f l in
          let u = norm t in
          if T.equal t u then t else norm_deref subst (u, sc)
      | _ ->
          tt
    in
    if T.equal tt t' then t else T.fun_l pref t'
  ...
  let project_flex_rigid ~subst ~scope flex rigid =
    assert (T.is_var (T.head_term flex)) ;
    let flex_var, flex_args =
      ( T.as_var_exn @@ T.head_term flex
      , List.mapi (fun i a -> (i, a)) (T.args flex) )
    in
        ...
        CCList.filter_map
          (fun (i, fa) ->
            ...
            | Some id' when ID.equal id id' && Type.equal (T.ty fa) ret_ty ->
                let subs_term =
                  T.fun_l pref_tys (T.bvar ~ty:ret_ty (n - 1 - i))
                in
                let subst =
                  Subst.FO.bind' subst (flex_var, scope) (subs_term, scope)
                in
                let flex_arg =
                  if CCList.is_empty trailing_tys then fa
                  else T.fun_l trailing_tys fa
                in
  ...
  let rec unify ~scope ~counter ~subst constraints =
    ...
    | [] ->
        [subst]
    | (s, t) :: rest ->
        if not (Type.equal (T.ty s) (T.ty t)) then raise NotInFragment ;
        let s', t' =
          (norm_deref subst (s, scope), norm_deref subst (t, scope))
        in
        if not (Term.equal s' t') then
          let pref_s, body_s = T.open_fun s' in
          let pref_t, body_t = T.open_fun t' in
          let body_s', body_t', _ =
            eta_expand_otf ~subst:(US.subst subst) ~scope pref_s pref_t body_s
              body_t
          in
          ...
          | T.Var _, T.Var _ ->
              let solver =
                if not (T.equal hd_s hd_t) then solve_flex_flex_diff
                else solve_flex_flex_same
              in
              let subst =
                solver ~subst:(US.subst subst) ~scope ~counter body_s' body_t'
              in
              unify ~scope ~counter ~subst rest
          | T.Var _, _ ->
              let projected =
                project_flex_rigid ~subst:(US.subst subst) ~scope body_s'
                  body_t'
              in
              let covered =
                cover_flex_rigid ~subst:(US.subst subst) ~counter ~scope body_s'
                  body_t'
              in
              CCList.flat_map
                (fun subst -> unify ~scope ~counter ~subst rest)
                covered
              @ CCList.flat_map
                  (fun (subst, s, t) ->
                    unify ~scope ~counter ~subst:(US.of_subst subst)
                      ((s, t) :: rest) )
                  projected
          | _, T.Var _ ->
              let projected =
                project_flex_rigid ~subst:(US.subst subst) ~scope body_t'
                  body_s'
              in
              let covered =
                cover_flex_rigid ~subst:(US.subst subst) ~counter ~scope body_t'
                  body_s'
              in
              CCList.flat_map
                (fun subst -> unify ~scope ~counter ~subst rest)
          ...
          | T.AppBuiltin (hd_s, args_s'), T.AppBuiltin (hd_t, args_t')
            ...
            try
              ...
              let args_lhs, args_rhs =
                Unif.norm_logical_disagreements ~mode hd_s (args_s' @ args_s)
                  (args_t' @ args_t)
              in
              unify ~subst ~counter ~scope
          ...
          | T.Const f, T.Const g
            when ID.equal f g && List.length args_s = List.length args_t ->
              unify ~subst ~counter ~scope
              @@ build_constraints args_s args_t rest
          ...
          | _ ->
              raise NotUnifiable
        else unify ~subst ~counter ~scope rest
    ...
    try
      let res =
        ...
        if US.is_empty subst then
          let t0', t1', scope, subst =
            US.FO.rename_to_new_scope ~counter t0_s t1_s
          in
          if var_conditions t0' t1' then
            let t0', t1' = (solidify t0', solidify t1') in
            unify ~scope ~counter ~subst [(t0', t1')]
          else raise NotInFragment
        else if Scoped.scope t0_s != Scoped.scope t1_s then
      ...
      assert (
        CCList.for_all
          (fun sub ->
            ...
            let lhs_o = norm @@ US_A.apply subst t0_s
            and rhs_o = norm @@ US_A.apply subst t1_s in
            (* CCFormat.printf "l_o:@[%a@];r_0:@[%a@]@.sub:@[%a@]@." T.pp lhs_o T.pp rhs_o US.pp sub; *)
            let lhs = norm @@ US_A.apply sub t0_s
            and rhs = norm @@ US_A.apply sub t1_s in
            if T.equal lhs rhs then true
            else (
              CCFormat.printf "orig: @[%a@]=?=@[%a@]@." (Scoped.pp T.pp) t0_s
                (Scoped.pp T.pp) t1_s ;
              CCFormat.printf "orig_sub: @[%a@]@." US.pp subst ;
              CCFormat.printf "orig_app: @[%a@]=?=@[%a@]@." T.pp lhs_o T.pp
                rhs_o ;
