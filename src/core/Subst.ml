...
module Renaming = struct
  type t = R_none | R_some of {mutable map: var M.t; mutable n: int}
...
let[@unroll 2] rec deref subst ((t, sc_t) as term) =
  match T.view t with
  | T.Var v -> (
    match find subst (v, sc_t) with Some t' -> deref subst t' | None -> term )
  | _ ->
      term
  ...
  let rec aux sc t =
    if T.equal t T.tType then t
      ...
      | T.Const id ->
          T.const ~ty id
      | T.App (f, l) ->
          T.app ~ty (aux sc f) (List.map (aux sc) l)
      | T.AppBuiltin (b, l) ->
          T.app_builtin b ~ty (List.map (aux sc) l)
      | T.Bind (b, varty, body) ->
    ...
    if T.is_ground t then t
      ...
      | T.HasType ty ->
          ...
          let res =
            ...
            | T.Const id ->
                (* regular constant *)
                if T.equal ty ty' then t else T.const ~ty:ty' id
            | T.DB i ->
                if T.equal ty ty' then t else T.bvar ~ty:ty' i
            | T.Var v -> (
...
module Projection = struct
  type t = {scope: Scoped.scope; subst: subst; renaming: Renaming.t}
