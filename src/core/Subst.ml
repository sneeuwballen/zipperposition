...
module Renaming = struct
  type t =
    | R_none
    | R_some of {mutable map : var M.t; mutable n : int}
...
let bind : t -> var Scoped.t -> T.t Scoped.t -> t =
 fun subst v t ->
  assert (not (M.mem v subst));
  M.add v t subst

let update : t -> var Scoped.t -> T.t Scoped.t -> t =
 fun subst v t ->
...
let merge s1 s2 =
  M.merge
    (fun v b1 b2 ->
      ...
      | _, None -> b1
      | Some t1, Some t2 ->
        if Scoped.equal T.equal t1 t2 then
          Some t1
        else
          raise (InconsistentBinding (v, t1, t2)) )
    s1 s2
...
let of_list ?(init = empty) l =
  match l with
  | [] -> init
  | _ :: _ -> List.fold_left (fun subst (v, t) -> bind subst v t) init l
  ...
  let rec aux t sc_t : T.t =
    ...
    | T.HasType ty ->
      ...
      | T.Var v ->
        ...
        ( match find_exn subst (v, sc_t) with
        | t', sc' ->
      ...
      | T.App (hd, l) ->
        ...
        if T.equal ty ty' && T.equal hd hd' && T.same_l l l' then
          t
        else
          T.app ~ty:ty' hd' l'
      | T.AppBuiltin (s, l) ->
        let l' = aux_list l sc_t in
        if T.equal ty ty' && T.same_l l l' then
          t
        else
          T.app_builtin ~ty:ty' s l' )
  and aux_list l sc =
    match l with
    | [] -> []
    | [t1] -> [aux t1 sc]
    | t1 :: t2 :: l' -> aux t1 sc :: aux t2 sc :: aux_list l' sc
  in
  aux t

(* Apply substitution to a term and rename variables not bound by [subst]*)
let apply renaming subst (t, sc) =
  if is_empty subst && Renaming.is_none renaming then
    t
  else
...
module Ty : SPECIALIZED with type term = Type.t = struct
  ...
  let apply renaming subst t =
    Type.of_term_unsafe
      (apply renaming subst (t : term Scoped.t :> T.t Scoped.t))
