...
let debugf_real ~section msg k =
  ...
  if section == Section.root then
    Format.fprintf debug_fmt_ "@{<Black>@[<4>%.3f[]@}@ " now
  else
    Format.fprintf debug_fmt_ "@{<Black>@[<4>%.3f[%s]@}@ " now
...
type profiler = {
  ...
  (* max time in the profiled function (ns) *)
  mutable prof_enter : timestamp
      (* time at which we entered the profiler (ns) *)
}
...
let enter_prof profiler =
  if !enable_profiling then
    profiler.prof_enter <- get_time_mon_ ()
...
let show_profilers out () =
  ...
  List.iter
    (fun profiler ->
      if profiler.prof_calls > 0 then (* print content of the profiler *)
