...
module Constraint = struct
  ...
  let rec pp out t =
    ...
    | LT (a, b) ->
        Format.fprintf out "(< %a %a)" pp_expr a pp_expr b
    | And l ->
        Format.fprintf out "(and %a)" (Util.pp_list ~sep:" " pp) l
    | Or l ->
        Format.fprintf out "(or %a)" (Util.pp_list ~sep:" " pp) l
    | Not t ->
        Format.fprintf out "(not %a)" pp t
    | True ->
  ...
  let rec simplify t =
    match t with
    | EQ (a, b) when ID.equal a b ->
        true_
    | LE (a, b) when ID.equal a b ->
        true_
    | LT (a, b) when ID.equal a b ->
        false_
    | Not (Not t) ->
        simplify t
    | Not True ->
    ...
    | Not False ->
        true_
    | Not (And l) ->
        simplify (or_ (List.map not_ l))
    | Not (Or l) ->
        simplify (and_ (List.map not_ l))
    | And [] ->
        true_
    | Or [] ->
        true_
...
module MakeSolver (X : sig end) = struct
  ...
  let rec encode_constr ~n c =
    ...
    | C.LT (a, b) ->
        encode_lt ~n a b
    | C.And l ->
        F.make_and (List.rev_map (encode_constr ~n) l)
    | C.Or l ->
        F.make_or (List.rev_map (encode_constr ~n) l)
    | C.Not c' ->
        F.make_not (encode_constr ~n c')
    | C.True ->
        F.f_true
    | C.False ->
        F.f_false
...
let any_bigger ~orient_lpo l b =
  ...
  | [x] ->
      orient_lpo x b
  | _ ->
      (* any element of [l] bigger than [r]? *)
...
module FO = struct
  ...
  let rec orient_lpo a b =
    ...
    | TC.AppBuiltin _, _ | _, TC.AppBuiltin _ | TC.NonFO, _ ->
        (* no clue... *)
        C.false_
