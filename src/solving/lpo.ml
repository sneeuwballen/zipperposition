...
module MakeSolver (X : sig end) = struct
  ...
  let solve_list l =
    ...
    let symbols =
      Iter.of_list l |> Iter.flat_map C.Seq.exprs |> ID.Set.of_seq
      |> ID.Set.elements
    in
...
and all_bigger ~orient_lpo a l =
  ...
  | [] -> C.true_
  | [x] -> orient_lpo a x
  | _ -> C.and_ (List.rev_map (fun y -> orient_lpo a y) l)
...
module FO = struct
  ...
  let rec orient_lpo a b =
    ...
    | (TC.Var _ | TC.DB _), _ -> C.false_ (* a variable cannot be > *)
...
module TypedSTerm = struct
  ...
  let rec orient_lpo a b =
    ...
    | T.Var _, _ -> C.false_ (* a variable cannot be > *)
