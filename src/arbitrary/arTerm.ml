...
module PT = struct
  ...
  and shrink_sub t =
    ...
    | PT.Bind (b, v, bod) -> shrink bod >|= PT.bind ~ty:(PT.ty_exn t) b v
    | _ -> empty

  let mk_ gen = QA.make ~print:TypedSTerm.to_string ~shrink gen
          ...
          if n <= 0 then
            ...
            let l =
              if ho then (
                (* fun term -> x *)
                let g_fun_t gen =
                  map2 lam (oneofl [var_x; var_y; var_z]) gen
                in
  ...
  let pred_g =
    ...
    let open QA.Gen in
    oneof [map2 p sub sub; map q sub; map r sub; return s]
  ...
  | T.App (f, l) ->
    append
      ...
      ( Iter.(0 -- (List.length l - 1)) >>= fun i ->
        ...
        let l' = CCList.set_at_idx i sub' l in
        T.app f l' )
  | T.Fun (ty_arg, bod) -> shrink bod >|= T.fun_ ty_arg
  | _ -> empty
...
let pred = QA.map (Term.Conv.of_simple_term_exn ctx) PT.pred
...
let pos t =
  ...
  let rec recurse t pb st =
    ...
    match T.view t with
    | T.App (_, []) | T.Const _ | T.Var _ | T.DB _ -> PB.to_pos pb
    | T.AppBuiltin (_, l) | T.App (_, l) ->
      let len = List.length l in
      oneof
        (stop :: List.mapi (fun i t' -> recurse t' (PB.arg (len - 1 - i) pb)) l)
        st
    | T.Fun (_, bod) -> oneof [stop; recurse bod (PB.body pb)] st
  in
  recurse t PB.empty
