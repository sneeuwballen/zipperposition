...
let shrink_lit (lit : lit) =
  let open Q.Iter in
  match lit with
  | Literal.Prop (t, sign) -> AT.shrink t >|= fun t -> Literal.mk_prop t sign
  | Literal.Equation (t, u, sign) ->
    AT.shrink t
    >|= (fun t -> Literal.mk_lit t u sign)
    <+> (AT.shrink u >|= fun u -> Literal.mk_lit t u sign)
  | _ -> empty

let mk_lit_ gen = Q.make ~print:Literal.to_string ~shrink:shrink_lit gen

let shrink_lits (lits : clause) = Q.Shrink.array ~shrink:shrink_lit lits

let mk_lits gen = Q.make ~print:Literals.to_string ~shrink:shrink_lits gen
