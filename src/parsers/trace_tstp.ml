...
let hash p =
  ...
  | Axiom _ | Theory _ ->
      Hashtbl.hash p
  | InferForm (_, (lazy s)) | InferClause (_, (lazy s)) ->
      Hashtbl.hash s.id
...
let is_proof_of_false = function
  ...
  | InferClause ([], _) ->
      true
  | InferClause (l, _) ->
      List.for_all SLiteral.is_false l
  | _ ->
      false
...
let get_id = function
  ...
  | Axiom (_, name) ->
      A.NameString name
  | Theory _ ->
      invalid_arg "Trace_tstp: Theory does not have an ID"

let force = function
  | InferForm (_, step) | InferClause (_, step) ->
      ignore (Lazy.force step)
  | Axiom _ | Theory _ ->
      ()
          ...
          lazy
            (Array.map
               (fun data ->
                 match data with
                 | A.GInt i ->
                     find_step (A.NameInt i)
                 | A.GString s ->
                     find_step (A.NameString s)
...
let _print_parent p =
  match p with
  | InferForm _ | InferClause _ ->
      CCFormat.to_string A.pp_name (get_id p)
  | Theory s ->
      CCFormat.sprintf "theory(%s)" s
  | Axiom (f, n) ->
      CCFormat.sprintf "file('%s', %s)" f n
...
let pp0 out proof =
  match proof with
  | Axiom (f, n) ->
      Format.fprintf out "axiom(%s, %s)" f n
  | Theory s ->
