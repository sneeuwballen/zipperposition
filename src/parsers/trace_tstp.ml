...
let hash p =
  match p with
  | Axiom _ | Theory _ -> Hashtbl.hash p
  | InferForm (_, (lazy s)) | InferClause (_, (lazy s)) -> Hashtbl.hash s.id

let compare p1 p2 = Stdlib.compare p1 p2 (* FIXME *)

(** {2 Constructors and utils} *)

let mk_f_axiom ~id f ~file ~name =
  let step =
    { id; rule = "axiom"; parents = [| Axiom (file, name) |]; esa = false }
  in
...
let is_step = function
  | InferClause _ | InferForm _ -> true
  | Axiom _ | Theory _ -> false
...
let get_id = function
  | InferClause (_, (lazy step)) | InferForm (_, (lazy step)) -> step.id
  | Axiom (_, name) -> A.NameString name
  | Theory _ -> invalid_arg "Trace_tstp: Theory does not have an ID"

let force = function
  | InferForm (_, step) | InferClause (_, step) -> ignore (Lazy.force step)
  | Axiom _ | Theory _ -> ()
...
let is_dag proof =
  ...
  let rec check_proof proof =
    if StepTbl.mem closed proof then
      ()
    (* ok *)
      ...
      (match proof with
      | InferClause (_, (lazy step)) | InferForm (_, (lazy step)) ->
...
let traverse ?(traversed = StepTbl.create 11) proof k =
  ...
  while not (Queue.is_empty queue) do
    ...
    if StepTbl.mem traversed proof then
      ...
      (match proof with
      | Axiom _ | Theory _ -> ()
      | InferForm (_, (lazy step)) | InferClause (_, (lazy step)) ->
...
let depth proof =
  ...
  while not (Queue.is_empty q) do
    ...
    if StepTbl.mem explored proof then
      ()
    else (
...
let of_decls decls =
  ...
  let read_info info =
        ...
        lazy
          (Array.map
             (fun data ->
               ...
               | _ ->
                 failwith
                   (CCFormat.sprintf "not a valid parent: %a" A.pp_general data))
    ...
    | A.GNode ("trivial", _) -> `Parents ("trivial", false, Lazy.from_val [||])
    | A.GInt i ->
  ...
  Iter.iter
    (fun decl ->
      ...
      | A.CNF (_name, _role, _c, info :: _) ->
        ...
        | `Proof | `NoIdea -> ()
        | `Parents (_rule, _esa, _parents) -> assert false)
        (* FIXME
      ...
      | A.FOF (name, _role, f, info :: _) | A.TFF (name, _role, f, info :: _) ->
        ...
        | `Parents (rule, esa, parents) ->
          ...
          add_step name p)
      | A.TypeDecl _ | A.FOF _ | A.CNF _ | A.TFF _ | A.THF _ | A.Include _
      | A.IncludeOnly _ | A.NewType _ ->
        ())
    decls;
  match !root with
...
let parse ?(recursive = true) filename =
  Err.(
    Util_tptp.parse_file ~recursive filename >>= fun decls ->
...
let _pp_clause out c =
  ...
  | [] -> T.pp out T.false_
  | _ ->
    Format.fprintf out "@[%a@]" (Util.pp_list ~sep:" | " (SLiteral.pp T.pp)) c

let _print_parent p =
  match p with
  | InferForm _ | InferClause _ -> CCFormat.to_string A.pp_name (get_id p)
  ...
  traverse proof (fun p ->
      ...
      | InferClause (c, (lazy step)) ->
        ...
        Format.fprintf out
          ...
          id _pp_clause c step.rule status
          (CCFormat.array CCFormat.string)
          ids)
