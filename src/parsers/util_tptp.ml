...
let find_file name dir =
  ...
  let rec search dir =
    ...
    if Sys.file_exists cur_name then
      Some cur_name
    else (
...
let parse_file ?cache ~recursive f =
  let parse_cache =
    lazy
      ( if recursive then
          CCOpt.get_lazy create_parse_cache cache
      else
        assert false )
  in
  ...
  let rec parse_this_file ?names filename =
    ...
    try
      ...
      List.iter
        (fun decl ->
          match decl, names with
          | ( ( A.CNF _ | A.FOF _ | A.TFF _ | A.THF _ | A.TypeDecl _
              | A.NewType _ ),
              None ) ->
...
let print_into ppt oc decls =
  fpf oc "@[<v>%a@]@?" (Util.pp_seq ~sep:"" (A.pp ppt)) decls
...
let to_ast st =
  let conv_form name role f =
    ...
    | A.R_definition when !enable_def_as_rewrite && looks_like_def f ->
      ...
      UA.rewrite ~attrs f
    | A.R_definition | A.R_axiom | A.R_hypothesis | A.R_assumption
    | A.R_theorem | A.R_plain | A.R_finite _ | A.R_type | A.R_unknown ->
      UA.assert_ ~attrs f
