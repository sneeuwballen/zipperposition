...
let rec conv_ty ty =
  ...
  | A.Ty_app ("Int", []) ->
      T.builtin Builtin.TyInt
  | A.Ty_app ("Rat", []) ->
      T.builtin Builtin.TyRat
  | A.Ty_app (s, []) ->
...
let rec app_reduce f zero l =
  ...
  | [x; y] ->
      T.app_builtin f [x; y]
  | x :: y :: tail ->
      app_reduce f zero (T.app_builtin f [x; y] :: tail)
...
let rec conv_term (t : A.term) : T.t =
  ...
  | A.App (f, l) -> (
      ...
      | "/", _ ->
          quotient_l l
      | ">=", [a; b] ->
          T.app_builtin BA.greatereq [a; b]
      | "<=", [a; b] ->
          T.app_builtin BA.lesseq [a; b]
      | ">", [a; b] ->
          T.app_builtin BA.greater [a; b]
      | "<", [a; b] ->
          T.app_builtin BA.less [a; b]
      | "mod", [a; b] ->
          T.app_builtin BA.remainder_e [a; b]
      | "div", [a; b] ->
          T.app_builtin BA.quotient_e [a; b]
      | _ ->
          T.app_const f l )
  | A.App_poly (f, tys, l) ->
  ...
  | A.Imply (a, b) ->
      T.imply (conv_term a) (conv_term b)
  | A.And l ->
      T.and_ (List.map conv_term l)
  | A.Or l ->
      T.or_ (List.map conv_term l)
  | A.Not a ->
      T.not_ (conv_term a)
  | A.Forall (vars, body) ->
