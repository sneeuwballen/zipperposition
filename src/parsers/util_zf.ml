    ...
    CCList.flat_map
      (fun st ->
        ...
        | A.Include s -> (
          ...
          | None ->
              Util.errorf ~where:"utils_zf" "could not find included file `%s`"
                s
...
let parse_lexbuf ?cache ?recursive file : parser_res =
  try
    parse_lexbuf_ ?cache ?recursive ~dir:"." file
...
let parse_file ?cache ?recursive file : parser_res =
  if file = "stdin" then parse_stdin ()
    ...
    try
      parse_file_ ?cache ?recursive file |> Iter.of_list |> CCResult.return
    with
    | Sys_error e ->
