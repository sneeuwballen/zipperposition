...
let find_file name ~dir : string option =
  ...
  if Sys.file_exists abs_path then Some abs_path else None

let rec parse_lexbuf_ ?cache ?(recursive = true) ~dir lex =
  let l = Parse_zf.parse_statement_list Lex_zf.token lex in
  if recursive then
    ...
    CCList.flat_map
      (fun st ->
        ...
        | A.Include s -> (
          match find_file s ~dir with
          | None ->
              Util.errorf ~where:"utils_zf" "could not find included file `%s`" s
          | Some s' when Hashtbl.mem cache s' ->
              [] (* already included *)
          | Some s' ->
              (* put in cache *)
              Hashtbl.add cache s' () ; parse_file_ ~cache ~recursive s' )
        | _ ->
            [st] )
      l
  else l
...
let parse_lexbuf ?cache ?recursive file : parser_res =
  try parse_lexbuf_ ?cache ?recursive ~dir:"." file |> Iter.of_list |> CCResult.return
  with e -> CCResult.of_exn e

let parse_stdin () : parser_res =
  let lexbuf = Lexing.from_channel stdin in
  ParseLocation.set_file lexbuf "stdin" ; parse_lexbuf ~recursive:false lexbuf

let parse_file ?cache ?recursive file : parser_res =
  if file = "stdin" then parse_stdin ()
  else
    try parse_file_ ?cache ?recursive file |> Iter.of_list |> CCResult.return with
    | Sys_error e ->
