...
module Prover = struct
  ...
  let register name prover =
    if Hashtbl.mem __table name then
      ...
      Hashtbl.add __table name prover

  let p_E =
    { name = "E";
      command =
        "eprover --cpu-limit=${timeout} --auto -l0 --tstp-in --tstp-out";
...
let call_with_out ?(timeout = 30) ?(args = []) ~prover decls =
  ...
  Err.(
    ...
    (* parse output *)
    let result =
      if _find_mem prover.Prover.unsat output then
        Unsat
      else if _find_mem prover.Prover.sat output then
        Sat
      else
        Unknown
    in
...
let call ?timeout ?args ~prover decls =
  Err.(
    call_with_out ?timeout ?args ~prover decls >>= fun (res, _) -> return res)
...
module Eprover = struct
  ...
  let parse_answer output =
    if CCString.mem ~sub:"SZS status Theorem" output then
      Theorem
  ...
  let discover ?(opts = []) ~steps decls =
    ...
    (* build stdin *)
    let input =
      CCFormat.sprintf "@[%a@]" (Util.pp_seq ~sep:"" (A.pp ST.pp)) decls
    in
    Err.(
      (* call E *)
      Util.popen ~cmd ~input >>= fun output ->
