...
let rec pp_term out (t : term) =
  ...
  | Match (lhs, cases) ->
    let pp_case out = function
      ...
      | Match_case (c, [], rhs) -> fpf out "(@[<2>%s@ %a@])" c pp_term rhs
...
let pp_par pp_x out (ty_vars, x) =
  match ty_vars with
  | [] -> pp_x out x
  | _ -> fpf out "(@[<2>par (@[%a@])@ (%a)@])" (pp_list pp_tyvar) ty_vars pp_x x
    ...
    let pp_cstor out c =
      if c.cstor_args = [] then
        ...
        fpf out "(@[<1>%s@ %a@])" c.cstor_name (pp_list pp_cstor_arg)
          c.cstor_args
    in
    fpf out "(@[<hv2>declare-datatype@ %s@ (par@ (@[%a@])@ (@[<v>%a@])@]))" name
      (pp_list pp_tyvar) tyvars (pp_list pp_cstor) cstors
  | Stmt_datas (tyvars, l) ->
    let pp_cstor_arg out (sel, ty) = fpf out "(@[%s %a@])" sel pp_ty ty in
    let pp_cstor out c =
      if c.cstor_args = [] then
