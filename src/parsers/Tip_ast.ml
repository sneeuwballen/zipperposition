...
(** Polymorphic types *)
type ty = Ty_bool | Ty_app of ty_var * ty list | Ty_arrow of ty list * ty
...
type funs_rec_def = {fsr_decls: typed_var fun_decl list; fsr_bodies: term list}

type statement = {stmt: stmt; loc: Loc.t option}
...
let rec pp_ty out (ty : ty) =
  ...
  | Ty_app (s, []) ->
      pp_str out s
  | Ty_app (s, l) ->
      Format.fprintf out "(@[<hv1>%s@ %a@])" s (pp_list pp_ty) l
  | Ty_arrow (args, ret) ->
...
let rec pp_term out (t : term) =
  ...
  | Const s ->
      pp_str out s
  | App (f, l) ->
      fpf out "(@[<1>%s@ %a@])" f (pp_list pp_term) l
  | App_poly (f, tys, l) ->
      fpf out "(@[<1>_@ %s@ %a@ %a@])" f (pp_list pp_ty) tys (pp_list pp_term) l
  | HO_app (a, b) ->
      fpf out "(@[<1>@@@ %a@ %a@])" pp_term a pp_term b
  | Match (lhs, cases) ->
      let pp_case out = function
        | Match_default rhs ->
            fpf out "(@[<2>_@ %a@])" pp_term rhs
        | Match_case (c, [], rhs) ->
  ...
  | Fun (v, body) ->
      fpf out "(@[<1>lambda @ (%a)@ %a@])" pp_typed_var v pp_term body
  | Let (l, t) ->
  ...
  | Imply (a, b) ->
      fpf out "(@[=>@ %a@ %a@])" pp_term a pp_term b
  | And l ->
      fpf out "(@[<hv>and@ %a@])" (pp_list pp_term) l
  | Or l ->
      fpf out "(@[<hv>or@ %a@])" (pp_list pp_term) l
  | Not t ->
      fpf out "(not %a)" pp_term t
  | Distinct l ->
      fpf out "(@[distinct@ %a@])" (pp_list pp_term) l
  | Cast (t, ty) ->
...
let pp_stmt out (st : statement) =
  match view st with
  | Stmt_decl_sort (s, n) ->
      fpf out "(@[declare-sort@ %s %d@])" s n
  | Stmt_assert t ->
      fpf out "(@[assert@ %a@])" pp_term t
  | Stmt_lemma t ->
      fpf out "(@[lemma@ %a@])" pp_term t
