...
let role_of_string = function
  ...
  | "assumption" ->
      R_assumption
  | "lemma" ->
      R_axiom (* R_lemma makes problems harder *)
  | "theorem" ->
      R_theorem
  | "conjecture" ->
      R_conjecture
  ...
  | "unknown" ->
      R_unknown
  | s ->
      failwith ("not a proper TPTP role: " ^ s)
...
let rec pp_general out d =
  ...
  | GVar s ->
      CCFormat.string out s
  | GColumn (a, b) ->
      Format.fprintf out "%a: %a" pp_general a pp_general b
  | GNode (f, l) ->
...
let rec pp_general_debugf out d =
  match d with
  | GString s ->
      Format.fprintf out "GSstr %s" s
  | GInt i ->
      Format.fprintf out "GInt %d" i
  | GVar s ->
      Format.fprintf out "GVar %s" s
  | GColumn (a, b) ->
