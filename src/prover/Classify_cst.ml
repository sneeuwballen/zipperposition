  ...
  aux
    [ (Ind_ty.as_constructor |>> fun (c, t) -> Cstor (c, t));
      ...
      ( Rewrite.as_defined_cst |>> fun cst ->
        DefinedCst
          (Rewrite.Defined_cst.level cst, Rewrite.Defined_cst.rules cst) ) ]
...
let prec_constr_ a b =
  let to_int_ = function
    ...
    | Parameter _ -> 3
    | Inductive_cst _ -> 4
    | Skolem | Other -> 10 (* skolem and other constants, at the same level *)
    | DefinedCst _ -> 20
    (* defined: biggest *)
  in
  ...
  | Inductive_cst c1, Inductive_cst c2 ->
    ...
    if dominates_ c1 c2 then
      1
    else if dominates_ c2 c1 then
      -1
    else
      0
...
let weight_fun (id : ID.t) : Precedence.Weight.t =
  ...
  | Skolem | Other -> W.omega_plus 4
  | DefinedCst _ -> W.omega_plus 5
