...
let pp_res out = function
  | Ty _ ->
      Format.fprintf out "ind_ty"
  | Cstor (_, ity) ->
      Format.fprintf out "cstor of %a" Ind_ty.pp ity
  | Inductive_cst _ ->
      Format.fprintf out "ind_cst"
  | Projector id ->
      Format.fprintf out "projector_%a" ID.pp id
  | DefinedCst (lev, _) ->
      Format.fprintf out "defined (level %d)" lev
  | Parameter i ->
      Format.fprintf out "parameter %d" i
  | Skolem ->
      CCFormat.string out "skolem"
  | Other ->
      CCFormat.string out "other"
...
let prec_constr_ a b =
  let to_int_ = function
    ...
    | Inductive_cst _ ->
        4
    | Skolem | Other ->
        10 (* skolem and other constants, at the same level *)
    | DefinedCst _ ->
        20 (* defined: biggest *)
  in
  ...
  | Inductive_cst c1, Inductive_cst c2 ->
      ...
      if dominates_ c1 c2 then 1 else if dominates_ c2 c1 then -1 else 0
...
let weight_fun (id : ID.t) : Precedence.Weight.t =
  ...
  | Skolem | Other ->
      W.omega_plus 4
  | DefinedCst _ ->
      W.omega_plus 5 (* defined: biggest *)
