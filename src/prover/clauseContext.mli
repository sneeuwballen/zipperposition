...
(** [extract lits t] returns [None] if [t] doesn't occur in [lits]. Otherwise,
    it creates a fresh var [x], replaces [t] with [x] within [lits], and returns
    the corresponding context. Basically, if [extract lits t = Some c] then
    [apply c t = lits] *)
...
(** [apply c t] fills the hole of [c] with the given term [t]. [t] and [c] share
    no free variables. *)

val apply_same_scope : t -> term -> Literals.t
(** Same as {!apply}, but now variables from the context and variables from the
    term live in the same scope *)

val raw_lits : t -> Literals.t
(** give access to the underlying literals. Careful not to depend on the
