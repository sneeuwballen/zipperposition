...
type result = Sat_solver_intf.result = Sat | Unsat of proof
...
module Make () = (*   : Sat_solver_intf.S *) struct
  ...
  *)
  ...
  let pp_form fmt f : unit = Format.fprintf fmt "[@[<hv>%a@]]" pp_form_simpl f
  ...
  let get_proof () = match !proof_ with None -> assert false | Some p -> p
  ...
  let conv_proof_atomic_ p : proof =
    let rec aux p =
      ...
      | {P.conclusion= c; step= P.Hyper_res {P.hr_init; hr_steps}} -> (
          ...
          match ResTbl.get tbl_res (c, q1 :: q2) with
          | Some s ->
              s
          | None ->
              let parents =
  ...
  let get_proof_of_lit lit =
    ...
    match P.prove_atom a with Some p -> conv_proof_ p | None -> assert false
