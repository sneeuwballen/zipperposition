  ...
  if Lit.is_neg lits.(i) then (
    ...
    | `NoUnappliedVarOccurringApplied ->
      ...
      |> Iter.exists (fun (t, _) ->
             vars_args
             |> CCList.exists (fun (head, args) ->
                    head = t && not (CCList.is_empty args) ) )
...
let validate_fun_ ~ord lits bv =
  if BV.is_empty bv then
    ...
    |> Iter.exists (fun (i, _) -> can_select_lit ~ord lits i && BV.get bv i)
...
let l =
  let basics = [("NoSelection", fun ~ord:_ -> no_select); "default", default]
  and by_ord =
    CCList.flat_map
      (fun (name, p) ->
        [ (name, fun ~ord -> p ~strict:true ~ord);
          (name ^ "NS", fun ~ord -> p ~strict:false ~ord) ] )
      ["MaxGoal", max_goal; "MaxGoalExceptRRHorn", except_RR_horn max_goal]
  in
  basics @ by_ord
