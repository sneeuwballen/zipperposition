...
let ho_sel_driver lits f =
  ...
  in
  if CCArray.length neg_max = 0 then
    BV.empty ()
  else (
...
let e_sel ~blocker ~ord lits =
  let chooser ~freq_tbl (i, l) =
    ( (if Lit.is_positivoid l then
         1
       else
         0),
      ((if Lits.is_max ~ord lits i then
          0
        else
          100)
...
let e_sel2 ~blocker ~ord lits =
  ...
  let chooser (idx, l) =
      ...
      if T.is_var (T.head_term lhs) then
        ...
        let prec_weight =
          if not hd_is_cst then
            ...
            Precedence.sel_prec_weight prec (T.head_exn lhs)
        in
        let alpha_rank =
          if not hd_is_cst then
            max_int
    ...
    | _ -> sign_val, max_int, max_int, diff_val
    (* won't be chosen *)
  in
...
let e_sel8 ~blocker ~ord lits =
  ...
  let is_propositional = function
    | Lit.Equation (lhs, _, _) as l -> Lit.is_predicate_lit l && T.is_const lhs
    | _ -> false
  in
  ...
  let lhs_head_arity lit =
    ...
    | Lit.Equation (lhs, _, _) ->
      ...
      if T.is_const hd then
        -CCList.length (Type.expected_args (Term.ty hd))
      else
        max_int
    | _ -> max_int
  in
...
let e_sel11 ~blocker ~ord lits =
  ...
  let chooser (i, l) =
    if Lit.is_positivoid l then
      ...
      0, lhs_weight l, get_pred_freq ~freq_tbl l
    else if not (Lit.is_typex_pred l) then
...
let e_sel12 ~blocker ~ord lits =
  if Lits.is_unique_max_horn_clause ~ord lits then
    BV.empty ()
  else
    e_sel3 ~blocker ~ord lits
...
let e_sel14 ~blocker ~ord lits =
  ...
  let chooser (i, l) =
    ...
    let block b i =
      ...
      max_val
    in
    if Lit.is_positivoid l || hd_is_fresh_pred l then
      block blocked i
    else if Lit.is_ground l then
      0, Lit.weight l, hd_freq
    else if not (Lit.is_typex_pred l) then
      10, Lit.max_term_positions ~ord l, hd_freq
    else if not (Lit.is_type_pred l) then
      20, -Lit.ho_weight l, hd_freq
    else
      block blocked i
  in
  ...
  let app_var_pen l =
    ...
    | Lit.Equation (lhs, rhs, _) ->
      let num_app_var_sides =
        (if T.is_app_var lhs then
           1
         else
           0)
        +
        if T.is_app_var rhs then
          1
        else
          0
      in
      if num_app_var_sides = 1 then
        0
      else if num_app_var_sides = 2 then
        1
      else
        2
    | _ -> max_int
  ...
  let lit_penalty = function
    | Lit.Equation (lhs, rhs, _) ->
      ...
      if var_headed lhs then
        if Term.is_var lhs && Term.is_var rhs then
          0
        else if Term.is_fun rhs || Term.is_formula rhs then
          1
          +
          if Term.is_app_var lhs then
            3
          else
            0
          ...
          if Term.is_var lhs then
            ...
            10
        else if Term.is_app_var lhs && Term.is_app_var rhs then
          20
        else if Term.has_ho_subterm rhs then
          5
        else
          10
      else
...
let from_string ~ord s =
  ...
  | Some name ->
    ...
    let bool_blocker i l =
      Lit.Seq.terms l |> Iter.flat_map selectable_sub |> fun i ->
      not @@ Iter.is_empty i
    in
