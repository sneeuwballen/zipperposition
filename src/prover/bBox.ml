...
let compare_payload l1 l2 =
  ...
  | Fresh, Fresh ->
      0
  | Clause_component l1, Clause_component l2 ->
      Lits.compare l1 l2
  | Lemma l1, Lemma l2 ->
      Cut_form.compare l1 l2
  | Case p1, Case p2 ->
      CCList.compare Cover_set.Case.compare p1 p2
...
let inject_lit lit =
  ...
  try
    ...
    | Some t ->
        Lit.apply_sign (Literal.is_positivoid lit) t
    | None ->
...
let pp_zf out i =
  let pp_payload out = function
    | Fresh ->
        CCFormat.string out "'dummy_sym'"
    | Clause_component lits ->
        Format.fprintf out "(@[<hv>%a@])" Lits.pp_zf lits
    | Lemma f ->
        Cut_form.pp_zf out f
    | Case c ->
