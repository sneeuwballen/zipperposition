...
let inject_lits_ lits =
  ...
  let lits, sign =
    match lits with
    | [|lit0|]
      when Literal.is_ground lit0 && Literal.is_neg lit0
...
let inject_lits lits = Util.with_prof prof_inject_lits inject_lits_ lits
...
let inject_lemma f = Util.with_prof prof_inject_lemma inject_lemma_ f
