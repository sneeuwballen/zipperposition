...
module Make (E : Env.S) = struct
  ...
  let given_clause_step ?(generating = true) num =
    ...
    | None ->
        ...
        let clauses = Env.do_generate ~full:true () in
        if Iter.is_empty clauses then (
          Util.debugf ~section 3 "saturated set: @[%a@]@." (fun k ->
              k (Iter.pp_seq Env.C.pp_tstp_full) (Env.get_active ()) ) ;
          Sat )
        else
          let clauses =
            clauses
    ...
    | Some c -> (
        ...
        | c :: l', state -> (
            ...
            if Env.C.is_redundant c then (ZProf.exit_prof _span ; Unknown)
            else
              ...
              | None ->
                  ZProf.exit_prof _span ; Unknown
              | Some c ->
                  let pr = Env.C.proof c in
                  ZProf.exit_prof _span ; Unsat pr ) )
