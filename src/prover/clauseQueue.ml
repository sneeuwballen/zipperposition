...
module Make (C : Clause_intf.S) = struct
  ...
  let take_first = function
    | FIFO q -> if Queue.is_empty q then raise Not_found else Queue.pop q
  ...
  let explore () : t =
    ...
    let weight =
      combine
        [default, 4; favor_small_num_vars, 1; favor_all_neg, 1; penalty, 1]
    in
