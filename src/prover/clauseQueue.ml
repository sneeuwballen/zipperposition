...
module WeightFun = struct
      ...
      (fun acc i l ->
        let max_terms =
          if CCBV.get max_lits i then
            Literal.Comp.max_terms ~ord:(Ctx.ord (Clause.ctx_of c)) l
          else
            []
        in
        let lit_weight =
  ...
  let pn_refined_weight ~pv_w ~pf_w ~nv_w ~nf_w ~max_t_m ~max_l_m ~pos_m c =
    ...
    let res =
      CCArray.foldi
        (fun sum i lit ->
          let pterm_w =
           fun t -> float_of_int (Term.weight ~var:pv_w ~sym:(fun _ -> pf_w) t)
          in
          let nterm_w =
           fun t -> float_of_int (Term.weight ~var:nv_w ~sym:(fun _ -> nf_w) t)
          in
          let w =
            match lit with
            | Lit.Equation (l, r, _) ->
    ...
    let lit_weight is_max lit =
      ...
      let base_weight =
        ...
        | Literal.Equation (lhs, rhs, _) ->
          (match Ordering.compare (Ctx.ord (Clause.ctx_of c)) lhs rhs with
          | Comparison.Gt | Geq ->
  ...
  let explore_fun =
    penalize (combine [ default, 4; favor_small_num_vars, 1; favor_all_neg, 1 ])
  ...
  let calc_poly base x ~c ~lin ~sq =
    base
    * int_of_float (c +. (lin *. float_of_int x) +. (sq *. float_of_int (x * x)))
    ...
    try
      ...
      Signal.once (CCOpt.get_exn !_current_queue).on_init (fun cls ->
          ...
          Name.Tbl.filter_map_inplace
            (fun id lvl ->
              let ty = Signature.find_exn (Ctx.signature ctx) id in
              let base =
                if Type.returns_prop ty then
                  pw
                else if Type.is_fun ty then
              ...
              in
              let v =
                calc_poly ~c:l_poly_const ~lin:l_poly_lin ~sq:l_poly_sq base lvl
              in
              Some v)
            _f_weights);
  ...
  let parse_clauseweight s =
    let or_lmax_regex =
      Str.regexp "clauseweight(\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+[.]?[0-9]*\\)"
    in
    try
      ignore (Str.search_forward or_lmax_regex s 0);
...
let goal_oriented () : t =
  ...
  let weight =
    penalize
      (combine
         [
           default, 4; favor_small_num_vars, 2; favor_goal, 1; favor_all_neg, 1;
         ])
  in
...
let ground () : t =
  ...
  let weight =
    penalize
      (combine [ favor_pos_unit, 1; favor_ground, 2; favor_small_num_vars, 10 ])
  in
