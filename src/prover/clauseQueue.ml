  ...
  try
    ...
    | Some suffix ->
      ...
      if List.length args != 4 then
        ...
        let var_mul =
          try float_of_string (List.nth args 1) with _ -> invalid_arg err_msg
        in
        ...
        if CCOpt.is_none ratio then
          ...
          parameters_magnitude :=
            if CCString.equal par_mag "l" then
              `Large
            else if CCString.equal par_mag "s" then
              `Small
            else
              invalid_arg err_msg;
          goal_penalty :=
            if CCString.prefix ~pre:"t" goal_pen then
              true
            else if CCString.prefix ~pre:"f" goal_pen then
              false
            else
              invalid_arg err_msg;
...
module Make (C : Clause_intf.S) = struct
  ...
  module WeightFun = struct
      ...
      CCArray.foldi
        (fun acc i l ->
          ...
          let lit_weight =
            ...
            |> Iter.map (fun t ->
                   let var, f_nc, f_c =
                     if List.mem t max_terms then
                       2, 6, 3
                     else
                       1, 2, 1
                   in
                   let sym id_ =
                     if Signature.sym_in_conj id_ signature then
                       f_c
                     else
                       f_nc
                   in
                   Term.weight ~var ~sym t
                   *
                   if Term.is_app_var t then
                     2
                   else
          ...
          in
          let multiplier =
            (if Literal.is_typex_pred l then
               1.2
             else
               1.0)
            *. (if Literal.is_type_pred l then
                  1.8
                else
                  1.0)
            *.
            if Literal.is_ground l then
              0.5
            else
              1.0
          in
    ...
    let orient_lmax_weight ~v_w ~f_w ~pos_m ~unord_m ~max_l_mul c =
      ...
      let res =
        CCArray.foldi
          (fun sum i lit ->
            let term_w t =
              if Term.is_true_or_false t then
                0.0
              else
                float_of_int (Term.weight ~var:v_w ~sym:(fun _ -> f_w) t)
            in
            let w =
              match lit with
              | Lit.Equation (l, r, _) ->
    ...
    let rec calc_tweight t sg v w c_mul =
      ...
      | Term.Const id ->
        int_of_float
          ((if Signature.sym_in_conj id sg then
              c_mul
            else
              1.0)
          *. float_of_int w)
    ...
    let conjecture_relative_e ~conj_mul ~fresh_mul ~f ~cst ~p ~v ~max_term_mul
      ...
      let f_weight sym ty =
        let multipliers =
          (if Signature.sym_in_conj sym sgn then
             conj_mul
           else
             1.0)
          *.
      ...
      let t_weight ~mul t =
        let rec aux t =
          if Type.is_tType (Term.ty t) then
            0
          else (
            match Term.view t with
      ...
      let lit_weight is_max lit =
        ...
        let base_weight =
          ...
          | Literal.Equation (lhs, rhs, _) ->
            ...
            | Lt | Leq -> t_weight ~mul:max_term_mul rhs + t_weight ~mul:1.0 lhs
            | _ -> t_weight ~mul:1.0 lhs + t_weight ~mul:1.0 rhs)
          | _ -> 1
        in
    ...
    let diversity_weight ~var_w ~sym_w ~pos_mul ~max_t_mul ~max_l_mul ~fdiff_a
      ...
      let mul_max_t (l, w_l) (r, w_r) =
        ...
        | Gt | Geq -> (max_t_mul *. float_of_int w_l) +. float_of_int w_r
        | Lt | Leq -> (max_t_mul *. float_of_int w_r) +. float_of_int w_l
        | _ -> float_of_int (w_l + w_r)
      in
      let max_lits =
        if C.has_selected_lits c then
          C.selected_lits_bv c
    ...
    let favor_all_neg c =
      if all_ground_neg c then
        0
      else
        penalty_coeff_

    let favor_ground c =
      if C.is_ground c then
        0
      else
        penalty_coeff_

    let favor_non_all_neg c =
      if not (all_ground_neg c) then
        0
      else
        penalty_coeff_
    ...
    let parse_crv s =
      ...
      try
        ...
        let parameters_magnitude =
          ...
          |> fun s ->
          if CCString.prefix ~pre:"l" s then
            `Large
          else
            `Small
        in
  ...
  module PriorityFun = struct
    ...
    let prefer_sos c =
      if C.proof_depth c = 0 || CCOpt.is_some (C.distance_to_goal c) then
        0
      else
        1

    let prefer_non_goals c =
      if Iter.exists Literal.is_positivoid (C.Seq.lits c) then
        0
      else
        1
    ...
    let prefer_processed c =
      if C.is_backward_simplified c then
        0
      else
        1
    ...
    let prefer_easy_ho c =
        ...
        let rec aux t =
          ...
          | App (hd, args) ->
            if Term.is_const hd then
              ...
              false, List.for_all Term.is_bvar args
      ...
      in
      if is_arg_cong_child c then
        0
      else if has_lam_eq c || in_pattern_fragment c then
        1
      else if prefer_formulas c == 1 then
        2
      else
        3
    ...
    let prefer_short_trail c =
      if Trail.is_empty (C.trail c) then
        max_int
      else
    ...
    let prefer_fo c =
      let rec almost_fo t =
        ...
        match Term.view t with
        | App (hd, args) -> Term.is_const hd && List.for_all almost_fo args
    ...
    let by_neg_lit c =
      ...
      @@ Array.fold_left
           (fun acc lit ->
             if Lit.is_positivoid lit then
               acc + 400
             else if Lit.is_ground lit then
               acc - 3
             else
               acc - 1)
           0 (C.lits c)
  ...
  let add_to_mixed_eval ~ratio ~weight_fun mixed_eval : unit =
    ...
    mixed_eval.weight_funs <-
      Array.append mixed_eval.weight_funs [| weight_fun |];
