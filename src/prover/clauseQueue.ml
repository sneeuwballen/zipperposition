...
module Make (C : Clause_intf.S) = struct
  ...
  let norm_app hd arg =
    ...
    let normalize =
      if Term.is_fun hd then Lambda.whnf else if Term.is_comb hd then Combinators_base.narrow else CCFun.id
    in
    normalize body
  ...
  let unroll_logical_symbols t =
    let rec aux t =
      ...
      | AppBuiltin (b, l) when Builtin.is_logical_binop b ->
          List.fold_left (fun acc t -> Term.Set.union acc (aux t)) Term.Set.empty l
      | _ ->
          Term.Set.singleton t
    in
    aux t |> Term.Set.filter (fun t -> (not (Term.is_true_or_false t)) && not (Term.is_const t))

  let add_related_term_ t =
    if Term.Set.cardinal !_related_terms < max_related_ then (
      let new_terms = unroll_logical_symbols t in
      Util.debugf ~section 20 "addding related terms:@.@[%a@]@." (fun k -> k (Term.Set.pp Term.pp) new_terms) ;
      _related_terms := Term.Set.union !_related_terms new_terms )

  let register_conjecture_clause cl =
    match C.distance_to_goal cl with
    | Some 0 when !_rel_terms_enabled && Term.Set.cardinal !_related_terms < max_related_ ->
        C.Seq.terms cl |> Iter.filter (fun t -> not (Term.is_true_or_false t)) |> Iter.iter add_related_term_
    | _ ->
        ()
  ...
  module WeightFun = struct
    ...
    let weight_lits_ l = Array.fold_left (fun acc lit -> acc + Lit.heuristic_weight term_weight lit) 0 l

    let default c =
      (* maximum depth of types. Avoids reasoning on list (list (list .... (list int))) *)
      let _depth_ty =
        Lits.Seq.terms (C.lits c)
        |> Iter.map Term.ty |> Iter.map Type.depth |> Iter.max ?lt:None |> CCOpt.map_or CCFun.id ~default:0
      in
      let w_lits = weight_lits_ (C.lits c) in
      (w_lits * Array.length (C.lits c)) + _depth_ty

    let ho_weight_calc c =
      let all_terms c = C.Seq.terms c |> Iter.flat_map (Term.Seq.subterms ~include_builtin:true) in
      let app_var_num c =
        float_of_int @@ Iter.fold (fun acc t -> acc + if Term.is_app_var t then 1 else 0) 0 (all_terms c)
      in
      let formulas_num c =
        float_of_int @@ Iter.fold (fun acc t -> acc + if Term.is_formula t then 1 else 0) 0 (C.Seq.terms c)
      in
      let p_depth c = float_of_int (C.proof_depth c) in
      let t_depth c =
        C.Seq.terms c |> Iter.map Term.depth |> Iter.max |> CCOpt.get_or ~default:0 |> float_of_int
      in
      let weight c = float_of_int (weight_lits_ (C.lits c)) in
      let res =
        int_of_float
          ( weight c
          *. (1.25 ** (app_var_num c *. (1.35 ** p_depth c)))
          *. (0.85 ** formulas_num c)
          *. (1.05 ** t_depth c) )
      in
      Util.debugf ~section 5 "[C_W:]@ @[%a@]@ :@ %d(%g, %g, %g).\n" (fun k ->
          k C.pp c res (app_var_num c) (formulas_num c) (p_depth c) ) ;
      res
    ...
    let avoid_expensive c =
      ...
      CCArray.foldi
        (fun acc i l ->
          let max_terms = if CCBV.get max_lits i then Literal.Comp.max_terms ~ord:(C.Ctx.ord ()) l else [] in
          let lit_weight =
            Literal.Seq.terms l
            |> Iter.map (fun t ->
          ...
          in
          let multiplier =
            (if Literal.is_typex_pred l then 1.2 else 1.0)
            *. (if Literal.is_type_pred l then 1.8 else 1.0)
            *. if Literal.is_ground l then 0.5 else 1.0
          in
          int_of_float (multiplier *. float_of_int lit_weight) + acc )
        0 (C.lits c)

    let orient_lmax_weight ~v_w ~f_w ~pos_m ~unord_m ~max_l_mul c =
      let max_lits = if C.has_selected_lits c then C.selected_lits_bv c else C.maxlits (c, 0) Subst.empty in
      let ord = C.Ctx.ord () in
      let res =
        CCArray.foldi
          (fun sum i lit ->
            let term_w t =
              if Term.is_true_or_false t then 0.0 else float_of_int (Term.weight ~var:v_w ~sym:(fun _ -> f_w) t)
            in
            let w =
              match lit with
              | Lit.Equation (l, r, _) ->
    ...
    let pn_refined_weight ~pv_w ~pf_w ~nv_w ~nf_w ~max_t_m ~max_l_m ~pos_m c =
      ...
      let res =
        CCArray.foldi
          (fun sum i lit ->
            let pterm_w t = float_of_int (Term.weight ~var:pv_w ~sym:(fun _ -> pf_w) t) in
            let nterm_w t = float_of_int (Term.weight ~var:nv_w ~sym:(fun _ -> nf_w) t) in
            let w =
              match lit with
              | Lit.Equation (l, r, _) ->
                  let term_w = if Lit.is_positivoid lit then pterm_w else nterm_w in
                  let ord_side = Ordering.compare ord l r in
                  let l_mul = match ord_side with Comparison.Gt | Geq | Incomparable -> max_t_m | _ -> 1.0 in
                  let r_mul = match ord_side with Comparison.Lt | Leq | Incomparable -> max_t_m | _ -> 1.0 in
                  let eq_inc =
                    if not (Lit.is_predicate_lit lit) then
                      float_of_int (if Lit.is_positivoid lit then pf_w else nf_w)
                    else 0.0
                  in
    ...
    let ho_weight_initial c = if C.proof_depth c = 0 then 1 else ho_weight_calc c

    let rec calc_tweight t sg v w c_mul =
      match Term.view t with
      | Term.AppBuiltin (_, l) ->
          w + List.fold_left (fun acc t -> acc + calc_tweight t sg v w c_mul) 0 l
      | Term.Var _ ->
          v
      | Term.DB _ ->
          w
      | Term.App (f, l) ->
          calc_tweight f sg v w c_mul + List.fold_left (fun acc t -> acc + calc_tweight t sg v w c_mul) 0 l
      | Term.Const id ->
          int_of_float ((if Signature.sym_in_conj id sg then c_mul else 1.0) *. float_of_int w)
      | Term.Fun (_, t) ->
          calc_tweight t sg v w c_mul
    ...
    let calc_lweight l sg v w c_mul =
      ...
      | Lit.Equation (lhs, rhs, sign) ->
          (calc_tweight lhs sg v w c_mul + calc_tweight rhs sg v w c_mul, sign)
      | _ ->
          (0, false)
    ...
    let conj_relative ?(distinct_vars_mul = -1.0) ?(parameters_magnitude = `Large) ?(goal_penalty = false) c =
      ...
      let pos_mul, max_mul, v, f =
        match parameters_magnitude with `Large -> (1.5, 1.5, 100, 100) | `Small -> (2.0, 1.5, 2, 3)
      in
      let conj_mul = 0.5 in
      Array.mapi (fun i xx -> (i, xx)) (C.lits c)
      |> Array.fold_left
           (fun acc (i, l) ->
             ...
             +.
             let l_w, l_s = calc_lweight l sgn v f conj_mul in
             (if l_s then pos_mul else 1.0) *. (if CCBV.get max_lits i then max_mul else 1.0) *. float_of_int l_w
             )
           0.0
      |> fun res ->
      if distinct_vars_mul < 0.0 then int_of_float res
      else
        let dist_vars = Literals.vars (C.lits c) |> List.filter (fun v -> not (Type.is_tType (HVar.ty v))) in
    ...
    let conj_relative_struct ~inst_penalty ~gen_penalty ~var_w ~sym_w c =
      ...
      let struct_diff_weight t =
        ...
        let w t = float_of_int @@ Term.weight ~var:var_w ~sym:(fun _ -> sym_w) t in
        let rec w_diff ~given_term ~conj_term =
          match (T.view given_term, T.view conj_term) with
          | T.Var _, T.Var _ ->
              1
          | T.Const x, T.Const y when ID.equal x y ->
              1
          | T.DB i, T.DB j when i = j ->
              1
          | T.Var _, _ ->
              int_of_float (inst_penalty *. w conj_term)
          | T.App (hd, _), _ when T.is_var hd ->
              int_of_float (w given_term +. (inst_penalty *. w conj_term))
          | _, T.Var _ ->
              int_of_float (gen_penalty *. w given_term)
          | _, T.App (hd, _) when T.is_var hd ->
              int_of_float (w conj_term +. (inst_penalty *. w given_term))
          | T.App (hd1, args1), T.App (hd2, args2) when T.equal hd1 hd2 && List.length args1 = List.length args2
            ->
        ...
        and w_diff_l xs ys =
          CCList.fold_left2 (fun acc x y -> acc + w_diff ~given_term:x ~conj_term:y) 0 xs ys
        in
      ...
      |> Iter.foldi
           (fun acc idx lit ->
             ...
             |> Iter.fold
                  ...
                  acc
             |> fun res ->
             let mul = (if is_pos then pos_mul else 1.0) *. if is_max then max_mul else 1.0 in
    ...
    let conjecture_relative_e ~conj_mul ~fresh_mul ~f ~cst ~p ~v ~max_term_mul ~max_lit_mul ~pos_mul c =
      ...
      let f_weight sym ty =
        let multipliers =
          ...
          *. if ID.is_postcnf_skolem sym then fresh_mul else 1.0
        in
        int_of_float
          (float_of_int (if Type.returns_prop ty then p else if Type.is_fun ty then f else cst) *. multipliers)
      in
      let t_weight ~mul t =
        let rec aux t =
          if Type.is_tType (Term.ty t) then 0
          else
            match Term.view t with
            | App (hd, args) ->
                aux_l (hd :: args)
            ...
            | DB _ | Var _ ->
                v
            | Const sym ->
                f_weight sym (Term.ty t)
        and aux_l xs = List.fold_left (fun acc arg -> acc + aux arg) 0 xs in
        int_of_float (mul *. float_of_int (aux t))
      in
      let lit_weight is_max lit =
        let is_pos = Lit.is_positivoid lit in
        let multipliers = (if is_max then max_lit_mul else 1.0) *. if is_pos then pos_mul else 1.0 in
        let base_weight =
          match lit with
          | Literal.Equation (lhs, rhs, _) -> (
            match Ordering.compare (C.Ctx.ord ()) lhs rhs with
            | Comparison.Gt | Geq ->
          ...
          | _ ->
              1
        in
        int_of_float (multipliers *. float_of_int base_weight)
      in
      let max_lits = if C.has_selected_lits c then C.selected_lits_bv c else C.maxlits (c, 0) Subst.empty in
      C.Seq.lits c
      |> Iter.foldi
    ...
    let dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag ~pos_t_reset ~pos_eqn_reset
        ~neg_use_dag ~neg_t_reset ~neg_eqn_reset ~pos_neg_reset c =
      ...
      let _w = ref 0 in
      CCArray.iter
        (fun lit -> if Lit.is_positivoid lit then _w := !_w + calc_w pos_use_dag pos_eqn_reset pos_t_reset lit)
        (C.lits c) ;
      if pos_neg_reset then Term.Tbl.clear _tbl ;
      CCArray.iter
        (fun lit -> if Lit.is_negativoid lit then _w := !_w + calc_w neg_use_dag neg_eqn_reset neg_t_reset lit)
        (C.lits c) ;
      !_w

    let diversity_weight ~var_w ~sym_w ~pos_mul ~max_t_mul ~max_l_mul ~fdiff_a ~fdiff_b ~vdiff_a ~vdiff_b c =
      let ord = C.Ctx.ord () in
      ...
      let res =
        ...
        int_of_float (w +. f_factor +. v_factor)
      in
    ...
    let favor_pos_unit c =
      let is_unit_pos c = match C.lits c with [|lit|] when Lit.is_positivoid lit -> true | _ -> false in
    ...
    let favor_small_num_vars c =
      ...
      let n_vars =
        Literals.vars (C.lits c) |> List.filter (fun v -> not (Type.is_tType (HVar.ty v))) |> List.length
      in
      let n = if n_vars < 4 then 0 else if n_vars < 6 then 1 else if n_vars < 8 then 3 else n_vars in
      n * penalty_coeff_

    let favor_horn c = if Lits.is_horn (C.lits c) then 0 else penalty_coeff_
    ...
    let favor_goal c =
      if C.is_empty c then 0
        ...
        let d = match C.distance_to_goal c with Some d -> min d goal_threshold_ | None -> goal_threshold_ in
        1 + d
    ...
    let favor_all_neg c = if all_ground_neg c then 0 else penalty_coeff_

    let favor_ground c = if C.is_ground c then 0 else penalty_coeff_

    let favor_non_all_neg c = if not (all_ground_neg c) then 0 else penalty_coeff_

    let combine ws =
      assert (ws <> []) ;
      assert (List.for_all (fun (_, c) -> c > 0) ws) ;
      fun c -> List.fold_left (fun sum (w, coeff) -> sum + (coeff * w c)) 0 ws

    let explore_fun = penalize (combine [(default, 4); (favor_small_num_vars, 1); (favor_all_neg, 1)])

    let default_fun =
      penalize
        (combine
           [(default, 3); (favor_all_neg, 1); (favor_small_num_vars, 2); (favor_goal, 1); (favor_pos_unit, 1)] )

    (* imitation of the E function of the same name *)
    let parse_diversity_weight s =
      let crv_regex =
        Str.regexp
          ( "diversity-weight(" ^ "\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+[.]?[0-9]*\\),"
          ^ "\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),"
          ^ "\\([+-]?[0-9]+[.]?[0-9]*\\),\\([+-]?[0-9]+[.]?[0-9]*\\),"
          ^ "\\([+-]?[0-9]+[.]?[0-9]*\\),\\([+-]?[0-9]+[.]?[0-9]*\\))" )
      in
      try
        ...
        diversity_weight ~sym_w ~var_w ~max_t_mul ~max_l_mul ~pos_mul ~fdiff_a ~fdiff_b ~vdiff_a ~vdiff_b
      with Not_found | Invalid_argument _ ->
        invalid_arg
          ( "expected diversity-weight(" ^ "f_w:int,v_w:int,max_t_mul:float,max_l_mul:float,pos_mul:float,"
    ...
    let parse_crv s =
      ...
      try
        ...
        let parameters_magnitude =
          ...
          |> fun s -> if CCString.prefix ~pre:"l" s then `Large else `Small
        in
        let goal_penalty =
          Str.matched_group 3 s |> CCString.trim |> String.lowercase_ascii |> CCString.prefix ~pre:"t"
        in
    ...
    let parse_dag_weight s =
      let crv_regex =
        Str.regexp
          ( "dagweight(\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+[.]?[0-9]*\\),\\([0-9]+\\),"
          ^ "\\([tfTF]\\),\\([tfTF]\\),\\([tfTF]\\),\\([tfTF]\\)," ^ "\\([tfTF]\\),\\([tfTF]\\),\\([tfTF]\\))" )
      in
      try
        ignore (Str.search_forward crv_regex s 0) ;
        ...
        dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag ~pos_t_reset ~pos_eqn_reset
          ~neg_use_dag ~neg_t_reset ~neg_eqn_reset ~pos_neg_reset
      with Not_found | Invalid_argument _ ->
        invalid_arg
          ( "expected dagweight(fweight,vweight,pos_mul,dup_weight,"
          ^ "pos_use_dag,pos_t_reset,pos_eqn_reset,neg_use_dag," ^ "neg_t_reset,neg_eqn_reset,pos_neg_reset)" )
    ...
    let parse_cr_struct s =
      ...
      let crs_regex =
        Str.regexp
          "conjecture-relative-struct(\\([0-9][.]?[0-9]*+\\),\\([0-9][.]?[0-9]*+\\),\\([0-9]+\\),\\([0-9]+\\))"
      in
      try
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg
          "expected \
           conjecture-relative-struct(inst_penalty:float,gen_penalty:float,var_weight:int,sym_weight:int)"

    let parse_orient_lmax s =
      let or_lmax_regex =
        Str.regexp
          ( "orient-lmax(\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\),"
          ^ "\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\))" )
      in
      try
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected orient-lmax(var_weight:int,fun_weight:int"
        ^ "pos_lit_mult:float, unorderable_lit_mul:float, max_lit_mul:float)"

    let parse_pnrefine s =
      let or_lmax_regex =
        Str.regexp
          ( "pnrefined(\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\),"
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected pnrefined(+fun_weight:int,+var_weight:int" ^ "-fun_weight:int,-var_weight:int"
    ...
    let calc_poly base x ~c ~lin ~sq =
      base * int_of_float (c +. (lin *. float_of_int x) +. (sq *. float_of_int (x * x)))
          ...
          (fun sum i lit ->
            ...
            let w =
              ...
              | Lit.Equation (l, r, _) ->
                  let term_w () t =
                    float_of_int
                    @@ Term.weight ~var:vw ~sym:(fun id -> ID.Tbl.get_or _f_weights ~default:!_default id) t
                  in
                  let ord_side = Ordering.compare ord l r in
                  let l_mul = match ord_side with Comparison.Gt | Geq | Incomparable -> max_t_mul | _ -> 1.0 in
                  let r_mul = match ord_side with Comparison.Lt | Leq | Incomparable -> max_t_mul | _ -> 1.0 in
    ...
    let parse_rel_lvl_weight s =
      let r_str =
        "rel_lvl_weight(\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),"
        ^ "\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+\\),"
        ^ "\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\))"
      in
      ...
      try
        ...
        Signal.once on_proof_state_init (fun cls ->
            let goals, axs =
              Iter.fold
                (fun (goals, axs) cl ->
                  match C.distance_to_goal cl with Some _ -> (cl :: goals, axs) | None -> (goals, cl :: axs) )
                ([], []) cls
            in
              ...
              (fun cl ->
                ...
                |> ID.Set.iter (fun k ->
                       ID.Tbl.update cl_map
                         ~f:(fun _ -> function Some old -> Some (cl :: old) | None -> Some [cl])
                         ~k ) )
              axs ;
              ...
              let new_syms =
                ID.Set.fold
                  (fun id new_syms ->
                    ...
                    ID.Set.fold
                      (fun id acc -> if not (ID.Tbl.mem _f_weights id) then ID.Set.add id acc else acc)
                      (C.symbols (Iter.of_list cls))
                      new_syms )
                  syms_at_level ID.Set.empty
              in
              if not (ID.Set.is_empty new_syms) then fill_levels new_syms (level + 1) else level
            in
            let max_lvl = fill_levels goal_syms 1 in
            _default :=
              calc_poly ~c:l_poly_const ~lin:l_poly_lin ~sq:l_poly_sq (max cw (max pw fw)) (def_l + max_lvl) ;
            ID.Tbl.filter_map_inplace
              (fun id lvl ->
                let ty = Signature.find_exn (C.Ctx.signature ()) id in
                let base = if Type.returns_prop ty then pw else if Type.is_fun ty then fw else cw in
                let v = calc_poly ~c:l_poly_const ~lin:l_poly_lin ~sq:l_poly_sq base lvl in
                Some v )
              _f_weights ) ;
        rel_weight ~vw ~max_t_mul ~max_lit_mul ~pos_lit_mul
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected rel_lev_weight(poly_const:float,,poly_lin:float,poly_sq:float,"
    ...
    let parse_conj_pref_weight s =
      let r_str =
        "conj_pref_weight(\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),"
        ^ "\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\),\\([0-9]+[.]?[0-9]*\\))"
      in
      let rel_w_regex = Str.regexp r_str in
      try
                ...
                | Some 0 ->
                    ...
                    |> Iter.iter (function
                         | Literal.Equation (lhs, rhs, _) as l ->
                             ( if Lit.is_predicate_lit l then Term.Seq.subterms ~include_builtin:true lhs
                               else
                                 Iter.append
                                   (Term.Seq.subterms ~include_builtin:true lhs)
                                   (Term.Seq.subterms ~include_builtin:true rhs) )
                             |> Iter.iter (fun t -> if not (Term.is_type t) then PW.insert_term t)
                         | _ ->
                             () )
                | _ ->
                    () )
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected conj_pref_weight(match_weight:float,miss_weight:float,"
        ^ "max_t_mul:float,max_lit_mul:float,pos_lit_mul:float)"

    let parse_clauseweight s =
      let or_lmax_regex = Str.regexp "clauseweight(\\([0-9]+\\),\\([0-9]+\\),\\([0-9]+[.]?[0-9]*\\)" in
      try
        ignore (Str.search_forward or_lmax_regex s 0) ;
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected clauseweight(+fun_weight:int,+var_weight:int," ^ "pos_lit_mult:float)"

    let parse_staggered s =
      let or_lmax_regex = Str.regexp "staggered(\\([0-9]+[.]?[0-9]*\\))" in
      try
        ignore (Str.search_forward or_lmax_regex s 0) ;
        let stagger_factor = CCFloat.of_string_exn (Str.matched_group 1 s) in
        staggered ~stagger_factor
      with Not_found | Invalid_argument _ -> invalid_arg @@ "expected staggered(+stagger_factor:int)"

    let parse_conj_relative_cheap s =
      let or_lmax_regex =
        Str.regexp
          ( "conjecture-relative-cheap(\\([0-9]+\\)," ^ "\\([0-9]+\\)," ^ "\\([0-9]+[.]?[0-9]*\\),"
          ^ "\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\))" )
      in
      try
        ignore (Str.search_forward or_lmax_regex s 0) ;
    ...
    let parsers =
      ...
      ; ( "conjecture-relative"
        , fun _ -> conj_relative ~distinct_vars_mul:1.0 ~parameters_magnitude:`Large ~goal_penalty:false )
    ...
    let of_string s =
      try
        ...
        fun c ->
          ...
          res
      with Not_found | Failure _ -> invalid_arg (CCFormat.sprintf "unknown weight function %s" s)
  end
  ...
  module PriorityFun = struct
    ...
    let prefer_sos c = if C.proof_depth c = 0 || CCOpt.is_some (C.distance_to_goal c) then 0 else 1

    let prefer_non_goals c = if Iter.exists Literal.is_positivoid (C.Seq.lits c) then 0 else 1

    let prefer_unit_ground_non_goals c =
      if Iter.exists Literal.is_positivoid (C.Seq.lits c) && C.is_unit_clause c && C.is_ground c then 0 else 1

    let prefer_unit_ground_goals c =
      if (not (Iter.exists Literal.is_positivoid (C.Seq.lits c))) && C.is_unit_clause c && C.is_ground c then 0
      else 1

    let prefer_goals c = -prefer_non_goals c

    let prefer_processed c = if C.is_backward_simplified c then 0 else 1

    let prefer_lambdas c =
      if
        C.Seq.terms c
        |> Iter.exists (fun t -> Iter.exists (fun t -> Term.is_fun t || Term.is_comb t) (Term.Seq.subterms t))
      then 0
      else 1

    let defer_lambdas c = -prefer_lambdas c

    let prefer_formulas c =
      if C.Seq.terms c |> Iter.exists (fun t -> Iter.exists Term.is_formula (Term.Seq.subterms t)) then 0 else 1
    ...
    let prefer_easy_ho c =
      let is_arg_cong_child c =
        let rec aux proof =
          ...
          if Proof.Step.is_simpl step then
            ...
            | Some rule ->
                String.equal (Proof.Rule.name rule) "ho_complete_eq"
            | None ->
                false
          else false
        in
      ...
      let has_lam_eq c =
        C.Seq.lits c
        |> Iter.exists (fun l ->
               match l with Literal.Equation (lhs, _, _) -> Type.is_fun (Term.ty lhs) | _ -> false )
      in
      let in_pattern_fragment c =
        ...
        let rec aux t =
          match Term.view t with
          | App (hd, args) ->
              if Term.is_const hd then aux_l args else (false, List.for_all Term.is_bvar args)
        ...
        and aux_l args =
          CCList.fold_while
            (fun (has_lambda, is_pattern) arg ->
              assert is_pattern ;
              let h_l', i_p' = aux arg in
              if i_p' then ((h_l' || has_lambda, true), `Continue) else ((false, false), `Stop) )
            (false, true) args
        in
        C.Seq.terms c |> Iter.map aux
        (* at least one subterm is functional and all subterms are patterns *)
        |> fun seq -> Iter.exists fst seq && Iter.for_all snd seq
      in
    ...
    let defer_formulas c = -prefer_formulas c

    let prefer_short_trail c = if Trail.is_empty (C.trail c) then max_int else Trail.length (C.trail c)

    let prefer_empty_trail c = if Trail.is_empty (C.trail c) then 0 else 1

    let prefer_long_trail c = -Trail.length (C.trail c)
    ...
    let defer_fo c = -prefer_fo c

    let prefer_ground c = if C.is_ground c then 0 else 1

    let defer_ground c = if C.is_ground c then 1 else 0

    let defer_sos c = if C.proof_depth c = 0 || CCOpt.is_some (C.distance_to_goal c) then 1 else 0
    ...
    let prefer_top_level_app_var c =
      ...
      if Array.length lits > 1 then max_int
        ...
        let no_app_vars = Lit.Seq.terms lits.(0) |> Iter.filter Term.is_app_var |> Iter.length in
        if no_app_vars = 0 then max_int else no_app_vars

    let defer_top_level_app c = -prefer_top_level_app_var c

    let prefer_shallow_app_var c =
      let app_var_depth t =
        let rec aux depth t =
          match Term.view t with
          | Term.DB _ | Term.Const _ | Term.Var _ ->
              0
          | Term.Fun (_, body) ->
              aux (depth + 1) body
          ...
          | Term.AppBuiltin (_, args) ->
              aux_l (depth + 1) args
        and aux_l depth = function [] -> min_int | t :: ts -> max (aux depth t) (aux_l depth ts) in
        aux 0 t
      in
      C.Seq.terms c |> Iter.map app_var_depth |> Iter.max |> CCOpt.get_or ~default:min_int

    let by_app_var_num c =
      C.Seq.terms c
      |> Iter.flat_map (Term.Seq.subterms ~include_builtin:true)
      |> Iter.filter Term.is_app_var |> Iter.length

    let prefer_deep_app_var c = -prefer_shallow_app_var c

    let prefer_neg_unit c = match C.lits c with [|l|] when Lit.is_negativoid l -> 0 | _ -> 1

    let defer_neg_unit c = -prefer_neg_unit c

    let by_neg_lit c =
      abs
      @@ Array.fold_left
           (fun acc lit ->
             if Lit.is_positivoid lit then acc + 400 else if Lit.is_ground lit then acc - 3 else acc - 1 )
           0 (C.lits c)

    (* defer if there are no ho subterms *)
    let by_ho_depth ?(modifier = fun x -> x) ~depth_fun c =
      let max_opt a b =
        match (a, b) with Some x, Some y -> if x > y then a else b | Some x, None -> a | _ -> b
      in
      C.Seq.terms c
      |> Iter.fold (fun acc t -> max_opt acc (depth_fun t)) None
      |> CCOpt.map modifier |> CCOpt.get_or ~default:max_int

    let prefer_shallow_lambdas c = by_ho_depth ~depth_fun:Term.lambda_depth c

    let prefer_deep_lambdas c = by_ho_depth ~depth_fun:Term.lambda_depth ~modifier:(fun x -> -x) c

    let prefer_shallow_combs c = by_ho_depth ~depth_fun:Term.comb_depth c

    let prefer_deep_combs c = by_ho_depth ~depth_fun:Term.comb_depth ~modifier:(fun x -> -x) c

    let prefer_shallow c =
      Literals.Seq.terms (C.lits c) |> Iter.map Term.depth |> Iter.max |> CCOpt.get_or ~default:0

    let defer_shallow c = -prefer_shallow c
    ...
    *)
      ...
      let norm_fact c =
        let aux proof =
          ...
          if not (List.mem Proof.Tag.T_ho_norm (Proof.Step.tags step)) then default
            ...
            let calc_w pr =
              TypedSTerm.Seq.subterms (Proof.Result.to_form @@ Proof.S.result pr) |> Iter.length |> float_of_int
            in
    ...
    let of_string s =
      ...
      with Not_found ->
        let err_msg =
          CCFormat.sprintf "unknown priortity: %s.\noptions:@ {@[%a@]}" s (CCList.pp CCString.pp)
            (List.map fst parsers)
        in
        invalid_arg err_msg
  end
  ...
  module H = CCHeap.Make (struct
    ...
    let leq (i10, i11, c1) (i20, i21, c2) =
      ...
      else if i10 = i20 then if i11 < i21 then true else if i11 = i21 then C.compare c1 c2 < 0 else false
      else false
  end)

  (** A priority queue of clauses + FIFO queue *)
  type t = FIFO of C.t Queue.t | Mixed of mixed
  ...
  let is_empty (q : t) = match q with FIFO q -> Queue.is_empty q | Mixed q -> is_empty_mixed q

  let length q = match q with FIFO q -> Queue.length q | Mixed q -> C.Tbl.length q.tbl
  ...
  let rec take_first_mixed q =
    ...
    let move_queue q =
      ...
      if q.current_step >= q.ratios_limit then
        if (* we have to choose the next heap *)
           q.current_heap_idx + 1 = Array.length q.heaps then (
    ...
    let is_orphaned c = (!_ignoring_orphans && C.is_orphaned c) || C.is_redundant c in
  ...
  let add_to_mixed_eval ~ratio ~weight_fun mixed_eval : unit =
    ...
    if was_empty then mixed_eval.ratios_limit <- ratio ;
    ()
  ...
  let name q = match q with FIFO _ -> "bfs" | Mixed q -> "mixed"

  (** {5 Combination of queues} *)

  let const_prioritize_fun wf c = (wf c, 1)
  ...
  let goal_oriented () : t =
    ...
    let weight =
      penalize (combine [(default, 4); (favor_small_num_vars, 2); (favor_goal, 1); (favor_all_neg, 1)])
    in
  ...
  let ground () : t =
    let open WeightFun in
    let weight = penalize (combine [(favor_pos_unit, 1); (favor_ground, 2); (favor_small_num_vars, 10)]) in
  ...
  let conj_var_relative_mk () : t =
    ...
    let weight_fun =
      const_prioritize_fun
        (WeightFun.conj_relative ~distinct_vars_mul:!cr_var_mul ~parameters_magnitude:!parameters_magnitude
  ...
  let all_clauses q =
    match q with FIFO q -> CCSeq.to_iter (Queue.to_seq q) | Mixed q -> Iter.map fst (C.Tbl.to_iter q.tbl)
  ...
  let remove q cl =
    match q with
    | FIFO q ->
        invalid_arg "legacy queue, removal unsupported"
    | Mixed q ->
        if C.Tbl.mem q.tbl cl then (C.Tbl.remove q.tbl cl ; true) else false
...
let parse_wf_with_priority s =
  ...
  with Not_found | Invalid_argument _ ->
    invalid_arg "weight function is of the form \"ratio:int|priority:name|weight:name(options..)\""

let () =
  let o = Arg.Symbol ("<custom>" :: List.map fst profiles_, parse_profile) in
  let add_queue = Arg.String parse_wf_with_priority in
  Params.add_opts
    [ ("--clause-queue", o, " choose which set of clause queues to use (for selecting next active clause)")
    ; ("-cq", o, " alias for --clause-queue")
    ; ( "--add-queue"
      , add_queue
      , " create a new clause evaluation queue. Its description is of the form"
        ^ " RATIO|PRIORITY_FUN|WEIGHT_FUN" )
    ; ("-q", add_queue, "alias to --add-queue")
    ; ( "--ignore-orphans"
      , Arg.Bool (( := ) _ignoring_orphans)
