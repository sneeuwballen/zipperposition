...
let profile_of_string s =
  ...
  try
    ...
    | Some suffix ->
        let err_msg =
          "conjecture-relative-var(ratio:int,var_mul:float,par:[L,S],goal_penalty:[true,false])"
        in
        let args =
          List.map
            (fun s ->
...
module Make (C : Clause_intf.S) = struct
  ...
  let unroll_logical_symbols t =
    let rec aux t =
      ...
      | AppBuiltin (b, l) when Builtin.is_logical_binop b ->
          List.fold_left
            (fun acc t -> Term.Set.union acc (aux t))
            Term.Set.empty l
      | _ ->
  ...
  module WeightFun = struct
    ...
    let default c =
      ...
      let _depth_ty =
        ...
        |> Iter.map Term.ty |> Iter.map Type.depth |> Iter.max ?lt:None
    ...
    let ho_weight_calc c =
      let all_terms c =
        C.Seq.terms c |> Iter.flat_map (Term.Seq.subterms ~include_builtin:true)
      in
      let app_var_num c =
        float_of_int
        @@ Iter.fold
             (fun acc t -> acc + if Term.is_app_var t then 1 else 0)
             0 (all_terms c)
      in
      let formulas_num c =
        float_of_int
        @@ Iter.fold
             (fun acc t -> acc + if Term.is_formula t then 1 else 0)
             0 (C.Seq.terms c)
      in
      let p_depth c = float_of_int (C.proof_depth c) in
      let t_depth c =
        C.Seq.terms c |> Iter.map Term.depth |> Iter.max
        |> CCOpt.get_or ~default:0 |> float_of_int
      in
    ...
    let avoid_expensive c =
      ...
      CCArray.foldi
        (fun acc i l ->
          let max_terms =
            if CCBV.get max_lits i then
              Literal.Comp.max_terms ~ord:(C.Ctx.ord ()) l
            else []
          in
          let lit_weight =
            Literal.Seq.terms l
    ...
    let orient_lmax_weight ~v_w ~f_w ~pos_m ~unord_m ~max_l_mul c =
      let max_lits =
        ...
        else C.maxlits (c, 0) Subst.empty
      in
        ...
        CCArray.foldi
          (fun sum i lit ->
            let term_w t =
              ...
              else float_of_int (Term.weight ~var:v_w ~sym:(fun _ -> f_w) t)
            in
            let w =
              match lit with
              | Lit.Equation (l, r, _) ->
                  ...
                  let t_w =
                    if Lit.is_positivoid lit then pos_m *. t_w else t_w
                  in
                  let t_w =
                    if CCBV.get max_lits i then max_l_mul *. t_w else t_w
                  in
                  let ordered =
    ...
    let pn_refined_weight ~pv_w ~pf_w ~nv_w ~nf_w ~max_t_m ~max_l_m ~pos_m c =
      ...
      let res =
        CCArray.foldi
          (fun sum i lit ->
            let pterm_w t =
              float_of_int (Term.weight ~var:pv_w ~sym:(fun _ -> pf_w) t)
            in
            let nterm_w t =
              float_of_int (Term.weight ~var:nv_w ~sym:(fun _ -> nf_w) t)
            in
            let w =
              match lit with
              | Lit.Equation (l, r, _) ->
                  let term_w =
                    if Lit.is_positivoid lit then pterm_w else nterm_w
                  in
                  let ord_side = Ordering.compare ord l r in
                  let l_mul =
                    match ord_side with
                    | Comparison.Gt | Geq | Incomparable ->
                        max_t_m
                    | _ ->
                        1.0
                  in
                  let r_mul =
                    match ord_side with
                    | Comparison.Lt | Leq | Incomparable ->
                        max_t_m
                    | _ ->
                        1.0
                  in
                  ...
                  in
                  let t_w =
                    (l_mul *. term_w l) +. (r_mul *. term_w r) +. eq_inc
                  in
                  let t_w =
                    if Lit.is_positivoid lit then pos_m *. t_w else t_w
                  in
                  let t_w =
                    if CCBV.get max_lits i then max_l_m *. t_w else t_w
                  in
                  t_w
              | _ ->
                  1.0
      ...
      Util.debugf ~section 5 "pnrefined(@[%a@],@[%g@])=%d@." (fun k ->
          k C.pp c pos_m w ) ;
      w

    let ho_weight_initial c =
      if C.proof_depth c = 0 then 1 else ho_weight_calc c
    ...
    let rec calc_tweight t sg v w c_mul =
      ...
      | Term.Const id ->
          int_of_float
            ( (if Signature.sym_in_conj id sg then c_mul else 1.0)
            *. float_of_int w )
      | Term.Fun (_, t) ->
    ...
    let conj_relative ?(distinct_vars_mul = -1.0)
        ?(parameters_magnitude = `Large) ?(goal_penalty = false) c =
      let sgn = C.Ctx.signature () in
      let max_lits =
        if C.has_selected_lits c then C.selected_lits_bv c
        else C.maxlits (c, 0) Subst.empty
    ...
    let conj_relative_struct ~inst_penalty ~gen_penalty ~var_w ~sym_w c =
      ...
      let struct_diff_weight t =
        ...
        let w t =
          float_of_int @@ Term.weight ~var:var_w ~sym:(fun _ -> sym_w) t
        in
        let rec w_diff ~given_term ~conj_term =
          ...
          | _, _ ->
              int_of_float
                ((inst_penalty *. w conj_term) +. (gen_penalty *. w given_term))
        and w_diff_l xs ys =
          CCList.fold_left2
            (fun acc x y -> acc + w_diff ~given_term:x ~conj_term:y)
            0 xs ys
        in
           ...
           (fun acc idx lit ->
             ...
             |> Iter.fold
                  (fun acc t ->
                    ...
                    Util.debugf ~section 5 "struct(@[%a@])=%d" (fun k ->
                        k Term.pp t w ) ;
                    acc + w )
                  acc
             |> fun res ->
             let mul =
               ...
               *. if is_max then max_mul else 1.0
             in
             int_of_float (mul *. float_of_int res) )
           0

    let conjecture_relative_e ~conj_mul ~fresh_mul ~f ~cst ~p ~v ~max_term_mul
        ~max_lit_mul ~pos_mul c =
      let sgn = C.Ctx.signature () in
      let f_weight sym ty =
        let multipliers =
    ...
    let dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag
        ~pos_t_reset ~pos_eqn_reset ~neg_use_dag ~neg_t_reset ~neg_eqn_reset
        ~pos_neg_reset c =
      ...
      let calc_w use_dag reset_lit reset_term lit =
        ...
        | Lit.Equation (lhs, rhs, _) ->
            int_of_float
              (p_m *. float_of_int (t_w false lhs + t_w reset_term rhs))
        | _ ->
            0
      in
      let _w = ref 0 in
    ...
    let diversity_weight ~var_w ~sym_w ~pos_mul ~max_t_mul ~max_l_mul ~fdiff_a
        ~fdiff_b ~vdiff_a ~vdiff_b c =
      let ord = C.Ctx.ord () in
      ...
      in
      Util.debugf ~section 5 "diversity_weight(@[%a@])=%d@." (fun k ->
          k C.pp c res ) ;
      res
    ...
    let favor_pos_unit c =
      let is_unit_pos c =
        match C.lits c with
        | [|lit|] when Lit.is_positivoid lit ->
            true
        | _ ->
            false
      in
    ...
    let explore_fun =
      penalize
        (combine [(default, 4); (favor_small_num_vars, 1); (favor_all_neg, 1)])
    ...
    let parse_diversity_weight s =
      ...
      try
        ...
        diversity_weight ~sym_w ~var_w ~max_t_mul ~max_l_mul ~pos_mul ~fdiff_a
          ~fdiff_b ~vdiff_a ~vdiff_b
    ...
    let parse_crv s =
      let crv_regex =
        Str.regexp
          "conjecture-relative-var(\\([0-9]+[.]?[0-9]*\\),\\([lsLS]\\),\\([tfTF]\\))"
      in
      try
        ignore (Str.search_forward crv_regex s 0) ;
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg
          "expected \
           conjecture-relative-var(dist_var_mul:float,parameters_magnitude:l/s,goal_penalty:t/f)"
    ...
    let parse_dag_weight s =
      ...
      try
        ignore (Str.search_forward crv_regex s 0) ;
        let parse_bool s =
          CCString.prefix ~pre:"t" (String.lowercase_ascii s)
        in
        let fweight =
          CCOpt.get_exn @@ CCInt.of_string (Str.matched_group 1 s)
        in
        let vweight =
          CCOpt.get_exn @@ CCInt.of_string (Str.matched_group 2 s)
        in
        let pos_multiplier = CCFloat.of_string_exn (Str.matched_group 3 s) in
        let dup_weight =
          CCOpt.get_exn @@ CCInt.of_string (Str.matched_group 4 s)
        in
    ...
    let parse_cr_e s =
      ...
      try
        ...
        conjecture_relative_e ~conj_mul ~fresh_mul ~f ~cst ~p ~v ~max_term_mul
          ~max_lit_mul ~pos_mul
      with Not_found | Invalid_argument _ ->
        invalid_arg
          ( "expected conjecture-relative-e(conj_mul:float, fresh_mul:float, \
             f:int, const:int,"
          ^ "p:int, v:int, max_term_mul:float, max_lit_mul:float, \
             pos_mul:float)" )
    ...
    let parse_pnrefine s =
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected pnrefined(+fun_weight:int,+var_weight:int"
    ...
    let calc_poly base x ~c ~lin ~sq =
      base
      * int_of_float
          (c +. (lin *. float_of_int x) +. (sq *. float_of_int (x * x)))
          ...
          (fun sum i lit ->
            ...
            let w =
              ...
              | Lit.Equation (l, r, _) ->
                  ...
                  let t_w =
                    if Lit.is_positivoid lit then pos_lit_mul *. t_w else t_w
                  in
                  let t_w =
                    if CCBV.get max_lits i then max_lit_mul *. t_w else t_w
                  in
                  t_w
              | _ ->
                  1.0
    ...
    let conj_pref_weight (module PW : PrefWeight.S) ~max_t_mul ~max_lit_mul
        ~pos_lit_mul c =
      let max_lits =
        if C.has_selected_lits c then C.selected_lits_bv c
        else C.maxlits (c, 0) Subst.empty
    ...
    let parse_rel_lvl_weight s =
      ...
      try
        ...
        Signal.once on_proof_state_init (fun cls ->
            ...
            _default :=
              calc_poly ~c:l_poly_const ~lin:l_poly_lin ~sq:l_poly_sq
                (max cw (max pw fw))
                (def_l + max_lvl) ;
            ID.Tbl.filter_map_inplace
              (fun id lvl ->
                ...
                let base =
                  if Type.returns_prop ty then pw
                  else if Type.is_fun ty then fw
                  else cw
                in
                let v =
                  calc_poly ~c:l_poly_const ~lin:l_poly_lin ~sq:l_poly_sq base
                    lvl
                in
                Some v )
              _f_weights ) ;
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg
        @@ "expected \
            rel_lev_weight(poly_const:float,,poly_lin:float,poly_sq:float,"
      ...
      try
        ...
        Signal.once on_proof_state_init (fun cls ->
            Iter.iter
              ...
              cls ) ;
        conj_pref_weight
          (module PW : PrefWeight.S)
          ~max_t_mul ~max_lit_mul ~pos_lit_mul
      with Not_found | Invalid_argument _ ->
        invalid_arg
    ...
    let parse_clauseweight s =
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected clauseweight(+fun_weight:int,+var_weight:int,"
        ^ "pos_lit_mult:float)"
    ...
    let parse_staggered s =
      ...
      with Not_found | Invalid_argument _ ->
        invalid_arg @@ "expected staggered(+stagger_factor:int)"

    let parse_conj_relative_cheap s =
      let or_lmax_regex =
        Str.regexp
          ( "conjecture-relative-cheap(\\([0-9]+\\)," ^ "\\([0-9]+\\),"
          ^ "\\([0-9]+[.]?[0-9]*\\)," ^ "\\([0-9]+[.]?[0-9]*\\),"
          ^ "\\([0-9]+[.]?[0-9]*\\))" )
      in
      try
    ...
    let of_string s =
      try
        ...
        fun c ->
          ...
          Util.debugf ~section 2 "@[%s(%a)@]=@[%d@]" (fun k ->
              k name C.pp c res ) ;
          res
  ...
  module PriorityFun = struct
    ...
    let prefer_ho_steps c =
      if Proof.Step.has_ho_step (C.proof_step c) then 0 else 1

    let prefer_sos c =
      if C.proof_depth c = 0 || CCOpt.is_some (C.distance_to_goal c) then 0
      else 1
    ...
    let prefer_unit_ground_non_goals c =
      if
        Iter.exists Literal.is_positivoid (C.Seq.lits c)
        && C.is_unit_clause c && C.is_ground c
      then 0
      else 1

    let prefer_unit_ground_goals c =
      if
        (not (Iter.exists Literal.is_positivoid (C.Seq.lits c)))
        && C.is_unit_clause c && C.is_ground c
      then 0
      else 1
    ...
    let prefer_short_trail c =
      if Trail.is_empty (C.trail c) then max_int else Trail.length (C.trail c)
    ...
    let prefer_fo c =
      ...
      in
      let all_terms =
        Iter.filter (fun t -> not (Term.is_true_or_false t)) (C.Seq.terms c)
      in
    ...
    let defer_sos c =
      if C.proof_depth c = 0 || CCOpt.is_some (C.distance_to_goal c) then 1
      else 0
    ...
    let prefer_top_level_app_var c =
      ...
      if Array.length lits > 1 then max_int
        ...
        let no_app_vars =
          Lit.Seq.terms lits.(0) |> Iter.filter Term.is_app_var |> Iter.length
        in
      ...
      let app_var_depth t =
        ...
        and aux_l depth = function
          | [] ->
              min_int
          | t :: ts ->
              max (aux depth t) (aux_l depth ts)
        in
        aux 0 t
      in
      C.Seq.terms c |> Iter.map app_var_depth |> Iter.max
    ...
    let prefer_neg_unit c =
      match C.lits c with [|l|] when Lit.is_negativoid l -> 0 | _ -> 1
    ...
    let prefer_shallow c =
      ...
      |> Iter.map Term.depth |> Iter.max |> CCOpt.get_or ~default:0
    ...
    let by_normalization_factor c =
      ...
      let norm_fact c =
        let aux proof =
          let p_res, step = (Proof.S.result proof, Proof.S.step proof) in
          (* clause is not obtained by normalization *)
          if not (List.mem Proof.Tag.T_ho_norm (Proof.Step.tags step)) then
            default
          else (
      ...
      with Not_found ->
        let err_msg =
          CCFormat.sprintf "unknown priortity: %s.\noptions:@ {@[%a@]}" s
            (CCList.pp CCString.pp) (List.map fst parsers)
        in
        invalid_arg err_msg
  end
  ...
  module H = CCHeap.Make (struct
    ...
    let leq (i10, i11, c1) (i20, i21, c2) =
      if i10 < i20 then true
        ...
        else if i11 = i21 then C.compare c1 c2 < 0
        else false
      else false
  end)
  ...
  let is_empty (q : t) =
    match q with FIFO q -> Queue.is_empty q | Mixed q -> is_empty_mixed q

  let length q =
    match q with FIFO q -> Queue.length q | Mixed q -> C.Tbl.length q.tbl
  ...
  let rec take_first_mixed q =
    ...
    let move_queue q =
      ...
      if q.current_step >= q.ratios_limit then
        if
          ...
          q.current_heap_idx + 1 = Array.length q.heaps
        then (
          ...
          q.ratios_limit <-
            q.ratios_limit + Array.get q.ratios q.current_heap_idx )
    in
    ...
    let is_orphaned c =
      (!_ignoring_orphans && C.is_orphaned c) || C.is_redundant c
    in
    (* if clause was picked by another queue
       or it should be ignored, repeat clause choice. *)
    if not (C.Tbl.mem q.tbl c) then (
      Util.debugf ~section 2 "clause is not in the table:@ @[%a@]@." (fun k ->
          k C.pp c ) ;
      take_first_mixed q )
    else if is_orphaned c then (
      C.Tbl.remove q.tbl c ;
      Util.debugf ~section 2 "ignoring orphaned clause:@ @[%a@]@." (fun k ->
          k C.pp c ) ;
      Util.debugf ~section 5 "proof:@ @[%a@]@." (fun k ->
          k Proof.S.pp_tstp (C.proof c) ) ;
      take_first_mixed q )
    else (
  ...
  let ground () : t =
    ...
    let weight =
      penalize
        (combine
           [(favor_pos_unit, 1); (favor_ground, 2); (favor_small_num_vars, 10)] )
    in
    ...
    let weight_fun =
      const_prioritize_fun
        (WeightFun.conj_relative ~distinct_vars_mul:!cr_var_mul
           ~parameters_magnitude:!parameters_magnitude
  ...
  let all_clauses q =
    match q with
    | FIFO q ->
        CCSeq.to_iter (Queue.to_seq q)
    | Mixed q ->
        Iter.map fst (C.Tbl.to_iter q.tbl)
...
let parse_wf_with_priority s =
  ...
  with Not_found | Invalid_argument _ ->
    invalid_arg
      "weight function is of the form \
       \"ratio:int|priority:name|weight:name(options..)\""
...
let () =
  ...
  Params.add_opts
    [ ( "--clause-queue"
      , o
      , " choose which set of clause queues to use (for selecting next active \
         clause)" )
  ...
  Params.add_to_mode "best" (fun () ->
      ...
      parse_wf_with_priority
        "2|defer-formulas|conjecture-relative-e(0.1,0.5,100,100,100,100,1.5,1.5,1.5)" ;
