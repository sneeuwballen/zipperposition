...
type case = {
  case_top : cst;
  (* copy of the coverset's top constant *)
  case_term : Term.t;
  (* rhs *)
  case_kind : [`Base | `Rec];
  (* at least one sub-constant? *)
  case_sub : cst list;
  (* set of sub-constants *)
  case_skolems : (ID.t * Type.t) list (* set of other skolems *)
}

type t = {
  cs_top : cst;
  (* inductive constant that is unique to that set *)
...
module State_ = struct
  ...
  type t = {
    cst : Ind_cst.Cst_set.t;
    (* raw set of constants *)
  ...
  let ( >>>= ) : 'a mm -> ('a -> 'b mm) -> 'b mm =
   fun x_mm f st ->
    let xs = x_mm st in
    CCList.flat_map (fun (st, x) -> f x st) xs
  ...
  let rec map_l : ('a -> 'b mm) -> 'a list -> 'b list mm =
   fun f l ->
    match l with
    | [] -> yield []
    | x :: tl -> f x >>>= fun x' -> map_l f tl >>>= fun tl' -> yield (x' :: tl')
...
let make_coverset_ ~cover_set_depth ~depth (ty : Type.t) (ity : Ind_ty.t) : t =
  ...
  let mk_skolem ty : T.t mm =
    ...
    add_constant id ty >|= fun () -> T.const ~ty id
  in
  ...
  let decl_sub ty : T.t mm =
    if Ind_ty.is_inductive_type ty then (
      ...
      add_sub_case sub >|= fun () -> Ind_cst.to_term sub )
    else
      mk_skolem ty
  in
  ...
  let rec make (cs_depth : int) (subst : Subst.t) : T.t mm =
    ...
    if cs_depth = 0 then (
      ...
      in
      decl_sub ty )
    else (* inner nodes or base cases: constructors *)
      yield_l ity.Ind_ty.ty_constructors >>>= fun cstor ->
      ...
      if ty_args_f = [] then
        if cs_depth > 0 then
          yield (T.app (T.const ~ty:ty_f f) ty_params)
        (* only one answer : f *)
        else
          fail
      else (* make fresh type variables and apply *)
        map_l (make_of_ty (cs_depth - 1)) ty_args_f >>|= fun args ->
