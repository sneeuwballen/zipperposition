...
module H = CCHeap.Make (struct
  ...
  let leq (i1, s1) (i2, s2) = i1 < i2 || (i1 = i2 && Stream.compare s1 s2 <= 0)
end)
...
let rec _take_first guard q =
  ...
  (* TODO: replace with cheaper test q.stm_nb = 0 ? *)
    ...
    let new_hp =
      ...
      with Stream.Empty_Stream ->
        assert (q.stm_nb > 0);
        (* TODO: stronger but more costly assert using H.size ?*)
        q.stm_nb <- q.stm_nb - 1;
        reduced_hp
    in
    q.hp <- new_hp;
...
let take_fair ?(full = false) tries q =
  ...
  (* q.hp <- H.filter (fun (_,s) -> not (Stream.is_empty s)) q.hp;
     H.fold (fun res (_,s) -> Stream.drip s :: res) [] q.hp *)
  q.fair_tries <- q.fair_tries + 1;
  (* H.fold (fun res (_,s) -> Stream.drip s :: res) [] q.hp *)
  let all_stms =
    CCList.sort
      (fun (_, s) (_, s') -> CCInt.compare (Stream.id s) (Stream.id s'))
      (H.to_list q.hp)
  in
  ...
  let dripped =
    let rec consume_until_empty n acc = function
      | [] -> acc
      | (_, s) :: s_rest ->
        (match Stream.drip s with
        | result ->
          let n =
            n
            +
            if CCOpt.is_some result then
              1
            else
              0
          in
  ...
  let new_stms =
    List.rev_append
      (List.rev_map (fun (_, s) -> Stream.penalty s, s) dripped)
      rest
  in
  ...
  Util.debugf ~section 1 "taken clauses: @[%a@]@." (fun k ->
      k (CCList.pp (CCOpt.pp Clause.pp)) taken);
  taken
...
let rec _take_nb q nb prev_res =
  if H.is_empty q.hp || nb = 0 then
    prev_res
  else (
    try
      let res = take_first q in
      (* if empty clause is found do not go futher *)
      if is_empty_clause res then
        res :: prev_res
      else
        _take_nb q (nb - 1) (res :: prev_res)
...
let rec _take_stm_nb_fix_stm q n res =
  if n = 0 || H.is_empty q.hp then
    res
  else (
    let red_hp, (w, s) = H.take_exn q.hp in
    try
      if Stream.is_empty s then (
        q.stm_nb <- q.stm_nb - 1;
        q.hp <- red_hp;
        _take_stm_nb_fix_stm q n res
      ) else (
        let res = Stream.drip_n s n q.guard in
        q.hp <- H.insert (q.weight s, s) red_hp;
        res
      )
    with Stream.Drip_n_Unfinished (res', _, n') ->
