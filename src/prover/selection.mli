...
(** See "E: a brainiac theorem prover". A selection function returns a bitvector
    of selected literals.

    The [strict] parameter, if true, means only one negative literal is selected
    (at most); if [strict=false] then all positive literals are also selected.
*)
...
(** [except_RR_horn p] behaves like [p], except if the clause is a
    range-restricted Horn clause. In that case, we assume the clause is a
    (conditional) rewrite rule and we don't prevent using it as an active
    clause. *)
...
(** Selection function that tries to take into account the number of nested
    applied variables.

    The assumption is that they are hard for unification. *)

val from_string : ord:Ordering.t -> string -> t * bool
(** selection function from string (may fail) -- returns functiona and a boolean
    saying whether function retains completeness *)
