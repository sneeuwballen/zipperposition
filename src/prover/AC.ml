...
module Make (Env : Env.S) : S with module Env = Env = struct
  ...
  let mk_axioms_ proof s ty : C.t list =
    ...
    if List.length ty_args <> 2 then Util.errorf ~where:"AC" "AC symbol `%a`must be of arity 2" ID.pp s ;
    ...
    | [a; b] ->
        if not (Type.equal a b && Type.equal a ty_ret) then
          Util.errorf ~where:"AC" "AC symbol `%a` argument types must be `@[%a@]`" ID.pp s Type.pp ty_ret
    | _ ->
        assert false ) ;
  ...
  let add_ proof ~ty s =
    ...
    with Not_found ->
      ...
      ID.Tbl.add tbl s cell ; Signal.send on_add spec ; cell
  ...
  let is_trivial c =
    let res = (not (C.get_flag flag_axiom c)) && CCArray.exists is_trivial_lit (C.lits c) in
    if res then (
  ...
  let simplify c =
    ...
    if exists_ac () then
      ...
      if n' < n && not (C.get_flag SClause.flag_persistent c) then (
        ...
        let premises = C.proof_parent c :: List.map (fun id -> (ID.Tbl.find tbl id).proof) symbols in
        let proof = Proof.Step.simp premises ~rule:(Proof.Rule.mk "AC.normalize") ~tags in
        let new_c = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) lits proof in
        ZProf.exit_prof _span ;
        Util.incr_stat stat_ac_simplify ;
        Util.debugf ~section 3 "@[<2>@[%a@]@ AC-simplify into @[%a@]@]" (fun k -> k C.pp c C.pp new_c) ;
        SimplM.return_new new_c )
      else (
        (* no simplification *)
        ZProf.exit_prof _span ;
        SimplM.return_same c )
    else (ZProf.exit_prof _span ; SimplM.return_same c)

  let install_rules_ () = Env.add_is_trivial is_trivial ; Env.add_basic_simplify simplify ; ()

  let add ~proof s ty =
    Util.debugf ~section 1 "@[enable AC redundancy criterion@ for `@[%a : @[%a@]@]`@ :proof %a@]" (fun k ->
        k ID.pp s Type.pp ty Proof.pp_parent proof ) ;
    ...
    (* add clauses *)
    Util.debugf ~section 3 "@[<2>add AC axioms for `%a : @[%a@]`:@ @[<hv>%a@]@]" (fun k ->
        k ID.pp s Type.pp ty (Util.pp_list C.pp) cell.axioms ) ;
    (* add axioms to either passive, or active set *)
    if Env.ProofState.ActiveSet.clauses () |> C.ClauseSet.for_all (C.get_flag flag_axiom) then
    ...
    else Env.add_passive (Iter.of_list cell.axioms) ;
    ()

  (* TODO: proof stuff *)
  let scan_statement st =
    let module St = Statement in
    let has_ac_attr = List.exists (function St.A_AC -> true | _ -> false) (Statement.attrs st) in
    if has_ac_attr then
      ...
      | St.Data _ | St.Rewrite _ | St.Assert _ | St.Lemma _ | St.Goal _ | St.NegatedGoal _ ->
          Util.error ~where:"AC" "attribute 'AC' only supported on def/decl statements"
      ...
      try
        ...
        | T.App (hd_s, [x_s; fyz_s]), T.App (hd_t, [fxy_t; z_t]) -> (
          ...
          | T.App (hd_s', [y_s; z_s]), T.App (hd_t', [x_t; y_t]) ->
              fail_on (not (T.equal hd_s hd_t && T.equal hd_t hd_s' && T.equal hd_s' hd_t')) ;
              fail_on (not (T.is_const hd_s)) ;
              fail_on (not (is_binary_sym hd_s)) ;
  ...
  let setup () =
    ...
    if !_scan_cl_ac then
      Signal.on_every Env.ProofState.PassiveSet.on_add_clause (fun c ->
          scan_clause c ; Signal.ContinueListening ) ;
...
let () =
  Options.add_opts [("--scan-clause-ac", Arg.Bool (( := ) _scan_cl_ac), " scan clauses for AC definitions")]
