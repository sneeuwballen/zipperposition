...
module Make (Env : Env.S) : S with module Env = Env = struct
  ...
  let mk_axioms_ proof s ty : C.t list =
    ...
    let ty_vars =
      CCList.init ty_args_n (fun i -> HVar.make ~ty:Type.tType i)
    in
  ...
  let add ~proof s ty =
    ...
    Util.debugf ~section 3
      "@[<2>add AC axioms for `%a : @[%a@]`:@ @[<hv>%a@]@]" (fun k ->
  ...
  let setup () = Signal.on_every Env.on_input_statement scan_statement
end
