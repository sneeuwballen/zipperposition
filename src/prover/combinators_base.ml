...
let[@inline] mk_comb comb_head ty ty_args args =
  ...
  if CCList.is_empty args then
    ...
    T.app (T.app_builtin ~ty comb_head ty_args) args
    ...
    | T.AppBuiltin (hd, args) ->
      ...
      if T.equal t t' then (
        ...
        if T.same_l args args' then
          ...
          T.app_builtin hd args' ~ty:(T.ty t)
      ) else
        do_narrow t'
    | T.App (hd, args) ->
...
type state = {
  ...
  balance: CCInt.t Term.Tbl.t;
  var_map: T.t Term.Tbl.t;
}
...
let add_pos_var state var =
  ...
  if n = 0 then
    state.pos_counter <- state.pos_counter + 1
  else if n = -1 then
    state.neg_counter <- state.neg_counter - 1;
...
let max_weak_reduction_length var_handler ~state orig_t =
  let rec aux t =
    ...
    | T.DB _ | T.Fun _ ->
      let err_msg =
        CCFormat.sprintf
          "lambdas should be removed@.orig:@[%a@];subterm:@[%a@]@." T.pp orig_t
          T.pp t
      in
      invalid_arg err_msg
    | T.Const _ -> 0
    ...
    | T.AppBuiltin (b, _l) when Builtin.is_combinator b ->
      ...
      | Builtin.SComb | Builtin.CComb | Builtin.BComb ->
        if CCList.length args < 3 then
          ...
          aux (narrow_one t) + 1
      | _ -> invalid_arg "only combinators are supported")
...
let cmp_by_max_weak_r_len t1 t2 =
  ...
  if t1_len > t2_len then
    if state.neg_counter > 0 then
      return Comparison.Incomparable
    else
      return Comparison.Gt
  else if t1_len < t2_len then
    if state.pos_counter > 0 then
      return Comparison.Incomparable
    else
...
let abf ~rules t =
  let rec abstract ~bvar_ty t =
    ...
    | T.Fun _ -> invalid_arg "all lambdas should be abstracted away!"
  in
    ...
    | T.AppBuiltin _ | T.App _ ->
      ...
      (try
         if T.same_l args args' then
           ...
           T.app hd_mono args' (* flattens AppBuiltin if necessary *)
