...
let ty_c =
  ...
  let open Type in
  let prefix ty = forall @@ forall @@ forall ty in
  prefix ([[db_alpha; db_beta] ==> db_gamma; db_beta; db_alpha] ==> db_gamma)
...
let[@inline] mk_comb comb_head ty ty_args args =
  ...
  if CCList.is_empty args then T.app_builtin ~ty comb_head ty_args
...
let[@inline] unpack_comb t =
  ...
  | _ ->
      raise IsNotCombinator

(* Given type arguments of S, calculate correct type arguments
   for B *)
let s2b_tyargs ~alpha ~beta ~gamma = (beta, gamma, alpha)
...
let narrowK t =
  try
    ...
    if Builtin.equal Builtin.KComb c_kind then
      match args with x :: y :: rest -> Some (T.app x rest) | _ -> None
    else None
  with IsNotCombinator -> None
  ...
  try
    ...
    if Builtin.equal Builtin.SComb c_kind then
      ...
      | [bkx; y] -> (
        ...
        | Builtin.BComb, _, [k; x] -> (
          match unpack_comb k with Builtin.KComb, _, [] -> Some x | _ -> None )
        | _ ->
            None )
      | _ ->
          None
  ...
  let rec do_narrow t =
    ...
    | T.AppBuiltin (hd, args) ->
        ...
        if T.equal t t' then
          ...
          if T.same_l args args' then t else T.app_builtin hd args' ~ty:(T.ty t)
        else do_narrow t'
    | T.App (hd, args) ->
    ...
    | T.Fun _ ->
        ...
        if T.equal body body' then t else T.fun_l tys body'
  in
  do_narrow t
...
let add_pos_var state var =
  ...
  else if n = -1 then state.neg_counter <- state.neg_counter - 1 ;
...
let add_neg_var state var =
  ...
  else if n = 1 then state.pos_counter <- state.pos_counter - 1 ;
  Term.Tbl.add state.balance var (n - 1)
...
let max_weak_reduction_length var_handler ~state orig_t =
  let rec aux t =
    ...
    | T.AppBuiltin (b, l) when Builtin.is_combinator b -> (
        ...
        match c_kind with
        | Builtin.IComb ->
            if CCList.is_empty args then 0 else aux (narrow_one t) + 1
        | Builtin.KComb ->
    ...
    | T.AppBuiltin (b, l) ->
        aux_l l
  and aux_l = function [] -> 0 | t :: ts -> aux t + aux_l ts
  and narrow_one t =
...
let abf ~rules t =
  let rec abstract ~bvar_ty t =
    ...
    | T.Fun _ ->
        invalid_arg "all lambdas should be abstracted away!"
  in
  let rec aux t =
    match T.view t with
    | T.AppBuiltin _ | T.App _ -> (
