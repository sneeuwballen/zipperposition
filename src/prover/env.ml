...
module Make (X : sig
  ...
  let get_passive () = ProofState.PassiveSet.clauses () |> C.ClauseSet.to_seq

  let get_active () = ProofState.ActiveSet.clauses () |> C.ClauseSet.to_seq
  ...
  let add_generate name rule =
    if not (List.mem_assoc name !_generate_rules) then
      _generate_rules := (name, rule) :: !_generate_rules
  ...
  let do_binary_inferences c =
    ...
    Util.debugf ~section 5
      "@[<2>do binary inferences with current active set:@ `@[%a@]`@]"
  ...
  let is_passive c = C.ClauseSet.mem c (ProofState.PassiveSet.clauses ())
    ...
    let rec reduce_term rules t =
      ...
      | (name, r) :: rules' ->
        ...
        | Some (t', proof) ->
          ...
          Util.debugf ~section 5
            "@[<2>rewrite `@[%a@]`@ into `@[%a@]`@ :proof (@[%a@])@]" (fun k ->
              k T.pp t T.pp t' (Util.pp_list Proof.pp_parent) proof );
    ...
    let res =
      fix_simpl c ~f:(fun c ->
          ...
          >>= (* simplify with unit clauses, then all active clauses *)
              rewrite >>= rw_simplify >>= unary_simplify >>= active_simplify
          >|= fun c ->
  ...
  let backward_simplify given =
    ...
    let back_simplify c =
      ...
      fix_simpl c ~f:(fun c ->
          ...
          basic_simplify c
          >>= (* simplify with unit clauses, then all active clauses *)
              rewrite >>= rw_simplify >>= unary_simplify
          >|= fun c ->
          if not (Lits.equal_com (C.lits c) (C.lits old_c)) then
            Util.debugf ~section 2
              "@[clause `@[%a@]`@ simplified into `@[%a@]`@]" (fun k ->
  ...
  let is_redundant c =
    C.is_redundant c || Util.with_prof prof_is_redundant is_redundant_ c
  ...
  let convert_input_statements stmts : C.t Clause.sets =
    ...
    Util.debugf ~section 1
      "@[<v>@[<2>clauses:@ @[<v>%a@]@]@ @[<2>sos:@ @[<v>%a@]@]@]" (fun k ->
        k
          (Util.pp_seq ~sep:" " C.pp)
          (CCVector.to_seq c_set)
          (Util.pp_seq ~sep:" " C.pp)
          (CCVector.to_seq c_sos) );
