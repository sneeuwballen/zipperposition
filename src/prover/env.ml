...
module Make (X : sig
  ...
  let add_passive cs =
    ...
    Iter.iter (fun c -> if C.is_empty c then add_empty c) cs ;
    ()
  ...
  let get_passive () = ProofState.PassiveSet.clauses () |> C.ClauseSet.to_iter
  ...
  let add_rw_simplify r = _rw_simplify := r :: !_rw_simplify

  let add_active_simplify r = _active_simplify := r :: !_active_simplify

  let add_backward_simplify r = _backward_simplify := r :: !_backward_simplify
  ...
  let add_unary_simplify r = _unary_simplify := r :: !_unary_simplify

  let add_is_trivial_trail r = _is_trivial_trail := r :: !_is_trivial_trail

  let add_is_trivial r = _is_trivial := r :: !_is_trivial
  ...
  let get_ho_normalization_rule () = !_norm_rule

  let add_lit_rule name rule = _lit_rules := (name, rule) :: !_lit_rules
  ...
  let add_cheap_multi_simpl_rule rule = _cheap_msr := rule :: !_cheap_msr
  ...
  let do_binary_inferences c =
    ...
    let clauses =
      List.fold_left
        ...
        [] !_binary_rules
    in
    ZProf.exit_prof _span ; Iter.of_list clauses
  ...
  let is_trivial_trail trail =
    ...
    | [f] ->
        f trail
    | f1 :: f2 :: tl ->
        f1 trail || f2 trail || List.exists (fun f -> f trail) tl
    ...
    if C.get_flag SClause.flag_persistent c then false
      ...
      let res =
        ...
        | [f] ->
            f c
        | f :: g :: tl ->
            f c || g c || List.exists (fun f -> f c) tl
      in
  ...
  let immediate_simplify given immediate =
    let rec aux = function
      | [] ->
          immediate
      | f :: fs -> (
        match f given immediate with Some res -> res | None -> aux fs )
    in
    aux !_immediate_simpl

  let is_active c = C.ClauseSet.mem c (ProofState.ActiveSet.clauses ())
  ...
  let rewrite c =
    ...
    let rec reduce_term rules t =
      ...
      | (name, r) :: rules' -> (
        match r t with
        | None ->
            reduce_term rules' t (* try next rules *)
        | Some (t', proof) ->
  ...
  let ho_normalize c =
    ...
    let lits' =
      Array.map
        ...
        (C.lits c)
    in
    if not !did_reduce then SimplM.return_same c (* no simplification *)
    else (
      C.mark_redundant c ;
      (* FIXME: put the rules as parameters *)
  ...
  let rewrite_lits c =
    ...
    if StrSet.is_empty !applied_rules then SimplM.return_same c
    else (
  ...
  let rec fix_simpl : f:(C.t -> C.t SimplM.t) -> C.t -> C.t SimplM.t =
   fun ~f c ->
    let open SimplM.Infix in
    let new_c = f c in
    if C.equal c (SimplM.get new_c) then new_c (* fixpoint reached *)
    else (
  ...
  let rw_simplify c =
    let open SimplM.Infix in
    fix_simpl c ~f:(fun c ->
        if C.get_flag SClause.flag_persistent c then SimplM.return_same c
        else
          match !_rw_simplify with
          | [] ->
              SimplM.return_same c
          | [f] ->
              f c
    ...
    let rec try_next ~depth c rules =
      if flex_get k_max_multi_simpl_depth != -1 && depth > flex_get k_max_multi_simpl_depth then None
        ...
        | [] ->
            None
        | r :: rules' -> (
          match r c with Some l -> Some l | None -> try_next ~depth c rules' )
    in
  ...
  *)
  let all_simplify c =
    ...
    while not (Queue.is_empty q) do
      ...
      if is_trivial c || is_redundant c then ()
      else
    ...
    ZProf.exit_prof _span ;
    if !did_simplify then SimplM.return_new res else SimplM.return_same res
  ...
  let convert_input_statements stmts : C.t Clause.sets =
    ...
    let rec conv_clause_ rules st =
      ...
      | r :: rules' -> (
        match r st with
        | CR_skip ->
            conv_clause_ rules' st
        | CR_drop ->
            []
        | CR_return l ->
            l
        | CR_add l ->
            List.rev_append l (conv_clause_ rules' st) )
    in
    CCVector.iter
      (fun st ->
