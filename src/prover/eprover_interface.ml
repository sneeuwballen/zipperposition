...
let encode_ty_args_t ~encoded_symbols ~env t =
  ...
  let rec aux ~sym_map t =
    ...
    | T.AppBuiltin (b, _)
      when (b == Builtin.Eq || b == Builtin.Neq) && not (Type.is_prop (T.ty t))
      ->
...
let try_e env active_set passive_set =
  ...
  let take_ho_clauses ~converter ~encoded_symbols clauses =
    ...
    |> Iter.sort ~cmp:(fun c1 c2 ->
           ...
           if pd1 = pd2 || !_sort_by_weight_only then
             CCInt.compare (C.ho_weight c1) (C.ho_weight c2)
           else
  ...
  try
    let converter =
      ...
      | `Drop ->
        fun c ->
          if Iter.exists T.has_lambda (C.Seq.terms c) then
            []
          else
            [ c ]
      ...
      | _ ->
        fun c ->
          ...
          if CCList.is_empty lifted then
            [ c ]
          else
            lifted
    in

    let encoded_symbols, initial = take_initial ~converter () in
    let _, ho_clauses =
    ...
    let res =
      ...
      | Some ids ->
        ...
        in
        let penalty =
          CCOpt.get_exn @@ Iter.max (Iter.map C.penalty (Iter.of_list clauses))
        in
        let trail = C.trail_l clauses in
        Some
          (C.create ~ctx:(C.ctx_of (List.hd clauses)) ~penalty ~trail [] proof)
      | _ -> None
    in
