...
let stat_refine = Util.mk_stat "ho.refine.steps"
...
module Make(E : Env.S) : S with module Env = E = struct
  ...
  (* refine subterms of the form [F t1…tn] where [F] is fully applied,
     using combinators.
  ...
  let setup () =
    ...
    (* force rules *)
    let () = ignore (HO_combinators.rules @@ Ctx.combinators ()) in
    declare_combinators ();
    Env.add_unary_inf "ho_refine" refine;
