...
let direct_arg_mask =
  premise_ctx lor consequent_ctx lor and_ctx lor or_ctx lor neg_ctx
...
let get_forbidden_vars ~ord lits =
  ...
  |> CCArray.fold
       (fun vars lit ->
         Lit.fold_terms ~vars:true ~ty_args:false ~which:`Max ~ord
           ~subterms:false lit
...
let select_leftmost ~ord ~kind lits =
  ...
  and aux_term ~top ~pos_builder t =
    ...
    if T.is_app_var t then
      CCOpt.return_if
        (is_selectable ~top ~forbidden t)
        (t, PB.to_pos pos_builder)
    else
      match T.view t with
      | T.App (_, args) | T.AppBuiltin (_, args) ->
          (* reversing the argument indices *)
          let inner =
            aux_term_args args ~idx:(List.length args - 1) ~pos_builder
          in
          let outer =
...
let collect_green_subterms_ ~forbidden ~filter ~ord ~pos_builder t k =
  let rec aux_term ~top ~pos_builder t k =
    ...
    if T.is_app_var t then ( (* only green subterms are eligible *) )
      ...
      match T.view t with
      | T.AppBuiltin
          ((BIn.Eq | BIn.Neq | BIn.Xor | BIn.Equiv), (([a; b] | [_; a; b]) as l))
...
let get_selectable_w_ctx ~ord lits =
  let open CCOpt in
  let forbidden = get_forbidden_vars ~ord lits in
  let selectable_with_ctx ?(forbidden = T.VarSet.empty) ~block_top ~ord
      ~pos_builder t ctx log_depth k =
    let negate_sgn ctx =
    ...
    let rec aux_term ~top ~pos_builder t ctx log_depth k =
      if is_selectable ~forbidden ~top:(top && block_top) t then
        k (t, (ctx, log_depth), PB.to_pos pos_builder) ;
      if T.is_app_var t then ( (* only green subterms are eligible *) )
      else
        match T.view t with
        | T.AppBuiltin
        ...
        | T.AppBuiltin (Builtin.Imply, [p; c]) ->
            ...
            aux_term ~top:false ~pos_builder:(PB.arg 0 pos_builder) c ctx_c
              (log_depth + 1) k
        | T.AppBuiltin (hd, args) when not (Builtin.is_quantifier hd) ->
            aux_term_args
              ~idx:(List.length args - 1)
              ~pos_builder args ctx log_depth k
        | _ ->
            ()
    and aux_term_args ~idx ~pos_builder args ctx log_depth k =
      ...
      | x :: xs ->
          assert (idx >= 0) ;
          aux_term ~top:false ~pos_builder:(PB.arg idx pos_builder) x ctx
            log_depth k ;
  ...
  let rec aux_lits idx k =
    if idx < Array.length lits then
      ...
      | Lit.Equation (lhs, rhs, sign) as lit ->
          ...
          ( match Ordering.compare ord lhs rhs with
          | Comparison.Lt | Leq ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden
                ~pos_builder:(PB.right pos_builder) rhs ctx_sign 0 k
          | Gt | Geq ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden
                ~pos_builder:(PB.left pos_builder) lhs ctx_sign 0 k
          | _ ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden
                ~pos_builder:(PB.left pos_builder) lhs ctx_sign 0 k ;
              selectable_with_ctx ~block_top:sign ~ord ~forbidden
...
let by_size ~ord ~kind lits =
  ...
  |> selector ~lt:(fun (s, _, _) (t, _, _) ->
         Term.ho_weight s < Term.ho_weight t )
...
let by_context_weight_combination ~ord ~ctx_fun ~weight_fun lits =
  ...
  |> Iter.map (fun ((t, (ctx, _), _) as arg) ->
         Util.debugf ~section 2 "selectable @[%a/%8s@]@." (fun k ->
             k T.pp t (bin_of_int ctx) ) ;
         arg )
...
let is_eq t =
  match T.view t with T.AppBuiltin ((Eq | Neq), _) -> true | _ -> false
...
let sel2 lits s t =
  ...
  <?> (compare, T.VarSet.cardinal (T.vars s), T.VarSet.cardinal (T.vars t))
...
let sel3 =
  let is_type_pred t =
    match T.view t with
    | T.App (hd, args) ->
        T.is_const hd && List.for_all T.is_var args
    | _ ->
        false
  in
  fun lits s t ->
    let ( <?> ) = CCOrd.( <?> ) in
    compare (T.is_appbuiltin s) (T.is_appbuiltin t) (* defer formulas *)
    <?> (compare, is_type_pred s, is_type_pred t)
        (* defer p(x) where x are variables *)
    <?> (compare, T.is_app_var s, T.is_app_var t) (* defer app vars *)
    <?> (compare, T.ho_weight s, T.ho_weight t)
(* by weight *)

let prefer_pos_ctx (ctx1, _) (ctx2, _) =
  compare (get_sgn_ctx ctx1 != pos_ctx) (get_sgn_ctx ctx2 != pos_ctx)

let prefer_neg_ctx (ctx1, _) (ctx2, _) =
  compare (get_sgn_ctx ctx1 != neg_ctx) (get_sgn_ctx ctx2 != neg_ctx)

let prefer_and_arg (ctx1, _) (ctx2, _) =
  compare (get_arg_ctx ctx1 != and_ctx) (get_arg_ctx ctx2 != and_ctx)

let prefer_or_arg (ctx1, _) (ctx2, _) =
  compare (get_arg_ctx ctx1 != or_ctx) (get_arg_ctx ctx2 != or_ctx)
...
let prefer_concl_arg (ctx1, _) (ctx2, _) =
  compare
    (get_arg_ctx ctx1 != consequent_ctx)
...
let from_string ~ord name lits =
  let res =
    ...
    with _ -> (
      try parse_combined_function ~ord name
      with _ -> invalid_arg (name ^ " is not a valid bool selection name") )
  in
  res lits
  |> CCFun.tap (fun l ->
