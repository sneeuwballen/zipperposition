...
let inv_idx args idx = List.length args - idx - 1
...
let get_forbidden_vars ~ord lits =
  ...
  |> CCArray.fold
       (fun vars lit ->
         ...
         |> T.VarSet.of_iter |> T.VarSet.union vars )
       T.VarSet.empty
...
let select_leftmost ~ord ~kind lits =
  ...
  and aux_term ~top ~pos_builder t =
    ...
    if T.is_app_var t then CCOpt.return_if (is_selectable ~top ~forbidden t) (t, PB.to_pos pos_builder)
    else
      match T.view t with
      | T.App (_, args) | T.AppBuiltin (_, args) ->
          (* reversing the argument indices *)
          let inner = aux_term_args args ~idx:(List.length args - 1) ~pos_builder in
          let outer = if not (is_selectable ~top ~forbidden t) then None else Some (t, PB.to_pos pos_builder) in
          if kind == `Inner then inner <+> outer else outer <+> inner
      | T.Const _ when is_selectable ~top ~forbidden t ->
          Some (t, PB.to_pos pos_builder)
      | _ ->
          None
...
let all_eligible_subterms ~ord ~pos_builder =
  ...
  let filter ~forbidden ~top t = (not top) && not (Type.is_tType (T.ty t)) in
...
let get_selectable_w_ctx ~ord lits =
  ...
  let selectable_with_ctx ?(forbidden = T.VarSet.empty) ~block_top ~ord ~pos_builder t ctx log_depth k =
    let negate_sgn ctx =
      ...
      if sgn == pos_ctx then neg_ctx else if sgn == neg_ctx then pos_ctx else under_equiv_ctx
    in
    let rec aux_term ~top ~pos_builder t ctx log_depth k =
      if is_selectable ~forbidden ~top:(top && block_top) t then k (t, (ctx, log_depth), PB.to_pos pos_builder) ;
      if T.is_app_var t then ( (* only green subterms are eligible *) )
      else
        match T.view t with
        | T.AppBuiltin ((BIn.Eq | BIn.Neq | BIn.Xor | BIn.Equiv), (([a; b] | [_; a; b]) as l)) -> (
  ...
  let rec aux_lits idx k =
    if idx < Array.length lits then
      ...
      | Lit.Equation (lhs, rhs, sign) as lit ->
          let ctx_sign =
            if Lit.is_predicate_lit lit then if Lit.is_positivoid lit then pos_ctx else neg_ctx
            else under_equiv_ctx
          in
          ( match Ordering.compare ord lhs rhs with
          | Comparison.Lt | Leq ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.right pos_builder) rhs
                ctx_sign 0 k
          | Gt | Geq ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.left pos_builder) lhs ctx_sign
                0 k
          | _ ->
              selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.left pos_builder) lhs ctx_sign
                0 k ;
...
let by_size ~ord ~kind lits =
  let selector = if kind = `Max then Iter.max else Iter.min in
...
let by_context_weight_combination ~ord ~ctx_fun ~weight_fun lits =
  let bin_of_int d =
    if d < 0 then invalid_arg "bin_of_int"
      ...
      let rec aux acc d = if d = 0 then acc else aux (string_of_int (d land 1) :: acc) (d lsr 1) in
      String.concat "" (aux [] d)
  in
  get_selectable_w_ctx ~ord lits
  |> Iter.map (fun ((t, (ctx, _), _) as arg) ->
         Util.debugf ~section 2 "selectable @[%a/%8s@]@." (fun k -> k T.pp t (bin_of_int ctx)) ;
         arg )
  |> Iter.min ~lt:(fun (s, ctx_s, _) (t, ctx_t, _) ->
         CCOrd.( <?> ) (ctx_fun ctx_s ctx_t) (weight_fun lits, s, t) < 0 )
  |> CCOpt.map_or ~default:[] (fun (t, ctx, pos) -> [(t, pos)])

let is_eq t = match T.view t with T.AppBuiltin ((Eq | Neq), _) -> true | _ -> false

let sel1 lits =
  let w_tbl = ID.Tbl.create 16 in
  let incr_id = ID.Tbl.incr ~by:1 w_tbl in
  let id_w = ID.Tbl.get_or ~default:0 w_tbl in
  Iter.of_array lits |> Iter.flat_map (Lit.Seq.symbols ~include_types:false) |> Iter.iter incr_id ;
  fun s t ->
    let ( <?> ) = CCOrd.( <?> ) in
    ...
    <?> (compare, T.weight ~var:1 ~sym:id_w s, T.weight ~var:1 ~sym:id_w t)
...
let sel2 lits s t =
  ...
  <?> (compare, T.VarSet.cardinal (T.vars s), T.VarSet.cardinal (T.vars t)) (* prefer less distinct vars *)
  <?> (compare, T.ho_weight s, T.ho_weight t)
(* by weight, where the weight of symbol
   is its number of occs*)

let sel3 =
  let is_type_pred t =
    match T.view t with T.App (hd, args) -> T.is_const hd && List.for_all T.is_var args | _ -> false
  in
  fun lits s t ->
    let ( <?> ) = CCOrd.( <?> ) in
    compare (T.is_appbuiltin s) (T.is_appbuiltin t) (* defer formulas *)
...
let prefer_pos_ctx (ctx1, _) (ctx2, _) = compare (get_sgn_ctx ctx1 != pos_ctx) (get_sgn_ctx ctx2 != pos_ctx)

let prefer_neg_ctx (ctx1, _) (ctx2, _) = compare (get_sgn_ctx ctx1 != neg_ctx) (get_sgn_ctx ctx2 != neg_ctx)

let prefer_and_arg (ctx1, _) (ctx2, _) = compare (get_arg_ctx ctx1 != and_ctx) (get_arg_ctx ctx2 != and_ctx)

let prefer_or_arg (ctx1, _) (ctx2, _) = compare (get_arg_ctx ctx1 != or_ctx) (get_arg_ctx ctx2 != or_ctx)

let prefer_neg_arg (ctx1, _) (ctx2, _) =
  compare (get_arg_ctx ctx1 != neg_sym_ctx) (get_arg_ctx ctx2 != neg_sym_ctx)

let prefer_premises_arg (ctx1, _) (ctx2, _) =
  compare (get_arg_ctx ctx1 != premise_ctx) (get_arg_ctx ctx2 != premise_ctx)

let prefer_concl_arg (ctx1, _) (ctx2, _) =
  compare (get_arg_ctx ctx1 != consequent_ctx) (get_arg_ctx ctx2 != consequent_ctx)

let prefer_deep_log (_, log_d1) (_, log_d2) = compare (-log_d1) (-log_d2)

let prefer_shallow_log (_, log_d1) (_, log_d2) = compare log_d1 log_d2

let discard_ctx _ _ = 0

let leftmost_innermost ~ord lits = select_leftmost ~ord ~kind:`Inner lits

let leftmost_outermost ~ord lits = select_leftmost ~ord ~kind:`Outer lits

let smallest ~ord lits = by_size ~ord ~kind:`Min lits

let largest ~ord lits = by_size ~ord ~kind:`Max lits
...
let parse_combined_function ~ord s =
  ...
  in
  let or_lmax_regex = Str.regexp "sel\\([1-3]\\)(\\(.+\\))" in
  try
  ...
  with Not_found | Invalid_argument _ ->
    Util.invalid_argf "expected sel[123](%a)"
      (CCList.pp ~pp_sep:(CCFormat.return "|") CCString.pp)
      (List.map fst ctx_funs)

let from_string ~ord name lits =
  let res =
    try (List.assoc name fun_names) ~ord
    with _ -> (
      try parse_combined_function ~ord name with _ -> invalid_arg (name ^ " is not a valid bool selection name") )
  in
  res lits
  |> CCFun.tap (fun l ->
         Util.debugf ~section 1 "bool_select(@[%a@])=@.@[%a@]@." (fun k ->
             k Literals.pp lits (CCList.pp Term.pp) (List.map fst l) ) )

let () =
  let set_bselect s = Params.bool_select := s in
  Params.add_opts [("--bool-select", Arg.String set_bselect, " set boolean literal selection function")] ;
  Params.add_to_mode "best" (fun () -> Params.bool_select := "sel1(consequent_ctx)")
