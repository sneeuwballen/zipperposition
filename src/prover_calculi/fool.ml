...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let fool_param_sign ~sub sign c =
    ...
    in
    let new_c =
      C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (new_lit :: lits)
        proof
    in
