...
let ty_decls = Iter.singleton eq_clone_tydecl
...
let eq_encode_stmt stmt =
  ...
  | Statement.Def _ | Statement.Rewrite _ | Statement.TyDecl _ ->
      stmt
  ...
  | Statement.Assert lits ->
      Statement.assert_ ~proof (eq_encode_lits lits)
...
let extension =
  ...
  in
  Extensions.{default with name= "eq_encode"; post_cnf_modifiers= [modifier]}
