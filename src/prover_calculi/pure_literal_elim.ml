    ...
    let rec ocurrences map =
      ...
      | x :: xs -> (
        try
          ...
          | _ ->
              ocurrences map xs
        with Invalid_argument _ -> (* invalid arg is raised if the head is not a symbol *)
                                   raise AppVarFound )
    in
  ...
  Iter.fold
    (fun (forbidden, ids2clauses, clauses) (stmt : (TST.t SLiteral.t list, TST.t, TST.t) Statement.t) ->
      ...
      | Statement.Data _ | Statement.Lemma _ | Statement.Def _ | Statement.Rewrite _ ->
          let f_syms =
            Statement.Seq.forms stmt |> Iter.fold (fun acc lits -> ID.Set.union acc (cl_syms lits)) ID.Set.empty
          in
...
let get_pure_symbols forbidden ids2clauses clauses =
  ...
  let all_clauses =
    List.fold_left (fun acc cl -> IDMap.union (fun _ a b -> CCOpt.return @@ (a + b)) acc cl) IDMap.empty clauses
  in
    ...
    (fun stmt ->
      ...
      | Statement.NegatedGoal (skolems, list_of_lits) ->
          ...
          if CCList.is_empty new_cls then None
            ...
            let new_stm =
              Statement.neg_goal ~attrs:(Statement.attrs stmt) ~proof:(Statement.proof_step stmt) ~skolems
                new_cls
            in
            Some new_stm
      | Statement.Goal lits ->
...
let extension =
  ...
  in
  Extensions.
    {default with name= "pure_literal_elimination"; post_cnf_modifiers= [modifier]; env_actions= [print_stats]}

let () =
  Options.add_opts
    [("--pure-literal-preprocessing", Arg.Bool (fun v -> _enabled := v), " remove all pure literals in fixpoint")] ;
