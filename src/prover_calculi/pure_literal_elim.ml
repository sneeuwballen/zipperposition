...
let compute_occurence_map
    (seq : (TST.t SLiteral.t list, TST.t, TST.t) Statement.t Iter.t) =
    ...
    let rec ocurrences map =
      ...
      | x :: xs ->
        (try
           ...
           | (SLiteral.Neq (lhs, rhs) | SLiteral.Eq (lhs, rhs))
             when TST.Ty.is_prop (TST.ty_exn lhs) ->
  ...
  in
  Iter.fold
    (fun (forbidden, ids2clauses, clauses)
         (stmt : (TST.t SLiteral.t list, TST.t, TST.t) Statement.t) ->
      match Statement.view stmt with
      (* Ignoring type declarations *)
      | Statement.TyDecl _ -> forbidden, ids2clauses, clauses
      | Statement.Data _ | Statement.Lemma _ | Statement.Def _
      | Statement.Rewrite _ ->
        let f_syms =
      ...
      | Statement.Assert lits ->
        ...
        let ids2clauses', new_cl =
          process_literals ids2clauses (List.length clauses, lits)
        in
        ...
        let ids2clauses', clauses' =
          List.fold_left
            (fun (ids2cls, cls) lits ->
              let ids2cls', new_cl =
                process_literals ids2cls (List.length cls, lits)
              in
              ids2cls', new_cl :: cls)
...
let remove_pure_clauses
  ...
  let filter_if_has_pure stmt lits =
    ...
    if CCOpt.is_none ans then (
      ...
      Util.debugf ~section 2 "removed: @[%a@]@." (fun k ->
          k (CCList.pp (SLiteral.pp TST.pp)) lits)
    );
    ans
  in
  Iter.filter_map
    (fun stmt ->
      match Statement.view stmt with
      | Statement.TyDecl _ | Statement.Data _ | Statement.Lemma _
      | Statement.Def _ | Statement.Rewrite _ ->
        Some stmt
      | Statement.Assert lits ->
        (* normal clause *)
      ...
      | Statement.NegatedGoal (skolems, list_of_lits) ->
        ...
        in
        if CCList.is_empty new_cls then
          None
        else if List.length new_cls == List.length list_of_lits then
          Some stmt
        else (
          let new_stm =
            Statement.neg_goal ~attrs:(Statement.attrs stmt)
...
let extension =
  let modifier (seq : (TST.t SLiteral.t list, TST.t, TST.t) Statement.t Iter.t)
      =
    if !_enabled then (
      try remove_pure_clauses seq with AppVarFound | Not_found -> seq
    ) else
      seq
  in
  let print_stats _ =
