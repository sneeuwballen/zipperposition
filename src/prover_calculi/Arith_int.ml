...
let stat_arith_semantic_tautology_steps =
  Util.mk_stat "int.semantic_tauto.steps"
...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let update f c =
    let ord = Ctx.ord () in
    _idx_eq :=
      Lits.fold_terms ~vars:false ~ty_args:false ~which:`Max ~ord
        ~subterms:false
    ...
    let left, right =
      ...
      |> Iter.fold
           (fun (left, right) (t, pos) ->
             let with_pos = C.WithPos.{term = t; pos; clause = c} in
             match pos with
             | Position.Arg (_, Position.Left _) -> f left t with_pos, right
             | Position.Arg (_, Position.Right _) -> left, f right t with_pos
             | _ -> assert false )
  ...
  let update_simpl f c =
    ...
    | [|Lit.Int (AL.Binary (AL.Lesseq, _, _) as alit)|] ->
      let pos = Position.(arg 0 stop) in
      _idx_unit_ineq :=
        if !enable_trivial_ineq_ || !enable_demod_ineq_ then
          AL.fold_terms ~subterms:false ~vars:false ~pos ~which:`Max ~ord alit
          |> Iter.fold
               (fun acc (t, pos) ->
                 assert (not (T.is_var t));
  ...
  let () =
    ...
    Signal.on PS.SimplSet.on_add_clause (fun c ->
    ...
    Signal.on PS.ActiveSet.on_remove_clause (fun c ->
        ...
        Signal.ContinueListening );
    Signal.on PS.SimplSet.on_remove_clause (fun c ->
  ...
  let _do_canc info acc =
    ...
    Util.debugf ~section 5
      "@[<2>arith superposition@ between @[%a[%d]@]@ and @[%a[%d]@]@ (subst \
       @[%a@])...@]" (fun k ->
        k C.pp info.active s_a C.pp info.passive s_p Subst.pp subst );
    (* check ordering conditions *)
    if
      C.is_maxlit (info.active, s_a) subst ~idx:idx_a
    ...
    then (
      ...
      let mf_a, m_a =
        match lit_a with
        | ALF.Left (AL.Equal, mf, m) | ALF.Right (AL.Equal, m, mf) -> mf, m
        | _ -> assert false
      in
      let new_lit =
        match lit_p with
      ...
      (* build clause *)
      let proof =
        Proof.Step.inference ~tags:[Proof.Tag.T_lia] ~rule:rule_canc
          [ C.proof_parent_subst renaming (info.active, s_a) subst;
            C.proof_parent_subst renaming (info.passive, s_p) subst ]
      in
    ...
    let res =
      ...
      |> Iter.fold
           (fun acc (t, active_lit, active_pos) ->
             ...
             Util.debugf ~section 5
               "@[<2>active canc. sup.@ with @[%a@]@ in @[%a@]@]" (fun k ->
                 k ALF.pp active_lit C.pp c );
             ...
             |> Iter.fold
                  (fun acc (t', with_pos, subst) ->
                    ...
                    let passive_lit =
                      Lits.View.get_arith_exn (C.lits passive) passive_pos
                    in
                    Util.debugf ~section 5
                      "@[<4>  possible match:@ @[%a@]@ in @[%a@]@]" (fun k ->
                        k ALF.pp passive_lit C.pp passive );
                    (* now to unify active_lit and passive_lit further *)
                    if T.is_var t || T.is_var t' then
                      acc
                    else
    ...
    let res =
      ...
      |> Iter.fold
           (fun acc (t, passive_lit, passive_pos) ->
             Util.debugf ~section 5
               "@[<2>passive canc. sup.@ with @[%a@]@ in @[%a@]@]" (fun k ->
                 k ALF.pp passive_lit C.pp c );
             ...
             |> Iter.fold
                  (fun acc (t', with_pos, subst) ->
                    ...
                    let active_lit =
                      Lits.View.get_arith_exn (C.lits active) active_pos
                    in
                    (* must have an equation as active lit *)
                    match ALF.op active_lit with
                    | `Binary AL.Equal
                      when (not (T.is_var t)) && not (T.is_var t') ->
                      Util.debugf ~section 5
                        "@[<4>  possible match:@ @[%a@]@ in @[%a@]@]" (fun k ->
                          k ALF.pp passive_lit C.pp c );
  ...
  let _try_demod_step ~subst passive_lit _s_p c pos active_lit s_a c' _pos' =
    ...
    if
      ALF.is_strictly_max ~ord active_lit'
      && C.Seq.vars c'
         |> Iter.for_all (fun v ->
                S.mem subst ((v : Type.t HVar.t :> InnerTerm.t HVar.t), s_a) )
      && ( C.lits c |> Array.length > 1
         || (not (Lit.equal (C.lits c).(i) (C.lits c').(0)))
         || not
              (ALF.is_max ~ord passive_lit && C.is_maxlit (c, 0) S.empty ~idx:i)
         )
    ...
    then (
      (* we know all variables of [active_lit] are bound, no need
         for a renaming *)
      let active_lit =
        ALF.apply_subst Subst.Renaming.none subst (active_lit, s_a)
      in
      let active_lit, passive_lit = ALF.scale active_lit passive_lit in
      match active_lit, passive_lit with
      | ALF.Left (AL.Equal, mf1, m1), _ | ALF.Right (AL.Equal, m1, mf1), _ ->
        let new_lit =
      ...
      | ALF.Div d1, ALF.Div d2 when d1.AL.sign ->
        let n1 = Z.pow d1.AL.num d1.AL.power
        and n2 = Z.pow d2.AL.num d2.AL.power in
        ...
        (* simplification: we only do the rewriting if both
             ...
             TODO: generalize *)
        if Z.equal n1 n2 && Z.lt gcd Z.(pow d2.AL.num d2.AL.power) then (
          let new_lit =
            ALF.replace passive_lit (M.uminus (MF.rest d1.AL.monome))
          in
          raise (SimplifyInto (new_lit, c', subst)) )
      | _ -> () )
    else
  ...
  let rec _demod_lit_nf ~add_lit ~add_premise ~i c a_lit =
    ...
    in
    try
      AL.fold_terms ~pos:Position.stop ~vars:false ~which:`Max ~ord
        ~subterms:false a_lit
      |> Iter.iter (fun (t, lit_pos) ->
             assert (not (T.is_var t));
    ...
    with SimplifyInto (a_lit', c', subst) ->
      ...
      Util.debugf ~section 4
        "@[<2>rewrite arith lit (@[%a@])@ into (@[%a@])@ using clause @[%a@]@ \
         and subst @[%a@]@]" (fun k ->
          k AL.pp a_lit AL.pp a_lit' C.pp c' S.pp subst );
  ...
  let _demodulation c =
    ...
    |> Iter.iter (fun (lit, i) ->
           match lit with
           | Lit.Int a_lit -> _demod_lit_nf ~add_lit ~add_premise ~i c a_lit
           | _ -> add_lit lit
           (* keep non-arith literals *) );
    ...
    let res =
      if !did_simplify then (
        ...
        let new_c =
          C.create ~penalty:(C.penalty c) ~trail (List.rev !lits) proof
        in
        Util.incr_stat stat_arith_demod;
        Util.debugf ~section 5
          "@[<2>arith demodulation@ of @[%a@]@ with [@[%a@]]@ gives @[%a@]@]"
          (fun k ->
            let pp_c_s out (c, s) =
               ...
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                     ...
                     its literal (and clause) after substitution *)
                      ...
                      let alit' =
                        ALF.apply_subst Subst.Renaming.none subst (alit', 1)
                      in
                      if C.trail_subsumes c' c && ALF.is_max ~ord alit' then (
                    ...
                    acc )
             C.ClauseSet.empty
      | [|Lit.Int (AL.Binary (AL.Lesseq, _m1, _m2))|] -> res (* TODO *)
      | [|Lit.Int (AL.Divides d)|] when d.AL.sign -> res (* TODO *)
      | _ -> res
      (* no demod *)
    in
    ...
    let res =
      ...
      |> Iter.fold
           (fun acc (a_lit, pos) ->
             ...
             match a_lit with
             | AL.Binary (op, m1, m2) ->
               Util.debugf ~section 5 "@[<2>try cancellation@ in @[%a@]@]"
                 (fun k -> k AL.pp a_lit);
               ...
               |> Iter.fold
                    (fun acc (mf1, mf2, us) ->
                      ...
                      Util.debugf ~section 5
                        "@[<4>... candidate:@ @[%a@] (max lit ? %B)@ :mf1 %a@ \
                         :mf2 %a@]" (fun k ->
                          k S.pp subst is_max_lit MF.pp mf1' MF.pp mf2' );
                      if
                        is_max_lit && MF.is_max ~ord mf1'
                        && MF.is_max ~ord mf2'
                      then (
                        (* do the inference *)
                        let lits' = CCArray.except_idx (C.lits c) idx in
                        let lits' =
                          Lit.apply_subst_list renaming subst (lits', 0)
                        in
                        let new_lit =
                          Lit.mk_arith_op op (MF.rest mf1') (MF.rest mf2')
                        in
                        ...
                        Util.debugf ~section 3
                          "@[<2>cancellation@ of @[%a@]@ (with %a)@ into \
                           @[%a@]@]" (fun k ->
                    ...
                    acc
             | AL.Divides d ->
               Util.debugf ~section 5 "@[try cancellation@ in @[%a@]@]"
                 (fun k -> k AL.pp a_lit);
               ...
               |> Iter.fold
                    (fun acc (mf, us) ->
                      ...
                      if C.is_maxlit (c, 0) subst ~idx && MF.is_max ~ord mf
                      then (
                        let lits' = CCArray.except_idx (C.lits c) idx in
                        let lits' =
                          Lit.apply_subst_list renaming subst (lits', 0)
                        ...
                        let new_lit =
                          Lit.mk_divides ~sign:d.AL.sign d.AL.num
                            ~power:d.AL.power (MF.to_monome mf')
                        in
                        let c_guard = Literal.of_unif_subst renaming us in
                        let all_lits = (new_lit :: c_guard) @ lits' in
                        let proof =
      ...
      |> Iter.fold
           (fun acc (t1, lit1, pos1) ->
             ...
             Lits.fold_arith_terms ~which:`Max ~ord
               ~eligible:C.Eligible.(filter Lit.is_arith_eq)
               (C.lits c)
             |> Iter.fold
                  (fun acc (t2, lit2, pos2) ->
                    ...
                    try
                      ...
                      Util.debugf ~section 5
                        "@[<2>arith canc. eq. factoring:@ possible match in \
                         @[%a@]@ (at %d, %d)@]" (fun k -> k C.pp c idx1 idx2);
                      ...
                      |> Iter.fold
                           (fun acc (_, _, us) ->
                             ...
                             let subst = US.subst us in
                             let lit1' =
                               ALF.apply_subst renaming subst (lit1, 0)
                             in
                             let lit2' =
                               ALF.apply_subst renaming subst (lit2, 0)
                             in
                             if
                               C.is_maxlit (c, 0) subst ~idx:idx1
                               && ALF.is_max ~ord lit1'
                               && ALF.is_max ~ord lit2'
                             then (
                               (*  lit1 is   a.t + l1 = l2,
                             ...
                             a'.l1 + a.l2' != a'.l2 + a.l1' *)
                               ...
                               in
                               let other_lits =
                                 CCArray.except_idx (C.lits c) idx1
                               in
                               let other_lits =
                                 Lit.apply_subst_list renaming subst
                                   (other_lits, 0)
                               in
                               let c_guard =
                                 Literal.of_unif_subst renaming us
                               in
                               (* apply subst and build clause *)
                               let all_lits =
                                 (new_lit :: c_guard) @ other_lits
                               in
                               let proof =
                                 Proof.Step.inference ~tags:[Proof.Tag.T_lia]
                               ...
                               let new_c =
                                 C.create ~trail ~penalty all_lits proof
                               in
                               Util.debugf ~section 5
                                 "@[<2>arith_eq_factoring:@ @[%a@]@ gives \
                                  @[%a@]@]" (fun k -> k C.pp c C.pp new_c);
           ...
           []
    in
    Util.exit_prof prof_arith_eq_factoring;
    res
  ...
  let _range low len =
    let rec make acc i len =
      if Z.sign len < 0 then
        acc
      else
        make (i :: acc) (Z.succ i) (Z.pred len)
    in
    make [] low len
  ...
  let _do_chaining info acc =
    ...
    Util.debugf ~section 5
      "@[<2>arith chaining@ between @[%a[%d]@]@ and @[%a[%d]@]@ (subst \
       @[%a@])...@]" (fun k ->
        k C.pp info.left s_l C.pp info.right s_r Subst.pp subst );
    (* check ordering conditions *)
    if
      C.is_maxlit (info.left, s_l) subst ~idx:idx_l
      && C.is_maxlit (info.right, s_r) subst ~idx:idx_r
      && ALF.is_max ~ord lit_l && ALF.is_max ~ord lit_r
    then (
      (* scale literals *)
      ...
      | ALF.Left (AL.Lesseq, mf_1, m1), ALF.Right (AL.Lesseq, m2, mf_2) ->
        ...
        let proof =
          Proof.Step.inference ~tags:[Proof.Tag.T_lia]
            ...
            [ C.proof_parent_subst renaming (info.left, s_l) subst;
              C.proof_parent_subst renaming (info.right, s_r) subst ]
        in
        let trail = C.trail_l [info.left; info.right] in
        (* penalty for some chaining *)
        let penalty =
          C.penalty info.left + C.penalty info.right
        ...
        let new_c = C.create ~penalty ~trail all_lits proof in
        Util.debugf ~section 5
          "@[<2>ineq chaining@ of @[%a@]@ and @[%a@]@ gives @[%a@]@]" (fun k ->
            k C.pp info.left C.pp info.right C.pp new_c );
        ...
        (* now, maybe we can also perform case switch! We can if
             ...
             Or_{i=0...k} mf_2 = m2 + i *)
        let diff =
          M.difference (M.sum m1 (MF.rest mf_2)) (M.sum m2 (MF.rest mf_1))
        in
        if
          M.is_const diff && Z.leq (M.const diff) Z.(of_int !case_switch_limit)
        then (
          ...
          let new_c = C.create ~trail ~penalty all_lits proof in
          Util.debugf ~section 5
            "@[<2>case switch@ of @[%a@]@ and @[%a@]@ gives @[%a@]@]" (fun k ->
              k C.pp info.left C.pp info.right C.pp new_c );
      ...
      |> Iter.fold
           (fun acc (t, lit, pos) ->
             ...
             | ALF.Left (AL.Lesseq, mf_l, _) ->
               ...
               PS.TermIndex.retrieve_unifiables (!_idx_ineq_right, sc_r)
                 (t, sc_l)
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                      ...
                      let lit_r =
                        Lits.View.get_arith_exn (C.lits right) right_pos
                      in
                      match lit_r with
                               ...
                               let info =
                                 ChainingInfo.
                                   { left = c;
                                     ...
                                     subst }
                               in
                               _do_chaining info acc )
                             acc
                      | _ -> acc )
                    acc
             | ALF.Right (AL.Lesseq, _, mf_r) ->
               (* find a right-chaining literal in some other clause *)
               PS.TermIndex.retrieve_unifiables (!_idx_ineq_left, sc_l)
                 (t, sc_r)
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                      ...
                      let lit_l =
                        Lits.View.get_arith_exn (C.lits left) left_pos
                      in
                      match lit_l with
                      | ALF.Left (AL.Lesseq, mf_l, _) ->
                        MF.unify_ff ~subst (mf_l, sc_l) (mf_r, sc_r)
  ...
  let canc_ineq_factoring c =
    ...
    let _do_factoring ~subst:us lit1 lit2 i j =
      ...
      | ALF.Right (AL.Lesseq, m1, mf1), ALF.Right (AL.Lesseq, m2, mf2) ->
        ...
        if
          (C.is_maxlit (c, 0) subst ~idx:i || C.is_maxlit (c, 0) subst ~idx:j)
        ...
        then (
          ...
          let other_lits =
            Lit.apply_subst_list renaming subst (other_lits, 0)
          in
          (* build new literal *)
          let new_lit =
            if left then
          ...
          Util.debugf ~section 5
            "@[<2>ineq factoring@ of @[%a@]@ gives @[%a@]@" (fun k ->
              k C.pp c C.pp new_c );
    ...
    |> Iter.iter (fun (lit1, pos1) ->
           ...
           |> Iter.iter (fun (lit2, pos2) ->
                  ...
                  | AL.Binary (AL.Lesseq, l1, r1), AL.Binary (AL.Lesseq, l2, r2)
                    ...
                    MF.unify_mm (r1, 0) (r2, 0) (fun (mf1, mf2, subst) ->
                    ...
                    MF.unify_mm (l1, 0) (l2, 0) (fun (mf1, mf2, subst) ->
  ...
  let rec _ineq_find_sufficient ~ord ~trace c lit k =
    match lit with
    ...
    | AL.Binary (AL.Lesseq, _, _) ->
      ...
      Util.debugf ~section 5
        "(@[try_ineq_find_sufficient@ :lit `%a`@ :trace (@[%a@])@])" (fun k ->
          k AL.pp lit (Util.pp_list C.pp) trace );
      ...
      |> Iter.iter (fun (t, pos) ->
             ...
             |> Iter.iter (fun (_t', with_pos, subst) ->
                    ...
                    let active_pos = with_pos.C.WithPos.pos in
                    match
                      Lits.View.get_arith (C.lits active_clause) active_pos
                    with
                    | None -> assert false
                    | Some (ALF.Left (AL.Lesseq, _, _) as alit') when is_left
                      ...
                      in
                      if
                        C.trail_subsumes active_clause c
                      ...
                      then (
                        ...
                        let mf1', m2' =
                          match
                            Lits.View.get_arith (C.lits active_clause)
                              active_pos
                          with
                        ...
                        in
                        let mf1' =
                          MF.apply_subst Subst.Renaming.none subst (mf1', 1)
                        in
                        let m2' =
                          M.apply_subst Subst.Renaming.none subst (m2', 1)
                        in
                        ...
                        let new_plit =
                          ALF.replace plit (M.difference m2' (MF.rest mf1'))
                        in
                        (* transitive closure *)
                        let trace = active_clause :: trace in
                        _ineq_find_sufficient ~ord ~trace c new_plit k )
                    | Some (ALF.Right (AL.Lesseq, _, _) as alit')
                      when not is_left ->
                      (* symmetric case *)
                      let alit' =
                        ALF.apply_subst Subst.Renaming.none subst (alit', 1)
                      ...
                      then (
                        ...
                        in
                        let mf2' =
                          MF.apply_subst Subst.Renaming.none subst (mf2', 1)
                        in
                        let m1' =
                          M.apply_subst Subst.Renaming.none subst (m1', 1)
                        in
                        let new_plit =
                          ALF.replace plit (M.difference m1' (MF.rest mf2'))
                        in
                        (* transitive closure *)
                        let trace = active_clause :: trace in
                        _ineq_find_sufficient ~ord ~trace c new_plit k )
  ...
  let _ineq_is_redundant_by_unit c lit =
    ...
    | Lit.Int (AL.Binary (AL.Lesseq, _m1, _m2) as alit) ->
      let ord = Ctx.ord () in
      let traces =
        _ineq_find_sufficient ~ord ~trace:[] c alit |> Iter.head
        (* one is enough *)
      in
      ( match traces with
      | Some (trace, _lit') ->
  ...
  let is_redundant_by_ineq c =
    ...
    let res =
      CCArray.exists
        (fun lit ->
          match _ineq_is_redundant_by_unit c lit with
          | None -> false
          | Some trace ->
            Util.debugf ~section 3
              "@[<2>clause @[%a@]@ trivial by inequations @[%a@]@]" (fun k ->
                k C.pp c (CCFormat.list C.pp) trace );
  ...
  let rec ineq_find_necessary_ ~ord ~trace c lit k =
    match lit with
    ...
    | AL.Binary (AL.Lesseq, _, _) ->
      ...
      Util.debugf ~section 5
        "(@[try_ineq_find_necessary@ :lit `%a`@ :trace (@[%a@])@])" (fun k ->
          k AL.pp lit (Util.pp_list C.pp) trace );
      AL.fold_terms ~vars:false ~which:`Max ~ord ~subterms:false lit
      |> Iter.iter (fun (t, pos) ->
             let plit = ALF.get_exn lit pos in
             ...
             |> Iter.iter (fun (_t', with_pos, subst) ->
                    ...
                    with
                    | None -> assert false
                    | Some (ALF.Left (AL.Lesseq, _, _) as alit')
                      when not is_left ->
                      let alit' =
                        ALF.apply_subst Subst.Renaming.none subst (alit', 1)
                      in
  ...
  let _ineq_is_absurd_by_unit c lit =
    ...
    | Lit.Int (AL.Binary (AL.Lesseq, _m1, _m2) as alit) ->
      let ord = Ctx.ord () in
      let traces =
        ineq_find_necessary_ ~ord ~trace:[] c alit |> Iter.head
        (* one is enough *)
      in
      ( match traces with
      | Some (trace, _lit') ->
  ...
  let demod_ineq c : C.t SimplM.t =
    ...
    let res =
      CCArray.findi
        (fun i lit ->
          match _ineq_is_absurd_by_unit c lit with
          | None -> None
          | Some trace ->
            Util.debugf ~section 3
              "@[<2>clause @[%a@]@ rewritten by inequations @[%a@]@]" (fun k ->
                k C.pp c (CCFormat.list C.pp) trace );
    ...
    let res =
      ...
      | Some (i, cs) ->
        ...
        let proof =
          Proof.Step.simp ~tags:[Proof.Tag.T_lia]
  ...
  let canc_div_chaining c =
    ...
    let _do_chaining ~sign n power c1 lit1 pos1 c2 lit2 pos2 us acc =
      ...
      (* check that we didn't "overflow", and that ordering conditions
         are good *)
      Util.debugf ~section 5
        "@[<2>div. chaining@ with @[%a@]@ between @[%a@] (at %a)@ and@ @[%a@] \
         (at %a)@]" (fun k ->
          k Subst.pp subst C.pp c1 Position.pp pos1 C.pp c2 Position.pp pos2 );
      if
        Z.lt gcd Z.(pow n power)
        && C.is_maxlit (c1, sc1) subst ~idx:idx1
        && C.is_maxlit (c2, sc2) subst ~idx:idx2
        && ALF.is_max ~ord lit1' && ALF.is_max ~ord lit2'
      then (
        let new_lit =
          Lit.mk_divides ~sign n ~power
             ...
             | ALF.Div d1 when AL.Util.is_prime d1.AL.num ->
               ...
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                      ...
                      match lit2 with
                      | ALF.Div d2
                        when (d1.AL.sign || d2.AL.sign) && Z.equal n d2.AL.num
                        ->
                        ...
                        |> Iter.fold
                             (fun acc (_, _, subst) ->
                               _do_chaining ~sign n power c lit1 pos1 c2 lit2
                                 pos2 subst acc )
                             acc
                      | _ -> acc )
                    acc
  ...
  let canc_div_case_switch c =
      ...
      |> Iter.iter (fun (lit, pos) ->
             ...
             | AL.Divides d ->
               ...
               if Z.gt n Z.one && AL.Util.is_prime n then
                 if Z.leq n (Z.of_int !div_case_switch_limit) then (
                   let idx = Lits.Pos.idx pos in
                   ...
                   for e = 0 to power - 1 do
                     for i = 1 to Z.to_int n - 1 do
                       ...
                       let m' =
                         M.add_const d.AL.monome Z.((n ** e) * of_int i)
                       in
                       let new_lit = Lit.mk_divides n ~power:(e + 1) m' in
                       lits := new_lit :: !lits
                     done
                   done;
    ...
    with ReplaceLitByLitsInSameClause (i, lits) ->
      ...
      let proof =
        Proof.Step.inference ~tags:[Proof.Tag.T_lia]
          ~rule:(Proof.Rule.mk "div_case_switch")
          [C.proof_parent c]
      in
      let new_c =
  ...
  let canc_div_prime_decomposition c =
             ...
             | AL.Divides d when d.AL.sign ->
               ...
               if Z.gt n Z.one && not (AL.Util.is_prime n) then (
                 ...
                 let lits =
                   List.map
                     (fun div ->
                       Lit.mk_divides ~sign:true div.AL.Util.prime
                         ~power:div.AL.Util.power d.AL.monome )
                     divisors
                 in
             ...
             | AL.Divides d ->
               ...
               if Z.gt n Z.one && not (AL.Util.is_prime n) then (
                 ...
                 let lits =
                   List.map
                     (fun div ->
                       Lit.mk_divides ~sign:false div.AL.Util.prime
                         ~power:div.AL.Util.power d.AL.monome )
                     divisors
                 in
                 raise (ReplaceLitByLitsInSameClause (idx, lits)) )
    ...
    | ReplaceLitByLitsInSameClause (i, lits) ->
      ...
      in
      Util.debugf ~section 5
        "@[<2>prime_decomposition- of@ @[%a@]@ into @[%a@]@]" (fun k ->
          k C.pp c C.pp new_c );
      Some [new_c]
    | ReplaceLitByLitsInManyClauses (i, lits) ->
      let clauses =
        List.map
          (fun lit ->
            ...
            all_lits.(i) <- lit;
            let proof =
              Proof.Step.inference ~tags:[Proof.Tag.T_lia] [C.proof_parent c]
                ~rule:(Proof.Rule.mk "div_prime_decomposition")
            in
            let new_c =
              C.create_a ~trail:(C.trail c) ~penalty:(C.penalty c) all_lits
                proof
            in
            new_c )
          lits
      in
      Util.debugf ~section 5
        "@[<2>prime_decomposition+@ of @[%a@]@ into set {@[%a@]}@]" (fun k ->
  ...
  let canc_divisibility c =
    ...
    let eligible =
      C.Eligible.(
        max c
        ** (filter Lit.is_arith_eq ++ (pos ** filter Lit.is_arith_divides)))
    in
    let ord = Ctx.ord () in
    let res =
      Lits.fold_arith_terms ~eligible ~which:`Max ~ord (C.lits c)
             ...
             |> Iter.fold
                  (fun acc (_, us) ->
                   ...
                   power-of-prime *)
                      match lit' with
                      | ALF.Div d ->
                        Z.sign (Z.rem (Z.pow d.AL.num d.AL.power) n) = 0
                      | _ -> true
                    then (
                      (* do the inference *)
                      Util.debugf ~section 5
                        "@[<2>divisibility@ on @[%a@]@ at @[%a@]@ with \
                         @[%a@]...@]" (fun k ->
                          k C.pp c Position.pp pos Subst.pp subst );
                      let new_lit =
                        match lit' with
                        | ALF.Left (AL.Equal, mf, m)
                        | ALF.Right (AL.Equal, m, mf) ->
                          (* remove the max term from [mf], and inject into the Z/nZ group *)
                          Lit.mk_divides n ~power:1
                            (M.difference m (MF.rest mf))
                        | ALF.Div d ->
                          assert d.AL.sign;
                          let n', power' =
                            match AL.Util.prime_decomposition n with
                      ...
                      let new_c =
                        C.create ~trail:(C.trail c) ~penalty:(C.penalty c)
                          all_lits proof
                      in
                      Util.debugf ~section 5 "@[<4>... gives@ @[%a@]@]"
                        (fun k -> k C.pp new_c);
  ...
  let canc_lit_of_lit lit =
    ...
    | Lit.Equation (l, r, sign) when Type.equal Type.int (T.ty l) ->
      ...
      | opp, T.AppBuiltin (Builtin.Remainder_e, [l'; r']) ->
        ( match Monome.Int.of_term l', T.view r', opp with
        | ( Some m,
        ...
        | _ -> None )
      | _ ->
        ( match Monome.Int.of_term l, Monome.Int.of_term r with
        | Some m1, Some m2 ->
          if sign then
            Some (Lit.mk_arith_eq m1 m2, [], [Proof.Tag.T_lia])
          else
            Some (Lit.mk_arith_neq m1 m2, [], [Proof.Tag.T_lia])
        | _, None | None, _ -> None ) )
    | _ -> None

  (** {3 Others} *)

  let _has_arith c = CCArray.exists Lit.is_arith (C.lits c)
  ...
  let is_tautology c =
    if C.get_flag flag_computed_tauto c then
      ...
      if res then (
        ...
        Util.debugf ~section 4 "@[<2>clause@ @[%a@]@ is an arith tautology@]"
          (fun k -> k C.pp c) );
  ...
  let canc_eq_resolution c =
    ...
    let is_unary_var m =
      ...
      | [(c, t)] ->
        ( match T.view t with
        | T.Var v when Z.(equal c one) && Z.(equal (M.const m) zero) -> Some v
        | _ -> None )
      | _ -> None
    in
    try
      ...
      |> Iter.iter (fun (lit, pos) ->
             match lit with
             | AL.Binary (AL.Different, m1, m2) ->
               ( match is_unary_var m1, is_unary_var m2 with
               | Some v1, Some v2 ->
                 let subst =
                   S.FO.bind S.empty
    ...
    with VarElim (i, subst) ->
      ...
      in
      Util.debugf ~section 4
        "@[<2>arith_eq_res:@ simplify @[%a@]@ into @[%a@]@]" (fun k ->
          k C.pp c C.pp c' );
      SimplM.return_new c'
  ...
  let canc_diff_to_lesseq c =
    ...
    try
      ...
      |> Iter.iter (fun (lit, i) ->
             ...
             | Lit.Int (AL.Binary (AL.Different, m1, m2))
               ...
               let proof =
                 Proof.Step.inference [C.proof_parent c] ~tags:[Proof.Tag.T_lia]
                   ~rule:(Proof.Rule.mk "arith_diff_to_lesseq")
               in
               let c' =
                 C.create ~trail:(C.trail c) ~penalty:(C.penalty c)
                   (new_lits @ lits) proof
             ...
             | _ -> assert false );
      SimplM.return_same c
    with DiffToLesseq c -> SimplM.return_new c
  ...
  let naked_vars lits =
    Literals.unshielded_vars lits ~filter:(fun var ->
        Type.equal (HVar.ty var) Type.int )
  ...
  module NakedVarElim = struct
    type t = {
      ...
      (* m1 ≤ c.x + m2 *)
      lcm : Z.t;
      (* scaling coefficient (divisibility guard) *)
      ...
      Array.fold_left
        (fun acc lit ->
          ...
          | Lit.Int o when Lit.var_occurs x lit ->
            ...
            ( match AL.Focus.focus_term (AL.negate o) (T.var x) with
            | None -> assert false
    ...
    let a_set c =
      List.concat
        [ List.map
            (function
              | ALF.Left (AL.Equal, mf, m) | ALF.Right (AL.Equal, m, mf) ->
                M.difference (M.succ m) (MF.rest mf)
              | _ -> assert false )
            c.a_lit;
          List.map
            (function
              | ALF.Left (AL.Different, mf, m) | ALF.Right (AL.Different, m, mf)
                ->
                M.difference m (MF.rest mf)
              | _ -> assert false )
            c.b_lit;
    ...
    let b_set c =
      List.concat
        [ List.map
            (function
              | ALF.Left (AL.Equal, mf, m) | ALF.Right (AL.Equal, m, mf) ->
                M.difference (M.pred m) (MF.rest mf)
              | _ -> assert false )
            c.a_lit;
          List.map
            (function
    ...
    let eval_at c x =
      ...
      :: map_lits
           (function
             ...
             | ALF.Div d ->
               Lit.mk_divides ~sign:d.AL.sign d.AL.num ~power:d.AL.power
                 (M.sum (MF.rest d.AL.monome) x) )
           c
    ...
    let eval_minus_infty c x =
      ...
      :: map_lits
           (function
             ...
             | ALF.Left (AL.Lesseq, _, _) ->
               Lit.mk_tauto
             | ALF.Left (AL.Equal, _, _)
             | ALF.Right (AL.Equal, _, _)
             | ALF.Right (AL.Lesseq, _, _) ->
               Lit.mk_absurd
             | ALF.Div d ->
               Lit.mk_divides ~sign:d.AL.sign d.AL.num ~power:d.AL.power
                 (M.sum (MF.rest d.AL.monome) x)
             | ALF.Left (AL.Less, _, _) | ALF.Right (AL.Less, _, _) ->
               assert false )
           c
  ...
  let eliminate_unshielded c =
    ...
    | x :: _ ->
      (* eliminate v *)
      Util.debugf ~section 3 "@[<2>eliminate naked variable %a@ from @[%a@]@]"
        (fun k -> k HVar.pp x C.pp c);
      ...
      if not (Z.fits_int delta) then
        None
      else (
        let delta = Z.to_int delta in
        let a_set = NVE.a_set view and b_set = NVE.b_set view in
        ...
        let add_clause ~by ~which lits =
          ...
          let proof =
            Proof.Step.inference ~tags:[Proof.Tag.T_lia] ~infos ~rule
              [C.proof_parent c]
          in
          let new_c =
            C.create ~trail:(C.trail c) ~penalty:(C.penalty c) lits proof
          in
          Util.debugf ~section 5
            "@[<2>elimination of %s×%a@ by %a (which:%s)@ in @[%a@]:@ gives \
             @[%a@]@]" (fun k ->
              k (Z.to_string view.NVE.lcm) HVar.pp x M.pp by which C.pp c C.pp
                new_c );
          acc := new_c :: !acc
        in
        (* choose which form to use *)
        if List.length a_set > List.length b_set then (
          (* use B *)
          ...
          for i = 1 to delta do
            ...
            let lits =
              view.NVE.rest @ _negate_lits (NVE.eval_minus_infty view x')
            in
          ...
          for i = 1 to delta do
            List.iter
              (fun x' ->
                ...
                let lits =
                  view.NVE.rest @ _negate_lits (NVE.eval_at view x'')
                in
          ...
          for i = 1 to delta do
            ...
            let lits =
              view.NVE.rest @ _negate_lits (NVE.eval_plus_infty view x')
            in
            add_clause ~by:x' ~which:"+∝" lits
          done;
          (* then the enumeration *)
  ...
  let setup_dot_printers () =
    CCOpt.iter
      (fun f ->
        Signal.once Signals.on_dot_output (fun () -> _print_idx f !_idx_unit_eq)
        )
      !dot_unit_;
    ()
  ...
  and post_typing_action stmts state =
    ...
    let has_int =
      ...
      |> Iter.flat_map (function
           ...
           | `Form t | `Term t -> PT.Seq.subterms t |> Iter.filter_map PT.ty )
      |> Iter.exists (PT.Ty.equal PT.Ty.int)
    in
    let should_reg = !enable_arith_ && has_int in
    Util.debugf ~section 2 "decision to register arith: %B" (fun k ->
        k should_reg );
    state
