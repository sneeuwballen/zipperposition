...
let get_unif_alg env =
  ...
  OSeq.empty
    ...
    Env.flex_get_of env Superposition.k_unif_alg
...
let get_triggers env c =
  ...
  let rec get_terms t k =
    ...
    | T.AppBuiltin ((ForallConst | ExistsConst), [ _; body ]) ->
      if Env.flex_get_of env k_include_quants then k body
    | _ -> ()
  in
...
let instantiate_w_bool env ~clause ~var ~trigger =
  ...
  let lits =
    Literals.map (fun t -> Lambda.eta_reduce ~expand_quant @@ Lambda.snf t) lits
  in
  let proof =
  ...
  in
  let res =
    C.create_a ~ctx:(Env.get_ctx env) lits proof ~penalty:(C.penalty clause)
      ~trail:(C.trail clause)
  in
...
let insert_new_trigger env t =
  let do_insert t =
    inst_clauses_w_trigger env t |> CCList.to_iter |> Env.add_passive env
  in
  do_insert t;
  if Type.returns_prop (T.ty t) then (
    let t = Lambda.eta_expand t in
    match Env.flex_get_of env k_generalize_trigger with
    | `Neg ->
...
let update_triggers env cl =
  ...
  if C.proof_depth cl < Env.flex_get_of env k_trigger_bool_inst then (
    let new_triggers = get_triggers env cl in
    if not (Iter.is_empty new_triggers) then
      Iter.iter (insert_new_trigger env) new_triggers
  );
...
let handle_new_skolem_sym env (c, trigger) =
  let trig_hd = T.head_term trigger in
  assert (T.is_const trig_hd);
  assert (Name.is_postcnf_skolem (T.as_const_exn trig_hd));

  if C.proof_depth c < Env.flex_get_of env k_trigger_bool_inst then
    insert_new_trigger env trigger;

  Signal.ContinueListening

let trigger_induction env cl =
  ...
  let make_triggers lhs rhs sign =
    ...
    let immediate_args =
      if T.is_const (T.head_term lhs_body) && T.is_const (T.head_term rhs_body)
      then
  ...
  if
    C.proof_depth cl < Env.flex_get_of env k_trigger_bool_ind
...
let mk_res env ~proof ~old ~repl new_lit c =
...
let get_green_eligible env c =
  let ord = C.Ctx.ord (Env.get_ctx env) in
...
let get_bool_eligible env c =
  Iter.append
    (SClause.TPSet.to_iter (C.eligible_subterms_of_bool c))
    (get_green_eligible env c)

let get_bool_hoist_eligible env c =
...
let bool_hoist env (c : C.t) : C.t list =
  let proof =
    Proof.Step.inference
      [ C.proof_parent c ]
      ~rule:(Proof.Rule.mk "bool_hoist")
      ~tags:[ Proof.Tag.T_ho ]
  in
  ...
  List.map
    (fun (t, _) ->
      ...
      mk_res env ~proof ~old:t ~repl:T.false_ (yes t) c)
    (get_bool_hoist_eligible env c)
  |> CCFun.tap (fun res ->
         Util.debugf ~section 3 "hoist(@[%a@])" (fun k -> k C.pp c);
         if CCList.is_empty res then
           Util.debugf ~section 3 " = ∅ (%d)(%a)(%d)" (fun k ->
               k
                 (Iter.length (get_green_eligible env c))
                 (Iter.pp_seq Term.pp)
                 (Iter.map fst (get_bool_eligible env c))
                 (List.length (get_bool_hoist_eligible env c)))
         else
           Util.debugf ~section 3 " = @[%a@]" (fun k -> k (CCList.pp C.pp) res))

let bool_hoist_simpl env (c : C.t) : C.t list option =
  let proof =
    Proof.Step.simp
      [ C.proof_parent c ]
      ~rule:(Proof.Rule.mk "bool_hoist")
  ...
  let bool_subterms = get_bool_hoist_eligible env c in
  CCOpt.return_if
    ...
    (CCList.fold_left
       (fun acc (t, _) ->
         ...
         mk_res env ~proof ~old:t ~repl:repl_neg neg_lit c
         :: mk_res env ~proof ~old:t ~repl:repl_pos pos_lit c
         :: acc)
       [] bool_subterms)
  |> CCFun.tap (function
       | Some res ->
...
let eq_hoist env (c : C.t) : C.t list =
  let proof ~prefix =
    Proof.Step.inference
      [ C.proof_parent c ]
      ~rule:(Proof.Rule.mk (prefix ^ "_hoist"))
      ~tags:[ Proof.Tag.T_ho ]
  in
  ...
  |> List.map (fun (t, _) ->
         ...
         | T.AppBuiltin (Builtin.(Eq | Equiv), ([ a; b ] | [ _; a; b ])) ->
           ...
           mk_res env ~proof:(proof ~prefix:"eq") ~old:t ~repl:T.false_ new_lit
             c
         | T.AppBuiltin (Builtin.(Neq | Xor), ([ a; b ] | [ _; a; b ])) ->
           let new_lit = Literal.mk_eq a b in
           mk_res env ~proof:(proof ~prefix:"neq") ~old:t ~repl:T.true_ new_lit
             c
         | _ -> assert false)
  |> CCFun.tap (fun res ->
         ...
         if CCList.is_empty res then
           Util.debugf ~section 3 " = ∅ (%d)(%d)(%a)" (fun k ->
               k
                 (Iter.length (get_green_eligible env c))
                 (Iter.length (get_bool_eligible env c))
                 (Iter.pp_seq Term.pp)
                 (Iter.map fst (get_bool_eligible env c)))
         else
           Util.debugf ~section 3 " = @[%a@]" (fun k -> k (CCList.pp C.pp) res))

let fluid_hoist env (c : C.t) =
      ...
      let res =
        C.create ~ctx:(C.ctx_of c)
          ~penalty:
            ...
            +
            if Proof.Step.has_ho_step (C.proof_step c) then
              3
            else
              1)
          ~trail:(C.trail c)
  ...
  get_bool_eligible env c |> fun iter ->
  Iter.fold
    (fun acc (u, p) ->
      if T.is_app_var u || (T.is_fun u && not (T.is_ground u)) then (
        let unif_seq =
          get_unif_alg env (zx, sc_zx) (u, sc_cl)
        ...
        if Env.should_force_stream_eval env () then
          Env.get_finite_infs env [ unif_seq ] @ acc
        else (
          let stm_res =
...
let fluid_log_hoist env (c : C.t) =
  ...
  let y_quant =
    Lambda.eta_expand @@ T.var (HVar.fresh ~ty:(Type.arrow [ a ] Type.prop) ())
  in
  ...
  let partners =
    [
      {
        unif_partner = F.eq x_a y_a;
        repl = T.false_;
        new_lit = Some (L.mk_eq x_a y_a);
      };
      {
        unif_partner = F.neq x_a y_a;
        repl = T.true_;
        new_lit = Some (L.mk_eq x_a y_a);
      };
      {
        ...
        repl = T.false_;
        new_lit = Some (L.mk_eq yx T.true_);
      };
      {
        unif_partner = F.exists y_quant;
        repl = T.true_;
        new_lit = Some (L.mk_eq yx T.false_);
      };
      ...
      {
        unif_partner = F.and_ T.false_ T.false_;
        repl = T.false_;
        new_lit = None;
      };
      {
        unif_partner = F.and_ T.true_ T.false_;
        repl = T.false_;
        new_lit = None;
      };
      {
        unif_partner = F.and_ T.false_ T.true_;
        repl = T.false_;
        new_lit = None;
      };
      { unif_partner = F.and_ T.true_ T.true_; repl = T.true_; new_lit = None };
      {
        unif_partner = F.or_ T.false_ T.false_;
        repl = T.false_;
        new_lit = None;
      };
      ...
      {
        unif_partner = F.imply T.false_ T.false_;
        repl = T.true_;
        new_lit = None;
      };
      {
        unif_partner = F.imply T.true_ T.false_;
        repl = T.false_;
        new_lit = None;
      };
      {
        unif_partner = F.imply T.false_ T.true_;
        repl = T.true_;
        new_lit = None;
      };
  ...
  let mk_res sub at partner =
    ...
    let new_lits =
      CCOpt.map_or ~default:[]
        (fun x -> [ L.apply_subst renaming sub (x, sc_partner) ])
        partner.new_lit
      @ Array.to_list lits
    in
    ...
    [
      {
        unif_partner = F.forall y_quant;
        repl =
          T.fun_ a (F.not_ (Lambda.whnf @@ T.app y_quant [ T.bvar ~ty:a 0 ]));
        new_lit = None;
      };
      {
        unif_partner = F.exists y_quant;
...
let false_elim env c =
  ...
  let schedule app_var target idx =
    ...
    let seq =
      get_unif_alg env (app_var, 0) (target, 0)
      |> OSeq.filter_map
           (CCOpt.map (fun us ->
                assert (not (Unif_subst.has_constr us));
                let sub = Unif_subst.subst us in
                let renaming = Subst.Renaming.create () in
                let res =
                  C.apply_subst ~penalty_inc:1 ~renaming ~proof:(p sub renaming)
                    (c, 0) sub
                in
                (* not eligible under substitution *)
  ...
  |> Iter.fold
       (fun acc (lhs, rhs, sign, p) ->
         ...
         if T.equal T.true_ rhs then
           ...
           T.is_var (T.head_term lhs) && T.is_var (T.head_term rhs) && sign
         then
...
let replace_bool_vars env (c : C.t) =
  let p =
    Proof.Step.simp
      [ C.proof_parent c ]
...
let replace_bool_app_vars env (c : C.t) =
  let p sub renaming =
    Proof.Step.simp
      [ C.proof_parent_subst renaming (c, 0) sub ]
  ...
  let is_eligible xs =
    CCOpt.return_if
      (List.for_all is_var_headed xs && List.exists T.is_app_var xs)
      xs
  in
  ...
  let create_targets n =
    let rec aux = function
      ...
      | n ->
        let rest = aux (n - 1) in
        List.rev_append
          (List.rev_map (List.cons T.true_) rest)
  ...
  let stms =
    ...
    |> Iter.flat_map_l (fun args ->
           List.map
             (fun target ->
               get_unif_alg_l env (args, 0) (target, 0)
               |> OSeq.filter_map
                    (CCOpt.map (fun us ->
                         assert (not (Unif_subst.has_constr us));
                         let sub = Unif_subst.subst us in
  ...
  in
  if Env.should_force_stream_eval env () then
    Env.get_finite_infs env stms
  else (
    Env.StmQ.add_lst (Env.get_stm_queue env)
      (List.map
...
let quantifier_rw_and_hoist env (c : C.t) =
  let quant_rw ~at b body =
    let quant_rw_unapplicable =
      let module P = Pos in
      match at with
      | (P.Arg (i, P.Left P.Stop) | P.Arg (i, P.Right P.Stop))
        when L.is_predicate_lit (C.lits c).(i) ->
    ...
    if quant_rw_unapplicable then
      ...
      let proof =
        Proof.Step.simp
          ...
          ~tags:[ Proof.Tag.T_ho ]
      in
      let body = Combs.expand env body in
      let form_for_skolem =
      ...
      Some
        (C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail c)
           ~penalty:(C.penalty c) (Array.to_list new_lits) proof)
    )
  in
  ...
  let quant_hoist ~old b body =
    ...
    | Builtin.ForallConst ->
      ...
      let res =
        mk_res env ~proof:(proof ~prefix:"forall") ~old ~repl:T.false_ new_lit c
      in
      if Type.returns_prop (T.ty subst_t) then
        Signal.send (Env.on_pred_var_elimination env) (res, subst_t);
      res
    | Builtin.ExistsConst ->
      let new_lit = no (T.app body [ subst_t ]) in
      let res =
        mk_res env ~proof:(proof ~prefix:"exists") ~old ~repl:T.true_ new_lit c
      in
      if Type.returns_prop (T.ty subst_t) then
        Signal.send (Env.on_pred_var_elimination env) (res, subst_t);
      res
...
let nested_eq_rw env c =
  ...
  |> Iter.filter_map (fun (t, p) ->
         ...
         | T.AppBuiltin
           ...
           ->
           Some
             (get_unif_alg env (mk_sc a) (mk_sc b)
  ...
  |> Iter.flat_map_l (fun clause_seq ->
         if Env.should_force_stream_eval env () then
           Env.get_finite_infs env [ clause_seq ]
         else (
           let stm_res =
...
let simplify_bools env t =
  ...
  let simplify_and_or t b l =
    ...
    let res =
      ...
      if compl_in_l l || List.exists (equal absorbing_el) l then
        absorbing_el
      else (
        let l' = List.filter (fun s -> not (equal s netural_el)) l' in
        if List.length l = List.length l' then
          t
        else if CCList.is_empty l' then
          netural_el
  ...
  let rec aux t =
    ...
    | AppBuiltin (Builtin.Not, [ s ]) ->
      ...
      | _ ->
        if T.equal s' T.true_ then
          T.false_
        else if T.equal s' T.false_ then
          T.true_
    ...
    *)
      ...
      if not (T.Set.is_empty (T.Set.inter premises conclusions)) then
        T.true_
      else if T.equal p' c' then
        T.true_
      else if T.equal c' (negate p') then
        c'
      else if T.equal p' (negate c') then
        c'
      else if T.equal p' T.true_ then
        c'
      else if T.equal p' T.false_ then
        T.true_
      else if T.equal c' T.false_ then
        aux (T.Form.not_ p')
      else if T.equal c' T.true_ then
        T.true_
    ...
    | AppBuiltin (((Builtin.Eq | Builtin.Equiv) as hd), ([ a; b ] | [ _; a; b ]))
      ...
      if T.equal a' b' then
        T.true_
      else if T.equal a' T.true_ then
        b'
      else if T.equal b' T.true_ then
        a'
      else if T.equal a' T.false_ then
        aux (T.Form.not_ b')
      else if T.equal b' T.false_ then
        aux (T.Form.not_ a')
    ...
    | AppBuiltin (((ExistsConst | ForallConst) as b), [ tyarg; g ]) ->
      let g' = aux g in
      let exp_g = Combs.expand env g' in
      ...
      if T.Seq.subterms ~include_builtin:true body |> Iter.exists T.is_bvar then
        if T.equal g g' then
          ...
          T.app_builtin ~ty:(T.ty t) b [ tyarg; g' ]
      else
        body
    | AppBuiltin (hd, args) ->
...
let fix_unsupported_quant env t =
    ...
    | AppBuiltin (hd, args) ->
      ...
      if Builtin.is_quantifier hd && List.length args' == 2 then (
        ...
        if not (quant_normal var_ty q_body) then
          ...
          T.app_builtin ~ty:(T.ty t) hd args'
      ) else if T.same_l args args' then
        t
      else
        T.app_builtin ~ty:(T.ty t) hd args'
    | DB _ | Const _ | Var _ -> t
...
let replace_unsupported_quants env c =
  let new_lits = Literals.map (fix_unsupported_quant env) (C.lits c) in
...
let simpl_bool_subterms env c =
  try
    let new_lits = Literals.map (simplify_bools env) (C.lits c) in
    if Literals.equal (C.lits c) new_lits then
      ...
      | AppBuiltin (((ForallConst | ExistsConst) as b), ([ g ] | [ _; g ])) ->
        let flipped =
          if b = Builtin.ForallConst then
            ...
            Builtin.ForallConst
        in
        let g_ty_args, g_body = T.open_fun (Combs.expand env g) in
...
let nnf_bool_subters env c =
  let new_lits = Literals.map (nnf_bools env) (C.lits c) in
  if Literals.equal (C.lits c) new_lits then
    SimplM.return_same c
  else (
    let proof =
...
let normalize_bool_terms env c =
...
let solve_bool_formulas ~which env c =
  let normalize_not t =
    let rec aux t =
      match T.view t with
      | T.AppBuiltin (Not, [ f ]) ->
  ...
  let find_resolvable_form lit =
    ...
    let find_pos_var_headed_eq l r =
      if is_var_headed l && not (is_var_headed r) then
        Some (l, T.Form.not_ r)
      else if is_var_headed r && not (is_var_headed l) then
        Some (r, T.Form.not_ l)
      else
        None
    in
    ...
    | Some (l, r, sign) ->
      if (not (T.is_true_or_false r)) && Type.is_prop (T.ty l) then
        if (not sign) && which = `All then
          Some (l, r)
        else if sign then
          find_pos_var_headed_eq l r
        else
          None
  ...
  let unif_alg l r =
    if not (Env.flex_get_of env Combinators.k_enable_combinators) then
      Env.flex_get_of env Superposition.k_unif_alg (l, 0) (r, 0)
    else
...
let cnf_otf env c : C.t list option =
  let idx =
    CCArray.find_idx
      (fun l ->
  ...
  let max_formula_weight =
    C.Seq.terms c |> Iter.filter T.is_formula |> Iter.map T.size |> Iter.max
  in
  let opts =
    match max_formula_weight with
    | None -> [ Cnf.DisableRenaming ]
    | Some m ->
      if m < renaming_weight then
        [ Cnf.DisableRenaming ]
      else
        []
  in
  ...
  | Some _ ->
    ...
    let proof =
      Proof.Step.simp ~rule:(Proof.Rule.mk "cnf_otf") ~tags:[ Proof.Tag.T_ho ]
        [ C.proof_parent c ]
    in
    ...
    let cnf_vec =
      ...
      @@ Cnf.cnf_of ~opts ~ctx:(Env.Ctx.sk_ctx (Env.get_ctx env)) stmt
    in
    CCVector.iter
      (fun cl ->
      ...
      cnf_vec;
    let solved =
      if Env.flex_get_of env k_solve_formulas then
        CCOpt.get_or ~default:[] (solve_bool_formulas ~which:`All env c)
      else
        []
    in
    ...
    let clauses =
      CCVector.map
        (C.of_statement ~ctx:(Env.get_ctx env) ~convert_defs:true)
        cnf_vec
    ...
    List.iteri
      (fun i new_c -> assert (C.proof_depth c <= C.proof_depth new_c))
      clauses;
    Some (solved @ clauses)
  | None -> None

let cnf_infer env cl = CCOpt.get_or ~default:[] (cnf_otf env cl)

let interpret_boolean_functions env c =
           ...
           let forall_cl =
             C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail c)
               ...
               :: Array.to_list
                    (C.lits c
                    |> Literals.map
                         (T.replace ~old:t ~by:(interpret t' T.true_))))
               proof
           in
           let forall_neg_cl =
             C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail c)
               ...
               :: Array.to_list
                    (C.lits c
                    |> Literals.map
                         (T.replace ~old:t ~by:(interpret t' T.false_))))
               proof
           in

           Util.debugf ~section 5
             "interpret bool(@[%a@]):@.@[%a@] !!> @. @[%a@]@." (fun k ->
               k T.pp t Literals.pp (C.lits c) Literals.pp (C.lits forall_cl));
           Util.debugf ~section 5
             "interpret bool(@[%a@]):@.@[%a@] !!> @. @[%a@]@." (fun k ->
               k T.pp t Literals.pp (C.lits c) Literals.pp
                 (C.lits forall_neg_cl));
    ...
    Signal.once (Env.on_start env) (fun () ->
        ...
        |> Clause.ClauseSet.iter (fun cl ->
               ...
               | Some new_ ->
                 ...
                 Env.add_passive env (Iter.singleton new_)));
  match Env.flex_get_of env k_bool_reasoning with
  | BoolReasoningDisabled -> ()
  | BoolCasesPreprocess -> Env.add_unary_inf env "false_elim" false_elim
  | _ ->
    if Env.flex_get_of env k_solve_formulas then
      Env.add_unary_inf env "solve formulas" (fun env c ->
          CCOpt.get_or ~default:[]
          @@ solve_bool_formulas ~which:`OnlyPositive env c);
    if
      Env.flex_get_of env k_trigger_bool_inst > 0
      || Env.flex_get_of env k_trigger_bool_ind > 0
    then (
      Signal.on (Env.on_pred_var_elimination env) (fun (clause, var) ->
          handle_new_pred_var_clause env (clause, var));
      Signal.on FR.on_pred_skolem_introduction (handle_new_skolem_sym env)
    );
    if Env.flex_get_of env k_trigger_bool_ind > 0 then
      Env.add_unary_inf env "trigger bool ind" trigger_induction;
    if Env.flex_get_of env k_simplify_bools then
      Env.add_basic_simplify env simpl_bool_subterms;
    if Env.flex_get_of env k_nnf then
    ...
    if not !Lazy_cnf.enabled then (
      ...
      if Env.flex_get_of env k_cnf_non_simpl then
        ...
        Env.add_multi_simpl_rule env ~priority:2 cnf_otf
    );
    if Env.flex_get_of env k_interpret_bool_funs then
      Env.add_unary_inf env "interpret boolean functions"
        interpret_boolean_functions;

    Env.add_unary_inf env "false_elim" false_elim;
    if Env.flex_get_of env k_bool_reasoning = BoolHoist then (
      if Env.flex_get_of env k_bool_hoist_simpl then
        Env.add_multi_simpl_rule env ~priority:1000 bool_hoist_simpl;
      Env.add_unary_inf env "bool_hoist" bool_hoist;

      if Env.flex_get_of env k_rename_nested_bools then
        Env.add_multi_simpl_rule env ~priority:500 rename_nested_booleans;

      Env.add_unary_inf env "formula_hoist" eq_hoist;
      Env.add_multi_simpl_rule env ~priority:100 replace_bool_vars;
      Env.add_multi_simpl_rule env ~priority:90 quantifier_rw_and_hoist;
      Env.add_unary_inf env "eq_rw" nested_eq_rw;

      if Env.flex_get_of env Superposition.k_ho_basic_rules then (
        if Env.flex_get_of env k_bool_app_var_repl then
          Env.add_unary_inf env "replace_bool_app_vars" replace_bool_app_vars;
        if Env.flex_get_of env k_fluid_hoist then
          Env.add_unary_inf env "fluid_hoist" fluid_hoist;
        if Env.flex_get_of env k_fluid_log_hoist then (
          Env.add_unary_inf env "fluid_log_hoist" fluid_log_hoist;
          Env.add_unary_inf env "fluid_quant_rw" fluid_quant_rw
        )
      )
    )
...
let if_changed proof (mk : ?attrs:Logtk.Statement.attrs -> 'r) s f p =
  ...
  if fp == [ p ] then
    ...
    map (fun x -> mk ~proof:(proof s) x) fp
(* match fp with 
...
let name_quantifiers stmts =
  let proof s =
    Proof.Step.esa
      [ Proof.Parent.from (Statement.as_proof_i s) ]
    ...
    replaceTST
      (fun t ->
        ...
        | Bind (Binder.Forall, _, _) | Bind (Binder.Exists, _, _) ->
          ...
          let proof =
            Proof.Step.define_internal qid
              [ Proof.Parent.from (Statement.as_proof_i s) ]
          in
...
let rec replace old by t =
  ...
  let ty = ty_exn t in
  if TypedSTerm.equal t old then
    by
  else (
    match view t with
...
let rec case_bools_wrt vs t =
  with_subterm_or_id t (fun _ s ->
      ...
      | App (f, ps) ->
        ...
        if TypedSTerm.equal t t' then
          None
        else
          Some (case_bools_wrt vs t')
      | _ -> None)
      ...
      [
        with_subterm_or_id t (fun vs s ->
            ...
            | Bind (((Forall | Exists) as q), v, b) ->
              ...
              if TypedSTerm.equal b b' then
                None
              else
                Some (replace s (bind ~ty:prop q v b') t)
            | _ -> None)
    ...
    let rec aux ~top p =
      ...
      | Bind (Binder.Lambda, var, body) ->
        CCOpt.map
          ...
          (aux ~top:false body)
      | App (hd, args)
        when (not top) && T.Ty.is_prop p_ty && no_leaky_variables p ->
        return p
      | Const _ when (not top) && T.Ty.is_prop p_ty -> return p
      | AppBuiltin (b, args) ->
        CCOpt.map
          (fun (args_t, args_f, s) ->
      ...
      | AppBuiltin (((Builtin.Neq | Builtin.Eq) as hd), ([ _; a; b ] | [ a; b ]))
        ...
        | None ->
          ...
          | Some (b_t, b_f, subterm) ->
            ...
            let if_true =
              T.Form.or_ [ T.Form.not_ subterm'; aux ~vars @@ cons a b_t ]
            in
        ...
        | Some (a_t, a_f, subterm) ->
          let subterm' = aux ~vars subterm in
          let if_true =
            T.Form.or_ [ T.Form.not_ subterm'; aux ~vars @@ cons a_t b ]
          in
...
let post_eager_cases =
  let proof s =
    Proof.Step.esa
      [ Proof.Parent.from (Statement.as_proof_c s) ]
             ...
             %> Iter.fold
                  (fun res (s, d) ->
                    ...
                    then
                      ...
                      let replace_s_by by =
                        map (SLiteral.map ~f:(replace ~old:s ~by))
                      in
                      flatten
                        (map
                           (fun c ->
...
let preprocess_booleans stmts =
  ...
  | _ -> id)
    (if !_quant_rename then
       name_quantifiers stmts
     else
       stmts)
