...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let get_unif_alg () =
    if Env.flex_get k_disable_ho_bool_unif then fun _ _ -> OSeq.empty else Env.flex_get Superposition.k_unif_alg

  let get_unif_alg_l () =
    if Env.flex_get k_disable_ho_bool_unif then fun _ _ -> OSeq.empty
    else
  ...
  let get_triggers c =
    ...
    let rec get_terms t k =
      ...
      | _ ->
          ()
    in
    Literals.fold_terms ~ord ~subterms:false ~eligible:C.Eligible.always ~which:`All (C.lits c) ~fun_bodies:false
  ...
  let inst_clauses_w_trigger t =
    ...
    if
      ...
      && ((not (Env.flex_get k_bool_triggers_only)) || Type.returns_prop (T.ty t))
    then (
      _trigger_bools :=
        Type.Map.update (T.ty t) (function None -> Some [t] | Some res -> Some (t :: res)) !_trigger_bools ;
  ...
  let insert_new_trigger t =
    let do_insert t = inst_clauses_w_trigger t |> CCList.to_iter |> Env.add_passive in
    do_insert t ;
  ...
  let update_triggers cl =
    ...
    ( if C.proof_depth cl < Env.flex_get k_trigger_bool_inst then
        ...
        if not (Iter.is_empty new_triggers) then Iter.iter insert_new_trigger new_triggers ) ;
  ...
  let handle_new_pred_var_clause (clause, var) =
    ...
    _cls_w_pred_vars :=
      Type.Map.update ty
        (function None -> Some [(clause, var)] | Some res -> Some ((clause, var) :: res))
        !_cls_w_pred_vars ;
    Signal.ContinueListening
  ...
  let handle_new_skolem_sym (c, trigger) =
    ...
    if C.proof_depth c < Env.flex_get k_trigger_bool_inst then insert_new_trigger trigger ;
    Signal.ContinueListening
  ...
  let trigger_induction cl =
    ...
    let abstract ~subterm t =
      ...
      let rec aux ~depth t =
        if T.equal subterm t then T.bvar ~ty:(T.ty subterm) depth
        else
          match T.view t with
          | T.App (hd, args) ->
              let hd' = aux ~depth hd in
              let args' = List.map (aux ~depth) args in
              if T.equal hd hd' && T.same_l args args' then t else T.app hd' args'
      ...
      if T.equal res t then None else Some res
    in
    let make_triggers lhs rhs sign =
      let lhs_body, rhs_body = (snd (Term.open_fun lhs), snd (Term.open_fun rhs)) in
      let immediate_args =
        if T.is_const (T.head_term lhs_body) && T.is_const (T.head_term rhs_body) then
      ...
      CCList.filter_map
        ...
        immediate_args
    in
    if C.proof_depth cl < Env.flex_get k_trigger_bool_ind && CCOpt.is_some (C.distance_to_goal cl) then
      match C.lits cl with
      | [|Literal.Equation (lhs, rhs, _) as lit|] ->
          let res = CCList.flat_map inst_clauses_w_trigger (make_triggers lhs rhs (Literal.is_positivoid lit)) in
          res
      | _ ->
          []
    else []
  ...
  let get_green_eligible c =
    ...
    Ls.fold_terms ~vars:false ~var_args:false ~fun_bodies:false ~ty_args:false ~ord ~which:`Max ~subterms:true
      ~eligible:(C.Eligible.res c) (C.lits c)

  let get_bool_eligible c =
    Iter.append (SClause.TPSet.to_iter (C.eligible_subterms_of_bool c)) (get_green_eligible c)
  ...
  let get_bool_hoist_eligible c =
    ...
    |> Iter.filter (fun (t, p) ->
           ...
           | P.Arg (idx, P.Left P.Stop) | P.Arg (idx, P.Right P.Stop) -> (
             match (C.lits c).(idx) with L.Equation (_, _, false) -> true | _ -> false )
           | _ ->
               true )
    |> Iter.filter (fun (t, _) ->
           let ty = T.ty t in
           ...
           | T.AppBuiltin (hd, _) ->
               ...
               not (List.mem hd Builtin.[Eq; Neq; ForallConst; ExistsConst; Not] || Builtin.is_logical_binop hd)
           | T.App (hd, _) ->
               not @@ T.is_var hd
           | _ ->
               true )
  ...
  let bool_hoist (c : C.t) : C.t list =
    let proof =
      Proof.Step.inference [C.proof_parent c] ~rule:(Proof.Rule.mk "bool_hoist") ~tags:[Proof.Tag.T_ho]
    in
    List.map
      (fun (t, _) ->
  ...
  let bool_hoist_simpl (c : C.t) : C.t list option =
    let proof = Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "bool_hoist") ~tags:[Proof.Tag.T_ho] in
    let bool_subterms = get_bool_hoist_eligible c in
    CCOpt.return_if
      (not @@ CCList.is_empty bool_subterms)
      (CCList.fold_left
         (fun acc (t, _) ->
           let t, c = handle_poly_bool_hoist t c in
           let neg_lit, repl_neg = (no t, T.true_) in
           let pos_lit, repl_pos = (yes t, T.false_) in
           mk_res ~proof ~old:t ~repl:repl_neg neg_lit c :: mk_res ~proof ~old:t ~repl:repl_pos pos_lit c :: acc
           )
         [] bool_subterms )
    |> CCFun.tap (function
         | Some res ->
  ...
  let eq_hoist (c : C.t) : C.t list =
    let proof ~prefix =
      Proof.Step.inference [C.proof_parent c] ~rule:(Proof.Rule.mk (prefix ^ "_hoist")) ~tags:[Proof.Tag.T_ho]
    in
    get_bool_eligible c
    |> Iter.filter (fun (t, _) ->
           match T.view t with
           | T.AppBuiltin (hd, _) ->
               List.mem hd [Builtin.Eq; Neq; Xor; Equiv; ForallConst; ExistsConst] && Type.is_prop (T.ty t)
           | _ ->
               false )
    |> Iter.to_list
    (* since we are doing simultaneous version -- we take only unique terms *)
  ...
  let fluid_hoist (c : C.t) =
    ...
    let mk_res sign renaming sub at =
      ...
      let still_at_eligible = get_bool_eligible c' |> Iter.exists (fun (_, p) -> Position.equal p at) in
      if still_at_eligible then (
        let new_lit =
          let x_sub = Subst.FO.apply renaming sub (x, sc_zx) in
          Literal.mk_eq x_sub (if sign then T.true_ else T.false_)
        in
        let by = Subst.FO.apply renaming sub ((if sign then z_false else z_true), sc_zx) in
        Literals.Pos.replace (C.lits c') ~at ~by ;
        let rule = Proof.Rule.mk ("fluid_" ^ (if sign then "bool_" else "loob_") ^ "hoist") in
        let proof = Proof.Step.inference ~rule [C.proof_parent_subst renaming (c, sc_cl) sub] in
        let res =
          C.create
            ~penalty:(C.penalty c + C.proof_depth c + if Proof.Step.has_ho_step (C.proof_step c) then 3 else 1)
            ~trail:(C.trail c)
            (new_lit :: CCArray.to_list (C.lits c'))
            proof
        in
        Some res )
      else None
            ...
            |> OSeq.flat_map (fun us_opt ->
                   CCOpt.map_or ~default:OSeq.empty
                     (fun us ->
                       ...
                       let renaming = Subst.Renaming.create () in
                       let z_false_sub = Lambda.snf @@ Subst.FO.apply renaming sub (z_false, sc_zx) in
                       let z_true_sub = Lambda.snf @@ Subst.FO.apply renaming sub (z_true, sc_zx) in
                       let x_sub = Lambda.snf @@ Subst.FO.apply renaming sub (x, sc_zx) in
                       let zx_sub = Lambda.snf @@ Subst.FO.apply renaming sub (zx, sc_zx) in
                       if T.is_true_or_false x_sub then OSeq.empty
                       else
                         let bool_res =
                           if T.equal z_false_sub zx_sub then [] else [mk_res true renaming sub p]
                         in
                         let loob_res =
                           if T.equal z_true_sub zx_sub then [] else [mk_res false renaming sub p]
                         in
                         OSeq.of_list (bool_res @ loob_res) )
                     us_opt )
          in
          if Env.should_force_stream_eval () then Env.get_finite_infs [unif_seq] @ acc
          else
  ...
  let fluid_log_hoist (c : C.t) =
    ...
    let y_quant = Lambda.eta_expand @@ T.var (HVar.fresh ~ty:(Type.arrow [a] Type.prop) ()) in
    ...
    let mk_res sub at partner =
      ...
      C.create
        ~penalty:(C.penalty c + C.proof_depth c + if Proof.Step.has_ho_step (C.proof_step c) then 2 else 0)
        ~trail:(C.trail c) new_lits step
    in
    ...
    |> Iter.fold
         (fun acc (lit, idx) ->
           ...
           match lit with
           | Literal.Equation (u, v, true) when (Type.is_prop (T.ty u) || Type.is_var (T.ty u)) && T.is_app_var u
             ->
               let app_vars =
                       ...
                       let seq =
                         ...
                         |> OSeq.filter_map
                              (CCOpt.map (fun us ->
                                   ...
                                   if not (CCBV.get eligible' idx) then None else Some (mk_res sub pos p) ) )
                       in
                       if Env.should_force_stream_eval () then Env.get_finite_infs [seq] @ acc
                       else
  ...
  let fluid_quant_rw (c : C.t) =
    ...
    let partners =
      [ { unif_partner= F.forall y_quant
        ...
        ; new_lit= None }
      ; {unif_partner= F.exists y_quant; repl= y_quant (*already eta-expanded *); new_lit= None} ]
    in
    let mk_res sub at partner =
      ...
      let expand_quant = not @@ Env.flex_get Combinators.k_enable_combinators in
      let repl_sub =
        Lambda.eta_reduce ~expand_quant @@ Lambda.snf @@ Subst.FO.apply renaming sub (partner.repl, sc_partner)
      in
      let sk = FR.get_skolem ~parent:c ~mode:`SkolemRecycle repl_sub in
      let y_sk = T.app (Subst.FO.apply renaming sub (y_quant, sc_partner)) [sk] in
      ...
      Util.debugf ~section 5 "fluid_quant_rw:@.@[%a@] -> @.@[%a@]@." (fun k -> k C.pp c C.pp res) ;
      res
    in
    let eligible = C.Eligible.res c in
    Literals.fold_lits ~eligible (C.lits c)
    |> Iter.fold
         (fun acc (lit, idx) ->
           let lit_pos = PB.arg idx PB.empty in
           match lit with
           | Literal.Equation (u, v, true) when (Type.is_var (T.ty u) || Type.is_prop (T.ty u)) && T.is_app_var u
             ->
               let app_vars =
                 if Literal.is_predicate_lit lit then [(u, PB.to_pos (PB.left lit_pos))]
                 else if Term.is_app_var v then
  ...
  let false_elim c =
    ...
    let p sub renaming =
      Proof.Step.inference
        ...
        ~rule:(Proof.Rule.mk "false_elim") ~tags:[Proof.Tag.T_ho]
    in
    let add_immediate acc sub idx =
      let renaming = Subst.Renaming.create () in
      let res = C.apply_subst ~renaming ~proof:(Some (p sub renaming)) (c, 0) sub in
      res :: acc
    in
      ...
      let seq =
        ...
        |> OSeq.filter_map
             (CCOpt.map (fun us ->
                  ...
                  let renaming = Subst.Renaming.create () in
                  let res =
                    C.apply_subst ~penalty_inc:(Some 1) ~renaming ~proof:(Some (p sub renaming)) (c, 0) sub
                  in
                  (* not eligible under substitution *)
                  if not @@ CCBV.get (C.eligible_res_no_subst res) idx then None else Some res ) )
      in
      if Env.should_force_stream_eval () then Env.get_finite_infs [seq]
      else
         ...
         (fun acc (lhs, rhs, sign, p) ->
           ...
           if T.equal T.true_ rhs then
             if T.is_var lhs then
               let sub = S.bind' Subst.empty (get_var lhs, 0) (T.false_, 0) in
               add_immediate acc sub idx
             else if T.is_app_var lhs then schedule lhs T.false_ idx @ acc
             else acc
           else if T.equal T.false_ rhs then
             if T.is_var lhs then
               let sub = S.bind' Subst.empty (get_var lhs, 0) (T.true_, 0) in
               add_immediate acc sub idx
             else if T.is_app_var lhs then schedule lhs T.true_ idx @ acc
             else acc
           else if T.is_var (T.head_term lhs) && T.is_var (T.head_term rhs) && sign then
             if T.is_var lhs && T.is_var rhs && Type.is_prop (T.ty lhs) then
               let sub_t_f =
                 S.bind' (S.bind' Subst.empty (get_var lhs, 0) (T.true_, 0)) (get_var rhs, 0) (T.false_, 0)
               in
               let sub_f_t =
                 S.bind' (S.bind' Subst.empty (get_var lhs, 0) (T.false_, 0)) (get_var rhs, 0) (T.true_, 0)
               in
  ...
  let replace_bool_vars (c : C.t) =
    let p =
      Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "replace_bool_vars") ~tags:[Proof.Tag.T_ho]
    in
    let all_bool_substs vars =
      ...
      let rec aux = function
        | [] ->
            assert false
        | [v] ->
            [Subst.FO.bind' Subst.empty (v, sc) (T.true_, sc); Subst.FO.bind' Subst.empty (v, sc) (T.false_, sc)]
        | v :: vs ->
            CCList.flat_map
              (fun subst ->
                [Subst.FO.bind' subst (v, sc) (T.true_, sc); Subst.FO.bind' subst (v, sc) (T.false_, sc)] )
              (aux vs)
      in
      aux vars
    in
    ...
    |> Iter.find_pred (fun (t, _) ->
           ...
           | T.AppBuiltin (hd, args) ->
               (Builtin.is_logical_binop hd || hd = Builtin.Not) && List.for_all T.is_var args
           | _ ->
               false )
    |> CCOpt.map (fun (t, _) ->
  ...
  let replace_bool_app_vars (c : C.t) =
    ...
    let is_eligible xs = CCOpt.return_if (List.for_all is_var_headed xs && List.exists T.is_app_var xs) xs in
    let create_targets n =
      let rec aux = function
        ...
        | n ->
            let rest = aux (n - 1) in
            List.rev_append (List.rev_map (List.cons T.true_) rest) (List.rev_map (List.cons T.false_) rest)
      in
      aux n
    in
    let stms =
      ...
      |> Iter.to_list
    in
    if Env.should_force_stream_eval () then Env.get_finite_infs stms
    else (
      Env.StmQ.add_lst (Env.get_stm_queue ())
        (List.map (fun seq -> Env.Stm.make ~penalty:(C.penalty c) ~parents:[c] seq) stms) ;
      [] )
  ...
  let quantifier_rw_and_hoist (c : C.t) =
    let quant_rw ~at b body =
      let quant_rw_unapplicable =
        let module P = Pos in
        match at with
        | (P.Arg (i, P.Left P.Stop) | P.Arg (i, P.Right P.Stop)) when L.is_predicate_lit (C.lits c).(i) ->
            (* CAUTION: Using is_pos to really mean is the RHS of the
               term true or false? *)
            let sign = L.is_positivoid (C.lits c).(i) in
            (Builtin.equal b Builtin.ForallConst && sign) || (Builtin.equal b Builtin.ExistsConst && not sign)
        | _ ->
            false
      in
      if quant_rw_unapplicable then None
        ...
        let proof =
          Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "quantifier_rw") ~tags:[Proof.Tag.T_ho]
        in
        let body = Combs.expand body in
        let form_for_skolem =
          (if b = Builtin.ForallConst then T.Form.not_ else CCFun.id) (snd @@ T.open_fun body)
        in
        let sk =
          FR.get_skolem ~parent:c ~mode:`SkolemRecycle (T.fun_l (fst @@ T.open_fun body) form_for_skolem)
        in
        let repl = T.app body [sk] in
        let new_lits = CCArray.copy (C.lits c) in
        Literals.Pos.replace ~at ~by:repl new_lits ;
        Some (C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (Array.to_list new_lits) proof)
    in
    let quant_hoist ~old b body =
      ...
      | Builtin.ForallConst ->
          ...
          if Type.returns_prop (T.ty subst_t) then Signal.send Env.on_pred_var_elimination (res, subst_t) ;
          res
      | Builtin.ExistsConst ->
  ...
  let nested_eq_rw c =
                 ...
                 |> OSeq.map (fun unif_subst_opt ->
                        CCOpt.map
                          (fun unif_subst ->
                            ...
                            let repl = if hd = Builtin.Eq || hd = Builtin.Equiv then T.true_ else T.false_ in
                            let new_lits = Array.copy (C.lits c) in
                            Literals.Pos.replace ~at:p ~by:repl new_lits ;
                            let renaming = Subst.Renaming.create () in
                            let new_lits =
                              Literals.apply_subst renaming subst (mk_sc new_lits) |> CCArray.to_list
                            in
                            let rule = Proof.Rule.mk ((if T.equal repl T.true_ then "eq" else "neq") ^ "_rw") in
                            let proof =
                              Proof.Step.inference ~tags:[Proof.Tag.T_ho] ~rule (parents renaming subst)
                            in
    ...
    |> Iter.flat_map_l (fun clause_seq ->
           if Env.should_force_stream_eval () then Env.get_finite_infs [clause_seq]
           else
  ...
  let rename_nested_booleans c =
    ...
    let rename_lit lit =
      ...
      if L.is_predicate_lit lit then
        ...
        let mk_form = if sign then T.Form.eq else T.Form.neq in
        match lit with
    ...
    | x :: xs when not (CCList.is_empty xs) ->
        ...
        let renamed = C.create_a ~penalty:(C.penalty c) ~trail:(C.trail c) new_lits proof in
        Util.debugf ~section 1 "renamed @[%a@] into@. @[%a@]" (fun k -> k C.pp c C.pp renamed) ;
  ...
  let simplify_bools t =
    ...
    let simplify_and_or t b l =
      ...
      let res =
        ...
        let netural_el, absorbing_el = if b = Builtin.And then (true_, false_) else (false_, true_) in
    ...
    let rec aux t =
      ...
      | AppBuiltin (Builtin.And, [x])
        when T.is_true_or_false x && ty_is_prop t && List.length (Type.expected_args (T.ty t)) = 1 ->
          if T.equal x T.true_ then T.fun_ Type.prop (T.bvar ~ty:Type.prop 0)
          else (
      ...
      | AppBuiltin (Builtin.Or, [x])
        when T.is_true_or_false x && ty_is_prop t && List.length (Type.expected_args (T.ty t)) = 1 ->
          ...
          if T.equal x T.true_ then T.fun_ prop T.true_
          else (
            assert (T.equal x T.false_) ;
            T.fun_ prop (T.bvar ~ty:prop 0) )
      | AppBuiltin (Builtin.And, l) when ty_is_prop t && List.length l > 1 ->
          let l' = List.map aux l in
          let t = if T.same_l l l' then t else T.app_builtin ~ty:Type.prop Builtin.And l' in
          simplify_and_or t Builtin.And l'
      | AppBuiltin (Builtin.Or, l) when ty_is_prop t && List.length l > 1 ->
          let l' = List.map aux l in
          let t = if T.same_l l l' then t else T.app_builtin ~ty:Type.prop Builtin.Or l' in
      ...
      | AppBuiltin (Builtin.Not, [s]) -> (
          ...
          | _ ->
              ...
              else T.app_builtin ~ty:Type.prop Builtin.Not [s'] )
      | AppBuiltin (Builtin.Imply, [p; c]) ->
          let unroll_and p =
            match T.view p with AppBuiltin (And, l) -> T.Set.of_list l | _ -> T.Set.singleton p
          in
          let unroll_or p =
            match T.view p with AppBuiltin (Or, l) -> T.Set.of_list l | _ -> T.Set.singleton p
          in
          let is_impl p = match T.view p with AppBuiltin (Imply, [l; r]) -> true | _ -> false in
          ...
          *)
          let unroll_impl p =
            ...
            let rec aux acc p =
              ...
              | AppBuiltin (Imply, [l; r]) ->
                  ...
                  if is_impl r then aux acc' r else (acc', unroll_or r)
              | _ ->
                  assert false
            in
          ...
          if not (T.Set.is_empty (T.Set.inter premises conclusions)) then T.true_
          ...
          else if T.equal p p' && T.equal c c' then t
      ...
      | AppBuiltin (((ExistsConst | ForallConst) as b), [tyarg; g]) ->
          ...
          if T.Seq.subterms ~include_builtin:true body |> Iter.exists T.is_bvar then
            if T.equal g g' then t else T.app_builtin ~ty:(T.ty t) b [tyarg; g']
          else body
      | AppBuiltin (hd, args) ->
  ...
  let fix_unsupported_quant t =
    ...
    let quant_normal _ q_body =
      let has_loosely_bound_0 t = List.mem 0 (T.DB.unbound t) in
      let rec aux t =
        match T.view t with
        | Fun (_, body) ->
            not (has_loosely_bound_0 t)
        | App (hd, args) ->
            if T.is_const hd then List.for_all aux args else not (List.exists has_loosely_bound_0 (hd :: args))
        | AppBuiltin (hd, args) ->
    ...
    let rec aux t =
      ...
      | AppBuiltin (((ExistsConst | ForallConst) as hd), [alpha]) ->
          ...
          let inner_quant =
            let body = if Builtin.equal hd ExistsConst then T.false_ else T.true_ in
            T.fun_ alpha body
          in
          let var = T.bvar ~ty:alpha2prop 0 in
          let body =
            if Builtin.equal hd ExistsConst then T.Form.neq var inner_quant else T.Form.eq var inner_quant
          in
      ...
      | AppBuiltin (hd, args) ->
          ...
          if Builtin.is_quantifier hd && List.length args' == 2 then
            ...
            if not (quant_normal var_ty q_body) then
              if Builtin.equal hd Builtin.ExistsConst then T.Form.neq (List.nth args' 1) (T.fun_ var_ty T.false_)
              else T.Form.eq (List.nth args' 1) (T.fun_ var_ty T.true_)
            else T.app_builtin ~ty:(T.ty t) hd args'
          else if T.same_l args args' then t
          else T.app_builtin ~ty:(T.ty t) hd args'
      | DB _ | Const _ | Var _ ->
          t
    in
    if Env.flex_get Combinators.k_enable_combinators then t else aux (Lambda.eta_reduce @@ Lambda.snf t)

  let replace_unsupported_quants c =
    let new_lits = Literals.map fix_unsupported_quant (C.lits c) in
    if Literals.equal (C.lits c) new_lits then None
    else
      let proof = Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "replace unsupported quants") in
  ...
  let simpl_bool_subterms c =
    try
      ...
      if Literals.equal (C.lits c) new_lits then SimplM.return_same c
        ...
        let proof = Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "simplify boolean subterms") in
        let new_ = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (Array.to_list new_lits) proof in
  ...
  let nnf_bools t =
    ...
    let rec aux t =
      ...
      | Fun _ ->
          ...
          let body' = aux body in
          if T.equal body body' then t else T.fun_l tyargs body'
      | App (hd, l) ->
          let hd' = aux hd and l' = List.map aux l in
          if T.equal hd hd' && T.same_l l l' then t else T.app hd' l'
      | AppBuiltin (Builtin.Not, [f]) -> (
        ...
        | AppBuiltin (((ForallConst | ExistsConst) as b), ([g] | [_; g])) ->
            let flipped = if b = Builtin.ForallConst then Builtin.ExistsConst else Builtin.ForallConst in
  ...
  let nnf_bool_subters c =
    ...
    if Literals.equal (C.lits c) new_lits then SimplM.return_same c
      ...
      let proof = Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "nnf boolean subterms") in
      let new_ = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (Array.to_list new_lits) proof in
      SimplM.return_new new_
  ...
  let normalize_bool_terms c =
    ...
    if Literals.equal (C.lits c) new_lits then SimplM.return_same c
      ...
      let proof = Proof.Step.simp [C.proof_parent c] ~rule:(Proof.Rule.mk "normalize subterms") in
      let new_ = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (Array.to_list new_lits) proof in
      SimplM.return_new new_
  ...
  let solve_bool_formulas ~which c =
      ...
      let rec aux t =
        ...
        | T.AppBuiltin (Not, [f]) -> (
          ...
          | T.AppBuiltin (((Eq | Equiv) as b), l) ->
              let flipped = if b = Builtin.Eq then Builtin.Neq else Builtin.Xor in
              T.app_builtin flipped l ~ty:(T.ty f)
          | T.AppBuiltin (((Neq | Xor) as b), l) ->
              let flipped = if b = Builtin.Neq then Builtin.Eq else Builtin.Equiv in
              T.app_builtin flipped l ~ty:(T.ty f)
          | _ ->
              t )
        | _ ->
    ...
    let find_resolvable_form lit =
      ...
      | Some (l, r, sign) ->
          if (not (T.is_true_or_false r)) && Type.is_prop (T.ty l) then
            if (not sign) && which = `All then Some (l, r) else if sign then find_pos_var_headed_eq l r else None
            ...
            | T.AppBuiltin ((Neq | Xor), ([f; g] | [_; f; g])) when Type.is_prop (T.ty f) && which == `All ->
    ...
    let unif_alg l r =
      ...
      else OSeq.return (Some (Unif.FO.unify_full (l, 0) (r, 0)))
    in
    ...
    |> CCArray.mapi (fun i lit ->
           ...
           | None ->
               Util.debugf ~section 5 "for lit %d(@[%a@]) of @[%a@] no resolvable lits found@." (fun k ->
           ...
           | Some (l, r) -> (
                   ...
                   |> OSeq.map
                        (CCOpt.map (fun subst ->
                             ...
                             let new_lits =
                               ...
                               |> CCArray.of_list
                               |> (fun l -> Literals.apply_subst renaming (US.subst subst) (l, 0))
                               |> CCArray.to_list
                             in
                             let proof =
                               Proof.Step.simp ~tags:[Proof.Tag.T_ho] ~rule:(Proof.Rule.mk "solve_formulas")
                                 [C.proof_parent_subst renaming (c, 0) (US.subst subst)]
                             in
               ...
               with _ ->
                 ...
                 None ) )
    |> CCArray.filter_map CCFun.id |> CCArray.to_list
  ...
  let cnf_otf c : C.t list option =
    ...
    let max_formula_weight = C.Seq.terms c |> Iter.filter T.is_formula |> Iter.map T.size |> Iter.max in
    let opts =
      match max_formula_weight with
      | None ->
          [Cnf.DisableRenaming]
      | Some m ->
          if m < renaming_weight then [Cnf.DisableRenaming] else []
    in
    match idx with
    | Some _ ->
        ...
        CCVector.iter
          ...
          cnf_vec ;
        let solved =
          if Env.flex_get k_solve_formulas then CCOpt.get_or ~default:[] (solve_bool_formulas ~which:`All c)
          else []
        in
        let clauses =
        ...
        List.iteri (fun i new_c -> assert (C.proof_depth c <= C.proof_depth new_c)) clauses ;
        Some (solved @ clauses)
    | None ->
        None
  ...
  let interpret_boolean_functions c =
    ...
    let collect_tl_bool_funs t k =
      let rec aux t =
        ...
        if (not (CCList.is_empty ty_args)) && Type.is_prop ret_ty && (not (T.is_var t)) && not (T.is_app_var t)
        then k t
        else
          match T.view t with
          | App (f, l) ->
    ...
    let forall_close t =
      let ty_args, body = T.open_fun t in
      assert (Type.is_prop (T.ty body)) ;
      List.fold_right (fun ty acc -> T.Form.forall (T.fun_ ty acc)) ty_args body
    in
         ...
         (fun res t ->
           ...
           if not (T.is_true_or_false t'_body) then (
             ...
             let forall_cl =
               C.create ~trail:(C.trail c) ~penalty:(C.penalty c)
                 ( as_forall
                 :: Array.to_list (C.lits c |> Literals.map (T.replace ~old:t ~by:(interpret t' T.true_))) )
                 proof
             in
             let forall_neg_cl =
               C.create ~trail:(C.trail c) ~penalty:(C.penalty c)
                 ( as_neg_forall
                 :: Array.to_list (C.lits c |> Literals.map (T.replace ~old:t ~by:(interpret t' T.false_))) )
                 proof
             in
             Util.debugf ~section 5 "interpret bool(@[%a@]):@.@[%a@] !!> @. @[%a@]@." (fun k ->
                 k T.pp t Literals.pp (C.lits c) Literals.pp (C.lits forall_cl) ) ;
             Util.debugf ~section 5 "interpret bool(@[%a@]):@.@[%a@] !!> @. @[%a@]@." (fun k ->
                 k T.pp t Literals.pp (C.lits c) Literals.pp (C.lits forall_neg_cl) ) ;
  ...
  let setup () =
    ...
    | _ ->
        ...
        if Env.flex_get k_trigger_bool_ind > 0 then Env.add_unary_inf "trigger bool ind" trigger_induction ;
        if Env.flex_get k_simplify_bools then Env.add_basic_simplify simpl_bool_subterms ;
        if Env.flex_get k_nnf then E.add_basic_simplify nnf_bool_subters ;
        if Env.flex_get k_norm_bools then Env.add_basic_simplify normalize_bool_terms ;
        ...
        if Env.flex_get k_bool_reasoning = BoolHoist then (
          if Env.flex_get k_bool_hoist_simpl then Env.add_multi_simpl_rule ~priority:1000 bool_hoist_simpl ;
          ...
          if Env.flex_get Superposition.k_ho_basic_rules then (
            ...
            if Env.flex_get k_fluid_hoist then Env.add_unary_inf "fluid_hoist" fluid_hoist ;
...
let map_propositions ~proof f =
  CCVector.flat_map_list (fun s ->
      match Statement.view s with
      | Assert p ->
          if_changed proof assert_ s f p
      | Lemma ps ->
          if_changed proof lemma s (map % f) ps
      | Goal p ->
          if_changed proof goal s f p
      | NegatedGoal (ts, ps) ->
          if_changed proof (neg_goal ~skolems:ts) s (map % f) ps
      | _ ->
          [s] )
...
let name_quantifiers stmts =
  let proof s =
    Proof.Step.esa [Proof.Parent.from (Statement.as_proof_i s)] ~rule:(Proof.Rule.mk "Quantifier naming")
  in
      ...
      (fun t ->
        ...
        | Bind (Binder.Forall, _, _) | Bind (Binder.Exists, _, _) ->
            ...
            let definition =
              ...
              bind_list ~ty:prop Binder.Forall vars (app_builtin ~ty:prop Builtin.Equiv [q_vars; t])
            in
  ...
  |> CCVector.map (fun s ->
         ...
         | Rewrite _ ->
             s
         | Assert p ->
             if_changed assert_ s (name_prop_Qs s p)
         | Lemma ps ->
             if_changed_list lemma s (map (name_prop_Qs s) ps)
         | Goal p ->
             if_changed goal s (name_prop_Qs s p)
         | NegatedGoal (ts, ps) ->
...
let rec replace old by t =
  ...
  if TypedSTerm.equal t old then by
    ...
    match view t with
    | App (f, ps) ->
        app_whnf ~ty (r f) (map r ps)
    | AppBuiltin (f, ps) ->
        app_builtin ~ty f (map r ps)
    | Ite (c, x, y) ->
        ite (r c) (r x) (r y)
    | Let (bs, e) ->
        let_ (map (CCPair.map_snd r) bs) (r e)
    | Bind (b, v, e) ->
        bind ~ty b v (r e)
    | _ ->
        t
...
let with_subterm_or_id t f =
  try
    Seq.subterms_with_bound t (fun (s, var_ctx) ->
        match f var_ctx s with None -> () | Some r -> raise (Return r) ) ;
    t
  with Return r -> r
...
let case_bool vs c p =
  if
    is_bool p
    && (not (is_T_F p))
    && (not (TypedSTerm.equal p c))
    && Var.Set.is_empty (Var.Set.diff (free_vars_set p) vs)
  then
    let ty = prop in
    app_builtin ~ty And
      [app_builtin ~ty Imply [p; replace p Form.true_ c]; app_builtin ~ty Or [p; replace p Form.false_ c]]
  else c
...
let eager_cases_far stms =
  let proof s =
    Proof.Step.esa [Proof.Parent.from (Statement.as_proof_i s)] ~rule:(Proof.Rule.mk "eager_cases_far")
  in
  map_propositions ~proof
    (fun _ t ->
...
let eager_cases_near stms =
  let proof s =
    Proof.Step.esa [Proof.Parent.from (Statement.as_proof_i s)] ~rule:(Proof.Rule.mk "eager_cases_near")
  in
  let module T = TypedSTerm in
  let find_fool_subterm ?(free_vars = Var.Set.empty) p =
    let rec aux ~top p =
      let p_ty = T.ty_exn p in
      let no_leaky_variables t = Var.Set.intersection_empty (T.free_vars_set t) free_vars in
      let return p =
      ...
      | AppBuiltin (hd, args)
        when (not top) && no_leaky_variables p
             && T.Ty.is_prop (T.ty_exn p)
             && (* making sure it is not T or F *)
             (Builtin.is_logical_op hd || Builtin.equal hd Builtin.Eq || Builtin.equal hd Builtin.Neq) ->
          CCFormat.printf "found OK eq@." ; return p
      | Bind ((Binder.Exists | Binder.Forall), var, body) when (not top) && no_leaky_variables p ->
          return p
      ...
      | AppBuiltin (b, args) ->
          CCOpt.map
            (fun (args_t, args_f, s) -> (T.app_builtin ~ty:p_ty b args_t, T.app_builtin ~ty:p_ty b args_f, s))
            (aux_l args)
      | App (hd, args) ->
          CCOpt.map
            (fun (args_t, args_f, s) -> (T.app ~ty:p_ty hd args_t, T.app ~ty:p_ty hd args_f, s))
            (aux_l args)
      | _ ->
          None
    and aux_l = function
      | [] ->
          None
      | x :: xs -> (
        match aux ~top:false x with
        | Some (x_t, x_f, s) ->
            Some (x_t :: xs, x_f :: xs, s)
        | None -> (
          match aux_l xs with Some (xs_t, xs_f, s) -> Some (x :: xs_t, x :: xs_f, s) | None -> None ) )
    in
    ...
    let rec aux ~vars p =
      ...
      | AppBuiltin (((Builtin.Neq | Builtin.Eq) as hd), ([_; a; b] | [a; b]))
        when not (T.Ty.is_prop (T.ty_exn a)) -> (
          ...
          match find_fool_subterm a with
          | None -> (
            match find_fool_subterm b with
            | None ->
                p
      ...
      | App (hd, args) -> (
        match find_fool_subterm p with
...
let post_eager_cases =
  let proof s =
    Proof.Step.esa [Proof.Parent.from (Statement.as_proof_c s)] ~rule:(Proof.Rule.mk "post_eager_cases")
  in
  map_propositions ~proof (fun _ c ->
      let cased = ref Set.empty in
      fold_left
        (SLiteral.fold (fun res ->
             (* Loop over subterms of terms of literals of a clause. *)
             Seq.subterms_depth
             %> Iter.fold
                  (fun res (s, d) ->
                    if
                      d = 0
                      || (not (Type.is_prop (ty s)))
                      || is_true_or_false s || is_var s || Set.mem s !cased
...
let preprocess_booleans stmts =
  (match !_bool_reasoning with BoolCasesPreprocess -> eager_cases_near | _ -> id)
    (if !_quant_rename then name_quantifiers stmts else stmts)

let preprocess_cnf_booleans stmts =
...
let extension =
  let register env =
    ...
    ET.setup ()
  in
  {Extensions.default with Extensions.name= "bool"; env_actions= [register]}

let () =
  Options.add_opts
    [ ( "--boolean-reasoning"
      , Arg.Symbol
          ( ["off"; "simpl-only"; "bool-hoist"; "cases-preprocess"]
          , fun s ->
              (_bool_reasoning :=
                 match s with
                 | "off" ->
    ...
    ; ("--quantifier-renaming", Arg.Bool (fun v -> _quant_rename := v), " turn the quantifier renaming on or off")
    ; ("--replace-quants", Arg.Bool (fun v -> _replace_quants := v), " replace unsupported quantifiers")
    ...
    ; ( "--trigger-bool-inst"
      ...
      , " instantiate predicate variables with boolean terms already in the proof state. Argument is the \
         maximal proof depth of predicate variable" )
          ...
          , fun s ->
              _generalize_trigger :=
                ...
                | "var" ->
                    `Var
                | _ ->
                    invalid_arg "off, neg or var are the only options" )
      , " generalize the trigger: neg adds the negation before the trigger body, "
        ^ " and var applies the body to a fresh variable" )
    ; ("--disable-simplifying-cnf", Arg.Set _cnf_non_simpl, " implement cnf on-the-fly as an inference rule")
    ; ( "--interpret-bool-funs"
    ...
    ; ("--simplify-bools", Arg.Bool (fun v -> _simplify_bools := v), " simplify boolean subterms")
    ; ("--fluid-hoist", Arg.Bool (fun v -> _fluid_hoist := v), " enable/disable Fluid(Bool|Loob)Hoist rules")
    ; ( "--fluid-log-hoist"
    ...
    ; ( "--boolean-reasoning-filter-literals"
      , Arg.Symbol
          ...
          , fun v ->
              match v with
              | "all" ->
                  _filter_literals := `All
              | "max" ->
                  _filter_literals := `Max
              | _ ->
                  assert false )
  ...
  Params.add_to_modes
    ...
    ; "ho-competitive"
    ; "lambda-free-purify-extensional"
    ; "fo-complete-basic" ] (fun () -> _replace_quants := false ) ;
  Params.add_to_modes
    [ "lambda-free-intensional"
    ; "lambda-free-purify-intensional"
    ; "lambda-free-extensional"
    ; "ho-comb-complete"
    ; "lambda-free-purify-extensional" ] (fun () -> _disable_ho_unif := true ) ;
