...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let instantiate_w_bool ~clause ~var ~trigger =
    ...
    let lits =
      Literals.map
        (fun t -> Lambda.eta_reduce ~expand_quant @@ Lambda.snf t)
        lits
    in
    let proof =
    ...
    then (
      _trigger_bools :=
        Type.Map.update (T.ty t)
          (function None -> Some [t] | Some res -> Some (t :: res))
          !_trigger_bools ;
  ...
  let trigger_induction cl =
    ...
    let make_triggers lhs rhs sign =
      let lhs_body, rhs_body =
        (snd (Term.open_fun lhs), snd (Term.open_fun rhs))
      in
      let immediate_args =
        if
          T.is_const (T.head_term lhs_body) && T.is_const (T.head_term rhs_body)
        then List.sort_uniq T.compare (T.args lhs_body @ T.args rhs_body)
        else []
      in
      CCList.filter_map
        (fun arg ->
    ...
    CCOpt.return_if
      ...
      (CCList.fold_left
         (fun acc (t, _) ->
           ...
           :: mk_res ~proof ~old:t ~repl:repl_pos pos_lit c
           :: acc )
         [] bool_subterms )
    |> CCFun.tap (function
         | Some res ->
  ...
  let eq_hoist (c : C.t) : C.t list =
    ...
    |> List.map (fun (t, _) ->
           ...
           | T.AppBuiltin (Builtin.(Eq | Equiv), ([a; b] | [_; a; b])) ->
               ...
               mk_res ~proof:(proof ~prefix:"eq") ~old:t ~repl:T.false_ new_lit
                 c
           | T.AppBuiltin (Builtin.(Neq | Xor), ([a; b] | [_; a; b])) ->
               let new_lit = Literal.mk_eq a b in
               mk_res ~proof:(proof ~prefix:"neq") ~old:t ~repl:T.true_ new_lit
                 c
           | _ ->
               assert false )
    |> CCFun.tap (fun res ->
  ...
  let fluid_hoist (c : C.t) =
    ...
    let mk_res sign renaming sub at =
      ...
      let still_at_eligible =
        get_bool_eligible c' |> Iter.exists (fun (_, p) -> Position.equal p at)
      in
      if still_at_eligible then (
        ...
        in
        let by =
          Subst.FO.apply renaming sub ((if sign then z_false else z_true), sc_zx)
        in
        Literals.Pos.replace (C.lits c') ~at ~by ;
        let rule =
          Proof.Rule.mk
            ("fluid_" ^ (if sign then "bool_" else "loob_") ^ "hoist")
        in
        let proof =
                     ...
                     (fun us ->
                       ...
                       if T.is_true_or_false x_sub then OSeq.empty
                         ...
                         let bool_res =
                           ...
                           else [mk_res true renaming sub p]
                         in
                         let loob_res =
                           if T.equal z_true_sub zx_sub then []
                           else [mk_res false renaming sub p]
                         in
  ...
  let fluid_log_hoist (c : C.t) =
    ...
    let mk_res sub at partner =
      ...
      let step =
        Proof.Step.inference ~rule [C.proof_parent_subst renaming (c, sc_cl) sub]
      in
      C.create
        ~penalty:
          ( C.penalty c + C.proof_depth c
         ...
         (fun acc (t, p) ->
           ...
           | T.AppBuiltin ((Builtin.(ForallConst | ExistsConst) as b), [_; body])
             ->
               ...
               let rest =
                 if CCArray.exists Literal.is_trivial (C.lits hoisted) then acc
                 else hoisted :: acc
               in
                 ...
                 |> OSeq.map (fun unif_subst_opt ->
                        CCOpt.map
                          (fun unif_subst ->
                            ...
                            let renaming = Subst.Renaming.create () in
                            let new_lits =
                              Literals.apply_subst renaming subst
                                (mk_sc new_lits)
                              |> CCArray.to_list
                            in
                            let rule =
                              Proof.Rule.mk
                                ( (if T.equal repl T.true_ then "eq" else "neq")
                                ^ "_rw" )
                            in
                            let proof =
        ...
        let new_defs, new_parents =
          CCList.fold_left
            (fun (defs, parents) (idx, _) ->
              ...
              let new_lit =
                Literal.mk_prop renamer (Literal.is_positivoid lit)
              in
    ...
    let simplify_and_or t b l =
      ...
      let res =
        ...
        let netural_el, absorbing_el =
          if b = Builtin.And then (true_, false_) else (false_, true_)
        in
    ...
    let rec aux t =
      ...
      | AppBuiltin (Builtin.And, l) when ty_is_prop t && List.length l > 1 ->
          ...
          let t =
            ...
            else T.app_builtin ~ty:Type.prop Builtin.And l'
          in
      ...
      | AppBuiltin (Builtin.Or, l) when ty_is_prop t && List.length l > 1 ->
          ...
          let t =
            if T.same_l l l' then t
            else T.app_builtin ~ty:Type.prop Builtin.Or l'
          in
      ...
      | AppBuiltin (((ExistsConst | ForallConst) as b), [tyarg; g]) ->
          ...
          assert (Type.is_prop (T.ty body)) ;
          if T.Seq.subterms ~include_builtin:true body |> Iter.exists T.is_bvar
          then
  ...
  let fix_unsupported_quant t =
    ...
    let quant_normal _ q_body =
      ...
      let rec aux t =
        ...
        | App (hd, args) ->
            ...
            else not (List.exists has_loosely_bound_0 (hd :: args))
        | AppBuiltin (hd, args) ->
    ...
    let rec aux t =
      ...
      | AppBuiltin (((ExistsConst | ForallConst) as hd), [alpha]) ->
          ...
          let inner_quant =
            let body =
              if Builtin.equal hd ExistsConst then T.false_ else T.true_
            in
            T.fun_ alpha body
          in
      ...
      let rec aux t =
        ...
        | T.AppBuiltin (Not, [f]) -> (
          ...
          | T.AppBuiltin (((Eq | Equiv) as b), l) ->
              let flipped =
                if b = Builtin.Eq then Builtin.Neq else Builtin.Xor
              in
              T.app_builtin flipped l ~ty:(T.ty f)
          | T.AppBuiltin (((Neq | Xor) as b), l) ->
              let flipped =
                if b = Builtin.Neq then Builtin.Eq else Builtin.Equiv
              in
              T.app_builtin flipped l ~ty:(T.ty f)
          | _ ->
              t )
...
let is_T_F t =
  match view t with AppBuiltin ((True | False), []) -> true | _ -> false
...
let rec replaceTST f top t =
  ...
  transformer
    ...
    | Bind (b, x, t) ->
        let top =
          Binder.equal b Binder.Forall || Binder.equal b Binder.Exists
        in
    ...
    (fun _ t ->
      [ with_subterm_or_id t (fun vs s ->
            ...
            | Bind (((Forall | Exists) as q), v, b) ->
                ...
                if TypedSTerm.equal b b' then None
                else Some (replace s (bind ~ty:prop q v b') t)
            | _ ->
                None )
  ...
  let find_fool_subterm ?(free_vars = Var.Set.empty) p =
    let rec aux ~top p =
      ...
      | AppBuiltin (b, args) ->
          CCOpt.map
            (fun (args_t, args_f, s) ->
              ( T.app_builtin ~ty:p_ty b args_t
              , T.app_builtin ~ty:p_ty b args_f
              , s ) )
            (aux_l args)
      | App (hd, args) ->
          CCOpt.map
