...
module Make(E : Env.S) : S with module Env = E = struct
  ...
  let cnf_otf c : C.t list option =   
    ...
    | Some _ ->
      ...
      let proof = Proof.Step.conv [C.proof_parent c] in
      ...
      let clauses = cnf_vec 
        |> CCVector.map (fun stmt -> C.of_statement ~convert_defs:true stmt, Statement.proof_step stmt) 
        |> CCVector.to_list 
        |> CCList.flat_map (fun (cs, proof) -> cs 
            |> List.map (fun c -> C.create ~penalty ~trail (CCArray.to_list (C.lits c)) proof)) in
