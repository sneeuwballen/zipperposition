...
let () =
  Printexc.register_printer (function
    | Error s -> Some ("error in enum_types: " ^ s)
    | _ -> None )
...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  type decl = {
    ...
    decl_ty : Type.t;
    (* id applied to ty_vars (shortcut) *)
  ...
  let check_uniq_var_is_ ~var cases =
    cases |> Iter.of_list |> Iter.flat_map T.Seq.vars
  ...
  let rec check_all_distinct_ acc l =
    match l with
    | [] -> true
    | v :: l' ->
  ...
  let declare_ ~proof ~ty_id:id ~ty_vars ~var cases =
    if not (check_all_distinct_ [] ty_vars) then
      errorf_ "invalid declaration %a: duplicate type variable"
        pp_id_or_builtin id;
  ...
  let detect_decl_ c =
    ...
    let rec _check_all_vars ~ty ~var acc lits =
      match lits with
      | [] ->
    ...
    if CCArray.exists (fun l -> not (Lit.is_eq l)) lits then
      None
    else (
  ...
  let naked_vars_ lits =
    let v = vars_under_eq_ lits |> T.Seq.add_set T.Set.empty in
    T.Set.diff v (_shielded_vars lits) |> T.Set.elements
  ...
  let find_ty_ sc_decl ty sc_ty =
    let find_aux i l =
      try
        ...
        if List.length l = List.length d.decl_ty_vars then (
  ...
  let _on_new_decl decl =
    let clauses =
      Signature.fold (Ctx.signature ()) [] (fun acc s ty ->
  ...
  let _detect_stmt stmt =
    ...
    | Stmt.TyDecl _ | Stmt.Def _ | Stmt.Rewrite _ | Stmt.Lemma _
    | Stmt.NegatedGoal _ | Stmt.Goal _ ->
      ()
