...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let declare_ ~proof ~ty_id:id ~ty_vars ~var cases =
    ...
    with Not_found ->
      let ty =
        match id with
        | I id ->
            Type.app id (List.map Type.var ty_vars)
        | B b ->
            assert (ty_vars = []) ;
            Type.builtin b
      in
  ...
  let instantiate_axiom_ ~ty_s s poly_args decl =
    if ID.Set.mem s decl.decl_symbols then None (* already declared *)
    else
  ...
  let _on_new_decl decl =
    let clauses =
      Signature.fold (Ctx.signature ()) [] (fun acc s (ty, _) ->
          match check_decl_ s ~ty decl with None -> acc | Some c -> c :: acc )
    in
