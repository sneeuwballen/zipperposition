...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let _do_canc info acc =
    ...
    then (
      ...
      (* build clause *)
      let proof =
        Proof.Step.inference ~tags:[Proof.Tag.T_lra] ~rule:rule_canc
           ...
           (fun acc (t, active_lit, active_pos) ->
             ...
             |> Iter.fold
                  (fun acc (t', with_pos, subst) ->
                    ...
                    let passive_lit =
                      Lits.View.get_rat_exn (C.lits passive) passive_pos
                    in
                    Util.debugf ~section 5
           ...
           (fun acc (t, passive_lit, passive_pos) ->
             ...
             |> Iter.fold
                  (fun acc (t', with_pos, subst) ->
                    ...
                    let active_lit =
                      Lits.View.get_rat_exn (C.lits active) active_pos
                    in
  ...
  let rec _demod_lit_nf ~add_lit ~add_premise ~i c a_lit =
    ...
    try
      ...
      |> Iter.iter (fun (t, lit_pos) ->
             ...
             |> Iter.iter (fun (_t', with_pos, subst) ->
                    ...
                    _try_demod_step ~subst passive_lit s_p c pos active_lit s_a
                      c' pos' ) );
    ...
    with SimplifyInto (a_lit', c', subst) ->
      ...
      Util.debugf ~section 4
        "(@[<hv2>rewrite_rat_arith@ :lit `%a`@ :into `%a`@ :using @[%a@]@ \
         :subst @[%a@]@])" (fun k ->
  ...
  let _demodulation c =
    ...
    |> Iter.iter (fun (lit, i) ->
           match lit with
           | Lit.Rat a_lit -> _demod_lit_nf ~add_lit ~add_premise ~i c a_lit
           | _ -> add_lit lit
           ...
           (fun acc (a_lit, pos) ->
             ...
             |> Iter.fold
                  (fun acc (mf1, mf2, us) ->
                    ...
                    Util.debugf ~section 5
                      "@[<4>... candidate:@ @[%a@] (max lit ? %B)@]" (fun k ->
                        k S.pp subst is_max_lit );
                    ...
                    then (
                      ...
                      in
                      let new_lit =
                        Lit.mk_rat_op op (MF.rest mf1') (MF.rest mf2')
                      in
                      let c_guard = Literal.of_unif_subst renaming us in
      ...
      |> Iter.fold
           (fun acc (t1, lit1, pos1) ->
             ...
             Lits.fold_rat_terms ~which:`Max ~ord
               ~eligible:C.Eligible.(filter Lit.is_rat_eq)
               (C.lits c)
             |> Iter.fold
           ...
           []
    in
    Util.exit_prof prof_rat_eq_factoring;
    res
  ...
  let _do_chaining info acc =
    ...
    then (
      ...
      | ALF.Left (AL.Less, mf_1, m1), ALF.Right (AL.Less, m2, mf_2) ->
        ...
        let new_lit =
          Lit.mk_rat_less (M.sum m2 (MF.rest mf_1)) (M.sum m1 (MF.rest mf_2))
        in
             ...
             | ALF.Left (AL.Less, mf_l, _) ->
               ...
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                      ...
                      let lit_r =
                        Lits.View.get_rat_exn (C.lits right) right_pos
                      in
                      match lit_r with
             ...
             | ALF.Right (AL.Less, _, mf_r) ->
               ...
               |> Iter.fold
                    (fun acc (_t', with_pos, subst) ->
                      ...
                      let lit_l =
                        Lits.View.get_rat_exn (C.lits left) left_pos
                      in
                      match lit_l with
                      | ALF.Left (AL.Less, mf_l, _) ->
                        MF.unify_ff ~subst (mf_l, sc_l) (mf_r, sc_r)
  ...
  let rec _ineq_find_sufficient ~ord ~trace c lit k =
    ...
    | {AL.op = AL.Less; _} when Iter.exists T.is_var (AL.Seq.terms lit) ->
    ...
    | {AL.op = AL.Less; _} ->
      ...
      |> Iter.iter (fun (t, pos) ->
             ...
             |> Iter.iter (fun (_t', with_pos, subst) ->
                    ...
                    let active_pos = with_pos.C.WithPos.pos in
                    match
                      Lits.View.get_rat (C.lits active_clause) active_pos
                    with
                    | None -> assert false
                    ...
                    | Some (ALF.Right (AL.Less, _, _) as alit')
                      when not is_left ->
                      (* symmetric case *)
                      let alit' =
                        ALF.apply_subst Subst.Renaming.none subst (alit', 1)
  ...
  let demod_ineq c : C.t SimplM.t =
    ...
    let res =
      ...
      | Some (i, cs) ->
        ...
        let proof =
          Proof.Step.simp ~tags:[Proof.Tag.T_lra]
    ...
    let conv_lit i lit =
      ...
      | Lit.Prop (f, sign) ->
        ...
        | T.AppBuiltin (Builtin.Less, [_; l; r]), true when type_ok l ->
          Monome.Rat.of_term l >>= fun m1 ->
          Monome.Rat.of_term r >|= fun m2 -> i, [Lit.mk_rat_less m1 m2]
        ...
        | T.AppBuiltin (Builtin.Eq, [_; l; r]), true when type_ok l ->
          Monome.Rat.of_term l >>= fun m1 ->
          Monome.Rat.of_term r >|= fun m2 -> i, [Lit.mk_rat_eq m1 m2]
      ...
      | _ -> None
    in
    match CCArray.findi conv_lit (C.lits c) with
    | None -> SimplM.return_same c
    | Some (i, new_lits) ->
      let lits = new_lits @ CCArray.except_idx (C.lits c) i
      and proof =
        Proof.Step.simp ~tags:[Proof.Tag.T_lra]
          ~rule:(Proof.Rule.mk "convert_lit")
          [C.proof_parent c]
      in
      let c' = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) lits proof in
      Util.debugf ~section 5 "(@[convert@ :from %a@ :to %a@])" (fun k ->
          k C.pp c C.pp c' );
      SimplM.return_new c'
  ...
  let unshielded_vars lits =
    Literals.unshielded_vars lits ~filter:(fun var ->
        Type.equal (HVar.ty var) Type.rat )
  ...
  module Elim_var = struct
    type t = {
      var : T.var;
      lower : Q.t M.t list;
      (* monomes smaller than x *)
      upper : Q.t M.t list;
      (* monomes bigger than x *)
      eq : Q.t M.t list;
      (* monomes equal to x *)
      side_lits : Literal.t list;
      (* other literals *)
    ...
    let pp out (e : t) =
      ...
      Format.fprintf out
        ...
        T.pp_var e.var pp_m_l e.lower pp_m_l e.upper pp_m_l e.eq
    ...
    module Form = struct
      ...
      let rec cnf (f : form) : Lit.t list list =
        match f with
        | Atom lit -> [[lit]]
        | True -> [[Lit.mk_tauto]]
        | False -> [[Lit.mk_absurd]]
      ...
      let rec pp out (f : form) : unit =
        match f with
        | True -> CCFormat.string out "⊤"
        | False -> CCFormat.string out "⊥"
        | Atom lit -> Lit.pp out lit
        | Or l ->
          Format.fprintf out "(@[<hv>or@ %a@])" (Util.pp_list ~sep:" " pp) l
        | And l ->
          Format.fprintf out "(@[<hv>and@ %a@])" (Util.pp_list ~sep:" " pp) l
    end
      ...
      Array.iter
        (fun lit ->
          ...
          | Lit.Rat {Rat_lit.op; left = m1; right = m2} ->
            ( match M.find m1 t_x, M.find m2 t_x with
          ...
          | _ ->
            assert (not (Lit.var_occurs x lit));
            (* shielding *)
            push_l side lit )
        lits;
      { var = x;
    ...
    let make lits x : t option =
      try Some (make_exn lits x) with Make_err -> None
  end
  ...
  let eliminate_unshielded (c : C.t) : C.t list option =
    ...
    | [] -> None
    | x :: _ ->
      ( match E.make (C.lits c) x with
      | None -> None
      | Some e ->
        let clauses = E.to_clauses e in
        let proof =
          Proof.Step.simp [C.proof_parent c] ~tags:[Proof.Tag.T_lra]
...
let extension =
  ...
  and post_typing_action stmts state =
    ...
    let has_rat =
      ...
      |> Iter.exists (PT.Ty.equal PT.Ty.rat)
    in
    let should_reg = !enable_rat_ && has_rat in
    Util.debugf ~section 2 "decision to register rat-arith: %B" (fun k ->
        k should_reg );
    state
