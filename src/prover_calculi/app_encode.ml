...
let rec app_encode_ty ty =
  let ty' =
    ...
    | T.Var _ ->
        ty
    | T.Meta _ ->
        failwith "Not implemented: Meta"
    | T.Bind (b, v, t) ->
        T.bind ~ty:T.tType b v (app_encode_ty t)
    | T.Ite (_, _, _) ->
        failwith "Not implemented: Ite"
    | T.Let _ ->
        failwith "Not implemented: Let"
    | T.Match (_, _) ->
        failwith "Not implemented: Match"
    | T.Multiset _ ->
        failwith "Not implemented: Multiset"
    | T.Record (_, _) ->
...
let rec app_encode_term toplevel t =
  ...
  let t' =
        ...
        CCList.fold_left
          (fun term arg ->
            ...
            | T.App (f, types) ->
                ...
                T.app ~ty:(List.nth types 1) const_ae_app
            ...
            | _ ->
                failwith "Expected quantified or function type" )
    ...
    | T.Const c ->
        T.const ~ty c
    | T.Var v ->
        T.var (app_encode_var v)
    | T.Bind (Binder.Forall, _, _) ->
        failwith "Not implemented: Forall"
    | T.Bind (Binder.Exists, _, _) ->
        failwith "Not implemented: Exist"
    | T.Bind (Binder.Lambda, _, _) ->
        failwith "Not implemented: Lambda"
    | _ ->
...
let pp_in pp_f pp_t pp_ty = function
  | Output_format.O_zf ->
      Statement.ZF.pp pp_f pp_t pp_ty
  | Output_format.O_tptp ->
      Statement.TPTP.pp pp_f pp_t pp_ty
  | Output_format.O_normal ->
      Statement.pp pp_f pp_t pp_ty
...
let app_encode_stmt stmt =
  ...
  | Statement.Lemma _ ->
      failwith "Not implemented: Lemma"
  | Statement.Goal lits ->
  ...
  | Statement.Assert lits ->
      Statement.assert_ ~proof (app_encode_lits lits)
...
let () =
  Options.add_opts
    [ ( "--app-encode"
      , Arg.Symbol (List.map fst options, fun o -> mode_ := List.assoc o options)
      , " enable applicative encoding" ) ]
