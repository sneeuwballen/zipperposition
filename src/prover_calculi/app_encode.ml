...
let app_encode_stmt stmt =
  ...
  | Statement.Rewrite def ->
    let new_def =
      Statement.map_def_rule ~form:app_encode_lits ~term:(app_encode_term true)
        ~ty:app_encode_ty def
    in
...
let extensionality_axiom =
  ...
  in
  let xdiff =
    T.app ~ty:(T.var beta) const_ae_app
      [ T.var alpha; T.var beta; T.var x; diff ]
  in
  let ydiff =
    T.app ~ty:(T.var beta) const_ae_app
      [ T.var alpha; T.var beta; T.var y; diff ]
  in
