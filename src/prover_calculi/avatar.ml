...
module Make (E : Env.S) (Sat : Sat_solver.S) = struct
  ...
  let simplify_split_ (c : C.t) : C.t list option =
    ...
    let uf_vars =
      C.Seq.vars c |> T.VarSet.of_seq |> T.VarSet.to_list |> UF.create
    ...
    | _ :: _ ->
      ...
      Util.debugf ~section 4 "@[split of @[%a@]@ yields @[%a@]@]" (fun k ->
      ...
      Util.debugf ~section 4 "@[constraint clause is @[%a@]@]" (fun k ->
  ...
  let split c =
    ...
    let res =
      if Array.length (C.lits c) <= 1 || Literals.is_trivial (C.lits c) then
        None
      else
        simplify_split_ c
    in
  ...
  let check_empty c =
    if Array.length (C.lits c) = 0 && !filter_absurd_trails_ (C.trail c) then (
  ...
  let simplify_trail_ c =
    ...
    | Tr_same | Tr_trivial ->
      SimplM.return_same c (* handled by [is_trivial] *)
  ...
  let introduce_cut ?reason ?(penalty = 0) ?(depth = 0) (f : Cut_form.t) proof
      : cut_res =
  ...
  let prove_lemma_handlers_ :
      (cut_res -> C.t list E.conversion_result) list ref =
    ref []
  ...
  let prove_lemma (c : cut_res) : unit =
    ...
    Util.debugf ~section 3
      "(@[prove_lemma@ :lemma %a@ :clauses (@[<hv>%a@])@])" (fun k ->
        k Cut_form.pp (cut_form c) (Util.pp_list C.pp) cs );
  ...
  let register ~split:do_split () =
    Util.debugf ~section:Const.section 2
      "register extension Avatar (split: %B)" (fun k -> k do_split);
