...
let remove_ff_constraints _env c =
  ...
  let extract_hd_vars = function
    | Literal.Equation (lhs, rhs, false) ->
      VS.of_list
        [ T.as_var_exn (T.head_term lhs); T.as_var_exn (T.head_term rhs) ]
    | _ -> assert false
  in
    ...
    CCArray.fold
      (fun ((allowed, bl) as acc) lit ->
        ...
        | Literal.Equation (lhs, rhs, false) when is_neg_ff lit ->
          let l_var, r_var =
            CCPair.map_same (fun t -> T.as_var_exn (T.head_term t)) (lhs, rhs)
          in
          if (not (VS.mem l_var bl)) && not (VS.mem r_var bl) then
            ...
            allowed, VS.add_list bl [ l_var; r_var ]
        | _ -> acc)
      (VS.empty, blocked_vars) (C.lits c)
  in

  if VS.is_empty vars_to_remove then
    ...
    let new_lits =
      ...
      |> CCList.filter_map (fun lit ->
             if is_neg_ff lit then (
               ...
               if VS.subset c_vars vars_to_remove then
                 None
               else
                 Some lit
             ) else
...
let rec declare_skolems env = function
  | [] -> ()
  | (sym, id) :: rest ->
    Ctx.declare (Env.get_ctx env) sym id;
    declare_skolems env rest
...
let ext_neg_lit env (lit : Literal.t) : _ option =
  ...
  | Literal.Equation (f, g, false)
    ...
    Util.debugf ~section 4 "(@[ho_ext_neg_lit@ :old `%a`@ :new `%a`@])"
...
let ext_pos_general ?(all_lits = false) env (c : C.t) : C.t list =
  ...
  let new_clauses =
    ...
    |> Iter.flat_map_l (fun (lit_idx, lit) ->
           let lit =
             Literal.map (fun t -> Lambda.eta_reduce ~expand_quant t) lit
           in
           match lit with
           | Literal.Equation (t, s, true) ->
             ...
             |> Iter.flat_map_l (fun new_lit ->
                    ...
                    let new_lits =
                      new_lit :: CCArray.except_idx (C.lits c) lit_idx
                    in
                    let proof =
  ...
  if new_clauses <> [] then
    Util.debugf ~section 4
      "(@[ext-pos-general-eq@ :clause %a@ :yields (@[<hv>%a@])@])" (fun k ->
...
let neg_ext env (c : C.t) : C.t list =
  let get_new_lits lhs rhs =
    ...
    let rec aux acc lhs rhs =
      ...
      if not (Type.is_fun (T.ty lhs)) then
        acc
      else (
        let sk = calc_skolem lhs rhs in
        let penalty =
          if List.length (fst @@ Type.open_fun (T.ty lhs)) = 1 then
            0
          else
            1
        in
  ...
  let new_lits =
    ...
    |> CCArray.mapi (fun i l ->
           ...
           | Literal.Equation (lhs, rhs, false)
             ...
             let free_vars =
               Literal.vars l |> T.VarSet.of_list |> T.VarSet.to_list
             in
             ...
             declare_skolems env !skolem_decls;
             Literal.mk_neq (T.app lhs skolems) (T.app rhs skolems)
           | _ -> l)
  in
  if not !applied_neg_ext then
    SimplM.return_same c
  else (
    let proof =
      Proof.Step.simp
...
let ord env = Ctx.ord (Env.get_ctx env)

let ext_rule_eligible env cl =
  Env.flex_get_of env k_ext_rules_max_depth < 0
  || C.proof_depth cl < Env.flex_get_of env k_ext_rules_max_depth

let update_ext_dec_indices env f c =
...
let find_ho_disagremeents env ?(unify = true) (orig_s, s_sc) (orig_t, t_sc) =
  let open CCFun in
  ...
  let cheap_unify ~subst (s, s_sc) (t, t_sc) =
    let unif_alg =
      if Env.flex_get_of env Combinators.k_enable_combinators then
        fun s t ->
      Unif_subst.of_subst
      @@ Unif.FO.unify_syn ~subst:(Unif_subst.subst subst) s t
        ...
        PatternUnif.unify_scoped ~subst ~counter
    in
    ...
    if T.equal s t && (s_sc == t_sc || T.is_ground s) then
      ...
      | T.App (s_hd, s_args), T.App (t_hd, t_args) when T.is_const s_hd ->
        let (s_hd, s_args), (t_hd, t_args) =
          CCPair.map_same T.as_app_mono (s, t)
        in
        if T.equal s_hd t_hd then
          find_diss_l s_args t_args
        else
          return ~top [ s, t ]
      | T.App (s_hd, s_args), T.App (t_hd, t_args)
        ...
        let lhs, rhs, args_lhs, args_rhs =
          if List.length s_args > List.length t_args then (
            let taken, dropped =
              CCList.take_drop (List.length s_args - List.length t_args) s_args
            in
            T.app s_hd taken, t_hd, dropped, t_args
          ) else (
            let taken, dropped =
              CCList.take_drop (List.length t_args - List.length s_args) t_args
            in
            s_hd, T.app t_hd taken, s_args, dropped
          )
        in
        if T.same_l args_lhs args_rhs && s_sc == t_sc then
          [ lhs, rhs ]
        else
          return ~top [ s, t ]
      | _ -> return ~top [ s, t ]
  ...
  and find_diss_l xs ys =
    ...
    | _ -> invalid_arg "args must be of the same length"
  in

  try
    if not (Type.equal (T.ty orig_s) (T.ty orig_t)) then raise StopSearch;

    if T.is_true_or_false orig_s || T.is_true_or_false orig_t then
      raise StopSearch;
    ...
    let _, _, unifscope, init_subst =
      if not unify then
        orig_s, orig_t, 0, US.empty
      else
        US.FO.rename_to_new_scope ~counter (orig_s, s_sc) (orig_t, t_sc)
    in
    let app_subst subst =
      if not unify then
        fun (s, _) ->
      s
        ...
        Subst.FO.apply Subst.Renaming.none (US.subst subst)
    in
    ...
    let diss =
      List.fold_left
        (fun (dis_acc, subst) (si, ti) ->
          let si', ti' =
            CCPair.map_same (app_subst subst) ((si, s_sc), (ti, t_sc))
          in
          if
            (not (Type.is_ground (T.ty si'))) || not (Type.is_ground (T.ty ti'))
          then
          ...
          | Some subst' ->
            assert (
              ...
              Type.equal
                (app_ty r s (T.ty si') unifscope)
                (app_ty r s (T.ty ti') unifscope));
            dis_acc, subst'
          | None ->
            assert (
              let r = Subst.Renaming.create () in
    ...
    if
      Env.flex_get_of env k_ho_disagremeents == `AllHo
...
let ext_inst env ~parents (s, s_sc) (t, t_sc) =
  ...
  let diff_const = Env.flex_get_of env k_diff_const in
  ...
  let proof =
    Proof.Step.inference
      ...
      ~rule:(Proof.Rule.mk "ext_inst")
  in
  let penalty = List.fold_left max 1 (List.map C.penalty parents) in

  C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail_l parents) ~penalty new_lits
    proof

let do_ext_inst env ~parents ((from_t, sc_f) as s) ((into_t, sc_t) as t) =
  match find_ho_disagremeents env ~unify:false s t with
  | Some (disagreements, subst) ->
    assert (US.is_empty subst);
    let ho_dis =
      List.filter (fun (s, t) -> Type.is_fun (T.ty s)) disagreements
    in
    (* assert (not (CCList.is_empty ho_dis)); *)

    CCList.map
      (fun (lhs, rhs) -> ext_inst env ~parents (lhs, sc_f) (rhs, sc_t))
      ho_dis
  | None -> []

let ext_inst_or_family_eqfact env cl =
  let try_ext_eq_fact (s, t) (u, v) idx =
    let sc = 0 in
    match find_ho_disagremeents env (s, sc) (u, sc) with
    | Some (disagrements, subst) ->
      ...
      let proof =
        Proof.Step.inference
          [ C.proof_parent cl ]
          ~rule:(Proof.Rule.mk "ext_eqfact")
      in
      (* ext_eqfact is rarely used *)
      let new_c =
        C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail cl)
  ...
  let try_ext_eq_factinst (s, _) (u, _) =
    do_ext_inst env ~parents:[ cl ] (s, 0) (u, 0)
  in

  let try_factorings (s, t) (u, v) idx =
    let ext_family =
      if
        Env.flex_get_of env k_ext_rules_kind = `Both
        || Env.flex_get_of env k_ext_rules_kind = `ExtFamily
      then
    ...
    let ext_inst =
      if
        Env.flex_get_of env k_ext_rules_kind = `Both
        || Env.flex_get_of env k_ext_rules_kind = `ExtInst
      then
       ...
       (fun i lit ->
         ...
         | Lit.Equation (s, t, _)
           when Lit.is_positivoid lit
                && (Env.flex_get_of env k_ext_dec_lits != `OnlyMax
                   || BV.get maximal i) ->
...
let retrieve_from_extdec_idx idx id =
  ...
  | Some cl_map ->
    ...
    |> Iter.flat_map (fun (c, l) ->
           Iter.of_list l |> Iter.map (fun (t, p) -> c, t, p))

let do_ext_sup env from_c from_p from_t into_c into_p into_t =
  let sc_f, sc_i = 0, 1 in
  if
    Type.equal (Term.ty from_t) (Term.ty into_t)
    && not (C.id from_c = C.id into_c && Position.equal from_p into_p)
  then (
    match find_ho_disagremeents env (from_t, sc_f) (into_t, sc_i) with
    | Some (disagreements, subst) ->
      assert (not @@ US.has_constr subst);
      let renaming = Subst.Renaming.create () in
      let subst = US.subst subst in
      ...
      let app_subst renaming scoped_t =
        Subst.FO.apply renaming subst scoped_t
      in
      ...
      let new_neq_lits =
        List.map
          (fun (arg_f, arg_i) ->
            Lit.mk_neq
              (app_subst renaming (arg_f, sc_f))
              (app_subst renaming (arg_i, sc_i)))
          disagreements
      in
      ...
      let new_lits =
        new_neq_lits @ CCArray.except_idx lits_f i @ CCArray.to_list lits_i
      in
      ...
      in
      let new_c =
        C.create ~ctx:(Env.get_ctx env) ~trail ~penalty new_lits proof
      in
      Some new_c
    | None -> None
  ) else
...
let ext_sup_act env given =
  if ext_rule_eligible env given then (
    let eligible =
      if Env.flex_get_of env k_ext_dec_lits = `OnlyMax then
        ...
        C.Eligible.always
    in
    Lits.fold_eqn ~ord:(ord env) ~both:true ~sign:true ~eligible (C.lits given)
           ...
           if T.is_const hd && T.has_ho_subterm l then (
             ...
             Iter.map
               (fun (into_c, into_t, into_p) ->
                 do_ext_sup env given pos l into_c into_p into_t)
               inf_partners
           ) else
             Iter.empty)
  ...
  if ext_rule_eligible env given then
    (let which, eligible =
     ...
     Lits.fold_terms ~vars:false ~var_args:false ~fun_bodies:false
       ~ty_args:false ~ord:(ord env) ~which ~subterms:true ~eligible
       (C.lits given)
            ...
            if T.is_const hd && T.has_ho_subterm t then (
              ...
              Iter.map
                (fun (from_c, from_t, from_p) ->
                  do_ext_sup env from_c from_p from_t given p t)
                inf_partners
            ) else
              Iter.empty))
    |> Iter.filter_map CCFun.id |> Iter.to_list
...
let ext_inst_sup_act env given =
  if ext_rule_eligible env given then (
    let eligible =
      if Env.flex_get_of env k_ext_dec_lits = `OnlyMax then
        C.Eligible.param given
    ...
    |> Iter.flat_map (fun (l, _, sign, pos) ->
           ...
           if T.is_const hd && T.has_ho_subterm l then (
             ...
             Iter.map
               (fun (into_c, into_t, _) ->
                 do_ext_inst env ~parents:[ given; into_c ] (l, 0) (into_t, 1))
               inf_partners
           ) else
             Iter.empty)
...
let ext_inst_sup_pas env given =
  if ext_rule_eligible env given then
    (let which, eligible =
       if Env.flex_get_of env k_ext_dec_lits = `OnlyMax then
         `Max, C.Eligible.res given
...
let ext_eqres env c =
  ...
  if ext_rule_eligible env c then (
    let res =
      Literals.fold_eqn (C.lits c) ~eligible ~ord:(ord env) ~both:false
        ~sign:false
      |> Iter.to_list
      |> CCList.filter_map (fun (lhs, rhs, sign, pos) ->
             ...
             then (
               let sc = 0 in
               match find_ho_disagremeents env (lhs, sc) (rhs, sc) with
               | Some (disagremeents, subst) ->
                 let new_neq_lits =
                   List.map (fun (s, t) -> Lit.mk_neq s t) disagremeents
                 in
                 let i, _ = Literals.Pos.cut pos in
                 let new_lits =
                   ( Array.of_list @@ new_neq_lits
    ...
    let res =
      ...
      |> CCList.flat_map (fun (lhs, rhs, sign, pos) ->
             ...
             if
               ...
               || BV.get (C.eligible_res_no_subst c) idx
             then
               do_ext_inst env ~parents:[ c ] (lhs, 0) (rhs, 0)
             else
               [])
    in
          ...
          | Literal.Equation (lhs, rhs, _) when Literal.is_predicate_lit lit ->
            ...
            | T.Var q when HVar.equal Type.equal v q ->
              ...
              ( others,
                ...
                (if Literal.is_positivoid lit then
                   [ lit ]
                 else
                   [])
                @ pos_lits )
    ...
    | Some (other_lits, constr_l, pos_lits) ->
      ...
      let subst =
        ...
        let body =
          ...
          |> T.Form.or_l
        in
        Util.debugf ~section 1 "(@[elim-pred-with@ (@[@<1>λ @[%a@].@ %a@])@])"
      ...
      let proof =
        proof_constructor
          ~rule:(Proof.Rule.mk "ho_elim_pred")
          ~tags:[ Proof.Tag.T_ho ]
          [ C.proof_parent_subst renaming (c, 0) subst ]
      in
      let new_c =
        C.create ~ctx:(Env.get_ctx env) new_lits proof ~penalty:(C.penalty c)
          ~trail:(C.trail c)
      in
...
let max_penalty_prim_ env = Env.flex_get_of env k_ho_prim_max_penalty

(* rule for primitive enumeration of predicates [P t1…tn]
   (using ¬ and ∧ and =) *)
let prim_enum_ ?(proof_constructor = Proof.Step.inference) ~mode env (c : C.t) :
    C.t list =
  (* set of variables to refine (only those occurring in "interesting" lits) *)
  let vars =
    Literals.fold_eqn ~both:false
      ~ord:(Ctx.ord (Env.get_ctx env))
  ...
  |> Iter.flat_map_l (fun v ->
         HO_unif.enum_prop
           ~add_var:(Env.flex_get_of env k_prim_enum_add_var)
           ~enum_cache:(Env.flex_get_of env k_prim_enum_terms)
           ~signature:(Ctx.signature (Env.get_ctx env))
           ~mode ~offset (v, sc_c))
  |> Iter.map (fun (subst, penalty) ->
         ...
         let proof =
           Proof.Step.inference
             ~rule:(Proof.Rule.mk "ho.refine")
             ~tags:[ Proof.Tag.T_ho ]
             [ C.proof_parent_subst renaming (c, sc_c) subst ]
         in
         let new_c =
           C.create_a ~ctx:(Env.get_ctx env) lits proof
             ~penalty:(C.penalty c + penalty)
             ~trail:(C.trail c)
         in
         Util.debugf ~section 1
           "(@[<hv2>ho.refine@ :from %a@ :subst %a@ :yields %a@])" (fun k ->
             k C.pp c Subst.pp subst C.pp new_c);
         Util.incr_stat stat_prim_enum;
         new_c)
  |> Iter.to_rev_list

let prim_enum ~mode env c =
  if C.proof_depth c < max_penalty_prim_ env then
    prim_enum_ ~mode env c
  else
    []
(* prim_enum_ ~mode env c *)
...
let insantiate_choice env ?(proof_constructor = Proof.Step.inference)
    ?(inst_vars = true) ?(choice_ops = choice_ops) c =
  let max_var =
    ref
      ((C.Seq.vars c |> Iter.map HVar.id |> Iter.max |> CCOpt.get_or ~default:0)
      + 1)
  in
  ...
  let choice_inst_of_hd ~def_clause hd arg =
    ...
    let new_lits =
      [ Literal.mk_prop choice_x false; Literal.mk_prop choice_arg true ]
    in
    let arg_str = CCFormat.sprintf "(%a)" T.TPTP.pp arg in
    let parents =
      match def_clause with
      | Some def -> [ C.proof_parent def; C.proof_parent c ]
      | None -> [ C.proof_parent c ]
    in
    let proof =
      proof_constructor
  ...
  let generate_instances_of_hd ~def_clause hd arg =
    ...
    ::
    (if not @@ Env.flex_get_of env k_generalize_choice_trigger then
       []
     else
      ...
      if Term.is_var hd && inst_vars then (
        ...
        let choice_ops =
          ...
          if CCList.is_empty l then
            [ choice_of_ty hd_ty, None ]
          else
            l
        in
        CCList.flat_map
          (fun (hd, def_clause) -> generate_instances_of_hd ~def_clause hd arg)
          choice_ops
      ) else (
  ...
  if not (CCList.is_empty res) then
    Util.debugf ~section 1 "inst(@[%a@])=@.@[%a@]@." (fun k ->
         ...
         (fun var_map subterm ->
           ...
           | T.App (hd, args)
             when T.is_var hd
                  && List.exists
                       (fun t ->
                         (not (Type.is_fun (T.ty t))) && not (T.is_bvar t))
                       args ->
               ...
               CCList.foldi
                 (fun acc idx arg ->
                   ...
                   if Type.is_ground arg_ty && Type.equal arg_ty var_ret_ty then (
                     let db =
                       T.bvar ~ty:arg_ty (List.length var_arg_tys - 1 - idx)
                     in
             ...
             in
             let old_bindings =
               Term.Map.get_or hd ~default:Term.Set.empty var_map
             in
        ...
        Term.Set.fold
          (fun binding acc ->
            ...
            let proof =
              Some
                (Proof.Step.inference
                   ~rule:(Proof.Rule.mk "simp.projection")
                   ~tags:[ Proof.Tag.T_ho ]
...
let recognize_choice_ops env c =
  let extract_not_p_x l =
    match l with
    | Literal.Equation (lhs, _, _)
      when Literal.is_negativoid l && Literal.is_predicate_lit l
           && T.is_app_var lhs ->
  ...
  let extract_p_choice_p p l =
    match l with
    | Literal.Equation (lhs, _, _)
      when Literal.is_positivoid l && Literal.is_predicate_lit l ->
      (match T.view lhs with
          ...
          let new_cls =
            ...
            |> Iter.flat_map_l (fun pas_cl ->
                   if C.id pas_cl = C.id c then
                     ...
                     insantiate_choice env ~inst_vars:false
                       ~choice_ops:(ref (Term.Map.singleton sym (Some c)))
                       pas_cl)
            |> Iter.map (Combinators.maybe_conv_lams env)
          in

          Env.add_passive env new_cls
        );
        C.mark_redundant c;
        true
      | None -> false)
...
let elim_leibniz_eq_ ?(proof_constructor = Proof.Step.inference) env c =
  let ord = Ctx.ord (Env.get_ctx env) in
  let eligible = C.Eligible.always in
    ...
    |> Iter.fold
         (fun (pos_vs, neg_vs, occ) (lhs, rhs, _, pos) ->
           ...
           if Literal.is_predicate_lit lit && Term.is_app_var lhs then (
             ...
             if sign then
               Term.VarSet.add var_hd pos_vs, neg_vs, Term.Map.add lhs true occ
             else
               pos_vs, Term.VarSet.add var_hd neg_vs, Term.Map.add lhs false occ
           ) else
             pos_vs, neg_vs, occ)
      ...
      CCList.flat_map
        (fun (t, sign) ->
          ...
          if Term.VarSet.mem (Term.as_var_exn hd) pos_neg_vars then (
            ...
            CCList.filter_map
              (fun (i, arg) ->
                if T.var_occurs ~var:var_hd arg then
                  None
                else (
                  let body =
                    (if sign then
...
let elim_leibniz_equality env c =
  if C.proof_depth c < Env.flex_get_of env k_elim_leibniz_eq then
    elim_leibniz_eq_ env c
  else
    []

let elim_andrews_eq_ ?(proof_constructor = Proof.Step.inference) env c =
  let ord = Ctx.ord (Env.get_ctx env) in
  let eligible = C.Eligible.always in
  Lits.fold_eqn ~both:false ~ord ~eligible (C.lits c)
  |> Iter.fold
         ...
         if Lit.is_predicate_lit lit && T.is_app_var lhs then (
           ...
           @@ CCList.flat_map_i
                (fun i arg_i ->
                  CCList.mapi
                    (fun j arg_j ->
                      if j > i && T.equal arg_i arg_j then
                        return i j
                      else
                        None)
                    args)
...
let elim_andrews_equality env c =
  if C.proof_depth c < Env.flex_get_of env k_elim_andrews_eq then
    elim_andrews_eq_ env c
  else
    []

let elim_andrews_equality_simpls env c =
  if C.proof_depth c < Env.flex_get_of env k_elim_andrews_eq then (
    let res = elim_andrews_eq_ env c in
...
let ho_unif_real_ env c pairs other_lits : C.t list =
  Util.debugf ~section 5
    "(@[ho_unif.try@ :pairs (@[<hv>%a@])@ :other_lits %a@])" (fun k ->
  ...
  |> List.map (fun (new_pairs, us, penalty, renaming) ->
         ...
         in
         let new_c =
           C.create ~ctx:(Env.get_ctx env) all_lits proof ~trail:(C.trail c)
             ~penalty:(C.penalty c + penalty)
         in
         Util.debugf ~section 5
...
let ho_unif env (c : C.t) : C.t list =
    ...
    let pairs, others =
      ...
      |> CCList.partition_map (function
           | Literal.Equation (t, u, false) as lit
             when Literal.is_ho_constraint lit ->
             `Left ([], t, u)
           | lit -> `Right lit)
    in
    assert (pairs <> []);
    ho_unif_real_ env c pairs others
  ) else
    []
...
let beta_reduce t =
  ...
  if T.equal t t' then (
    Util.debugf ~section 50 "(@[beta_reduce `%a`@ failed `@])" (fun k ->
        k T.pp t);
    None
  ) else (
    Util.debugf ~section 50 "(@[beta_reduce `%a`@ :into `%a`@])" (fun k ->
        k T.pp t T.pp t');
...
let get_eta_normalizer env =
  match Env.flex_get_of env k_eta with
  | `Reduce ->
    Lambda.eta_reduce
      ~expand_quant:(not @@ Env.flex_get_of env Combinators.k_enable_combinators)
      ~full:true
  | `Expand -> Lambda.eta_expand
  | `None -> fun t -> t

(* rule for eta-expansion/reduction *)
let eta_normalize env t =
  (* assert (T.DB.is_closed t); *)
  let t' = get_eta_normalizer env t in
  if T.equal t t' then (
    Util.debugf ~section 50 "(@[eta_normalize `%a`@ failed `@])" (fun k ->
        k T.pp t);
    None
  ) else (
    Util.debugf ~section 50 "(@[eta_normalize `%a`@ :into `%a`@])" (fun k ->
        k T.pp t T.pp t');
...
let mk_diff_const env =
  ...
  let diff_type =
    Type.forall_fvars [ alpha_var; beta_var ]
      (Type.arrow [ alpha_to_beta; alpha_to_beta ] alpha)
  in
  let diff = Term.const ~ty:diff_type diff_id in

  Env.Ctx.declare (Env.get_ctx env) diff_id diff_type;
  Env.flex_add_of env k_diff_const diff

let mk_extensionality_clause env =
  let diff = Env.flex_get_of env k_diff_const in
  let alpha_var = HVar.make ~ty:Type.tType 0 in
  let alpha = Type.var alpha_var in
  let beta_var = HVar.make ~ty:Type.tType 1 in
  let beta = Type.var beta_var in
  ...
  let x_diff =
    Term.app x [ Term.app diff [ T.of_ty alpha; T.of_ty beta; x; y ] ]
  in
  let y_diff =
    Term.app y [ Term.app diff [ T.of_ty alpha; T.of_ty beta; x; y ] ]
  in
  let lits = [ Literal.mk_eq x y; Literal.mk_neq x_diff y_diff ] in
  Env.C.create ~ctx:(Env.get_ctx env)
    ~penalty:(Env.flex_get_of env k_ext_axiom_penalty)
    ~trail:Trail.empty lits Proof.Step.trivial

let mk_choice_clause env =
  ...
  (* p x *)
  let p_choice =
    Term.app p [ Term.app choice [ p ] ]
    (* p (choice p) *)
  in
  (* ~ (p x) | p (choice p) *)
  let lits = [ Literal.mk_prop px false; Literal.mk_prop p_choice true ] in
  Env.Ctx.declare (Env.get_ctx env) choice_id choice_type;
  Env.C.create ~ctx:(Env.get_ctx env)
    ~penalty:(Env.flex_get_of env k_choice_axiom_penalty)
    ~trail:Trail.empty lits Proof.Step.trivial

let mk_ite_clauses env =
  ...
  let if_f_cl =
    C.create ~ctx:(Env.get_ctx env) ~penalty:1 ~trail:Trail.empty
      ...
      Proof.Step.trivial
  in
  Env.Ctx.declare (Env.get_ctx env) ite_id ite_ty;
  Iter.of_list [ if_t_cl; if_f_cl ]

let early_bird_prim_enum env cl var =
  assert (T.is_var var);
  let offset = C.Seq.vars cl |> T.Seq.max_var |> succ in
  let mode = Env.flex_get_of env k_ho_prim_mode in
  let add_var = Env.flex_get_of env k_prim_enum_add_var in
  let sc = 0 in
  ...
  |> CCList.map (fun (subst, p) ->
         ...
         let res =
           C.create_a ~ctx:(Env.get_ctx env) lits proof
             ...
             ~trail:(C.trail cl)
         in
         let res = Combinators.maybe_conv_lams env res in
         (* CCFormat.printf "orig:@[%a@]@.subst:@[%a@]@.res:@[%a@]@." C.pp cl Subst.pp subst C.pp res; *)
         res)
  |> CCList.to_iter |> Env.add_passive env

let recognize_injectivity env c =
  ...
  let find_in_args var args =
    fst
    @@ CCOpt.get_or ~default:(-1, T.true_) (CCList.find_idx (T.equal var) args)
  in
  ...
  try
    ...
    | [| lit1; lit2 |] ->
      ...
      | Equation (x, y, true), Equation (lhs, rhs, sign) ->
        ...
        fail_on
          (not (find_in_args x lhs_args != -1 || find_in_args x rhs_args != -1));
        fail_on
          (not (find_in_args y lhs_args != -1 || find_in_args y rhs_args != -1));
        ...
        let proof =
          Proof.Step.inference ~rule:(Proof.Rule.mk "inj_rec")
            [ C.proof_parent c ]
        in
        Ctx.declare (Env.get_ctx env) sk_id sk_ty;
        let new_clause =
          C.create ~ctx:(Env.get_ctx env) ~trail:(C.trail c)
            ~penalty:(C.penalty c) inv_lit proof
        in
...
let complete_eq_args env (c : C.t) : C.t list =
  ...
  let aux ?(start = 1) ~poly lits lit_idx t u =
    ...
    assert (ty_args <> []);
    let vars =
      List.mapi (fun i ty -> T.var @@ HVar.make ~ty (i + var_offset)) ty_args
    in
    CCList.(start -- List.length vars)
    |> List.map (fun prefix_len ->
           let vars_prefix = CCList.take prefix_len vars in
           let new_lit =
             Literal.mk_eq (T.app t vars_prefix) (T.app u vars_prefix)
           in
           ...
           in
           let new_c =
             C.create ~ctx:(Env.get_ctx env) new_lits proof ~penalty
               ~trail:(C.trail c)
           in
           ...
           | Literal.Equation (t, u, true)
             ...
             let res =
               ...
               |> OSeq.mapi (fun arrarg_idx arrow_args ->
                      ...
                      let funty =
                        T.of_ty
                          (Type.arrow arrow_args
                             (Type.var (HVar.fresh ~ty:Type.tType ())))
                      in
                      let subst = Unif_subst.FO.singleton (var, 0) (funty, 0) in
                      let renaming, subst =
                        Subst.Renaming.none, Unif_subst.subst subst
                      in
                      let lits' =
                        Lits.apply_subst renaming subst (C.lits c, 0)
                      in
                      let t' = Subst.FO.apply renaming subst (t, 0) in
                      let u' = Subst.FO.apply renaming subst (u, 0) in
                      let new_cl =
                        aux ~poly:true ~start:(arrarg_idx + 1) lits' lit_idx t'
                          u'
                      in
             ...
             in
             let first_two, rest =
               OSeq.take 2 res, OSeq.map CCOpt.return (OSeq.drop 2 res)
             in
             let stm =
               Stm.make ~penalty:(C.penalty c + 20) ~parents:[ c ] rest
             in
             StmQ.add (Env.get_stm_queue env) stm;
  ...
  if new_c <> [] then (
    ...
    Util.debugf ~section 4 "(@[complete-eq@ :clause %a@ :yields (@[<hv>%a@])@])"
...
let arg_cong_simpl _env c =
  ...
  if not !simplified then
    ...
    let proof =
      Proof.Step.simp
        ~rule:(Proof.Rule.mk "arg_cong_simpl")
        [ C.proof_parent c ]
    in
    SimplM.return_new
...
type fixed_arg_status =
  | Always of
      T.t (* This argument is always the given term in all occurrences *)
  | Varies
...
(** Removal of fixed/duplicate arguments of variables.
    - If within a clause, there exists a variable F that's always applied to at
      least i arguments and the ith argument is always the same DB-free term, we
      can systematically remove the argument (and repair F's type).
    - If within a clause, there exist a variable F, and indices i < j such that
...
let prune_arg_old env c =
         ...
         | Some var ->
           ...
           | Some var_status ->
             ...
             let rec update_das das arg =
               ...
               | AlwaysSameAs j ->
                 (try
                    if T.equal (List.nth args j) arg then
                      AlwaysSameAs j
                    else
                      update_das (snd (List.nth var_status j)) (List.nth args j)
             ...
             VTbl.replace status var
               (CCList.map
                  (fun ((fas, das), arg) ->
                    update_fas fas arg, update_das das arg)
                  (List.combine var_status args))
           | None ->
             (* First time to encounter this var *)
             let rec create_var_status ?(i = 0) args :
                 (fixed_arg_status * dupl_arg_status) list =
               match args with
               | [] -> []
               | arg :: args' ->
                 let fas =
                   if T.DB.is_closed arg then
                     Always arg
                   else
                     Varies
                 in
  ...
  let subst =
    ...
    |> CCList.filter_map (fun (var, var_status) ->
           ...
           in
           if CCList.for_all (( = ) true) keep then
             None
           else (
             ...
             let keep =
               CCList.(
                 append keep (replicate (length ty_args - length keep) true))
             in
             (* Create substitution: *)
             let ty_args' =
               ty_args |> CCList.combine keep |> CCList.filter fst
               |> CCList.map snd
             in
             ...
             let bvars =
               ...
               |> List.mapi (fun i (k, ty) ->
                      k, T.bvar ~ty (List.length ty_args - i - 1))
...
let prune_arg ~all_covers env c =
  let get_covers ?(current_sets = []) head args =
    let ty_args, _ = Type.open_fun (T.ty head) in
    let missing =
      CCList.replicate (List.length ty_args - List.length args) None
    in
    let args_opt =
      List.mapi
        (fun i a_i ->
          ...
          assert (
            CCList.is_empty current_sets
            || List.length current_sets = List.length args + List.length missing);
          if
            CCList.is_empty current_sets
            || not (Term.Set.is_empty (List.nth current_sets i))
          then
            Some
              (List.mapi
                 (fun j a_j ->
        ...
        (fun i arg_opt ->
          if i < List.length args then (
            ...
            | Some arg_l ->
              let res_l =
                if all_covers then
                  T.cover_with_terms t arg_l
                else
                  [ t; T.max_cover t arg_l ]
              in
              T.Set.of_list res_l
            | None -> Term.Set.empty
          ) else
  ...
  C.lits c
  |> Literals.map (fun t -> Combinators.expand env t)
  ...
  |> Iter.iter (fun (t, _) ->
         ...
         | Some var when T.VarSet.mem var free_vars ->
           ...
           | Some (current_sets, created_sk) ->
             ...
             let new_skolems =
               T.IntMap.bindings new_sk |> List.map snd |> Term.Set.of_list
             in
           ...
           | None ->
             ...
             let created_sk =
               T.IntMap.bindings created_sk |> List.map snd |> Term.Set.of_list
             in
               ...
               (fun i arg_set ->
                 ...
                 let arg_l =
                   List.filter
                     (fun t ->
                       List.for_all
                         (fun idx -> not @@ IntSet.mem idx !removed)
                         (T.DB.unbound t)
                       && T.Seq.subterms t
                          |> Iter.for_all (fun subt ->
                                 not @@ Term.Set.mem subt skolems))
                     arg_l
                 in
  ...
  if Subst.is_empty subst then
    ...
    Util.debugf ~section 3
      "@[<>@[%a@]@ @[<2>prune_arg_fun into@ @[%a@]@]@ with @[%a@]@]" (fun k ->
        k C.pp c C.pp c' Subst.pp subst);
    SimplM.return_new c'
  )
(* TODO: Simplified flag like in first-order? Profiler?*)
...
let ground_app_vars ~mode env c =
  assert (mode != `Off);
  let app_var =
    C.Seq.vars c
    |> Iter.filter (fun v ->
           Type.is_fun (HVar.ty v) && Type.is_ground (HVar.ty v))
    |> Iter.head
  in
  ...
  let get_groundings var =
    let introduce_new_const ty =
      ...
      C.Ctx.add_signature (Env.get_ctx env)
        (Signature.declare (C.Ctx.signature (Env.get_ctx env)) f_id f_ty);
      f
    in
    let ty = T.ty var in
    match mode with
    ...
    | `All ->
      let ids = Signature.find_by_type (C.Ctx.signature (Env.get_ctx env)) ty in
  ...
  CCOpt.map
    (fun v ->
      let inst repl =
        ...
        let p =
          Proof.Step.simp
            ~rule:(Proof.Rule.mk "ground_app_vars")
            ~tags:[ Proof.Tag.T_ho ]
            [ C.proof_parent_subst renaming (c, 0) subst ]
        in
...
let prim_enum_simpl ~mode env c =
  if C.proof_depth c < max_penalty_prim_ env then (
    ...
    if Env.flex_get_of env k_instantiate_choice_ax && recognize_choice_ops env c
    then
      None
    else (
      let other_insts =
        (if Env.flex_get_of env k_instantiate_choice_ax then
           insantiate_choice env ~proof_constructor c
         else
           [])
        @ (if C.proof_depth c < Env.flex_get_of env k_elim_leibniz_eq then
             elim_leibniz_eq_ env ~proof_constructor c
           else
             [])
        @
        if Env.flex_get_of env k_elim_pred_var then
          elim_pred_variable env ~proof_constructor c
        else
          []
      in

      let res = other_insts @ prim_enum_ ~proof_constructor ~mode env c in
...
let purify_applied_variable env c =
  (* set of new literals *)
  let new_lits = ref [] in
  ...
  let varidx =
    ...
    |> Iter.flat_map T.Seq.vars |> T.Seq.max_var |> succ |> CCRef.create
  in
  ...
  let same_class t1 t2 =
    ...
    if Env.flex_get_of env k_purify_applied_vars == `Ext then
      T.equal t1 t2
    else (
      assert (Env.flex_get_of env k_purify_applied_vars == `Int);
    ...
    let res =
      ...
      | Some v ->
        if should_purify t v then (
          (* purify *)
          Util.debugf ~section 5 "@[Purifying: %a.@ Untouched is: %a@]"
  ...
  let purify_lit lit =
    ...
    if Literal.for_all T.is_var lit then
      lit
    else
      Literal.map purify_term lit
  in
  (* try to purify *)
  ...
  | _ :: _ ->
    ...
    let proof =
      Proof.Step.simp
        ~rule:(Proof.Rule.mk "ho.purify_applied_variable")
...
let setup env =
  mk_diff_const env;
  if not (Env.flex_get_of env k_enabled) then
    Util.debug ~section 1 "HO rules disabled"
  else (
    Util.debug ~section 1 "setup HO rules";
    Env.Ctx.lost_completeness (Env.get_ctx env);

    if Env.flex_get_of env k_neg_ext_as_simpl then
      Env.add_unary_simplify env neg_ext_simpl
    else if Env.flex_get_of env k_neg_ext then
      Env.add_unary_inf env "neg_ext" neg_ext;

    if
      Env.flex_get_of env k_ext_rules_kind == `ExtFamily
      || Env.flex_get_of env k_ext_rules_kind == `Both
    then (
      Env.add_binary_inf env "ext_dec_act" ext_sup_act;
      Env.add_binary_inf env "ext_dec_pas" ext_sup_pas;
      Env.add_unary_inf env "ext_eqres_dec" ext_eqres
    );
    ...
    if
      ...
      || Env.flex_get_of env k_ext_rules_kind = `Both
    then (
      Env.add_binary_inf env "ext_dec_act" ext_inst_sup_act;
      Env.add_binary_inf env "ext_dec_pas" ext_inst_sup_pas;
      Env.add_unary_inf env "ext_eqres_dec" ext_inst_eqres
    );
    ...
    if Env.flex_get_of env k_arg_cong_simpl then
      Env.add_basic_simplify env arg_cong_simpl
    else if Env.flex_get_of env k_arg_cong then
      Env.add_unary_inf env "ho_complete_eq" complete_eq_args;

    if Env.flex_get_of env k_resolve_flex_flex then
      Env.add_basic_simplify env remove_ff_constraints;

    if Env.flex_get_of env k_ground_app_vars != `Off then (
      let mode = Env.flex_get_of env k_ground_app_vars in
      Env.add_cheap_multi_simpl_rule env (ground_app_vars ~mode);
      Env.add_multi_simpl_rule env ~priority:1000 (ground_app_vars ~mode)
    );

    if Env.flex_get_of env k_elim_pred_var then
      Env.add_unary_inf env "ho_elim_pred_var" (fun env c ->
          elim_pred_variable env c);
    if Env.flex_get_of env k_ext_neg_lit then
      Env.add_lit_rule env "ho_ext_neg_lit" ext_neg_lit;

    if Env.flex_get_of env k_elim_leibniz_eq > 0 then
      Env.add_unary_inf env "ho_elim_leibniz_eq" elim_leibniz_equality;
    if Env.flex_get_of env k_elim_andrews_eq > 0 then
      if Env.flex_get_of env k_elim_andrews_eq_simpl then
        Env.add_multi_simpl_rule env ~priority:100 elim_andrews_equality_simpls
      else
        Env.add_unary_inf env "ho_elim_leibniz_eq" elim_andrews_equality;

    if Env.flex_get_of env k_instantiate_choice_ax then (
      Env.add_redundant env recognize_choice_ops;
      Env.add_unary_inf env "inst_choice" (fun env c -> insantiate_choice env c)
    );

    if Env.flex_get_of env k_ext_pos then
      Env.add_unary_inf env "ho_ext_pos"
        (ext_pos_general ~all_lits:(Env.flex_get_of env k_ext_pos_all_lits));

    (* removing unfolded clauses *)
    if Env.flex_get_of env k_enable_def_unfold then
      Env.add_clause_conversion env (fun _env c ->
          match Statement.get_rw_rule c with
          | Some _ -> Env.CR_drop
          | None -> Env.CR_skip);

    (match Env.flex_get_of env k_prune_arg_fun with
    | `PruneMaxCover -> Env.add_unary_simplify env (prune_arg ~all_covers:false)
    | `PruneAllCovers -> Env.add_unary_simplify env (prune_arg ~all_covers:true)
    | `OldPrune -> Env.add_unary_simplify env prune_arg_old
    | `NoPrune -> ());

    if Env.flex_get_of env Combinators.k_enable_combinators then
      Env.set_ho_normalization_rule env "comb-normalize" (fun env t ->
          Combinators_base.comb_normalize t)
    else (
      let ho_norm env =
       fun t ->
        t |> beta_reduce |> fun opt ->
        match opt with
        | None -> eta_normalize env t
        | Some t' ->
          (match eta_normalize env t' with
          | None -> Some t'
          | Some tt -> Some tt)
      in
      Env.set_ho_normalization_rule env "ho_norm" (fun env t -> ho_norm env t)
    );

    if Env.flex_get_of env k_purify_applied_vars != `None then
      Env.add_unary_simplify env purify_applied_variable;

    if Env.flex_get_of env k_enable_ho_unif then
      Env.add_unary_inf env "ho_unif" ho_unif;

    if Env.flex_get_of env k_simple_projection >= 0 then (
      let penalty_inc = Env.flex_get_of env k_simple_projection in
      let max_depth = Env.flex_get_of env k_simple_projection_md in
      Env.add_unary_inf env "simple_projection"
        (simple_projection ~penalty_inc ~max_depth)
    );

    if not (Env.flex_get_of env k_prim_enum_early_bird) then (
      match Env.flex_get_of env k_ho_prim_mode with
      | `None -> ()
      | mode -> Env.add_unary_inf env "ho_prim_enum" (prim_enum ~mode)
    ) else
      Signal.on (Env.on_pred_var_elimination env) (fun (cl, var) ->
          early_bird_prim_enum env cl var;
          Signal.ContinueListening);
...
let enable_unif_ =
  ref false (* this unification seems very buggy, had to turn it off *)
...
let st_contains_ho (st : (_, _, _) Statement.t) : bool =
  ...
  and has_ho_sym () =
    ...
    |> Iter.exists (fun (_, ty) -> Type.order ty > 1 || has_prop_in_args ty)
  and has_ho_eq () =
    ...
    |> Iter.exists (fun c ->
           ...
           |> List.exists (function
                | SLiteral.Eq (t, u) | SLiteral.Neq (t, u) ->
                  T.is_ho_at_root t || T.is_ho_at_root u
                  || is_non_atomic_ty (T.ty t)
                | _ -> false))
  in
...
let extension =
  let register (env : Env.t) =
    ...
    if Env.flex_get_of env k_check_lambda_free != `False then
      Env.add_fragment_check env (fun _env c ->
    ...
    if !def_unfold_enabled_ then
      ...
      CCVector.iter
        (fun c ->
          match Statement.get_rw_rule c with
          | Some (sym, r) ->
            Util.debugf ~section 1 "@[<2> Adding constant def rule: `@[%a@]`@]"
