...
let k_eta : [`Reduce | `Expand | `None] Flex_state.key =
...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let ext_neg (lit : Literal.t) : _ option =
    match lit with
    | Literal.Equation (f, g, false)
      ...
      Util.debugf ~section 4 "(@[ho_ext_neg@ :old `%a`@ :new `%a`@])" (fun k ->
  ...
  let ext_pos (c : C.t) : C.t list =
    ...
    | [|Literal.Equation (t1, t2, true)|] ->
      ...
      | last1 :: l1, last2 :: l2 ->
        ( match T.view last1, T.view last2 with
        | T.Var x, T.Var y
          when HVar.equal Type.equal x y
               && (not (Type.is_tType (HVar.ty x)))
               && Iter.of_list
                    [Iter.doubleton f1 f2; Iter.of_list l1; Iter.of_list l2]
                  |> Iter.flatten |> Iter.flat_map T.Seq.vars
                  |> Iter.for_all (fun v' -> not (HVar.equal Type.equal v' x))
          ->
          (* it works! *)
          let new_lit =
            Literal.mk_eq (T.app f1 (List.rev l1)) (T.app f2 (List.rev l2))
          in
          let proof =
          ...
          Util.debugf ~section 4 "(@[ext_pos@ :clause %a@ :yields %a@])"
  ...
  *)
      ...
      |> Iter.flat_map_l (fun (lit_idx, lit) ->
             ...
             | Literal.Equation (t, u, true) when Type.is_fun (T.ty t) ->
               ...
               |> List.map (fun prefix_len ->
                      ...
                      let new_lit =
                        Literal.mk_eq (T.app t vars_prefix)
                          (T.app u vars_prefix)
                      in
                      let new_lits =
                        new_lit :: CCArray.except_idx (C.lits c) lit_idx
  ...
  let elim_pred_variable (c : C.t) : C.t list =
    ...
    let find_vars () : _ HVar.t Iter.t =
      C.Seq.vars c |> T.VarSet.of_seq |> T.VarSet.to_seq
      |> Iter.filter (fun v ->
  ...
  let prim_enum_ ~mode (c : C.t) : C.t list =
    ...
    let vars =
      ...
      |> Iter.filter_map (fun t ->
             ...
             match T.as_var hd, Type.arity (T.ty hd) with
    ...
    if not (T.VarSet.is_empty vars) then
      Util.debugf ~section 5
        "(@[<hv2>ho.refine@ :clause %a@ :terms {@[%a@]}@])" (fun k ->
          k C.pp c (Util.pp_seq T.pp_var) (T.VarSet.to_seq vars) );
    let sc_c = 0 in
    let offset = C.Seq.vars c |> T.Seq.max_var |> succ in
    vars |> T.VarSet.to_seq
    |> Iter.flat_map_l (fun v -> HO_unif.enum_prop ~mode (v, sc_c) ~offset)
  ...
  let prim_enum ~mode c =
    if C.penalty c < max_penalty_prim_ then
      prim_enum_ ~mode c
    else
      []
  ...
  let beta_reduce t =
    ...
    if T.equal t t' then
      ...
      Util.debugf ~section 4 "(@[beta_reduce `%a`@ :into `%a`@])" (fun k ->
          k T.pp t T.pp t' );
  ...
  let eta_expand t =
    ...
    if T.equal t t' then
      ...
      Util.debugf ~section 4 "(@[eta_expand `%a`@ :into `%a`@])" (fun k ->
          k T.pp t T.pp t' );
  ...
  let eta_reduce t =
    ...
    if T.equal t t' then
      ...
      Util.debugf ~section 4 "(@[eta_reduce `%a`@ :into `%a`@])" (fun k ->
          k T.pp t T.pp t' );
  ...
  let purify_applied_variable c =
    ...
    let same_class t1 t2 =
      ...
      if !_purify_applied_vars == `Ext then
        t1 = t2
      else (
...
let set_prim_mode_ =
  let l = ["neg", `Neg; "full", `Full; "none", `None] in
...
let st_contains_ho (st : (_, _, _) Statement.t) : bool =
  ...
  let has_ho_var () =
    Statement.Seq.terms st |> Iter.flat_map T.Seq.vars
    |> Iter.exists (fun v -> is_non_atomic_ty (HVar.ty v))
  (* is there a HO symbol? *)
  and has_ho_sym () =
    Statement.Seq.ty_decls st |> Iter.exists (fun (_, ty) -> Type.order ty > 1)
  and has_ho_eq () =
...
let extension =
  ...
  and check_ho vec state =
    let is_ho = CCVector.to_seq vec |> Iter.exists st_contains_ho in
    if is_ho then
...
let () =
  Options.add_opts
    [ "--ho", Arg.Set enabled_, " enable HO reasoning";
      ( "--force-ho",
        Arg.Set force_enabled_,
        " enable HO reasoning even if the problem is first-order" );
      ...
      ( "--ho-general-ext-pos",
        Arg.Set _general_ext_pos,
        " enable general positive extensionality rule" );
      "--ho-ext-axiom", Arg.Set _ext_axiom, " enable extensionality axiom";
      ( "--ho-no-ext-pos",
        Arg.Clear _ext_pos,
        " disable positive extensionality rule" );
      ( "--ho-no-ext-neg",
        Arg.Clear _ext_neg,
        " disable negative extensionality rule" ) ];
