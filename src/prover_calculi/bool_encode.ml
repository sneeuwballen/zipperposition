...
let app_bool hd args = T.app hd ~ty:bool_clone_ty args
...
let boolean_axioms =
  ...
  (* x != y \/ eq x y = T *)
  let eq_true = [SLiteral.neq alpha_x alpha_y; SLiteral.eq eq_x_y true_term] in
  (* x = y \/ eq x y = F *)
  ...
  let exists_def =
    [ SLiteral.eq
        ...
        (app_bool not_term [app_bool forall_term [alpha; l_not_p_x]]) ]
  in
  (* P X *)
...
let bool_encode_ty ty_orig =
  let rec aux ty =
    ...
    | T.App (f, args) ->
        T.app ~ty:T.tType (aux f) (CCList.map aux args)
...
let bool_encode_term t_orig =
  let rec aux t =
    let encode_var v =
      ...
      Var.update_ty v ~f:(fun _ -> ty)
    in
    try
      if T.equal (T.ty_exn t) T.tType then bool_encode_ty t
      else
...
let bool_encode_lit lit =
  match lit with
  | SLiteral.True ->
      SLiteral.eq true_term true_term
  | False ->
      SLiteral.neq true_term true_term
  | Atom (atom, sign) ->
...
let bool_encode_stmt stmt =
  ...
  let res =
    ...
    | Statement.TyDecl (id, ty) ->
        Statement.ty_decl ~proof:Proof.Step.trivial id (bool_encode_ty ty)
  in
  res
...
let extension =
  ...
  in
  Extensions.{default with name= "bool_encode"; post_cnf_modifiers= [modifier]}
