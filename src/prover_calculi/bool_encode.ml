...
let boolean_axioms =
  ...
  (* and T x = x *)
  let and_true =
    [ SLiteral.eq (app_bool and_term [ true_term; bool_x ]) bool_x ]
  in
  (* and F x = F *)
  let and_false =
  ...
  (* not F = T *)
  let not_false =
    [ SLiteral.eq (app_bool not_term [ false_term ]) true_term ]
  in
  (* or T x = T *)
  let or_true =
  ...
  (* or F x = x *)
  let or_false =
    [ SLiteral.eq (app_bool or_term [ false_term; bool_x ]) bool_x ]
  in
  (* impl T x = x *)
  let impl_t =
    [ SLiteral.eq (app_bool impl_term [ true_term; bool_x ]) bool_x ]
  in
  (* impl F x = T *)
  let impl_f =
  ...
  (* x != y \/ eq x y = T *)
  let eq_true =
    [ SLiteral.neq alpha_x alpha_y; SLiteral.eq eq_x_y true_term ]
  in
  (* x = y \/ eq x y = F *)
  let eq_false =
    [ SLiteral.eq alpha_x alpha_y; SLiteral.eq eq_x_y false_term ]
  in
  (* neq x y = not (eq x y) *)
  ...
  let p_choice_p =
    app_bool alpha2bool_p
      [ T.app ~ty:alpha choice_term [ alpha; alpha2bool_p ] ]
  in
...
let bool_encode_term t_orig =
  let rec aux t =
    ...
    with Invalid_argument ty_err ->
      ...
      in
      if CCString.prefix ~pre:"type" ty_err then
        invalid_arg err
      else
        invalid_arg ty_err
  in
...
let bool_encode_lit lit =
  ...
  | Eq _ | Neq _ -> SLiteral.map ~f:bool_encode_term lit
