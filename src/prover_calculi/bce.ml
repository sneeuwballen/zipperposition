...
let refine_logic new_val =
  if log_compare new_val !logic > 0 then logic := new_val
...
let symbol_occurs_too_often env sym_count =
  Env.flex_get_of env k_max_symbol_occ > 0
  && sym_count > Env.flex_get_of env k_max_symbol_occ
...
let add_lit_to_idx env lit_lhs sign cl =
  ...
  if not (Name.Set.mem sym !ignored_symbols) then (
    ...
    if symbol_occurs_too_often env total_sym_occs then (
      ...
      Util.debugf ~section 5 "ignoring symbol @[%a@]@." (fun k -> k Name.pp sym)
    ) else
      ss_idx :=
        SymSignIdx.update (sym, sign)
          (fun old ->
            Some
              (C.ClauseSet.add cl (CCOpt.get_or ~default:C.ClauseSet.empty old)))
          !ss_idx
  )
...
let find_candidates hd sign =
  C.ClauseSet.to_list
    (CCOpt.get_or ~default:C.ClauseSet.empty
...
let register_task ?(update_others = true) lit_idx clause =
  ...
  | L.Equation (lhs, rhs, _) as lit
    ...
    let task =
      { lit_idx; clause; cands = DEQ.of_list cands; heap_idx = init_heap_idx }
    in
  ...
  try
    List.iter
      (fun task ->
        ...
        Util.debugf ~section 3 " |@[%a@]|%d@." (fun k ->
            k C.pp task.clause task.lit_idx);
        TaskPriorityQueue.insert task_queue task)
...
let deregister_clause env clause =
  deregister_symbols clause;
  release_locks clause

let remove_from_proof_state env clause =
  (try
     if Env.is_active env clause then
       C.Tbl.add (Env.flex_get_of env k_removed_active) clause ()
     else if Env.is_passive env clause then
       C.Tbl.add (Env.flex_get_of env k_removed_passive) clause ()
   with _ ->
     (* we are in the preprocessing phase, so we can mark the clause *)
     C.mark_redundant clause);
  if Env.flex_get_of env k_processing_kind != `InprocessingSat then
    C.mark_redundant clause
  (* if we are doing the inprocessing in SAT mode, we cannot
       mark the clauses as redundant, since they might have to be returned
       to the proof state. *);
  Env.remove_active env (Iter.singleton clause);
  Env.remove_passive env (Iter.singleton clause);
  Env.remove_simpl env (Iter.singleton clause)
...
let resolvent_is_valid_neq lit_idx orig_cl partner =
  ...
  let split_partner lhs sign partner =
    CCArray.foldi
      (fun (unifiable, others) idx lit ->
        match lit with
        | L.Equation (lhs', _, _)
          when L.is_predicate_lit lit && L.is_positivoid lit != sign ->
          (try
  ...
  let are_opposite_lits_up_to ~subst (l1, sc1) (l2, sc2) =
    ...
    | Equation (lhs, rhs, _), Equation (lhs', rhs', _) ->
      UF.equal ~subst (lhs, sc1) (lhs', sc2)
      && UF.equal ~subst (rhs, sc1) (rhs', sc2)
      || UF.equal ~subst (lhs, sc1) (rhs', sc2)
         && UF.equal ~subst (rhs, sc1) (lhs', sc2)
    | True, False -> true
    | False, True -> true
    | _ -> false
  in
  ...
  let check_resolvents l_idx orig_cl (unifiable, nonunifiable) =
    ...
    let for_tautology_checking =
      List.filter
        (fun (lit, _) ->
          L.is_positivoid lit = orig_sign && L.is_predicate_lit lit)
        (List.map
           (fun x -> x, sc_orig)
           (CCArray.except_idx (C.lits orig_cl) l_idx)
        @ List.map (fun x -> x, sc_partner) nonunifiable)
    in
    if CCList.is_empty unifiable then
      true
    else (
      ...
      let rec check_lit lhs subst rest =
              ...
              CCOpt.is_some
                (CCArray.find_map_i
                   (fun idx lit ->
                     if
                       idx != l_idx
                       && are_opposite_lits_up_to ~subst (lit, sc_orig)
                            (lit', sc_partner)
                     then
                       Some ()
                     else
        ...
        Util.debugf ~section 30
          "check: @. lit: @[%a@]@. unif: @[%a@]@. non_unif: @[%a@]@. \
           partner_cl: @[%a@]@." (fun k ->
            k L.pp
              (C.lits orig_cl).(l_idx)
              (CCList.pp T.pp) (List.map fst unifiable) (CCList.pp L.pp)
              nonunifiable C.pp partner);
        ...
        || (not (CCList.is_empty rest))
           ...
           (* clause is not valid *)
           if CCList.is_empty contrasting then
             false
           else (
             try
               let subst =
                 List.fold_left
                   (fun subst (lhs', _) ->
                     ...
                     Unif.FO.unify_syn ~subst (lhs, sc_partner)
                       (lhs', sc_partner))
                   subst contrasting
               in
...
let resolvent_is_valid_eq lit_idx orig_cl partner =
  ...
  let check_resolvents l_idx orig_cl (same_hd_atms, diff_hd_lits) =
    let orig_args =
      T.args @@ CCOpt.get_exn @@ L.View.get_lhs (C.lits orig_cl).(l_idx)
    in
    ...
    let add_flat_resolvent ~cc new_args =
      ...
      List.fold_left
        (fun acc (lhs, rhs) -> CC.mk_eq acc lhs rhs)
        cc
        (List.combine orig_args new_args)
    in

    (* calculating positive resp. negative literals in both clauses *)
    let orig_pos, orig_neg =
      CCList.partition L.is_positivoid
        (CCArray.except_idx (C.lits orig_cl) l_idx)
    in
    ...
    if CCList.is_empty same_hd_atms then
      true
    (* no L-resolvent possible *)
    else (
          ...
          List.exists
            (fun lit' ->
              CCOpt.is_some
                (CCArray.find_map_i
                   (fun idx lit ->
                     if idx != l_idx && are_opposite_lits_up_to ~cc lit lit'
                     then
                       Some ()
                     else
                       None)
        ...
        || (not (CCList.is_empty rest))
           ...
           let congruent, rest =
             List.partition
               (fun lhs ->
                 List.exists
                   (fun lhs' -> CC.is_eq cc lhs lhs')
                   for_congruence_testing)
               rest
           in
           (* clause is not valid *)
           if CCList.is_empty congruent then
             false
             ...
             let cc =
               List.fold_left
                 (fun acc lhs -> add_flat_resolvent ~cc (T.args lhs))
                 cc congruent
             in
             check_lit ~cc rest
           )
...
let get_validity_checker () =
  ...
  if !logic == NonequationalFO then
    resolvent_is_valid_neq
  else
    resolvent_is_valid_eq

(* function that actually performs the blocked clause elimination *)
let do_eliminate_blocked_clauses env =
  ...
  let process_task task =
           ...
           (fun idx' lit' ->
             if idx' == lit_idx then
               ...
               | L.Equation (lhs', _, _) when L.is_predicate_lit lit' ->
                 ...
                 let sign' = L.is_positivoid lit' in
                 if Option.equal Name.equal sym' (Some hd_sym) && sign' == sign
                 then
                   Some lit'
                 else
                   None
    ...
    let rec task_is_blocked deq =
      ...
      if
        C.equal cl partner || C.is_redundant partner
        || validity_checker lit_idx cl partner
      then (
        Util.debugf ~section 5 "valid-res(@[%a@], @[%a@](%b))@." (fun k ->
            k C.pp cl C.pp partner (C.is_redundant partner));
        task_is_blocked deq
      ) else (
        Util.debugf ~section 5 "maybe-invalid-res(@[%a@], @[%a@])@." (fun k ->
            k C.pp partner C.pp cl);
    ...
    then (
      ...
      | true ->
        Util.debugf ~section 2 "removed(%d): @[%a@]" (fun k ->
            k task.lit_idx C.pp cl);
        deregister_clause env cl;
        remove_from_proof_state env cl;
        incr removed_cnt
      | false -> assert (not (TaskPriorityQueue.in_heap task))
    ) else
...
let eliminate_blocked_clauses env =
  ...
  if !steps = 0 then (
    ...
    in
    let eliminated = do_eliminate_blocked_clauses env in
    if eliminated != 0 then
      Util.debugf ~section 2 "original clause set:@.@[%a@]" (fun k ->
          k (CCList.pp C.pp) original_cls)
  )

let react_clause_addded env cl = add_clause env cl
let react_clause_removed env cl = deregister_clause env cl

let do_bce_sat env () =
  C.Tbl.clear (Env.flex_get_of env k_removed_active);
  C.Tbl.clear (Env.flex_get_of env k_removed_passive);

  Util.debugf ~section 1 "new BCE-SAT attempt" CCFun.id;

  ignore @@ do_eliminate_blocked_clauses env;

  if C.Tbl.length (Env.flex_get_of env k_bce_sat_tracked) == 0 then
    CCFormat.printf "%% BCE inprocessing removed all clauses"
  else (
    (* reinserting removed clauses *)
    let removed_actives = C.Tbl.keys (Env.flex_get_of env k_removed_active) in
    let removed_passives = C.Tbl.keys (Env.flex_get_of env k_removed_passive) in
    Util.debugf ~section 1 "reinserting %d/%d clauses" (fun k ->
        k
          (Iter.length removed_actives + Iter.length removed_passives)
          (C.Tbl.length (Env.flex_get_of env k_bce_sat_tracked)));
    Env.add_active env removed_actives;
    Env.add_simpl env removed_actives;
    Env.add_passive env removed_passives
  )

let eliminate_bce_sat env =
  steps := (!steps + 1) mod Env.flex_get_of env k_check_at;

  if !steps = 0 then do_bce_sat env ()
...
let initialize_regular env =
  ...
  (try
     ...
     List.iter (scan_cl_lits env) init_clauses;

     Util.debugf ~section 1 "logic is %s@." (fun k ->
         k
           (if !logic == NonequationalFO then
              "nonequational FO"
            else if !logic == EquationalFO then
              "equational FO"
            else if !logic == EquationalHO then
              "equational HO"
            else
     ...
     List.iter
       (fun cl ->
         CCArray.iteri
           (fun lit_idx _ -> register_task ~update_others:false lit_idx cl)
           (C.lits cl))
       init_clauses;
     (* eliminate clauses *)
     ignore (do_eliminate_blocked_clauses env);
     ...
     if
       Env.flex_get_of env k_processing_kind != `PreprocessingOnly
       || Env.flex_get_of env k_fp_mode
     then (
       if Env.flex_get_of env k_processing_kind == `InprocessingFull then
         Env.Ctx.lost_completeness (Env.get_ctx env);

       if Env.flex_get_of env k_processing_kind == `InprocessingSat then (
         Env.flex_add_of env k_removed_active (C.Tbl.create 256);
         Env.flex_add_of env k_removed_passive (C.Tbl.create 256);
         Env.flex_add_of env k_bce_sat_tracked (C.Tbl.create 256);

         let add_cl_sat cl =
           C.Tbl.add (Env.flex_get_of env k_bce_sat_tracked) cl ();
           react_clause_addded env cl
         in
         let remove_cl_sat cl =
           C.Tbl.remove (Env.flex_get_of env k_bce_sat_tracked) cl;
         ...
         Signal.on_every (Env.on_forward_simplified env) (fun (c, new_state) ->
             ...
             | _ -> react_clause_removed env c
             (* c is redundant *))
       );
       if not @@ Env.flex_get_of env k_fp_mode then
         if Env.flex_get_of env k_processing_kind = `InprocessingFull then
           Env.add_clause_elimination_rule env ~priority:1 "BCE"
             eliminate_blocked_clauses
         else
           Env.add_clause_elimination_rule env ~priority:1 "BCE_SAT"
             eliminate_bce_sat
     ) else
...
let begin_fixpoint env =
  ...
  try
    ...
    List.iter
      ...
      init_clauses;
    (* eliminate clauses *)
    let num_eliminated = do_eliminate_blocked_clauses env in
    Util.debugf ~section 2 "Step eliminates %d clauses" (fun k ->
        k num_eliminated);
...
let fixpoint_step env =
  let num_eliminated = do_eliminate_blocked_clauses env in
  Util.debugf ~section 1 "Step eliminates %d clauses" (fun k ->
      k num_eliminated);
...
let register env =
  Signal.on (Env.on_start env) (fun () -> initialize_regular env)

let setup ?(in_fp_mode = false) env =
  if Env.flex_get_of env k_enabled then (
    Env.flex_add_of env k_fp_mode in_fp_mode;
    if not (Env.flex_get_of env Avatar.k_avatar_enabled) then
      register env
    else
