...
module Make (Var : OrderedType) = struct
  ...
  let empty =
    {tab = []; basic = []; nbasic = []; assign = M.empty; bounds = M.empty}

  let mem x l = List.exists (fun y -> Var.compare x y = 0) l

  let rec empty_expr n =
    if n = 0 then [] else zero :: empty_expr (Pervasives.( - ) n 1)
  ...
  let find_expr_basic t x =
    let rec aux l l' =
      ...
      | [], [] ->
        raise
          (UnExpected "Trying to find an expression for a non-basic variable.")
      | _ ->
        raise
          (UnExpected "Internal representation error : different list length")
    in
    aux t.basic t.tab

  let find_expr_nbasic t x =
    List.map (fun y -> if Var.compare x y = 0 then one else zero) t.nbasic
  ...
  let value t x =
    try M.find x t.assign
    with Not_found ->
      ( try
          let lval = List.map (fun v' -> M.find v' t.assign) t.nbasic in
          List.fold_left ( + ) zero
            (List.map2 ( * ) lval (find_expr_basic t x))
        with Not_found ->
          raise
  ...
  let add_eq t (s, eq) =
    ...
    let l_eq =
      List.map
        (fun (c, x) -> List.map (fun y -> c * y) (find_expr_total t x))
        eq
    in
    let t_eq =
      List.fold_left (List.map2 ( + )) (empty_expr (List.length t.nbasic)) l_eq
    in
    {t with tab = t_eq :: t.tab; basic = s :: t.basic}
  ...
  let get_full_assign t =
    List.map
      (fun x -> x, value t x)
      (List.sort Var.compare (t.nbasic @ t.basic))
  ...
  let find_suitable t x =
    ...
    with Failure _ -> raise NoneSuitable
  ...
  let pivot t x y a =
    let l =
      List.map2
        ...
        (find_expr_basic t x) t.nbasic
    in
    List.map2
      (fun z e ->
        if Var.compare x z = 0 then
          l
        else (
          let k, l' = find_and_replace y e t.nbasic in
          List.map2 ( + ) l' (List.map (fun n -> k * n) l) ) )
      t.basic t.tab
  ...
  let solve_aux debug t =
    ...
    try
      while true do
        let x =
          List.find
            (fun y -> not (fst (is_within t y)))
            (List.sort Var.compare t.basic)
        in
        let _, v = is_within t x in
        try
          ...
          debug t
        with NoneSuitable -> raise (Unsat x)
      done
    with Not_found -> ()
  ...
  let ksolve ?debug:(f = fun _ _ -> ()) t =
    ...
    with
    | Unsat x -> Unsatisfiable (x, List.combine (find_expr_basic t x) t.nbasic)
    | AbsurdBounds x -> Unsatisfiable (x, [])
  ...
  *)
  ...
  let denlcm =
    List.fold_left (fun k c -> if is_real c then Z.lcm k (den c) else k) Z.one

  let lgcd k expr =
    let expr = List.filter (fun v -> is_real v && not (equal v zero)) expr in
    let aux g c = Z.gcd g (to_bigint (c * k)) in
    of_bigint
      (List.fold_left aux (to_bigint (List.hd expr * k)) (List.tl expr))

  let global_bound t =
    let m, max_coef =
      M.fold
        (fun x (l, u) (m, max_coef) ->
          let m =
            Pervasives.( + ) m
              (Pervasives.( + )
                 (if is_real l then 1 else 0)
                 (if is_real u then 1 else 0))
          in
          ...
          let max_coef =
            Z.max max_coef
              (to_bigint
                 (List.fold_left max zero
                    (List.filter is_real
                       (List.map (fun x -> abs (k * x / k')) (l :: u :: expr)))))
          in
          m, max_coef )
        t.bounds (0, Z.zero)
    in
    ...
    of_bigint
      (Z.pow (Z.mul (Z.of_int 2) (Z.mul (Z.pow (Z.of_int n) 2) max_coef)) n)
  ...
  let floor v =
    try of_bigint (Z.ediv (num v) (den v)) with Division_by_zero -> v
  ...
  let normalize int_vars t =
    ...
    let aux x expr =
      ...
      let k'' = k / k' in
      if
        List.for_all2 (fun b c -> b || equal c zero) mask expr
        && (not (equal k' one))
        && ((not (is_z (l * k / k'))) || not (is_z (u * k / k')))
      then (
  ...
  let apply_optims l t = List.fold_left (fun acc f -> acc @ f t) [] l

  let preprocess t int_vars =
    let l = [tighten int_vars; normalize int_vars] in
    apply_optims l t
    ...
    try
      while true do
        ...
        try
          ...
          let nsol =
            List.filter (fun (x, v) -> mem x int_vars && not (is_z v)) sol
          in
          if nsol = [] then
            raise (SolutionFound sol)
          else (
        ...
        | Unsat x ->
          res :=
            Some (Explanation (x, List.combine (find_expr_basic t x) t.nbasic))
        | AbsurdBounds x -> res := Some (Explanation (x, []))
      done;
      raise (UnExpected "")
    with
    | Queue.Empty -> Unsatisfiable final
    | SolutionFound sol -> Solution sol
  ...
  let print_bounds print_var fmt b =
    M.iter
      (fun x (l, u) ->
        Format.fprintf fmt "%s\t<= %a\t<= %s@\n" (to_string l) print_var x
          (to_string u) )
      b

  let print_tab print_var fmt (l, tab) =
    let aux fmt =
      List.iter (fun y -> Format.fprintf fmt "%s\t" (to_string y))
    in
    List.iter2
      (fun x e -> Format.fprintf fmt "%a\t%a@\n" print_var x aux e)
      l tab

  let print_assign print_var fmt l =
    List.iter
      (fun (x, c) -> Format.fprintf fmt "%a -> %s;@ " print_var x (to_string c))
      l
  ...
  let print_debug print_var fmt t =
    Format.fprintf fmt
      "@[<hov 2>*** System state ***@\n\
       ...
       %a@]@\n\
       ******** END ********@."
      (fun fmt -> List.iter (fun x -> Format.fprintf fmt "%a\t" print_var x))
      t.nbasic (print_tab print_var) (t.basic, t.tab) (print_bounds print_var)
      t.bounds (print_assign print_var) (get_assign t)
end
    ...
    List.fold_left
      (fun simpl c ->
        ...
        | LessEq ->
          ...
          if Q.sign coeff < 0 then
            add_bounds simpl (var, const', Q.inf)
          else
            add_bounds simpl (var, Q.minus_inf, const')
        | GreaterEq ->
          if Q.sign coeff < 0 then
            add_bounds simpl (var, Q.minus_inf, const')
          else
            add_bounds simpl (var, const', inf) )
      simpl l
end
