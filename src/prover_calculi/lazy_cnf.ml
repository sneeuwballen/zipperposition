...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let check_eq_cnf_ordering_conditions lhs rhs =
    ...
    | Comparison.Lt | Leq ->
        is_noninterpeted rhs
    | Gt | Geq ->
        is_noninterpeted lhs
    | _ ->
        is_noninterpeted lhs || is_noninterpeted rhs
             ...
             | Some f ->
                 let rule_name =
                   CCFormat.sprintf "lazy_cnf_%sscoping"
                     (if Env.flex_get k_scoping == `Maxi then "maxi" else "mini")
                 in
                 let app_sign = if sign then CCFun.id else T.Form.not_ in
...
let extension =
  let register env =
    ...
    ET.setup ()
  in
  {Extensions.default with Extensions.name= "lazy_cnf"; env_actions= [register]}
...
let () =
    ...
    ; ( "--lazy-cnf-skolem-mode"
      , Arg.Symbol
          ...
          , fun str ->
              match str with
              | "skolem-recycle" ->
                  _skolem_mode := `SkolemRecycle
              | "skolem-fresh" ->
                  _skolem_mode := `SkolemAlwaysFresh
              | "choice" ->
                  _skolem_mode := `Choice
              | _ ->
                  assert false )
  ...
  Params.add_to_modes
    ...
    ; "fo-complete-basic" ] (fun () -> enabled := false ) ;
