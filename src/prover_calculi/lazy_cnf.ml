...
let fold_lits env c =
  ...
  if Env.flex_get_of env k_only_eligible then (
...
let check_eq_cnf_ordering_conditions env lhs rhs =
  let is_noninterpeted t = CCOpt.is_some (Term.head t) in
  let ord = Ctx.ord (Env.get_ctx env) in
  Env.flex_get_of env k_clausify_eq_max_nonint
  ||
...
let rec enum_bool_funs env ~ty =
  assert (
    Iter.for_all (fun ty -> Type.is_fun ty || Type.is_prop ty) (Type.Seq.sub ty));
  ...
  let insert_defining_clauses id sym arg_combs table =
    List.iteri
      (fun i arg_comb ->
        ...
        let res =
          C.create ~ctx:(Env.get_ctx env) ~penalty:1 ~trail:Trail.empty lits
            proof
        in
        Util.debugf ~section 2 "defining: @[%a@]@." (fun k -> k C.pp res);
        Env.add_passive env (Iter.singleton res))
      arg_combs
  in
  ...
  try Type.Tbl.find _ty_map ty
  with Not_found ->
    let args, ret = Type.open_fun ty in
    let arg_members = List.map (fun ty -> enum_bool_funs env ~ty) args in
    let ret_members = enum_bool_funs env ~ty:ret in
    let new_members = create_members arg_members ret_members in

    Type.Tbl.add _ty_map ty new_members;

    Ctx.declare_syms (Env.get_ctx env)
...
let check_size_limits env sign f =
  let limit = Env.flex_get_of env k_simp_limit in
  Util.debugf ~section 1 "checking limit: %d@." (fun k -> k limit);
  Env.flex_get_of env k_lazy_cnf_kind != `Simp
  || limit < 0
...
let rename_eq env ~c ~should_rename lhs rhs sign =
  ...
  let app_sign =
    if sign then
      ...
      T.Form.not_
  in
  if Env.flex_get_of env k_rename_eq then
...
let cnf_scope_form env form =
  let kind = Env.flex_get_of env k_scoping in
  let open CCOpt in
  let rec maxiscoping_eligible l =
    let get_quant t =
      let t = Combinators.expand env t in
      match T.view t with
    ...
    | [ x ] -> get_quant x
    | x :: xs ->
      get_quant x >>= fun (b, ty, body) ->
      maxiscoping_eligible xs >>= fun (b', ty', bodies) ->
  ...
  let miniscope hd f =
    ...
    let f = Combinators.expand env f in
    if T.is_fun f then (
...
let lazy_clausify_driver env ?(ignore_eq = false)
  ...
  let eligible_to_ignore_eq ~ignore_eq lhs rhs =
    (not (check_eq_cnf_ordering_conditions env lhs rhs))
    || ignore_eq
  ...
  let should_clausify sign f =
    ...
    || Env.flex_get_of env k_lazy_cnf_kind != `Ignore
       && check_size_limits env sign f
  in

  fold_lits env c
  |> Iter.fold_while
       (fun acc (lhs, rhs, sign, pos) ->
         let i, _ = Ls.Pos.cut pos in
         let lit = (C.lits c).(i) in
         if L.is_predicate_lit lit then (
           ...
           | T.AppBuiltin (Imply, [ a; b ]) when should_clausify sign lhs ->
             ...
             if sign then
               if
                 force_clausification
                 || Env.flex_get_of env k_lazy_cnf_kind != `Simp
                 || Env.flex_get_of env k_clausify_implications
               then
                 return acc
           ...
           | T.AppBuiltin (((ForallConst | ExistsConst) as hd), [ _; f ])
             when Env.flex_get_of env k_lazy_cnf_kind != `Simp
                  || not (Env.flex_get_of env k_inf_quant) ->
             let var_offset = T.Seq.max_var (C.Seq.vars c) + 1 in
             let res, hd, subst_term, rule_name =
               clausify_quant env ~parent:c ~var_offset ~sign ~quant_body:f
                 ~quant_hd:hd
             in
             ...
             if Type.returns_prop (T.ty subst_term) && hd == ForallConst then (
               ...
               Signal.send
                 (Env.on_pred_var_elimination env)
                 (List.hd res_cl, subst_term)
             );

             let sub_ty = T.ty subst_term in
             if
               Env.flex_get_of env k_enum_bool_funs
               && Type.Seq.has_bools_only sub_ty
             then (
               let repls = enum_bool_funs env ~ty:sub_ty in
               if Env.flex_get_of env k_replace_bool_fun_quants then (
                 let bodies =
                   List.map
                     (fun r -> Lambda.eta_reduce @@ Lambda.whnf (T.app f [ r ]))
                     repls
                 in
                 return acc
                 @@
           ...
           if
             ...
             || not (check_size_limits env sign (T.Form.equiv lhs rhs))
           then
             continue acc
           else if sign then
             return acc
...
let clausify_quants env ~proof_cons c =
  C.lits c
  |> CCArray.find_map_i (fun i -> function
       | Literal.Equation (lhs, _, _) as lit when Literal.is_predicate_lit lit
         ...
         | T.AppBuiltin (((ForallConst | ExistsConst) as hd), [ _; body ]) ->
           ...
           let res, hd, subst_term, rule_name =
             clausify_quant env ~parent:c ~var_offset ~sign ~quant_body:body
               ~quant_hd:hd
           in
           assert (Type.is_prop (T.ty res));
           ...
           if Type.returns_prop (T.ty subst_term) && hd == ForallConst then (
             assert (T.is_var subst_term);
             Signal.send (Env.on_pred_var_elimination env) (res_cl, subst_term)
           );
           Some res_cl
         | _ -> None)
       | _ -> None)

let reduce_quantifiers env c =
  let proof_cons =
    Proof.Step.simp ~infos:[]
      ~tags:[ Proof.Tag.T_live_cnf; Proof.Tag.T_dont_increase_depth ]
  in
  clausify_quants env ~proof_cons c
...
let rename_subformulas env c =
  ...
  let clausify_defs new_defs =
    List.fold_left
      (fun acc c ->
        lazy_clausify_driver env ~ignore_eq:false ~force_clausification:true
          ~proof_cons c
        |> Iter.to_list
        |> fun l ->
...
let inf_quantifiers env c =
  let proof_cons =
    Proof.Step.inference ~infos:[]
      ~tags:[ Proof.Tag.T_live_cnf; Proof.Tag.T_dont_increase_depth ]
  in
  CCOpt.map_or ~default:[] (fun c -> [ c ]) @@ clausify_quants env ~proof_cons c

let clausify_eq env c =
  ...
  |> Iter.fold
       (fun acc (lhs, rhs, sign, pos) ->
         ...
         if
           (not (L.is_predicate_lit lit))
           && Type.is_prop (T.ty lhs)
           && check_eq_cnf_ordering_conditions env lhs rhs
           && check_size_limits env sign (T.Form.equiv lhs rhs)
         then (
           let new_cls =
             if sign then
               mk_or ~proof_cons ~rule_name [ T.Form.not_ lhs; rhs ] c i
           ...
           in
           let pen_inc =
             if T.is_ground lhs && T.is_ground rhs then
               0
             else if T.is_app_var lhs || T.is_app_var rhs then
               3
             else
               1
           in
           if Env.flex_get_of env k_penalize_eq_cnf then
...
let cnf_scope env c =
  ...
  |> Iter.fold_while
       (fun _ (lhs, rhs, sign, pos) ->
         ...
         in
         if L.is_predicate_lit lit && T.is_appbuiltin lhs then (
           match cnf_scope_form env lhs with
           | Some f ->
             let rule_name =
               CCFormat.sprintf "lazy_cnf_%sscoping"
                 (if Env.flex_get_of env k_scoping == `Maxi then
                    "maxi"
                  else
                    "mini")
             in
...
let lazy_clausify_simpl env c =
  update_form_counter ~action:`Increase env c;
  ...
  in
  let res =
    Iter.to_list @@ lazy_clausify_driver env ~ignore_eq:true ~proof_cons c
  in
  if not @@ CCList.is_empty res then (
    ...
    update_form_counter ~action:`Decrease env c;
    CCList.iter (update_form_counter ~action:`Increase env) res
  ) else
    Util.debugf ~section 3 "lazy_cnf_simp(@[%a@])=Ø" (fun k -> k C.pp c);
  if CCList.is_empty res then
    None
  else
    Some res

let lazy_clausify_inf env c =
  let proof_cons =
    Proof.Step.inference ~infos:[]
      ~tags:[ Proof.Tag.T_live_cnf; Proof.Tag.T_dont_increase_depth ]
  in
  let res =
    Iter.to_list (lazy_clausify_driver env ~ignore_eq:false ~proof_cons c)
  in
  if not @@ CCList.is_empty res then (
...
let setup env =
  if !enabled then (
    ...
    (match Env.flex_get_of env k_lazy_cnf_kind with
    | `Inf | `Ignore ->
      Env.add_unary_inf env "lazy_cnf" lazy_clausify_inf;
      if Env.flex_get_of env k_simplify_quant then
        Env.add_basic_simplify env reduce_quantifiers
    | `Simp ->
      Env.add_unary_inf env "elim eq" clausify_eq;
      if not (Env.flex_get_of env k_clausify_implications) then
        Env.add_unary_inf env "inf_imp" clausify_imp;
      if Env.flex_get_of env k_inf_quant then
        Env.add_unary_inf env "inf_quant" inf_quantifiers;
      Env.add_multi_simpl_rule env ~priority:5 lazy_clausify_simpl;
      if Env.flex_get_of env k_lazy_cnf_eager then
        Env.add_cheap_multi_simpl_rule env lazy_clausify_simpl);

    (* ** IMPORTANT **
       Due to correctly set priorioty, renaming will run before simplification *)
    if Env.flex_get_of env k_renaming_threshold > 0 then (
      Env.add_multi_simpl_rule env ~priority:4 rename_subformulas;
      if Env.flex_get_of env k_lazy_cnf_eager then
        Env.add_cheap_multi_simpl_rule env rename_subformulas
    );
    if Env.flex_get_of env k_scoping != `Off then
