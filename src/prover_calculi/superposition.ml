...
let stat_equality_resolution_call =
  Util.mk_stat "sup.equality_resolution calls"
...
let prof_infer_equality_resolution =
  Util.mk_profiler "sup.infer_equality_resolution"

let prof_infer_equality_factoring =
  Util.mk_profiler "sup.infer_equality_factoring"
...
module Make (Env : Env.S) : S with module Env = Env = struct
  ...
  let _update_active f c =
    ...
    _idx_sup_into :=
      Lits.fold_terms ~vars:!_sup_at_vars ~ty_args:false ~ord ~which:`Max
    ...
    _idx_sup_from :=
      Lits.fold_eqn ~ord ~both:true ~sign:true ~eligible:(C.Eligible.param c)
        (C.lits c)
      |> Iter.fold
    ...
    _idx_back_demod :=
      Lits.fold_terms ~vars:false ~ty_args:false ~ord ~subterms:true
  ...
  let _update_simpl f c =
    ...
    let idx' =
      ...
      | [|Lit.Equation (l, r, true)|] ->
        ...
        | Comparison.Gt -> f idx (l, r, true, c)
        | Comparison.Lt -> f idx (r, l, true, c)
        | Comparison.Incomparable ->
          let idx = f idx (l, r, true, c) in
          f idx (r, l, true, c)
        | Comparison.Eq -> idx (* no modif *) )
      | [|Lit.Equation (l, r, false)|] -> f idx (l, r, false, c)
      | [|Lit.Prop (p, sign)|] -> f idx (p, T.true_, sign, c)
      | _ -> idx
    in
    _idx_simpl := idx';
    Signal.ContinueListening
          ...
          if
            ...
            && CCList.(
               ...
               |> List.for_all (fun idx ->
                      match T.as_var s_args.(idx) with
                      | Some v ->
  ...
  let sup_at_var_condition info var replacement =
    ...
    then (
      ...
      let unique_args_of_var =
        ...
        |> Iter.fold_while
             (fun unique_args (t, _) ->
               if Head.term_to_head t == Head.term_to_head var then
                 if unique_args == Some (Head.term_to_args t) then
                   unique_args, `Continue
                 (* found the same arguments of var again *)
                 else
                   None, `Stop (* different arguments of var found *)
               else
                 unique_args, `Continue
    ...
    try
      ...
      | Lit.Prop (_, true), P.Arg (_, P.Left P.Stop) ->
        if T.equal t' T.true_ then
          raise (ExitSuperposition "will yield a bool tautology")
      ...
      if !_restrict_hidden_sup_at_vars then (
        ...
        | Some (var, replacement)
          when not (!_sup_at_vars && sup_at_var_condition info var replacement)
          ->
  ...
  let do_superposition info acc =
    ...
    if !_use_simultaneous_sup then
      do_simultaneous_superposition info acc
    else
      do_classic_superposition info acc
  ...
  let infer_active clause =
    ...
    let new_clauses =
      Lits.fold_eqn ~sign:true ~ord:(Ctx.ord ()) ~both:true ~eligible
        (C.lits clause)
      |> Iter.fold
           (fun acc (s, t, _, s_pos) ->
  ...
  let infer_equality_resolution clause =
    ...
    let new_clauses =
      Lits.fold_eqn ~sign:false ~ord:(Ctx.ord ()) ~both:false ~eligible
        (C.lits clause)
      |> Iter.filter_map (fun (l, r, _, l_pos) ->
             ...
             try
               ...
               if
                 BV.get (C.eligible_res_no_subst clause) pos
               ...
               then (
                 ...
                 None
             with Unif.Fail -> (* l and r not unifiable, try next *)
                               None )
      |> Iter.to_rev_list
    in
  ...
  let do_eq_factoring info acc =
    ...
    if
      ...
      <> Comp.Lt
      && C.is_eligible_param (info.clause, info.scope) subst
           ~idx:info.active_idx
    then (
  ...
  *)
  ...
  let demod_nf ?(restrict = lazy_false) (st : demod_state) c t : T.t =
    ...
    let rec reduce_at_root ~restrict t k =
      ...
      let step =
        ...
        |> Iter.find_map (fun (l, r, (_, _, sign, unit_clause), subst) ->
               ...
               then (
                 Util.debugf ~section 5
                   "@[<hv2>demod:@ @[<hv>t=%a[%d],@ l=%a[%d],@ r=%a[%d]@],@ \
                    subst=@[%a@]@]" (fun k ->
                     k T.pp t 0 T.pp l cur_sc T.pp r cur_sc S.pp subst );
                 (* sanity checks *)
    ...
    (* done one rewriting step, continue *)
    ...
    and normal_form ~restrict t k =
      ...
      | T.Fun (ty_arg, body) ->
        ...
        normal_form ~restrict:lazy_false body (fun body' ->
    ...
    and normal_form_l l k =
      ...
      | [] -> k []
      | t :: tail ->
        normal_form ~restrict:lazy_false t (fun t' ->
  ...
  let demodulate_ c =
      ...
      let strictly_max =
        lazy
          ...
          | Lit.Equation (t1, t2, true) ->
            ( match O.compare ord t1 t2 with
            | Comp.Gt -> [t1]
            | Comp.Lt -> [t2]
            | _ -> [] )
          | Lit.Prop (t, true) -> [t]
    ...
    if CCList.is_empty st.demod_clauses then (* no rewriting performed *)
      ...
      Util.debugf ~section 3
        "@[<hv2>demodulate@ @[%a@]@ into @[%a@]@ using {@[<hv>%a@]}@]"
        (fun k ->
  ...
  let is_tautology c =
    let is_tauto =
      Lits.is_trivial (C.lits c) || Trail.is_trivial (C.trail c)
    in
    if is_tauto then
    ...
    let cc =
      Array.fold_left
        (fun cc lit ->
          ...
          | Lit.Prop (p, false) -> Congruence.FO.mk_eq cc p T.true_
          | _ -> cc )
        cc (C.lits c)
    in
    let res =
      CCArray.exists
        (function
          ...
          | Lit.Prop (p, true) -> Congruence.FO.is_eq cc p T.true_
          | _ -> false )
        (C.lits c)
    in
    if res then (
  ...
  let positive_simplify_reflect c =
    ...
    let rec iterate_lits acc lits clauses =
      ...
      | (Lit.Equation (s, t, false) as lit) :: lits' ->
        ( match equatable_terms clauses s t with
        | None ->
          (* keep literal *)
          iterate_lits (lit :: acc) lits' clauses
        | Some new_clauses ->
          (* drop literal, remember clauses *)
    ...
    and equatable_terms clauses t1 t2 =
      ...
      | T.Classic.App (f, ss), T.Classic.App (g, ts)
        ...
        | None ->
          (* otherwise try to make subterms pairwise equal *)
          let ok, clauses =
            List.fold_left2
              (fun (ok, clauses) t1' t2' ->
                if ok then (
                  match equatable_terms clauses t1' t2' with
                  | None -> false, []
    ...
    (* try to solve it with a unit equality *)
    ...
    and equate_root clauses t1 t2 =
      try
        ...
        |> Iter.iter (fun (l, r, (_, _, _, c'), subst) ->
               assert (Unif.FO.equal ~subst (l, 1) (t1, 0));
               if Unif.FO.equal ~subst (r, 1) (t2, 0) && C.trail_subsumes c' c
               then (
                 (* t1!=t2 is refuted by l\sigma = r\sigma *)
                 Util.debugf ~section 4
                   "@[<2>equate @[%a@]@ and @[%a@]@ using @[%a@]@]" (fun k ->
                     k T.pp t1 T.pp t2 C.pp c' );
  ...
  let negative_simplify_reflect c =
    ...
    let rec iterate_lits acc lits clauses =
      ...
      | (Lit.Equation (s, t, true) as lit) :: lits' ->
        ( match can_refute s t, can_refute t s with
        | None, None ->
          (* keep literal *)
          iterate_lits (lit :: acc) lits' clauses
    ...
    and can_refute s t =
      try
        ...
        |> Iter.iter (fun (l, r, (_, _, _, c'), subst) ->
               ...
               if Unif.FO.equal ~subst (r, 1) (t, 0) && C.trail_subsumes c' c
               then (
                 (* TODO: useless? *)
                 let subst = Unif.FO.matching ~subst ~pattern:(r, 1) (t, 0) in
                 Util.debugf ~section 3
                   "@[neg_reflect eliminates@ @[%a=%a@]@ with @[%a@]@]"
  ...
  let subsumed_by_active_set c =
    ...
    if res then (
      Util.debugf ~section 3 "@[<2>@[%a@]@ subsumed by active set@]" (fun k ->
          k C.pp c );
  ...
  let subsumed_in_active_set acc c =
    ...
    let try_eq_subsumption = C.is_unit_clause c && Lit.is_pos (C.lits c).(0) in
    (* use feature vector indexing *)
    let res =
  ...
  let rec contextual_literal_cutting_rec c =
    ...
    then
      ...
      | Some (new_lits, _, c', subst, tags) ->
        ...
        let proof =
          Proof.Step.inference ~rule:(Proof.Rule.mk "clc") ~tags
            [ C.proof_parent c;
  ...
  let condensation c = Util.with_prof prof_condensation condensation_rec c
  ...
  let register () =
    let open SimplM.Infix in
    let rw_simplify c =
      demodulate c >>= basic_simplify >>= positive_simplify_reflect
  ...
  Params.add_opts
    [ ( "--semantic-tauto",
      ...
      ( "--restrict-hidden-sup-at-vars",
        ...
        " perform hidden superposition at variables only under certain \
         ordering conditions" ) ]
