...
module Make (E : Env_intf.S) = struct
           ...
           |> Iter.filter_map (fun (rule, us) ->
                  ...
                  let proof =
                    Proof.Step.inference
                      ...
                      ; Proof.Parent.from_subst renaming (RW.Rule.as_proof (RW.T_rule rule), sc_rule) subst ]
                      ~rule:(Proof.Rule.mk "narrow")
                  in
                  let c' =
                    C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (c_guard @ CCArray.to_list lits') proof
                  in
                  Util.debugf ~section 3
                    "@[<2>term narrowing:@ from `@[%a@]`@ to `@[%a@]`@ using rule `%a`@ and subst @[%a@]@]"
  ...
  let ctx_narrow_find (s, sc_a) sc_p : (RW.Rule.t * Position.t * Unif_subst.t) Iter.t =
    let find_term (r : RW.Term.rule) =
      ...
      |> Iter.filter (fun (t, _) ->
             ...
             | T.Classic.Var _ | T.Classic.DB _ | T.Classic.AppBuiltin (_, _) | T.Classic.NonFO ->
                 false )
  ...
  let ctx_narrow_with ~ord s t s_pos c acc : C.t list =
    ...
    let do_narrowing rule rule_pos (us : Unif_subst.t) =
      let rule_clauses =
        match rule with RW.T_rule r -> [[|RW.Term.Rule.as_lit r|]] | RW.L_rule r -> RW.Lit.Rule.as_clauses r
      in
      let renaming = Subst.Renaming.create () in
      let subst = Unif_subst.subst us in
      let c_guard = Literal.of_unif_subst renaming us in
      ...
      | _ ->
          ...
          |> List.map (fun rule_clause ->
                 ...
                 let ctx =
                   Literal.apply_subst_list renaming subst (CCArray.except_idx (C.lits c) idx_active, sc_a)
                 in
                 (* build new clause *)
                 let proof =
                   Proof.Step.inference
                 ...
                 Util.debugf ~section 4
                   "(@[<2>ctx_narrow@ :rule %a[%d]@ :clause %a[%d]@ :pos %a@ :subst %a@ :yield %a@])" (fun k ->
...
let rewrite_tst_stmt stmt =
  ...
  | Assert f -> (
    ...
    | Some (f', proof) ->
        Statement.assert_ ~proof:(Proof.S.step proof) f'
    | None ->
        stmt )
  | Lemma fs -> (
    match aux_l fs with
  ...
  | Goal g -> (
    ...
    | Some (g', proof) ->
        Statement.goal ~proof:(Proof.S.step proof) g'
    | None ->
        stmt )
