...
let simpl_term env t = simpl_term_ t
...
let setup env ?(ctx_narrow = true) ~narrowing ~has_rw () =
  ...
  Env.add_rewrite_rule env "rewrite_defs" simpl_term;
  if narrowing then (
    Env.add_binary_inf env "narrow_term_defs" narrow_term_passive;
    Env.add_unary_inf env "narrow_lit_defs" narrow_lits
  );
  if ctx_narrow then Env.add_binary_inf env "ctx_narrow" contextual_narrowing;
  if has_rw then Ctx.lost_completeness (Env.get_ctx env);
...
let post_cnf stmts st =
  ...
  let has_rw =
    ...
    |> Iter.exists (fun st ->
           match Statement.view st with
           | Statement.Rewrite _ | Statement.Def _ -> true
           | _ -> false)
  in
...
let rewrite_tst_stmt stmt =
  ...
  let aux_l fs =
    ...
    if List.for_all CCOpt.is_none ts then
      None
    else (
      let proof = ref [] in
  ...
  | Lemma fs ->
    ...
    | Some (fs', parents) ->
      ...
      let fs_parents =
        List.map
          (fun f -> Proof.Parent.from (Proof.S.mk_f_esa ~rule f stmt_parents))
          fs
        @ parents
      in
      ...
      (fun stmt ->
        ...
        with Type.Conv.Error t ->
          Util.debugf ~section 1
            "@[%a@] cannot be converted because it uses unsupported features \
             (such as ite, let and others)" (fun k -> k Statement.pp_input stmt);
          stmt)
      stmts
  ) else
    stmts
