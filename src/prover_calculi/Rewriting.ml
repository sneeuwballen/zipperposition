  ...
  let narrow_term_passive_ c : C.t list =
    ...
    |> Iter.flat_map (fun (u_p, passive_pos) ->
        ...
        |> Iter.filter_map (fun (rule, us) ->
            ...
            in
            Util.debugf ~section 3
              "@[<2>term narrowing:@ from `@[%a@]`@ to `@[%a@]`@ using rule \
               `%a`@ and subst @[%a@]@]" (fun k ->
  ...
  let ctx_narrow_with ~ord s t s_pos c acc : C.t list =
    ...
    let do_narrowing rule rule_pos (us : Unif_subst.t) =
      let rule_clauses =
        match rule with
        | RW.T_rule r ->
            [[|RW.Term.Rule.as_lit r|]]
        | RW.L_rule r ->
            RW.Lit.Rule.as_clauses r
      in
      let renaming = Subst.Renaming.create () in
      let subst = Unif_subst.subst us in
      let c_guard = Literal.of_unif_subst renaming us in
      ...
      | _ ->
          ...
          |> List.map (fun rule_clause ->
              ...
              let idx_active =
                match s_pos with P.Arg (n, _) -> n | _ -> assert false
              in
              let ctx =
...
let post_cnf stmts st =
  ...
  let has_rw =
    ...
    |> Iter.exists (fun st ->
        match Statement.view st with
        | Statement.Rewrite _ | Statement.Def _ ->
            true
        | _ ->
            false )
  in
        ...
        with Type.Conv.Error t ->
          Util.debugf ~section 1
            "@[%a@] cannot be converted because it uses unsupported features \
             (such as ite, let and others)" (fun k ->
              k Statement.pp_input stmt ) ;
          stmt )
      stmts
  else stmts
