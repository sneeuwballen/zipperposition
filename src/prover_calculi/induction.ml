...
let stat_generalize_vars_active_pos =
  Util.mk_stat "induction.generalize_vars_active_pos"

let stat_generalize_terms_active_pos =
  Util.mk_stat "induction.generalize_terms_active_pos"
...
module Make_goal (E : Env_intf.S) : sig
  ...
  let trivial : t = {cut = Cut_form.trivial; test_res = Lazy.from_val S_trivial}
  ...
  let test_ (cs : Literals.t list) : status =
    (* test and save *)
    if List.for_all trivial_c cs then
      S_trivial
    else (
      match Test_prop.check_form cs with
      | Test_prop.R_ok -> S_ok
  ...
  module UF_clauses = UnionFind.Make (struct
    type key = T.var
  ...
  let split (g : t) : t list =
    let uf = vars g |> T.VarSet.to_list |> UF_clauses.create
    and ground_ = ref [] in
    List.iter
      (fun c ->
        let vars = Literals.vars c in
        List.iter (fun v -> UF_clauses.add uf v [c]) vars;
        match vars with
        | [] ->
        ...
        | v :: other_vars ->
          ...
          List.iter (fun v' -> UF_clauses.union uf v v') other_vars )
      (cs g);
    let all_clusters =
      (UF_clauses.to_seq uf |> Iter.map snd |> Iter.to_rev_list) @ !ground_
    in
    let new_goals = List.rev_map of_form all_clusters in
    if List.length new_goals > 1 then (
      Util.incr_stat stat_split_goal;
      Util.debugf ~section 3
        "(@[<2>split_goal@ :goal %a@ :new_goals (@[<hv>%a@])@])" (fun k ->
          k pp g (Util.pp_list pp) new_goals ) );
    new_goals
  ...
  let test_goal_is_ok (g : t) : bool =
    ...
    | S_trivial ->
      ...
      false
    | S_falsifiable subst ->
      Util.debugf ~section 2 "(@[<2>lemma_absurd@ @[%a@]@ :subst %a@])"
  ...
  let check_not_absurd_or_trivial_ (g : t) : bool =
    ...
    try
        ...
        if C.comes_from_goal c then
          ...
          if !n + 2 < max_steps_ then (
            ...
            |> Iter.filter_map (fun new_c ->
                   ...
                   if C.comes_from_goal new_c then
                     None
      ...
      Util.debugf ~section 2
        "@[<2>lemma @[%a@]@ apparently not absurd (after %d steps; \
         trivial:%B)@]" (fun k -> k pp g !n !trivial);
  ...
  let is_acceptable_goal (g : t) : bool =
    ...
    let res = test_goal_is_ok g && check_not_absurd_or_trivial g in
  ...
  let add_lemma, has_been_tried =
    ...
    let add f = FV.add tbl f () and mem (g : t) = FV.mem tbl (form g) in
    add, mem
end
...
module T_view : sig
  ...
  let view (t : term) : term t =
    ...
    | T.Const id when Classify_cst.id_is_defined id ->
      ( match RW.as_defined_cst id with
  ...
  let active_subterms t yield : unit =
    let rec aux t =
      ...
      | T_app_defined (_, c, l) ->
        ...
        List.iteri
          (fun i sub -> if IArray.get pos i = Defined_pos.P_active then aux sub)
          l
      | T_fun (_, _) ->
        () (* no induction under λ, we follow WHNF semantics *)
      | T_var _ | T_db _ -> ()
      | T_app (f, l) ->
        aux f;
        List.iter aux l
      | T_app_cstor (_, l) -> List.iter aux l
      | T_builtin (_, l) | T_app_unin (_, l) -> List.iter aux l
    in
    aux t
end
...
module Make (E : Env.S) (A : Avatar_intf.S with module E = E) = struct
  module Env = E
  module Ctx = E.Ctx
  module C = E.C
  module BoolBox = BBox
  ...
  let ind_on_vars (cut : A.cut_res) (vars : T.var list) : C.t list =
    ...
    let clauses =
      ...
      |> CCList.flat_map (fun (cases : (T.var * Cover_set.case) list) ->
             ...
             Util.debugf ~section 2
               "(@[<2>induction on (@[%a@])@ :form %a@ @[<2>:cases \
                (@[%a@])@]@ :depth %d@ @[<2>:res [@[<hv>%a@]]@]@])" (fun k ->
                 k (Util.pp_list HVar.pp) vars Cut_form.pp g
                   (Util.pp_list
    ...
    *)
    let vars_at_active_pos (f : form) : generalization list =
      ...
      | _ ->
        ...
        let m =
          let offset =
            Cut_form.vars f |> T.VarSet.to_seq |> Iter.map HVar.id |> Iter.max
        ...
        Util.debugf ~section 5
          "(@[<2>candidate_generalize@ :of %a@ :gen_to %a@ :by \
           vars_active_pos :on (@[%a@])@ :map {@[%a@]}@])" (fun k ->
    ...
    let terms_at_active_pos (f : form) : generalization list =
      let relevant_subterms =
        ...
        |> Iter.filter_map (function
             | P_active, pos, t ->
               ( match T_view.view t with
               | T_view.T_app_unin (id, []) when Ind_cst.id_is_sub id -> None
      ...
      |> CCList.filter_map (fun t ->
             ...
             let v =
               Cut_form.vars f |> T.VarSet.to_seq |> Iter.map HVar.id
    ...
    let all =
      ...
      and g2 =
        if Env.flex_get k_generalize_term then terms_at_active_pos else id
      and ( <++> ) o (f, x) =
        match o with
        | [] -> f x
        | l -> l
  ...
  module UF_vars = UnionFind.Make (struct
    type key = T.var
  ...
  let find_var_clusters (f : Cut_form.t) (vars : T.var list) : T.var list list
    ...
    |> Iter.iter (fun t ->
           ...
           | _ -> () );
    let res =
      UF_vars.to_seq uf |> Iter.map snd
      |> Iter.filter_map (fun vars ->
             (* eliminate non-inductive variables *)
             let vars =
               List.filter
                 (fun v -> Ind_ty.is_inductive_type @@ HVar.ty v)
                 vars
             in
  ...
  let prove_by_ind (clauses : C.t list) ~ignore_depth ~generalize_on : unit =
    ...
    and no_pos_lemma_in_trail () =
      Iter.of_list clauses |> Iter.map C.trail |> Iter.flat_map Trail.to_seq
...
let () =
  Params.add_opts
    [ "--induction", Options.switch_set true enabled_, " enable induction";
      ...
      ( "--induction-depth",
        Arg.Set_int depth_,
        " maximum depth of nested induction" );
      ( "--ind-only-active-pos",
        Arg.Set limit_to_active,
        " limit induction to active positions" );
      ( "--no-ind-only-active-pos",
        Arg.Clear limit_to_active,
        " limit induction to active positions" );
      ( "--ind-coverset-depth",
        Arg.Set_int coverset_depth,
        " coverset depth in induction" );
      ( "--ind-goal-assess",
        Arg.Set_int goal_assess_limit,
        " number of steps for assessing potential lemmas" );
      "--ind-sub-cst", Arg.Set ind_sub_cst, " do induction on sub-constants";
      ( "--no-ind-sub-cst",
        Arg.Clear ind_sub_cst,
        " do not do induction on sub-constants" );
