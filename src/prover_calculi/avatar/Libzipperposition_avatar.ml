...
let extension =
  let action (env : Env.t) =
    ...
    Env.flex_add_of (Env.get_global ()) k_back_simplify_trail
      !back_simplify_trail_;
    Env.flex_add_of (Env.get_global ()) k_abstract_known_singletons
      !abstract_known_singletons;
    Env.flex_add_of (Env.get_global ()) k_split_only_initial !split_only_initial;
    Env.flex_add_of (Env.get_global ()) k_split_only_goals !split_only_goals;
    Env.flex_add_of (Env.get_global ()) k_split_only_ground !split_only_ground;
    Env.flex_add_of (Env.get_global ()) k_max_trail_size !max_trail_size;
    Env.flex_add_of (Env.get_global ()) k_infer_from_components
        ...
        Arg.Symbol
          ( [ "lazy"; "eager"; "off" ],
            fun s ->
              ...
              | "off" -> avatar_kind := `Off
              | _ -> assert false ),
        " enable Avatar splitting" );
      "--print-lemmas", Arg.Set show_lemmas_, " show status of Avatar lemmas";
      ( "--avatar-simp-trail",
        Arg.Set simplify_trail_,
        " simplify boolean trails in Avatar" );
      ( "--no-avatar-simp-trail",
        Arg.Clear simplify_trail_,
        " do not simplify boolean trails in Avatar" );
      ( "--avatar-backward-simp-trail",
        Arg.Set back_simplify_trail_,
        " backward-simplify boolean trails in Avatar" );
      ( "--no-avatar-backward-simp-trail",
        Arg.Clear back_simplify_trail_,
        " do not backward-simplify boolean trails in Avatar" );
      ( "--abstract-known-singletons",
        Arg.Bool (fun b -> abstract_known_singletons := b),
        " if a clause C <- [|A1, ..., An|] cannot be split, but its component \
         is "
        ^ " C is known, then add \\neg A1 \\lor ... \\lor \\neg An \\lor [| C \
           |] to the " ^ " set of SAT clauses." );
      ( "--split-only-initial",
        Arg.Bool (fun b -> split_only_initial := b),
        " split only initial clauses" );
      ( "--split-only-goals",
        Arg.Bool (fun b -> split_only_goals := b),
        " split only clauses that interacted with goal" );
      ( "--split-only-ground",
        Arg.Bool (( := ) split_only_ground),
        " split only ground clauses" );
      ( "--max-trail-size",
        Arg.Int (fun v -> max_trail_size := v),
        " sets the limit of the trail size that a clause can have to be \
         splittable. " ^ " Negative value sets the limit to infinity" );
      ( "--infer-from-components",
        Arg.Bool (fun b -> infer_from_components := b),
        " when empty clause \bot <- A is infered, if A has only one component,\n\
        \        skolemize and negate the component and add it to the proof \
         state. " );
    ];
  Params.add_to_modes
    [
