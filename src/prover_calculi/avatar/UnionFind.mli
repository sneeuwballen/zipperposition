...
(** This structure operates on arbitrary objects as long as they are hashable.
    It maps keys to values (values belong to a monoid, if they are not needed,
    unit makes for a simple implementation) and each equivalence class'
    representative maps to the monoid merge of all the class' elements values.
...
module Make (P : PAIR) : sig
  ...
  (** Create a union-find for the given elements. Elements are mapped to zero by
      default. *)
  ...
  (** Find value for the given element. The value is the monoid merge of all
      values associated to [key]'s equivalence class.
  ...
  (** Add the given value to the key's class (monoid). It modifies the value by
      merging it with [value]. If the key does not belong to the union-find, it
      is added. *)
