...
module type S = sig
  ...
  (** Forbid empty clauses with trails, i.e. adds the negation of their trails
      to the SAT-solver *)
  ...
  (** [filter_trails f] calls [f] on every trail associated with the empty
      clause. If [f] returns [false], the trail is ignored, otherwise it's
      negated and sent to the SAT solver *)
  ...
  (** This represents a cut on a formula, where we obtain a list of clauses
      [cut_pos] representing the formula itself with the trail [lemma], and a
      boolean literal [cut_lit] that is true iff the trail is true.

      Other modules, when a cut is introduced, will try to disprove the lemma
      (e.g. by induction or theory reasoning). *)
  ...
  (** [add_imply l res] means that the conjunction of lemmas in [l] implies that
      the lemma [res] is proven *)

  val on_input_lemma : cut_res Signal.t
  (** Triggered every time a cut is introduced for an input lemma (i.e. every
      time a statement of the form `lemma F` is translated) *)
  ...
  (** Register inference rules to the environment
      @param split
        if true, the clause splitting rule is added. Otherwise Avatar is only
        used for other things such as induction. *)
end
