...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  type logic =
    | Nonequational
    (* nonequational monomorphic FO without theories *)
    | Equational
  ...
  let logic_to_str = function Equational -> "eq" | Nonequational -> "neq"
    ...
    let eligible_for_non_singular_pe task =
      ...
      | Some (pos, neg) ->
          ...
          let max_resolvents =
            CS.fold
              (fun cl num_res ->
                if num_res == limit then limit (* shortcut *)
                else
                  let new_res =
                    CCArray.fold
                      (fun acc lit ->
      ...
      CCList.find_map
        (fun long_cl ->
          ...
          if List.length other_lits > CCArray.length bin_gates then None
            ...
            if maybe_gate then Some (long_cl, bin_cls) else None )
        long_clauses
    in
              ...
              (CCArray.find_map_i
                 (fun i lit ->
                   ...
                   | L.Equation (lhs, rhs, _) when L.is_predicate_lit lit -> (
                     ...
                     | Some head ->
                         if ID.equal task.sym head then Some (i, lhs) else None
                     | None ->
                         None )
                   | _ ->
                       None )
      ...
      let rec replace_by_lam t =
        ...
        | App (f, l) -> (
          ...
          | Const s ->
              ...
              else T.app f (List.map replace_by_lam l)
          | _ ->
              T.app f (List.map replace_by_lam l) )
        | Const s ->
            if ID.equal s sym then lam else t
        | Fun (ty, t') ->
            T.fun_ ty (replace_by_lam t')
      in
  ...
  let do_pred_elim () =
    ...
    let process_task task =
      ...
      else task.last_check <- Some (task.sq_var_weight, task.num_lits)
    in
  ...
  let initialize () =
    ...
    Signal.on_every Env.on_forward_simplified (fun (c, new_state) ->
        if not !_done then
          ...
          | _ ->
              ignore (react_clause_removed c) (* c is redundant *) ) ;
    ignore (do_pred_elim ()) ;
  ...
  let setup ?(in_fp_mode = false) () =
    ...
    if Env.flex_get k_enabled then
      ...
      else CCFormat.printf "AVATAR is not yet compatible with PredElim@."
end
