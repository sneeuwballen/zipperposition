...
let make_ctx ~signature ~ord ~select ~bool_select ~sk_ctx () =
  ...
  do_extensions ~field:(fun e -> e.Extensions.ctx_actions) ~x:ctx
  >>= fun () -> Phases.return_phase ctx
      ...
      let proof =
        if Env.params.Params.dot_all_roots then
          ...
          |> Iter.filter_map (fun c ->
              if Literals.is_absurd (Env.C.lits c) then Some (Env.C.proof c)
              else None )
...
let has_goal_decls_ decls =
  CCVector.exists
    (fun st ->
      match Statement.view st with Statement.Goal _ -> true | _ -> false )
    decls
...
let process_file ?(prelude = Iter.empty) file =
  ...
  >>= fun result -> Phases.return (Phases.Env_result (env, result))
...
let main_cli ?setup_gc:(gc = true) () =
  ...
  load_extensions
  >>= fun _ ->
  process_files_and_print ~params files
  >>= fun errcode -> Phases.exit >|= fun () -> errcode
...
let skip_parse_cli ?(params = Params.default) file =
  ...
  CCFormat.set_color_default true ;
  Phases.set_key Params.key params
  >>= fun () -> Phases.return_phase ([file], params)
