...
let load_extensions =
  ...
  Phases.start_phase Phases.LoadExtensions
  >>= fun () ->
...
let do_extensions ~x ~field =
  ...
  |> Phases.fold_l ~x:() ~f:(fun () e -> Phases.fold_l (field e) ~x:() ~f:(fun () f -> Phases.update ~f:(f x)))

let apply_modifiers ~field o =
  Extensions.extensions () |> CCList.fold_left (fun o e -> field e |> CCList.fold_left (fun o m -> m o) o) o

let start_file file =
  Phases.start_phase Phases.Start_file
  >>= fun () ->
  Util.debugf ~section 2 "@[@{<Yellow>### process file@ `%s` ###@}@]" (fun k -> k file) ;
  do_extensions ~field:(fun e -> e.Extensions.start_file_actions) ~x:file >>= fun () -> Phases.return_phase ()

let parse_prelude (params : Params.t) =
  Phases.start_phase Phases.Parse_prelude
  >>= fun () ->
  let prelude_files = params.Params.prelude in
  let res =
    if CCVector.is_empty prelude_files then CCResult.return Iter.empty
    else
      CCVector.to_list prelude_files
      |> CCResult.map_l (fun file ->
             Util.debugf ~section 2 "@[@{<Yellow>### parse prelude file@ `%s` ###@}@]" (fun k -> k file) ;
...
let parse_file file =
  Phases.start_phase Phases.Parse_file
  >>= fun () ->
  let input = Parsing_utils.input_of_file file in
  Parsing_utils.parse_file input file
  >>?= fun parsed ->
  do_extensions ~field:(fun e -> e.Extensions.post_parse_actions) ~x:parsed
  >>= fun () -> Phases.return_phase (input, parsed)

let has_arith stmt : bool =
  let module TS = TypedSTerm in
  let ty_is_arith ty = TS.equal ty TS.Ty.real || TS.equal ty TS.Ty.rat || TS.equal ty TS.Ty.int in
  CCVector.to_iter stmt |> Iter.flat_map Statement.Seq.to_iter
...
let sine_filter stmts =
  if !_sine_threshold < 0 || CCVector.length stmts < !_sine_threshold then stmts
    ...
    let filtered =
      Statement.sine_axiom_selector ~ignore_k_most_common_symbols:!_ignore_k_most_common_symbols
        ~take_conj_defs:!_take_conj_defs ~take_only_defs:!_take_only_defs ~trim_implications:!_trim_implications
        ~depth_start:!_sine_d_min ~depth_end:!_sine_d_max ~tolerance:!_sine_tolerance seq
    in
    CCVector.freeze (CCVector.of_iter filtered)

let typing ~file prelude (input, stmts) =
  Phases.start_phase Phases.Typing
  >>= fun () ->
  Phases.get_key Params.key
  >>= fun params ->
  let def_as_rewrite = params.Params.def_as_rewrite in
  TypeInference.infer_statements ~on_var:(Input_format.on_var input) ~on_undef:(Input_format.on_undef_id input)
    ~on_shadow:(Input_format.on_shadow input) ~implicit_ty_args:(Input_format.implicit_ty_args input)
    ~def_as_rewrite ?ctx:None ~file (Iter.append prelude stmts)
  >>?= fun stmts ->
  ...
  >>= fun () ->
  do_extensions ~field:(fun e -> e.Extensions.post_typing_actions) ~x:stmts
  >>= fun () -> Phases.return_phase stmts

(* obtain clauses  *)
let cnf ~sk_ctx decls =
  Phases.start_phase Phases.CNF
  >>= fun () ->
  let opts = if !Lazy_cnf.enabled then [Cnf.LazyCnf] else [] in
  let stmts =
    decls |> CCVector.to_iter |> Cnf.cnf_of_iter ~ctx:sk_ctx ~opts |> CCVector.to_iter
    |> apply_modifiers ~field:(fun e -> e.Extensions.post_cnf_modifiers)
    |> Cnf.convert
  in
  do_extensions ~field:(fun e -> e.Extensions.post_cnf_actions) ~x:stmts >>= fun () -> Phases.return_phase stmts

(* compute a precedence *)
let compute_prec ~signature stmts =
  Phases.start_phase Phases.Compute_prec
  >>= fun () ->
  (* use extensions *)
  Phases.get
  >>= fun state ->
  let cp =
    ...
    |> Compute_prec.set_weight_rule (fun stmts ->
           let sym_depth =
             stmts |> Iter.flat_map Statement.Seq.terms
             |> Iter.flat_map (fun t ->
                    Term.Seq.subterms_depth t
                    |> Iter.filter_map (fun (st, d) -> CCOpt.map (fun id -> (id, d)) (Term.head st)) )
           in
...
let compute_ord_select precedence =
  Phases.start_phase Phases.Compute_ord_select
  >>= fun () ->
  Phases.get_key Params.key
  >>= fun params ->
  ...
  do_extensions ~field:(fun e -> e.Extensions.ord_select_actions) ~x:(ord, fst select)
  >>= fun () ->
...
let make_ctx ~signature ~ord ~select ~bool_select ~sk_ctx () =
  ...
  Phases.start_phase Phases.MakeCtx
  >>= fun () ->
  ...
  let lost_comp = Flex_state.get_or ~or_:false Ctx.Key.lost_completeness st || not is_complete in
  if lost_comp then MyCtx.lost_completeness () ;
  do_extensions ~field:(fun e -> e.Extensions.ctx_actions) ~x:ctx >>= fun () -> Phases.return_phase ctx

let make_env ~ctx:(module Ctx : Ctx_intf.S) ~params stmts =
  Phases.start_phase Phases.MakeEnv
  >>= fun () ->
  Phases.get
  >>= fun state ->
  ...
  Extensions.extensions () |> List.iter (fun e -> List.iter (fun f -> f env1) e.Extensions.env_actions) ;
...
let print_stats_env (type c) (module Env : Env.S with type C.t = c) =
  ...
  let print_hashcons_stats what (sz, num, sum_length, small, median, big) =
    Format.printf
      "@[<h>%shashcons stats for %s: size %d, num %d, sum length %d, buckets: small %d, median %d, big %d@]@."
      comment what sz num sum_length small median big
  and print_state_stats (num_active, num_passive, num_simpl) =
    Format.printf "%sproof state stats: {active %d, passive %d, simpl %d}@." comment num_active num_passive
      num_simpl
  in
...
(* print stats *)
let print_stats () =
  Phases.start_phase Phases.Print_stats
  >>= fun () ->
  ...
  let print_gc () =
    ...
    Format.printf
      "@[<h>%sGC: minor words %.0f; major_words: %.0f; max_heap: %d; minor collections %d; major collections \
       %d@]@."
      comment stats.Gc.minor_words stats.Gc.major_words stats.Gc.top_heap_words stats.Gc.minor_collections
      stats.Gc.major_collections
  in
  Phases.get_key Params.key
  >>= fun params ->
  if params.Params.stats then (print_gc () ; Util.print_global_stats ~comment ()) ;
  Phases.return_phase ()

(* pre-saturation *)
let presaturate_clauses (type c) (module Env : Env.S with type C.t = c) (c_sets : c Clause.sets) =
  Phases.start_phase Phases.Pre_saturate
  >>= fun () ->
  ...
  if Env.params.Params.presaturate then (
    ...
    Util.debugf ~section 2 "@[<2>%d clauses pre-saturated into:@ @[<hv>%a@]@]" (fun k ->
        k num_clauses (Util.pp_iter ~sep:" " Env.C.pp_tstp_full) (CCVector.to_iter c_set) ) ;
...
let try_to_refute (type c) (module Env : Env.S with type C.t = c) clauses result =
  ...
  Util.debugf ~section 1 "active(%d): @[%a@]" (fun k ->
      k (Iter.length @@ Env.get_active ()) (Iter.pp_seq Env.C.pp) (Env.get_active ()) ) ;
  Util.debugf ~section 1 "passive(%d): @[%a@]" (fun k ->
      k (Iter.length @@ Env.get_passive ()) (Iter.pp_seq Env.C.pp) (Env.get_passive ()) ) ;
  Signal.send Env.on_start () ;
  let result, num =
    match result with
    | Saturate.Unsat _ ->
        (result, 0 (* already found unsat during presaturation *))
    | _ ->
        Sat.given_clause ~generating:true ?steps ?timeout ()
  in
  let comment = Options.comment () in
  Format.printf "%sdone %d iterations in %.3fs@." comment num (Util.total_time_s ()) ;
  Util.debugf ~section 2 "@[<2>final precedence:@ @[%a@]@]" (fun k -> k Precedence.pp (Env.precedence ())) ;
  Phases.return_phase result

(* Print some content of the state, based on environment variables *)
let print_dots (type c) (module Env : Env_intf.S with type C.t = c) (result : Saturate.szs_status) =
  Phases.start_phase Phases.Print_dot
  >>= fun () ->
  ...
  | Some dot_f, Saturate.Unsat proof ->
      ...
      let proof =
        if Env.params.Params.dot_all_roots then
          ...
          |> Iter.filter_map (fun c -> if Literals.is_absurd (Env.C.lits c) then Some (Env.C.proof c) else None)
...
let sat_to_str () = if !has_goal_ then "CounterSatisfiable" else "Satisfiable"

let unsat_to_str () = if !has_goal_ then "Theorem" else "Unsatisfiable"

let print_szs_result (type c) ~file (module Env : Env_intf.S with type C.t = c) (result : Saturate.szs_status) =
  Phases.start_phase Phases.Print_result
  >>= fun () ->
  let comment = Options.comment () in
  ( match result with
  ...
  | Saturate.Sat -> (
      ...
      | Options.O_none ->
          ()
      | Options.O_zf ->
          failwith "not implemented: printing in ZF" (* TODO *)
      | Options.O_tptp ->
          Util.debugf ~section 2 "@[<2>saturated set:@ @[<hv>%a@]@]" (fun k ->
              k (Util.pp_iter ~sep:" " Env.C.pp_tstp_full) (Env.get_active ()) )
      | Options.O_normal ->
...
let pp_weight prec out s = Format.fprintf out "w(%a)=%a" ID.pp s Precedence.Weight.pp (Precedence.weight prec s)

(* does the sequence of declarations contain at least one conjecture? *)
let has_goal_decls_ decls =
  CCVector.exists (fun st -> match Statement.view st with Statement.Goal _ -> true | _ -> false) decls

(* parse CLI options and list of files to deal with *)
let parse_cli =
  Phases.start_phase Phases.Parse_CLI
  >>= fun () ->
  CCFormat.set_color_default true ;
  (* parse arguments *)
  let params = Params.parse_args () in
  let files = CCVector.to_list params.Params.files in
  Phases.set_key Params.key params
  >>= fun () ->
...
let syms_in_conj decls =
  ...
  |> flat_map (fun st ->
         ...
         if CCOpt.is_some (Proof.Step.distance_to_goal pr) then Statement.Seq.symbols st else empty )
  ...
  CCVector.fold
    (fun acc f ->
      ...
      | Statement.Goal _ | Statement.NegatedGoal _ ->
          ...
          |> Iter.flat_map TypedSTerm.Seq.symbols |> Iter.append acc
      | _ ->
          acc )
    Iter.empty decls
...
let process_file ?(prelude = Iter.empty) file =
  ...
  compute_prec ~signature (CCVector.to_iter stmts)
  >>= fun precedence ->
  Util.debugf ~section 2 "@[<2>precedence:@ @[%a@]@]" (fun k -> k Precedence.pp precedence) ;
  compute_ord_select precedence
  >>= fun (ord, select, bool_select) ->
  (* HO *)
  Phases.get_key Params.key
  >>= fun params ->
  (* build the context and env *)
  make_ctx ~signature ~ord ~select ~bool_select ~sk_ctx ()
  >>= fun ctx ->
  make_env ~params ~ctx stmts
  >>= fun (Phases.Env_clauses (env, clauses)) ->
  (* main workload *)
  has_goal_ := has_goal ;
  (* FIXME: should be computed at Env initialization *)
  (* pre-saturation *)
  presaturate_clauses env clauses
  >>= fun (result, clauses) ->
  (* saturate, possibly changing env *)
  try_to_refute env clauses result >>= fun result -> Phases.return (Phases.Env_result (env, result))
...
let check res =
  Phases.start_phase Phases.Check_proof
  >>= fun () ->
  Phases.get_key Params.key
  >>= fun params ->
  let comment = Options.comment () in
  let errcode =
    ...
    | Saturate.Unsat p when params.Params.check ->
        ...
        Format.printf "%s(@[<h>proof_check@ :res %a@ :stats %a :time %.3fs@])@." comment LLProof_check.pp_res res
          LLProof_check.pp_stats stats (stop -. start) ;
...
let setup_gc =
  Phases.start_phase Phases.Setup_gc
  >>= fun () ->
...
let setup_signal =
  Phases.start_phase Phases.Setup_signal
  >>= fun () ->
...
let process_files_and_print ?(params = Params.default) files =
  parse_prelude params
  >>= fun prelude ->
  let f file =
    process_file ~prelude file
    >>= fun (Phases.Env_result (env, res)) -> print file env res >>= fun () -> check res
  in
  let phases = List.map f files in
  Phases.run_parallel phases >>= fun r -> print_stats () >>= fun () -> Phases.return r

let main_cli ?setup_gc:(gc = true) () =
  let open Phases.Infix in
  (if gc then setup_gc else Phases.return ())
  >>= fun () ->
  setup_signal
  >>= fun () ->
  parse_cli
  >>= fun (files, params) ->
  load_extensions
  >>= fun _ -> process_files_and_print ~params files >>= fun errcode -> Phases.exit >|= fun () -> errcode

let skip_parse_cli ?(params = Params.default) file =
  Phases.start_phase Phases.Parse_CLI
  >>= fun () ->
  CCFormat.set_color_default true ;
  Phases.set_key Params.key params >>= fun () -> Phases.return_phase ([file], params)
...
let () =
  ...
  Params.add_opts
    [ ("--de-bruijn-weight", Arg.Set_int _db_w, " Set weight of de Bruijn index for KBO")
    ; ("--lambda-weight", Arg.Set_int _lmb_w, " Set weight of lambda symbol for KBO")
    ; ("--kbo-weight-fun", Arg.Set_string _kbo_wf, " Set the function for symbol weight calculation.")
    ; ("--prec-gen-fun", Arg.Set_string _prec_fun, " Set the function used for precedence generation")
    ; ( "--sine-depth-min"
      , Arg.Int
          (fun v ->
    ...
    ; ( "--sine-ignore-k-most-common-syms"
      ...
      , " if conjecture symbol is within k most common occurring ones, then it will be disregarded as \
         conjecture symbol" )
    ; ( "--sine-take-conj-defs"
