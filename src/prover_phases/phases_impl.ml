...
let start_file file =
  ...
  Util.debugf ~section 1 "@[@{<Yellow>### process file@ `%s` ###@}@]" (fun k ->
      k file );
...
let typing ~file prelude (input, stmts) =
  ...
  Util.debugf ~section 3 "@[<hv2>@{<green>typed statements@}@ %a@]" (fun k ->
      k (Util.pp_seq Statement.pp_input) (CCVector.to_seq stmts) );
...
let cnf decls =
  ...
  let stmts =
    decls |> CCVector.to_seq |> Cnf.cnf_of_seq |> CCVector.to_seq
    |> Cnf.convert
  in
...
let make_ctx ~signature ~ord ~select () =
  ...
  do_extensions ~field:(fun e -> e.Extensions.ctx_actions) ~x:ctx >>= fun () ->
...
let presaturate_clauses (type c) (module Env : Env.S with type C.t = c)
  ...
  if Env.params.Params.presaturate then (
    ...
    Util.debugf ~section 2 "@[<2>%d clauses pre-saturated into:@ @[<hv>%a@]@]"
      (fun k ->
        k num_clauses (Util.pp_seq ~sep:" " Env.C.pp) (CCVector.to_seq c_set)
    );
    Phases.return_phase (result, clauses) )
  else
    Phases.return_phase (Saturate.Unknown, c_sets)
...
let try_to_refute (type c) (module Env : Env.S with type C.t = c) clauses
    result =
  ...
  let steps =
    if Env.params.Params.steps < 0 then
      ...
      Util.debugf ~section 1 "run for %d steps" (fun k ->
  ...
  and timeout =
    if Env.params.Params.timeout = 0. then
      ...
      Util.debugf ~section 1 "run for %.3f s" (fun k ->
  ...
  Format.printf "%sdone %d iterations in %.3fs@." comment num
    (Util.total_time_s ());
  Util.debugf ~section 1 "@[<2>final precedence:@ @[%a@]@]" (fun k ->
...
let print_dots (type c) (module Env : Env_intf.S with type C.t = c)
  ...
  | Some dot_f, (Saturate.Sat | Saturate.Unknown)
    when Env.params.Params.dot_sat ->
    (* print saturated set *)
    let name = "sat_set" in
...
let print_szs_result (type c) ~file (module Env : Env_intf.S with type C.t = c)
  ...
  | Saturate.Sat ->
    ...
    | Options.O_tptp ->
      Util.debugf ~section 1 "@[<2>saturated set:@ @[<hv>%a@]@]" (fun k ->
          k (Util.pp_seq ~sep:" " Env.C.pp_tstp_full) (Env.get_active ()) )
    | Options.O_normal ->
      Util.debugf ~section 1 "@[<2>saturated set:@ @[<hv>%a@]@]" (fun k ->
          k (Util.pp_seq ~sep:" " Env.C.pp) (Env.get_active ()) ) )
...
let process_file ?(prelude = Iter.empty) file =
  ...
  Util.debugf ~section 1 "parsed %d declarations (%s goal(s))" (fun k ->
  ...
  Util.debugf ~section 1 "@[<2>signature:@ @[<hv>%a@]@]" (fun k ->
      k Signature.pp signature );
  Util.debugf ~section 2 "(@[classification:@ %a@])" (fun k ->
      k Classify_cst.pp_signature signature );
  compute_prec (CCVector.to_seq stmts) >>= fun precedence ->
  Util.debugf ~section 1 "@[<2>precedence:@ @[%a@]@]" (fun k ->
