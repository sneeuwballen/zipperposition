...
val process_file :
  ?prelude:Phases.prelude -> Phases.filename -> (Phases.env_with_result, [`Parse_prelude], [`Saturate]) Phases.t
...
val print : Phases.filename -> 'c Env.packed -> Saturate.szs_status -> (unit, [`Saturate], [`Print_dot]) Phases.t
(** Printing of results *)

val check : Saturate.szs_status -> (Phases.errcode, [`Print_dot], [`Check_proof]) Phases.t

val process_files_and_print :
  ?params:Params.t -> Phases.filename list -> (Phases.errcode, [`LoadExtensions], [`Print_stats]) Phases.t
...
val main_cli : ?setup_gc:bool -> unit -> (Phases.errcode, [`Init], [`Exit]) Phases.t
(** Main for the command-line prover *)

val main : ?setup_gc:bool -> ?params:Params.t -> string -> (* file *)
