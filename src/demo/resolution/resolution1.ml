...
(** Global signature (maps symbols such as "f", "parent_of" or "greater" to
    their type). Every symbol has exactly one type. The initial signature is the
    TPTP signature (logic connectives) *)
let _signature = ref Logtk.Signature.empty

(** We do not have to do anything about terms, because they are already defined
    in {! Logtk.Term}. Terms are either variables or applications of a constant
    (symbol) to a list of sub-terms.
...
module Lit = struct
  ...
  (** We also define a few basic comparison and printing functions. Comparison
      functions are used by many data structures; Printing is useful for
      informing the user of results or for debugfging. *)
...
(** A clause is a disjunction ("or") of literals. We simply use a list of
    literals.

    Example: [ (lives_in(paris, X), false); (eats_baguette(X), true) ] (means
    "forall X, if X lives in paris then X eats baguette") *)
module Clause = struct
  ...
  (** A clause is trivial if it contains both a literal and its opposite. It
      means the clause is tautological, that is, always true; we can dispose of
      it because resolution is about {b refutation} (proving false) *)
  let is_trivial c =
    List.exists
      (fun (t, b) ->
        b && List.exists (fun (t', b') -> (not b') && T.equal t t') c)
      c

  (** A substitution maps some variables to terms. Here this function will be
      used to {i apply} the substitution to a clause - replace variables of the
      clause by their image in the substitution (or keep them unchanged if they
      do not appear in the substitution.

      Substitutions are pre-defined in Logtk, and applying a substitution to a
      term is defined too (the function {!Subst.FO.apply} that applies a
      substitution to a first-order term) *)
  ...
  (** Conversion from list of {!SLiteral.t} type: [Formula.t list -> clause] *)
  let _of_forms (c : T.t SLiteral.t list) : t =
    let _atom f =
      ...
      | _ ->
        failwith
          (CCFormat.sprintf "unsupported formula %a" (SLiteral.pp T.pp) f)
    in
    make (List.map _atom c)
end

(** A pair (clause, index) where the index refers to some literal in the clause.
*)
...
(** An Index is a multimap from terms to (clause,index). Basically when we
    process a clause [c], for each literal [(term,sign)] at position [i] in the
    clause [c], we add [term -> (c, i)] into the index. Later we will be able to
    retrieve the pair [(c,i)] using any term that {i unifies} with [term]. *)

module ClauseSet = Set.Make (Clause)
(** Set of clauses. Easy to define thanks to {!Clause.compare} *)

(** State for one proof:
    - One term index (maps terms to the pairs [(clause, index of literal)] they
      appear in);
    - One set of processed ("active") clauses;
    - One queue of clauses yet to be processed. We process those clauses one by
      one. *)
...
exception Unsat
(** Raise this exception when the empty clause (i.e., "false") is found. This
    means the original clause set implies false and therefore that it's absurd.
*)

(** add [c] to the passive set, if not already present in the active set nor it
    is trivial. *)
let _add_passive c =
  if c = [] then
    raise Unsat
  else if Clause.is_trivial c then
    Util.debugf 4 "clause %a is trivial" (fun k -> k Clause.pp c)
  else if not (ClauseSet.mem c !_active_set) then (
    Util.debugf 4 "new passive clause %a" (fun k -> k Clause.pp c);
    Queue.push c _passive_set
  )

(** When we process a clause [c], we put it into the active set (set of
    processed clauses). That also means every literal [(term,sign)] at index [i]
    will go into the index, so we can retrieve [c] by its literals later. *)
let _add_active c =
  _active_set := ClauseSet.add c !_active_set;
  List.iteri (fun i (t, _) -> _idx := Index.add !_idx t (c, i)) c
...
(** Factoring inference (takes one clause and deduce other clauses):
    ...
    with some substitution sigma such that sigma(A)=sigma(A'), then we can
    {i factor} those literals into (sigma(A),true) provided we also apply sigma
          ...
          (fun j (t', b') ->
            ...
            if i < j && b' then (
              try
                ...
                Util.debugf 3 "factoring of %a ----> %a" (fun k ->
                    k Clause.pp c Clause.pp c');
...
(** Resolution inference rule, between the clause [C] and any clause [D] from
    the active set.
    ...
    technique ever. If "resolves" together two complementary literals in two
    clauses (assuming those clauses do not share variables).

    Let us explain in the propositional case (ignoring variables), assuming
    [A = A']. The idea is, roughly:
    - we know that either [A] or either [not A] is true (excluded middle)
    - if [A] is true, it means that [not A' or D] can only be true if [D] is
      true (since [A=A'=true]). Therefore [D] must be true.
    - if [A] is false, then [A or C] can only be true if [C] is true; therefore
      [C] holds.
    - by excluded middle one of those must be true, so in any case [C or D] is
      true. Hence the conclusion.

    For the first-order case, we compute the {b most general unifier} or [A] and
    [A'] (if it exists), call this unifier substitution [sigma]. Then, the
    reasoning is the same as in the propositional case since the literals are
    actually equal.

    Note: the "0" and "1" are {b scopes}, a trick I use to avoid actually
    renaming variables in one of the clauses. More details can be found in the
    documentation for {!Subst} or in the talk I (Simon) gave at PAAR 2014. *)
let _resolve_with c =
  List.iteri
    (fun i (t, b) ->
      ...
      |> Iter.iter (fun (_t', (d, j), uf) ->
            ...
            *)
             if b <> b' then (
               ...
               Util.debugf 3 "resolution of %a and %a ---> %a" (fun k ->
                   k Clause.pp c Clause.pp d Clause.pp concl);
               _add_passive concl
             )))
    c

(** Main saturation algorithm, a simple "given clause" loop. This is the outer
    loop of the resolution procedure: given an initial set of [clauses], the
    algorithm does:

    - add all the clauses into the passive set
    - while some passive clauses remain unprocessed, pick one of them, call it
    ...
    - if at any point the empty clause [[]] is found, then the initial set of
      clauses is unsatisfiable (absurd).
    - otherwise, if the loop stops, we have computed a fixpoint of the initial
      clauses with respect to inferences without finding [false], which means
      the original set of clauses is satisfiable (admits a model) *)
let _saturate clauses =
  ...
  try
    while not (Queue.is_empty _passive_set) do
      ...
      if (not (Clause.is_trivial c)) && not (ClauseSet.mem c !_active_set) then (
...
(** Read the problem to solve from the file [f], (try to) solve it and return
    the result. We use an error monad to make error handling easier (the
  ...
  let res =
    E.(
      ...
      (* Perform type inference and type checking (possibly updating
          the signature) *)
      TypeInference.infer_statements ?ctx:None ~implicit_ty_args:true stmts
      >>= fun st ->
      ...
      let clauses =
        ...
        |> Iter.map Clause._of_forms |> Iter.to_rev_list
      in
