...
let rec lookup subst var offset = match subst with
  | SubstEmpty -> raise Not_found
  | SubstBind (v, o_v, t, o_t, subst') ->
    if v == var && o_v = offset then (t, o_t) else lookup subst' var offset
...
let is_in_subst subst v o_v =
  try ignore (lookup subst v o_v); true
...
let bind ?(recursive=true) subst v o_v t o_t =
  let (t', o_t') = if recursive then get_var subst v o_v else v, o_v in
  if t' == t && o_t' = o_t
...
module Renaming = struct
  ...
  type t = Term.t H.t   (* hashtable (term,offset) -> term *)
...
let apply_subst ?(recursive=true) ?renaming subst t offset =
  ...
  let rec replace binder_depth subst t offset =
    ...
    | T.Var i ->
      ...
      begin try
        let (t', o_t') = lookup subst t offset in
          ...
          if recursive && (t' != t || o_t' <> offset)
            then (* replace also in the image of t *)
              ...
              replace binder_depth empty t' o_t' 
      with Not_found -> (* variable not bound by [subst], rename it *)
        match renaming with
        | None when offset = 0 -> t
        | None -> T.mk_var (i+offset)             (* shift *)
        | Some r -> Renaming.rename r t offset    (* rename *)
      end
    | T.At (t1, t2) ->
      let t1' = replace binder_depth subst t1 offset in
      let t2' = replace binder_depth subst t2 offset in
...
let pp_full pp_term buf subst =
  ...
  let rec pp_rec buf subst =
    ...
    | SubstEmpty -> ()
    | SubstBind (v, o_v, t, o_t, subst') ->
      Printf.bprintf buf "%a[%d] -> %a[%d]" pp_term v o_v pp_term t o_t;
