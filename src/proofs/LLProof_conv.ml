...
let conv_subst ~ctx (p : Subst.Projection.t) : ll_subst =
  List.fold_left
    ...
    Var.Subst.empty (Subst.Projection.bindings p)

type state = {ctx: Term.Conv.ctx; tbl: LLProof.t Proof.S.Tbl.t}
...
let rec conv_proof st p : LLProof.t =
  ...
  | None ->
      ...
      Proof.S.Tbl.add st.tbl p res ; res

and conv_step st p =
  Util.debugf ~section 5 "(@[llproof.conv.step@ %a@])" (fun k -> k Proof.S.pp_notrec1 p) ;
  ...
  let intros =
    let l = List.mapi (fun i v -> (v, T.const ~ty:(Var.ty v) (ID.makef "sk_%d" i))) vars in
  ...
  let res =
    ...
    | Proof.Inference (rule, tags) | Proof.Simplification (rule, tags) ->
        ...
        let parents =
          List.map (conv_parent st res intros local_intros tags) (Proof.Step.parents @@ Proof.S.step p)
        in
        let local_intros = Var.Subst.to_list !local_intros |> List.rev_map snd in
        LLProof.inference ~intros ~local_intros ~tags (T.rename_all_vars res) (Proof.Rule.name rule) parents
    | Proof.Esa rule ->
        let l =
          List.map
            (function Proof.P_of p -> conv_proof st p | Proof.P_subst _ -> assert false)
    ...
    | Proof.Trivial ->
        LLProof.trivial res
    | Proof.By_def id ->
        LLProof.by_def id res
    | Proof.Define (id, _) ->
        LLProof.define id res
    | Proof.Intro (_, Proof.R_assert) ->
        LLProof.assert_ res
    | Proof.Intro (_, (Proof.R_goal | Proof.R_lemma)) ->
        LLProof.goal res
    | Proof.Intro (_, (Proof.R_def | Proof.R_decl)) ->
        LLProof.trivial res
  in
  Util.debugf ~section 4 "(@[llproof.conv.step.->@ :from %a@ :to %a@])" (fun k ->
      k Proof.S.pp_notrec1 p LLProof.pp res ) ;
  res

(* convert parent of the given result formula. Also make instantiations
   explicit. *)
and conv_parent st step_res intros local_intros tags (parent : Proof.Parent.t) : LLProof.parent =
  Util.debugf ~section 5 "(@[llproof.conv_parent@ %a@])" (fun k -> k Proof.pp_parent parent) ;
  ...
  if List.length intros <> List.length vars_step_res then
    errorf "length mismatch, cannot do intros@ :res %a@ :with [@[%a@]]" T.pp step_res
  ...
  let prev_proof, p_instantiated_res =
    ...
    | Proof.P_subst (p, subst) ->
        ...
        let p_instantiated_res, inst_subst = Proof.Result.to_form_subst ~ctx:st.ctx subst (Proof.S.result p) in
        let p_res = Proof.Result.to_form ~ctx:st.ctx (Proof.S.result p) in
        (* convert [p] itself *)
        let p = conv_proof st p in
              ...
              | None ->
                  errorf
                    "cannot find variable `%a`@ in inst-subst {%a}@ :inst-res %a@ :res %a@ :parent %a@ :intros \
                     [@[%a@]]"
                    Var.pp_fullc v (Var.Subst.pp T.pp) inst_subst T.pp p_instantiated_res T.pp step_res
                    Proof.pp_parent parent (Util.pp_list ~sep:"," T.pp) intros )
            vars_p
        in
      ...
      (fun v ->
        ...
        | None -> (
          ...
          | None ->
              ...
              let c =
                ID.makef "sk_%d" (List.length intros + Var.Subst.size !local_intros)
                |> T.const ~ty:(Var.ty v |> T.Subst.eval intro_subst)
              in
              local_intros := Var.Subst.add !local_intros v c ;
              Util.debugf ~section 5 "(@[llproof.conv.add_local_intro@ %a := %a@ :p-instantiated `%a`@])"
