...
and conv_step st p =
  ...
  let intros =
    let l =
      List.mapi
        (fun i v -> (v, T.const ~ty:(Var.ty v) (ID.makef "sk_%d" i)))
        vars
    in
  ...
  let res =
    ...
    | Proof.Inference (rule, tags) | Proof.Simplification (rule, tags) ->
        ...
        in
        let local_intros =
          Var.Subst.to_list !local_intros |> List.rev_map snd
        in
        ...
        let inst : LLProof.inst =
          ...
          List.map
            (fun v ->
              ...
              | None ->
                  errorf
                    "cannot find variable `%a`@ in inst-subst {%a}@ :inst-res \
                     %a@ :res %a@ :parent %a@ :intros [@[%a@]]"
