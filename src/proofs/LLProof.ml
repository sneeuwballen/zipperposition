...
let pp_step out (s : step) : unit =
  ...
  | Goal ->
      Fmt.string out "goal"
  | Assert ->
      Fmt.string out "assert"
  | Negated_goal _ ->
      Fmt.string out "negated_goal"
  | Trivial ->
      Fmt.string out "trivial"
  | By_def id ->
      Fmt.fprintf out "(by_def :of %a)" ID.pp id
  | Define id ->
      Fmt.fprintf out "(@[define@ %a@])" ID.pp id
  | Instantiate {inst; tags; _} ->
      Fmt.fprintf out "(@[instantiate %a%a@])" pp_inst inst pp_tags tags
  | Esa (n, _) ->
      Fmt.fprintf out "(esa %s)" n
...
let parents (p : t) : parent list =
  ...
  | Negated_goal p2 ->
      [p_of p2]
  | Instantiate {form= p2; _} ->
      [p_of p2]
  | Esa (_, l) ->
      List.map p_of l
  | Inference {parents= l; _} ->
      l
...
module Dot = struct
    ...
    CCGraph.make (fun p ->
        let descr =
          ...
          | Trivial ->
              "trivial"
          | By_def id ->
              Fmt.sprintf "by_def(%a)" ID.pp id
          | Define id ->
              Fmt.sprintf "define(%a)" ID.pp id
          | Instantiate _ ->
              "instantiate"
          | Esa (name, _) ->
              name
  ...
  let color p : string option =
    ...
    | _, F.False ->
        Some "red"
    | _ when is_bool_atom (concl p) ->
        Some "cyan"
    | _, F.Or l when List.for_all is_bool_atom l ->
        Some "cyan"
    | Goal, _ ->
        Some "green"
    | Assert, _ ->
