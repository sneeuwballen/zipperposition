...
let pp_res out = function R_ok -> Fmt.fprintf out "@{<Green>OK@}" | R_fail -> Fmt.fprintf out "@{<Red>FAIL@}"

let pp_stats out (s : stats) =
  Fmt.fprintf out "(@[<hv>:ok %d@ :fail %d@ :skip %d (:esa %d@ :tags %d)@])" s.n_ok s.n_fail s.n_skip
    s.n_skip_esa s.n_skip_tags
...
let () =
  Printexc.register_printer (function Error msg -> Some (Util.err_spf "llproof_check: %s" msg) | _ -> None)
...
let instantiate (f : form) (inst : LLProof.inst) : form =
  ...
  if List.length vars <> List.length inst then
    errorf "mismatched arities in instantiate `%a`@ :with %a" T.pp f LLProof.pp_inst inst ;
  let subst = List.fold_left2 Var.Subst.add Var.Subst.empty vars inst in
  let f' = T.Subst.eval subst body in
  Util.debugf ~section 5 "(@[<hv>instantiate@ :inst %a@ :from %a@ :into %a@ :subst {%a}@])" (fun k ->
...
let pp_csr out = function
  | CS_check r ->
      pp_res out r
  | CS_skip r ->
      let s = match r with `ESA -> "esa" | `Tags -> "tags" | `Other -> "other" | `Trivial -> "trivial" in
      Fmt.fprintf out "@{<Yellow>SKIP@} (%s)" s

let conv_res = function LLProver.R_ok -> R_ok | LLProver.R_fail -> R_fail
...
let check_step_ ?dot_prefix (p : proof) : check_step_res =
  ...
  | P.Instantiate {form= p'; inst; _} ->
      ...
      let subst =
        vars |> List.mapi (fun i v -> (v, T.const ~ty:(Var.ty v) (ID.makef "sk_%d" i))) |> Var.Subst.of_list
      in
  ...
  | P.Inference {parents; tags; intros; _} ->
      if LLProver.can_check tags then
        ...
        let all_premises = List.map concl_of_parent parents and concl = instantiate concl intros in
...
let check ?dot_prefix ?(before_check = fun _ -> ()) ?(on_check = fun _ _ -> ()) (p : proof) : res * stats =
  let tbl = P.Tbl.create 64 in
  let stats = ref {n_ok= 0; n_fail= 0; n_skip_esa= 0; n_skip_tags= 0; n_skip_trivial= 0; n_skip= 0} in
  ...
  let rec check (p : proof) : unit =
    if not (P.Tbl.mem tbl p) then (
      ...
      P.Tbl.add tbl p res ;
      Util.debugf ~section 3 "(@[<hv>@{<Yellow>done_checking_proof@}@ :of %a@ :res %a@])" (fun k ->
          k P.pp p pp_csr res ) ;
      on_check p res ;
      ( match res with
      | CS_check R_ok ->
