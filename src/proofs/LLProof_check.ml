...
let pp_res out = function
  | R_ok ->
      Fmt.fprintf out "@{<Green>OK@}"
  | R_fail ->
      Fmt.fprintf out "@{<Red>FAIL@}"
...
let () =
  Printexc.register_printer (function
    | Error msg ->
        Some (Util.err_spf "llproof_check: %s" msg)
    | _ ->
        None )
    ...
    if not (P.Tbl.mem tbl p) then (
      ...
      | CS_skip r ->
          ...
          upd_stats (fun s ->
              ...
              ; n_skip_tags=
                  (if r = `Tags then s.n_skip_tags + 1 else s.n_skip_tags)
              ; n_skip_trivial=
                  ( if r = `Trivial then s.n_skip_trivial + 1
                    else s.n_skip_trivial ) } ) ) ;
      (* now check premises *)
