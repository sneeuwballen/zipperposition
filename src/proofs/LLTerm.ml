...
module Int_op = struct
  ...
  let hash = function
    | Divisible_by n ->
        Hash.combine2 10 (Z.hash n)
    | Not_div_by n ->
        Hash.combine2 20 (Z.hash n)
    | x ->
        Hash.poly x
  ...
  let pp out = function
    ...
    | Neq0 ->
        Fmt.fprintf out "!= 0"
    | Divisible_by n ->
        Fmt.fprintf out "div_by %a" Z.pp_print n
    | Not_div_by n ->
        Fmt.fprintf out "not_div_by %a" Z.pp_print n
end
...
and 'a linexp = {const: 'a; coeffs: (t * 'a) I_map.t}
...
module Make_linexp (N : NUM) = struct
  ...
  let merge_ f a b : t =
    ...
    ; coeffs=
        I_map.merge_safe a.coeffs b.coeffs ~f:(fun _ o ->
            match o with
            | `Left n | `Right n ->
                Some n
            | `Both ((t, a), (t2, b)) ->
...
module H_cons = Hashcons.Make (struct
  ...
  let hash (a : t) : int =
    ...
    | Type ->
        1
    | Const id ->
        CCHash.combine2 10 (ID.hash id)
    | Var v ->
        CCHash.combine2 20 (HVar.hash v)
    | App (f, x) ->
        CCHash.combine3 30 (hash f) (hash x)
    | Arrow (a, b) ->
        CCHash.combine3 35 (hash a) (hash b)
    | Bind b ->
    ...
    | Int_pred (l, o) ->
        Hash.combine3 70 (Linexp_int.hash hash l) (Int_op.hash o)
    | Rat_pred (l, o) ->
        Hash.combine3 80 (Linexp_rat.hash hash l) (Rat_op.hash o)
...
let rec pp_rec depth out (t : t) =
  ...
  | Var v ->
      Format.fprintf out "Y%d" (depth - HVar.id v - 1)
...
let subterms (t : t) (k : t -> unit) : unit =
  let rec aux t =
    ...
    | Ite (a, b, c) ->
        aux a ; aux b ; aux c
    | Int_pred (l, _) ->
        Linexp_int.subterms l k
    | Rat_pred (l, _) ->
        Linexp_rat.subterms l k
  in
  aux t
...
let int_pred l o =
  if Linexp_int.is_const l then
    ...
    | O.Neq0 ->
        Z.sign n <> 0
    | O.Divisible_by k ->
        Z.equal Z.zero (Z.rem n k)
    | O.Not_div_by k ->
        not (Z.equal Z.zero (Z.rem n k)) )
    |> of_bool
...
let[@inline] map ~f ~bind:f_bind b_acc t =
  match view t with
  | Type ->
      t_type
  | Var v ->
      var (HVar.update_ty v ~f:(f b_acc))
  | Const id ->
      const ~ty:(f b_acc @@ ty_exn t) id
  | App (hd, a) ->
      app_ (f b_acc hd) (f b_acc a) ~ty:(f b_acc (ty_exn t))
  | Arrow (a, b) ->
  ...
  | Int_pred (l, o) ->
      int_pred (Linexp_int.map (f b_acc) l) o
  | Rat_pred (l, o) ->
      rat_pred (Linexp_rat.map (f b_acc) l) o
...
let bind ~ty binder ~ty_var body =
  match (binder, view body) with
...
let app_ f x =
  match (ty f, ty x) with
  | Some {view= Arrow (a, b); _}, Some a' when equal a a' ->
      app_ f x ~ty:b
...
let app f x =
  ...
  | Bind {binder= Binder.Lambda; ty_var; body} -> (
    ...
    | None ->
        errorf "type error: cannot apply `%a`@ to `%a : none`" pp f pp x )
  | _ ->
      app_ f x
...
let arrow a b =
  ...
  | Some {view= Type; _}, Some {view= Type; _} ->
      arrow_ a b
...
let as_eta_expansion body : _ option =
...
module Form = struct
  ...
  let view t =
    ...
    | AppBuiltin (Builtin.Not, [t]) ->
        Not t
    | AppBuiltin (Builtin.Eq, ([_; t; u] | [t; u])) ->
        Eq (t, u)
    | AppBuiltin (Builtin.Neq, ([_; t; u] | [t; u])) ->
        Neq (t, u)
    | AppBuiltin (Builtin.Imply, [t; u]) ->
        Imply (t, u)
    | AppBuiltin (Builtin.Equiv, [t; u]) ->
        Equiv (t, u)
    | AppBuiltin (Builtin.Xor, [t; u]) ->
        Xor (t, u)
  ...
  let not_ a =
    ...
    | Not f ->
        f
    | Int_pred (l, o) ->
        int_pred l (Int_op.not o)
    | Rat_pred (l, o) ->
        rat_pred l (Rat_op.not o)
    | _ ->
        app_builtin ~ty:bool Builtin.Not [a]
end
...
module Conv = struct
  ...
  let rec of_term (ctx : ctx) (t : T.t) : t =
    ...
    | T.Var v -> (
      ...
      | None ->
          errorf "cannot find var `%a`@ in %a" Var.pp v pp_ctx ctx
      | Some (i, ty) ->
          assert (ctx.depth > i) ;
  ...
  and conv_int_linexp ctx t : Linexp_int.t =
    ...
    | T.AppBuiltin (Builtin.Product, [_; a; b]) -> (
      ...
      | _ ->
          Linexp_int.monomial1 (of_term ctx t) )
    | _ ->
        Linexp_int.monomial1 (of_term ctx t)

  and conv_rat_linexp ctx t : Linexp_rat.t =
    match T.view t with
    | T.AppBuiltin (Builtin.Rat z, []) ->
        Linexp_rat.const z
    | T.AppBuiltin (Builtin.Sum, [_; a; b]) ->
    ...
    | T.AppBuiltin (Builtin.Product, [_; a; b]) -> (
      ...
      | _ ->
          Linexp_rat.monomial1 (of_term ctx t) )
    | _ ->
        Linexp_rat.monomial1 (of_term ctx t)
end
