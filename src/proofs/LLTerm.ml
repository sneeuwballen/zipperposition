...
module Int_op = struct
  ...
  let not = function
    ...
    | Eq0 -> Neq0
    | Neq0 -> Eq0
    | Divisible_by n -> Not_div_by n
    | Not_div_by n -> Divisible_by n
...
module Make_linexp (N : NUM) = struct
  ...
  let add c t e =
    ...
    let n = N.(n + c) in
    let coeffs =
      if N.equal N.zero n then
        I_map.remove t.id e.coeffs
      else
        I_map.add t.id (t, n) e.coeffs
    in
    { e with coeffs }
  ...
  let pp pp_t out (e : t) : unit =
    if is_const e then
      N.pp_print out e.const
    else (
      let pp_const out () =
        if N.equal N.zero e.const then
          ()
        else
          Fmt.fprintf out "@ + %a" N.pp_print e.const
      and pp_pair out (t, c) =
        if N.equal N.one c then
          pp_t out t
        else
          Fmt.fprintf out "@[<2>%a@ @<1>· %a@]" N.pp_print c pp_t t
      in
...
module H_cons = Hashcons.Make (struct
  ...
  let hash (a : t) : int =
    ...
    | Ite (a, b, c) -> CCHash.combine4 60 (hash a) (hash b) (hash c)
    | Int_pred (l, o) ->
...
let ite a b c =
  ...
  | Some ty1, Some ty2 when equal ty1 ty2 -> mk_ (Ite (a, b, c)) (ty b)
  | _ ->
    errorf "type error:@ cannot build `@[ite %a %a %a@]`@ incompatible types" pp
      a pp b pp c
...
let[@inline] map ~f ~bind:f_bind b_acc t =
  ...
  | Ite (a, b, c) -> ite (f b_acc a) (f b_acc b) (f b_acc c)
...
let db_shift n (t : t) : t =
  let rec aux k t =
    ...
    | Var v ->
      if HVar.id v >= k then
        var (HVar.make (HVar.id v + n) ~ty:(HVar.ty v))
      else
        t
    | _ -> map ~f:aux ~bind:succ k t
  in
  aux 0 t
...
let app_ f x =
  ...
  | ( Some { view = Bind { binder = Binder.ForallTy; ty_var; body }; _ },
      Some ty_x ) ->
    (* polymorphic type *)
    if not (equal ty_x t_type) then
      errorf "cannot apply@ `@[<2>%a@ : %a@]`@ to non-type `@[<2>%a@ : %a@]`" pp
        f (Fmt.opt pp) (ty f) pp x (Fmt.opt pp) (ty x);
    if not (equal ty_var t_type) then
      errorf "ill-formed polymorphic type@ `%a`" pp (ty_exn f);
    let ty = db_eval ~sub:x body in
    app_ f x ~ty
  | _ ->
    errorf "type error: cannot apply `@[<2>%a@ : %a@]`@ to `@[<2>%a@ : %a@]`" pp
      f (Fmt.opt pp) (ty f) pp x (Fmt.opt pp) (ty x)
...
let app f x =
  ...
  | Bind { binder = Binder.Lambda; ty_var; body } ->
    (* β-reduction *)
    (match ty x with
    | Some ty_x when equal ty_var ty_x -> db_eval ~sub:x body
    | Some ty_x ->
...
let arrow a b =
  ...
  | _ ->
    errorf
      "type error: cannot make arrow between non-types@ :from `%a`@ :to `%a`" pp
      a pp b
