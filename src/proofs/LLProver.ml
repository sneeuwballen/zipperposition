...
module Branch : sig
  ...
  let[@inline] add_eq t u b : t =
    if is_closed b then
      ...
      b |> add_cc_eq t u |> check_closed

  let[@inline] add_diseq t u b : t =
    if is_closed b then
      b
    else
      b |> add_cc_ t |> add_cc_ u |> add_diseq_l_ t u |> check_closed

  let[@inline] add_to_expand f b =
    if is_closed b then
      b
    else
      { b with to_expand = T_set.add f b.to_expand }
  ...
  let add1_ (br : t) (f : T.t) : t =
    ...
    | F.Not f' ->
      ...
      | F.Exists { ty_var; body } ->
        ...
        br |> add_eq f F.true_
      | F.Forall _ -> br |> add_eq f F.false_)
    ...
    | F.Equiv (a, b) ->
      add_form_to_expand
        (F.and_ [ F.or_ [ a; F.not_ b ]; F.or_ [ b; F.not_ a ] ])
        br
    | F.Forall _ -> br |> add_eq f F.true_
    | F.Exists { ty_var; body } ->
      let f = F.forall ~ty_var (F.not_ body) in
      br |> add_eq f F.false_

  let add1 br f : t =
    if is_closed br then
      br
    else
      add1_ br f
...
let solve_ (tab : t) : res =
  ...
  do
    ...
    | Some (f, b_tail) ->
      let new_branches =
        Rule.apply Rule.all f |> List.map (fun forms -> Branch.add b_tail forms)
      in
...
let pp_dot out (s : final_state) : unit =
  ...
  let attrs_v (b : Branch.t) : _ list =
    let color =
      if Branch.is_closed b then
        [ `Color "green" ]
      else if ISet.mem (Branch.id b) saturated_set then
        [ `Color "red" ]
      else
        []
    in
    let pp_closed out b =
      ...
      | Branch.C_closed_by_theory s -> Fmt.fprintf out "<closed by theory %S>" s
    in
    let label =
