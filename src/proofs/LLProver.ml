  ...
  let debug out (b : t) : unit =
    ...
    let pp_b out b =
      Fmt.fprintf out
        ...
        b.id (is_closed b) (Fmt.some T.pp) b.form (Util.pp_seq T.pp)
        (T_set.to_seq b.to_expand)
    in
...
let can_check : LLProof.tag list -> bool =
  ...
  let f = function
    | T_ho -> true
    | T_lra | T_lia | T_ind | T_data | T_distinct | T_ac _ | T_ext -> false
  in
  List.for_all f
...
let pp_dot out (s : final_state) : unit =
  ...
  in
  CCGraph.Dot.pp_seq ~tbl ~eq:br_eq ~graph:as_graph ~attrs_v out all_branches;
  ()
