...
module CC = Congruence.Make (struct
  ...
  let subterms t =
    ...
    | T.Arrow (a, b) ->
        [a; b]
    | T.AppBuiltin (Builtin.Box_opaque, _) ->
        [] (* simple equality *)
    | T.AppBuiltin (_, l) ->
        l
    | T.Ite (a, b, c) ->
    ...
    | T.Bind {body; _} ->
        [body]
    | Int_pred (l, _) ->
        T.Linexp_int.subterms l |> Iter.to_list
    | Rat_pred (l, _) ->
        T.Linexp_rat.subterms l |> Iter.to_list
...
module Branch : sig
  ...
  let[@inline] to_expand t = T_set.to_iter t.to_expand
  ...
  let check_closed (b : t) : t =
    if is_closed b then b
      ...
      ( match CCList.find_pred (fun (t, u) -> CC.is_eq b.cc t u) b.diseq with
      | None ->
          ()
      | Some (t, u) ->
          b.closed <- C_closed_by_diseq (t, u) ) ;
      b )
  ...
  let rec is_atomic f =
    match F.view f with
    | F.Eq _ | F.Neq _ | F.Atom _ | F.Int_pred _ | F.Rat_pred _ ->
        true
    | F.Not u ->
        is_atomic u
    | _ ->
  ...
  let add1_ (br : t) (f : T.t) : t =
    ...
    | F.True ->
        br
    | F.False ->
        add_eq F.true_ F.false_ br
    | F.Eq (t, u) ->
        add_eq t u br
    | F.Neq (t, u) ->
        add_diseq t u br
    | F.Int_pred (_, _) ->
        add_eq f F.true_ br (* TODO: decision proc *)
    | F.Rat_pred (_, _) ->
        add_eq f F.true_ br (* TODO: simplex *)
    | F.Not f' -> (
      match F.view f' with
      | F.Eq _ | F.Neq _ | F.Not _ | F.Int_pred _ | F.Rat_pred _ ->
          assert false
      | F.True ->
          add_diseq F.true_ F.false_ br
      | F.False ->
          br
      | F.Atom t ->
          add_eq t F.false_ br
      | F.And l ->
          add_form_to_expand (F.or_ (List.map F.not_ l)) br
      | F.Or l ->
          add_form_to_expand (F.and_ (List.map F.not_ l)) br
      | F.Imply (a, b) ->
  ...
  let add b l =
    ...
    let l =
      List.sort
        (fun a b ->
          match (is_atomic a, is_atomic b) with
  ...
  let pop_to_expand b =
    if is_closed b then None
      ...
      match T_set.choose b.to_expand with
      | f ->
...
module Rule : sig
  ...
  let or_ f = match F.view f with F.Or l -> List.map CCList.return l | _ -> []

  let and_ f = match F.view f with F.And l -> [l] | _ -> []
...
let prove (a : form list) (b : form) =
  ...
  let tab = {open_branches= [b_init]; closed_branches= []; saturated= None} in
  let res = solve_ tab in
  ZProf.exit_prof _span ; (res, tab)
