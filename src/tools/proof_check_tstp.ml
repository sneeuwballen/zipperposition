...
module CheckedTrace = struct
  ...
  type t = {
    trace : Trace_tstp.t;
    (* trace to check *)
    steps : check_result list StepTbl.t;
    (* result of checks for every step *)
    mutable failures : (Trace_tstp.t * check_result) list
  }

  let create trace = {trace; steps = StepTbl.create 15; failures = []}
...
let _do_not_check proof =
  match proof with
  | TT.Axiom _ | TT.Theory _ -> true
...
let check_step ~timeout ~prover step =
  ...
  | Some decls ->
    E.(
      CallProver.call ~timeout ~prover decls >>= fun res ->
      ...
      let res =
        ...
        | CallProver.Error e ->
          Util.debugf 1 "error trying to check %a: %s" (fun k ->
              k TT.pp1 step e );
          Failed p_name
        | CallProver.Unknown | CallProver.Sat -> Failed p_name
      in
    ...
    TT.traverse trace (fun step ->
        ...
        List.iter
          (fun prover ->
            ...
            | E.Ok res -> CheckedTrace.add ~checked step res
            | E.Error msg -> failwith msg )
          provers );
    (* clean line of progress *)
    if progress then Printf.printf "                                      \n";
    E.return ()
  with Failure msg -> E.fail msg
...
let main file =
  E.(
    TT.parse ~recursive:true file >>= fun trace ->
    ...
    if not (TT.is_dag trace) then (
      ...
      Util.debug 0 "derivation is a DAG";
    ...
    if check_structure ~minimum:!minimum ~checked then
      Util.debug 0 "validated steps form a correct proof. success."
    else
      Util.debug 0 "proof structure incorrect. failure.";
    E.return ())
