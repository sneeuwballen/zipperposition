...
type check_result = Unchecked | Succeeded of string | Failed of string

let is_success = function Unchecked | Succeeded _ -> true | Failed _ -> false
...
let pp_res out res =
  match res with
  | Unchecked ->
      CCFormat.string out "unchecked"
  | Succeeded p ->
      Format.fprintf out "prover %s succeeded" p
  | Failed p ->
...
let mk_proof_obligation proof =
  if _do_not_check proof then None (* no need to check *)
  else
    try
      let goal, step =
        match proof with
...
let check_step ~timeout ~prover step =
  ...
  | None ->
      E.return Unchecked (* nothing to check, no obligation! *)
  | Some decls ->
      E.(
