...
let refine old_status new_status =
  ...
  | InFOOL ->
    if new_status = OutsideFOOL then
      OutsideFOOL
    else
      InFOOL
  | PureFO -> new_status
  ...
  let rec aux ~top t =
    ...
    if not (CCList.is_empty args) then
      ...
      let res =
        match T.view t with
        | Var _ ->
          if Ty.is_prop ty then
            InFOOL
          else
            PureFO
        | Const _ -> PureFO
        | App (hd, args) ->
          if T.is_fun hd || T.is_var hd then
            OutsideFOOL
          else
            aux_l ~top:false args
        | AppBuiltin (hd, args) ->
          let top =
            top
