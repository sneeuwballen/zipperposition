...
let refine old_status new_status =
  ...
  | OutsideFOOL ->
      OutsideFOOL
  | InFOOL ->
      if new_status = OutsideFOOL then OutsideFOOL else InFOOL
  | PureFO ->
      new_status
  ...
  let rec aux ~top t =
    ...
    if not (CCList.is_empty args) then OutsideFOOL
      ...
      let res =
        match T.view t with
        | Var _ ->
            if Ty.is_prop ty then InFOOL else PureFO
        | Const _ ->
            PureFO
        | App (hd, args) ->
            if T.is_fun hd || T.is_var hd then OutsideFOOL else aux_l ~top:false args
        | AppBuiltin (hd, args) ->
            let top = top && (Builtin.is_logical_op hd || List.mem hd Builtin.[True; False; Eq; Neq]) in
            aux_l ~top args
        ...
        | _ ->
            OutsideFOOL
      in
      if (not top) && Ty.is_prop ty then refine res InFOOL else res
  and aux_l ~top = function
    | [] ->
        PureFO
    | x :: xs ->
        CCList.fold_left (fun acc y -> refine acc (aux ~top y)) (aux ~top x) xs
  in
  aux ~top:true f
