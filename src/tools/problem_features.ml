...
type feature =
  | Counter of string
  (* counter kind of feature *)
  | Vector of string
(* occurence kind of feautre,
   for max/avg features *)

let get_f_name = function Counter x -> x | Vector x -> x
...
let rec is_unit form =
  ...
  | Bind ((Binder.Forall | Binder.Exists), _, body) ->
      is_unit body
  | AppBuiltin (hd, args) ->
      (Builtin.equal hd Builtin.Eq || Builtin.equal hd Builtin.Neq)
      && (List.length args < 2 || not (TypedSTerm.Ty.is_prop (TypedSTerm.ty_exn (List.hd args))))
  | _ ->
      true
...
let update feature value =
  match feature with
  | Counter name ->
      StrTbl.replace cnt_map name (StrTbl.get_or cnt_map name ~default:0 + value)
  | Vector name ->
      StrTbl.replace vec_map name (value :: StrTbl.get_or vec_map name ~default:[])

let update_vec feature value =
  match feature with Counter _ -> invalid_arg "counter feature expected" | Vector _ -> update feature value

let incr_counter feature =
  match feature with Counter _ -> update feature 1 | Vector _ -> invalid_arg "vector feature expected"
...
let traverse_term t =
  ...
  let rec aux top t =
    if (not top) && T.Ty.is_prop (T.ty_exn t) then incr_counter num_nested_bools ;
    match T.view t with
    ...
    | Bind (b, v, t) ->
        if Binder.equal b Binder.Lambda then (
          ...
          if Binder.equal b Binder.Forall then incr_counter num_forall_quants
          else if Binder.equal b Binder.Exists then incr_counter num_exists_quants ;
          let var_order = (if snd @@ T.Ty.arity @@ Var.ty v != 0 then 1 else 0) + T.Ty.order (Var.ty v) in
...
let rec traverse_formula f =
  ...
  | AppBuiltin (b, l) ->
      ...
      then (
        ...
        if T.is_var lhs && T.is_var rhs then incr_counter num_var_atoms ;
        traverse_term lhs ;
        traverse_term rhs )
      else (incr_counter num_logical_connectives ; List.iter traverse_formula l)
  | App (hd, l) ->
      traverse_term f
  | Bind (b, v, t) ->
...
let collect_formula_features stmts =
  CCVector.iter
    (fun stm ->
      ...
      | Def _ | Rewrite _ ->
          incr_counter num_def_form ; incr_counter num_forms
      ...
      | TyDecl (id, ty) ->
          if not (TypedSTerm.Ty.returns_tType ty) then (
            ...
            if TypedSTerm.Ty.returns_prop ty then (* predicate *)
              update_vec pred_arity arity
            else (
              (* function symbol *)
              update_vec fun_arity arity ;
              if arity == 0 then incr_counter num_constants else incr_counter num_functions ) )
      | _ ->
          CCFormat.fprintf CCFormat.stderr "Warning: %a is ignored." Statement.pp_input stm ) ;
      Statement.Seq.forms stm |> Iter.iter update_form_statistics )
    stmts
           ...
           List.iter
             (function
               | SLiteral.Atom (t, sign) ->
                   if sign then update_vec pos_t_depth (Term.depth t) else update_vec neg_t_depth (Term.depth t)
               | SLiteral.Eq (lhs, rhs) ->
...
let options =
  ...
  @ Options.make ()
  |> List.sort CCShims_.Stdlib.compare |> Arg.align
...
let process file =
  ...
  let res =
    ...
    >>= TypeInference.infer_statements ?ctx:None ~file ~on_var:(Input_format.on_var input)
    ...
    if !cnf_features then collect_clause_features (Cnf.cnf_of_iter ~ctx:(Skolem.create ()) (CCVector.to_iter st))
  in
  match res with
  | CCResult.Ok () ->
      List.iter
        (function
          | Counter n ->
              if CCOpt.is_none (StrTbl.get cnt_map n) then StrTbl.replace cnt_map n 0
          | Vector n ->
              if CCOpt.is_none (StrTbl.get vec_map n) then StrTbl.replace vec_map n [-1] )
        all_features ;
      let res_map = StrTbl.create 128 in
      StrTbl.iter (fun key val_ -> StrTbl.replace res_map key (float_of_int val_)) cnt_map ;
      StrTbl.iter
        (fun key val_ ->
          ...
          let avg_v = float_of_int (Iter.sum iter_val) /. float_of_int (Iter.length iter_val) in
          StrTbl.replace res_map ("max_" ^ key) max_v ;
          StrTbl.replace res_map ("min_" ^ key) min_v ;
          StrTbl.replace res_map ("avg_" ^ key) avg_v )
        vec_map ;
      let sorted = List.sort (fun (a, _) (b, _) -> CCString.compare a b) (StrTbl.to_list res_map) in
      CCFormat.printf "@[%a@]@."
        (CCList.pp ~pp_start:(CCFormat.return "{@.") ~pp_stop:(CCFormat.return "@.}")
           ~pp_sep:(CCFormat.return ",@.") (fun out (key, val_) -> CCFormat.printf "  \"%s\" : %f" key val_ ) )
        sorted
  | CCResult.Error msg ->
...
let main () =
  ...
  (match !Options.output with Options.O_normal -> () (* Format.printf "%% @{<Green>success!@}@."; *) | _ -> ()) ;
  ()
