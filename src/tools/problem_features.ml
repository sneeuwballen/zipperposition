...
type feature =
  ...
  | Vector of
      string (* occurence kind of feautre,
  ...
  let rec aux top t =
    ...
    | Bind (b, v, t) ->
      if Binder.equal b Binder.Lambda then (
        ...
        let var_order =
          (if snd @@ T.Ty.arity @@ Var.ty v != 0 then
             1
           else
             0)
          + T.Ty.order (Var.ty v)
...
let rec traverse_formula f =
  ...
  | Bind (b, v, t) ->
    if Binder.equal b Binder.Lambda then
      failwith "should not appear on top."
...
let collect_formula_features stmts =
  CCVector.iter
    (fun stm ->
      ...
      | Assert _ | Goal _ | NegatedGoal _ -> incr_counter num_forms
      | TyDecl (id, ty) ->
        if not (TypedSTerm.Ty.returns_tType ty) then (
          ...
          if TypedSTerm.Ty.returns_prop ty then
            ...
            if arity == 0 then
              incr_counter num_constants
            else
              incr_counter num_functions
          )
        )
      | _ ->
           ...
           List.iter
             (function
               | SLiteral.Atom (t, sign) ->
                 if sign then
                   update_vec pos_t_depth (Term.depth t)
                 else
...
let options =
  ...
  @ Options.make ()
  |> List.sort Stdlib.compare |> Arg.align
