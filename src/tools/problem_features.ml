...
let update_vec feature value =
  match feature with
  | Counter _ ->
      invalid_arg "counter feature expected"
  | Vector _ ->
...
let incr_counter feature =
  ...
  | Counter _ ->
      update feature 1
  | Vector _ ->
      invalid_arg "vector feature expected"
...
let traverse_term t =
  ...
  let rec aux top t =
    ...
    | Bind (b, v, t) ->
        ...
        else if Binder.equal b Binder.Forall || Binder.equal b Binder.exists
        then (
...
let collect_formula_features stmts =
  CCVector.iter
    (fun stm ->
      ...
      | _ ->
          CCFormat.fprintf CCFormat.stderr "Warning: %a is ignored."
...
let options =
  [ ( "--cnf-features"
    , Arg.Bool (( := ) cnf_features)
    , " clausify the problem and include CNF features" ) ]
  @ Options.make ()
...
let process file =
  ...
  | CCResult.Ok () ->
      ...
      StrTbl.iter
        (fun key val_ -> StrTbl.replace res_map key (float_of_int val_))
        cnt_map ;
      StrTbl.iter
        (fun key val_ ->
          ...
          let avg_v =
            float_of_int (Iter.sum iter_val)
            /. float_of_int (Iter.length iter_val)
          in
      ...
      let sorted =
        List.sort
          (fun (a, _) (b, _) -> CCString.compare a b)
          (StrTbl.to_list res_map)
      in
      CCFormat.printf "@[%a@]@."
        (CCList.pp ~pp_start:(CCFormat.return "{@.")
           ~pp_stop:(CCFormat.return "@.}") ~pp_sep:(CCFormat.return ",@.")
           (fun out (key, val_) -> CCFormat.printf "  \"%s\" : %f" key val_ ) )
        sorted
  | CCResult.Error msg ->
