...
let pp_stmt_in o = Statement.pp_input_in o

let pp_stmts_in o out seq = CCList.pp ~sep:"" (pp_stmt_in o) out seq
...
let rec app_encode_ty ty =
  ...
  | T.AppBuiltin (Builtin.Arrow, ret :: args) when not (T.Ty.is_tType ret) ->
    CCList.fold_right
      (fun arg t -> T.app ~ty:T.tType function_type [app_encode_ty arg; t])
...
let is_type a = T.Ty.is_tType (T.ty_exn a)
...
let rec app_encode_term toplevel t =
  ...
  Util.debugf 5 "Encoded type %a into %a" (fun k ->
      k T.pp (CCOpt.get_exn (T.ty t)) T.pp ty );
  match T.view t with
  | T.App (f, []) -> app_encode_term false f
  | T.App (f, args) ->
    Util.debugf 5 "Attempting to encode application: %a" (fun k ->
        k T.pp_with_ty t );
    (* app encode *)
    CCList.fold_left
      (fun term arg ->
        Util.debugf 5 "Encoding application of %a to %a" (fun k ->
        ...
        | T.App (f, types) ->
          ...
          T.app ~ty:(List.nth types 1) app_const
  ...
  | T.AppBuiltin (f, ts) ->
    ...
    assert (
      f == Builtin.Eq || f == Builtin.Neq || f == Builtin.And
      || f == Builtin.Or || f == Builtin.Not || f == Builtin.Imply
      || f == Builtin.Equiv || f == Builtin.False || f == Builtin.True
      || f == Builtin.Equiv || f == Builtin.Xor );
    T.app_builtin ~ty f
      (List.map
         (app_encode_term (not (f == Builtin.Eq || f == Builtin.Neq)))
         ts)
...
let app_encode stmt =
  ...
  | Statement.Goal f ->
    Statement.goal ~proof:Proof.Step.trivial (app_encode_term true f)
  | Statement.NegatedGoal (_, _) -> failwith "Not implemented"
  | Statement.Assert f ->
    Statement.assert_ ~proof:Proof.Step.trivial (app_encode_term true f)
...
let extensionality_axiom =
  ...
  let xz =
    T.app ~ty:(T.var beta) app_const [T.var alpha; T.var beta; T.var x; T.var z]
  in
  let yz =
    T.app ~ty:(T.var beta) app_const [T.var alpha; T.var beta; T.var y; T.var z]
  in
...
let process file =
  ...
  Format.printf "@[<v>%a@]@." (pp_stmt_in o) decl_fun;
  Format.printf "@[<v>%a@]@." (pp_stmt_in o) decl_app;
  if !extensional_ then
    Format.printf "@[<v>%a@]@." (pp_stmt_in o) extensionality_axiom;
  Format.printf "@[<v>%a@]@." (pp_stmts_in o) app_encoded

let options =
  Options.add_opts
    [ ( "--app-encode-extensional",
        Arg.Set extensional_,
        " enable extensionality axiom in app-encoding" ) ];
  Options.make ()
