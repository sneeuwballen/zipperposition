...
let print_res (decls : _ CCVector.ro_vector) : unit =
  let close_c c = c |> List.map SLiteral.to_form |> TypedSTerm.Form.or_ |> TypedSTerm.Form.close_forall in
  match !Options.output with
  | Options.O_none ->
      ()
  | Options.O_normal ->
      let ppst = Statement.pp (Util.pp_list ~sep:" ∨ " (SLiteral.pp T.pp)) T.pp T.pp in
      Format.printf "@[<v2>%d statements:@ %a@]@." (CCVector.length decls)
        (CCVector.pp ~pp_sep:(CCFormat.return "@,") ppst)
        decls
  | Options.O_tptp ->
      let pp_c out c = TypedSTerm.TPTP.pp out (close_c c) in
      let ppst out st = Statement.TPTP.pp pp_c T.TPTP.pp T.TPTP.pp out st in
      Format.printf "@[<v>%a@]@." (CCVector.pp ~pp_sep:(CCFormat.return "@,") ppst) decls
  | Options.O_zf ->
      let pp_c out c = T.ZF.pp_inner out (close_c c) in
      let ppst out st = Statement.ZF.pp pp_c T.ZF.pp_inner T.ZF.pp_inner out st in
      Format.printf "val term : type.@." ;
      (* implicit *)
      Format.printf "@[<v>%a@]@." (CCVector.pp ~pp_sep:(CCFormat.return "@,") ppst) decls
...
let process file =
  ...
  let res =
    ...
    >|= fun st ->
    if !print_in then
      Format.printf "@[<v2>input:@ %a@]@." (CCVector.pp ~pp_sep:(CCFormat.return "@,") Statement.pp_input) st ;
    let opts =
    ...
    if !print_sig then
      Format.printf "@[<hv2>signature:@ (@[<v>%a@]@])@."
        (ID.Map.pp ~pp_sep:(CCFormat.return "@,") ~pp_arrow:(CCFormat.return "@ : ") ID.pp T.pp)
        sigma ;
    (* print *)
    print_res decls ;
    ()
  in
  match res with CCResult.Ok () -> () | CCResult.Error msg -> print_endline msg ; exit 1
