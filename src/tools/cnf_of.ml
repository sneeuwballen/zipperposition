...
let print_res (decls : _ CCVector.ro_vector) : unit =
  let close_c c =
    c |> List.map SLiteral.to_form |> TypedSTerm.Form.or_
  ...
  | Options.O_normal ->
    let ppst =
      Statement.pp (Util.pp_list ~sep:" ∨ " (SLiteral.pp T.pp)) T.pp T.pp
    in
...
let process file =
  ...
  let res =
    ...
    >|= fun st ->
    if !print_in then
      Format.printf "@[<v2>input:@ %a@]@."
        (CCVector.pp ~pp_sep:(CCFormat.return "@,") Statement.pp_input)
        st;
    let opts =
      (if !flag_distribute_exists then
         [ Cnf.DistributeExists ]
       else
         [])
      @ (if !flag_disable_renaming then
           [ Cnf.DisableRenaming ]
         else
           [])
      @ []
    in
    let decls =
      Cnf.cnf_of_iter ~opts ~ctx:(Skolem.create ()) (CCVector.to_iter st)
    in
