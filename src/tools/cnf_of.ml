...
let print_res decls =
  ...
  | Options.O_tptp ->
    let pp_c out c =
      ...
      in
      TypedSTerm.TPTP.pp out f
    in
    let ppst out st = Statement.TPTP.pp pp_c T.TPTP.pp T.TPTP.pp out st in
    Format.printf "@[<v>%a@]@." (CCVector.pp ~sep:"" ppst) decls
  | Options.O_zf ->
    let ppst out st =
      Statement.ZF.pp
        (Util.pp_list ~sep:" || " (SLiteral.ZF.pp T.ZF.pp_inner))
        T.ZF.pp_inner T.ZF.pp_inner out st
    in
    Format.printf "@[<v>%a@]@." (CCVector.pp ~sep:"" ppst) decls
  ...
  let res =
    ...
    if !print_in then
      Format.printf "@[<v2>input:@ %a@]@."
        (CCVector.pp ~sep:"" Statement.pp_input)
        st;
    let opts =
    ...
    if !print_sig then
      Format.printf "@[<hv2>signature:@ (@[<v>%a@]@])@."
        (ID.Map.pp ~sep:"" ~arrow:" : " ID.pp T.pp)
        sigma;
    (* print *)
    print_res decls;
    ()
