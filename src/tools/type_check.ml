...
let check file =
  ...
  ( CCVector.to_iter stmts |> Iter.flat_map Statement.Seq.terms |> fun trm ->
    try
      ignore
        (Iter.for_all
           (fun t ->
  ...
  |> fun x ->
  if x = "" then
    if !found_fool_subterm then
      CCResult.return ()
    else
      CCResult.fail "FAIL: Pure FO problem"
  else
    CCResult.fail ("FAIL: " ^ x)
...
let main () =
  ...
  match res with
  | Err.Ok () -> if not !dump then (* print_line (); *) print_endline "OK."
  | Err.Error msg -> print_endline msg
