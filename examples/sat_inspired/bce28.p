...
thf(sy_c_BCE_Oa, type,
    a : !>[A : $tType]: A).
thf(sy_c_BCE_Ob, type,
    b : !>[A : $tType]: A).
thf(sy_c_BCE_Op2, type,
    p2 : !>[A : $tType, B : $tType]: (A > B > $o)).
thf(sy_c_BCE_Oq, type,
    q : !>[A : $tType]: (A > $o)).

% Relevant facts (3)
thf(fact_0_assms__28_I3_J, axiom,
    (((a @ (nat)) = (b @ (nat))))). % assms_28(3)
thf(fact_1_assms__28_I2_J, axiom,
    (((p2 @ (nat > nat) @ (nat) @ (^[X : nat]: X) @ (b @ (nat))) | (~ ((q @ (nat) @ (b @ (nat)))))))). % assms_28(2)
thf(fact_2_assms__28_I1_J, axiom,
    ((![B : $tType, A : $tType]: (![C : nat, Y : A > $o]: ((~ ((p2 @ (nat > nat) @ (nat) @ (^[X : nat]: X) @ C))) | ((~ ((p2 @ (B > B) @ (nat) @ (^[X : B]: X) @ (a @ (nat))))) | ((q @ (nat) @ C) | ((q @ (nat) @ (a @ (nat))) | (Y @ (a @ (A))))))))))). % assms_28(1)
