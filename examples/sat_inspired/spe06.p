...
thf(fact_0_assms_I10_J, axiom,
    (((~ ((q @ two))) | ((f @ five) = six)))). % assms(10)
thf(fact_1_assms_I9_J, axiom,
    (((~ ((q @ one))) | ((f @ four) = five)))). % assms(9)
thf(fact_2_assms_I8_J, axiom,
    (((q @ three) | ((f @ three) = four)))). % assms(8)
thf(fact_3_assms_I7_J, axiom,
    (((q @ two) | ((f @ two) = three)))). % assms(7)
thf(fact_4_assms_I6_J, axiom,
    (((q @ one) | ((f @ one) = two)))). % assms(6)
thf(fact_5_assms_I5_J, axiom,
    (((~ ((p @ two))) | ((f @ five) = six)))). % assms(5)
thf(fact_6_assms_I4_J, axiom,
    (((~ ((p @ one))) | ((f @ four) = five)))). % assms(4)
thf(fact_7_assms_I3_J, axiom,
    (((p @ three) | ((f @ three) = four)))). % assms(3)
thf(fact_8_assms_I2_J, axiom,
    (((p @ two) | ((f @ two) = three)))). % assms(2)
thf(fact_9_assms_I1_J, axiom,
    (((p @ one) | ((f @ one) = two)))). % assms(1)
