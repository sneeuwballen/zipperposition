...
thf(sy_c_List_Orev_001tf__a, type,
    rev_a : list_a > list_a).
thf(sy_v_blue, type,
    blue : (list_a > list_a) > $o).
thf(sy_v_cyan, type,
    cyan : (nat > nat) > $o).
thf(sy_v_green, type,
    green : $o).
thf(sy_v_yellow, type,
    yellow : $o).

% Relevant facts (3)
thf(fact_0_assms_I3_J, axiom,
    (((cyan @ (^[X : nat]: X)) | ((blue @ rev_a) | yellow)))). % assms(3)
thf(fact_1_assms_I2_J, axiom,
    (((cyan @ (^[X : nat]: X)) => green))). % assms(2)
thf(fact_2_assms_I1_J, axiom,
    (((cyan @ (^[X : nat]: X)) => (blue @ rev_a)))). % assms(1)
